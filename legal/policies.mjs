...
export const LEGAL_POLICIES = [
  policy({
    ...
    summary: "This policy explains how Chi'llwood collects, uses, shares, retains, and protects information across accounts, Profile, Channel, uploads, Player, Watch-Party, Live, Chi'lly Chat, calls, Premium, support, moderation, legal evidence, diagnostics, and public legal surfaces.",
    sections: [
      section("Plain-English Summary", [
        "Chi'llwood uses information to run the app, keep accounts secure, deliver creator and social video features, operate live rooms and chat calls, process Premium entitlements, respond to support, moderate abuse, handle copyright and legal requests, debug reliability, and comply with law. Some information is visible because users choose public profile, channel, upload, live, replay, chat, or discovery settings. Other information is private, restricted by RLS, or available only to support, moderation, legal, and owner/admin tools when there is an allowed reason.",
      ...
      section("Information You Provide", [
        "You may provide account details such as email address, authentication identifiers, profile information, display name, username, bio, avatar, channel name, creator details, uploaded videos, thumbnails, titles, descriptions, visibility selections, live event details, chat messages, support requests, reports, copyright notices, counter-notices, legal request information, payment support details, and settings. Passwords and recovery tokens are handled through the authentication provider and should not be sent to support.",
        "Creator uploads can include video files, audio tracks embedded in videos, images, thumbnails, captions, metadata, categories, publish state, draft state, storage paths, processing status, channel associations, and moderation status. Live and chat surfaces can include room identifiers, participant identifiers, speaker roles, call mode, microphone or camera state, token request metadata, join/leave status, relay/connectivity metadata, replay policy, and related timestamps. These records help the app run honestly and help the team investigate disputes or reliability incidents.",
      ]),
      section("Information Created by Use of the Service", [
        "Chi'llwood may collect profile visits, channel interactions, player interactions, search and discovery activity, follows or audience relationships, watch-party room membership, live room membership, playback state, chat thread membership, notification preferences, push token fingerprints, device permission state, support and moderation history, report status, legal hold status, evidence export metadata, audit rows, and administrative action records. Some activity is retained in aggregated or diagnostic form even when a user later deletes public content.",
        "Device and technical information may include device type, operating system, app version, build channel, runtime configuration, crash reports, performance traces, network state, IP-derived operational signals, route names, feature flags, timing measurements, and errors. Firebase Crashlytics, Firebase Performance, Supabase logs, LiveKit connection telemetry, RevenueCat entitlement events, app-store billing records, and similar providers may help Chi'llwood understand whether the service is working and whether abuse, fraud, or unauthorized access occurred.",
      ]),
      section("Payments, Premium, and Entitlements", [
        "When Premium or subscriptions are available, Google Play, Apple, RevenueCat, Supabase, or other approved providers may process purchase, renewal, cancellation, refund, restore, entitlement, offer, transaction, receipt, and billing-support information. Chi'llwood uses that information to decide whether a normal user has paid access to gated features. Owner platform access is a separate server-side role exception for the invisible platform owner account; it is not a fake entitlement and it does not weaken premium gates for normal users.",
        "Do not send full payment card numbers, app-store passwords, private wallet keys, or unrelated financial data to support. Chi'llwood support may ask for limited purchase identifiers, store account context, screenshots with sensitive details removed, or RevenueCat/app-store identifiers when needed to troubleshoot billing. Store-managed subscriptions may remain active until canceled through the store or billing provider, even if a Chi'llwood account deletion request is pending.",
      ]),
      section("How Information Is Used", [
        "Chi'llwood uses information to create and authenticate accounts, provide Profile and Channel surfaces, host creator uploads, operate Player and discovery, run Watch-Party and Live features, deliver Chi'lly Chat and calls, send notifications, manage Premium, process reports, respond to support, handle copyright notices, investigate safety concerns, enforce rules, protect minors and vulnerable people, prevent fraud, maintain backups, debug crashes, improve performance, and comply with law.",
        "Chi'llwood may also use content and metadata to show uploads in feeds, search, discovery, profile, channel, player, spectator, live, replay, web, CTV, notification, recommendation, admin, moderation, support, and legal surfaces according to policy, visibility, licensing, and backend access rules. Operational use may include thumbnails, previews, compression, transcoding, captions, excerpts, abuse detection, evidence packages, and legal holds where allowed.",
      ]),
      section("Sharing and Service Providers", [
        "Information may be shared with service providers that help host, authenticate, store, stream, process payments, deliver notifications, monitor crashes, measure performance, send support replies, operate legal pages, route live media, analyze reliability, or secure the service. Providers may include Supabase, Firebase/Google, RevenueCat, app stores, LiveKit, Expo/EAS, email/support providers, hosting providers, and security or legal vendors when those providers are actually used.",
      ...
      section("User Rights, Choices, and Security", [
        "Users may be able to update profile, channel, visibility, notification, password, Premium restore, and account settings in the app. Users can request account deletion through the Account Deletion policy path. Depending on location, users may have legal rights to access, correct, delete, export, restrict, or object to certain processing. Chi'llwood may need to verify identity and may deny or limit requests when retention, safety, fraud, copyright, legal hold, or other lawful reasons apply.",
        "Chi'llwood uses reasonable technical and organizational safeguards, including server-side authorization, RLS, scoped admin permissions, audit records for sensitive admin actions, redaction patterns, and service-role-only paths where needed. No system is perfectly secure. Users are responsible for securing their email, password, device, and account sessions and should contact support if they suspect compromise.",
      ]),
  ...
  policy({
    ...
    path: "/terms",
    title: "Terms of Use",
    summary: "These Terms govern use of Chi'llwood accounts, Profile, Channel, creator uploads, Player, Watch-Party, Live, Chi'lly Chat, calls, Premium, support, reporting, moderation, legal processes, and related services.",
    sections: [
      section("Plain-English Summary", [
      ...
      section("Eligibility and Accounts", [
        "You must be at least 18 years old, legally able to agree to these Terms, and not barred from using the service by law or by a previous Chi'llwood restriction. You are responsible for your account credentials, email access, profile identity, channel identity, subscription status, device security, and all activity that occurs under your account. If you represent a business, creator team, agency, or other organization, you represent that you have authority to bind that organization.",
      ...
      section("Prohibited Conduct", [
        ...
        "You may not interfere with the service, overload infrastructure, exploit bugs, access data without permission, reverse engineer protected systems except where law allows, bypass RLS or server-side controls, attempt to obtain service-role keys or ops tokens, manipulate LiveKit tokens, create fake participants or fake proof, impersonate Chi'llwood staff, or misrepresent a creator, sponsor, official account, legal status, or public-safety role.",
      ]),
      section("Premium, Payments, and Creator Monetization", [
        "Premium features may require a subscription or entitlement for normal users. Store providers and RevenueCat or similar entitlement systems may be the source of truth for normal user paid access. Premium features can change, be paused, be unavailable on some devices, or require backend verification. Deleting a Chi'llwood account does not automatically cancel a store-managed subscription.",
  ...
  policy({
    ...
    sections: [
      section("Plain-English Summary", [
        "Chi'llwood is for adult viewers and creators who want social streaming, creator channels, live rooms, watch parties, chat, and calls without harassment, theft, exploitation, or unsafe conduct. Be honest about who you are, use content you have rights to use, respect other people, and do not abuse live, chat, reporting, support, billing, or moderation tools. If content or behavior creates risk, Chi'llwood may remove it, restrict it, preserve it, or take account action.",
        "These Guidelines apply to public and private-looking surfaces alike: Profile, Channel, uploads, thumbnails, titles, descriptions, Player, feeds, discovery, Watch-Party Live, Live Watch-Party, Live Stage, Chi'lly Chat, audio/video calls, comments, support, reports, account settings, legal requests, and future creator monetization surfaces. Private or limited-audience features still must follow law, rights, safety, and platform integrity rules.",
      ]),
      ...
      section("Scams, Fraud, Spam, Impersonation, and Platform Abuse", [
        ...
        "Do not bypass restrictions, create accounts to evade bans, exploit bugs, scrape data, automate abusive actions, overload rooms, manipulate playback, fake proof, create fake participants, misuse notifications, or attempt to access admin, legal, support, billing, Live Ops, service-role, or private user data without authorization. Attempts to break platform integrity can lead to account termination and legal escalation.",
      ]),
  ...
  policy({
    ...
    sections: [
      ...
      section("Specific Rights Included in the License", [
        "The license includes rights to host, store, cache, back up, stream, reproduce, display, publicly perform, distribute, publish, transmit, transcode, compress, resize, crop, format-shift, create thumbnails, create previews, create trailers or snippets, create captions or metadata where applicable, moderate, review, restrict, remove, promote the service using the content, advertise the service using content, and show the content in feeds, search, discovery, profile, channel, player, spectator, live, replay, web, CTV, notification, recommendation, admin, moderation, support, and legal surfaces.",
  ...
  policy({
    ...
    summary: "This policy explains how users can request account deletion, what may be deleted or de-identified, what may be retained, and how deletion interacts with subscriptions, backups, legal holds, and Google Play requirements.",
    sections: [
      section("Plain-English Summary", [
      ...
      section("Legal Holds, Evidence, and Safety Exceptions", [
        ...
        "The Legal Evidence tool is read-only for evidence packages and should not delete evidence. Deletion requests should not be used to destroy evidence, evade moderation, hide infringement, avoid chargebacks, or frustrate lawful process. Chi'llwood may preserve audit rows and evidence export records even after account deletion.",
      ]),
  ...
  policy({
    ...
    sections: [
      ...
      section("Billing Providers and Entitlement Source of Truth", [
        "Normal user Premium status may be processed through Google Play, Apple, RevenueCat, Supabase entitlement rows, or another approved provider. The provider may control price display, taxes, renewal timing, cancellation path, refunds, trials, receipts, and subscription management. Chi'llwood uses verified entitlement truth to decide whether normal users clear Premium gates. Users should be signed into the correct store account and Chi'llwood account when purchasing or restoring.",
        "Do not assume payment succeeded because a button was tapped. Do not assume Premium is active because a screenshot exists. Backend entitlement truth, provider status, receipt validation, and account identity matter. Fraud, chargebacks, refunds, revoked receipts, expired subscriptions, test purchases, mismatched accounts, unsupported stores, or provider outages may prevent Premium access.",
      ]),
  ...
  policy({
    ...
    sections: [
      ...
      section("Technical Reliability and Live Ops", [
        "Live and call reliability can be affected by tokens, signaling, TURN/media paths, cellular networks, device permissions, SDK behavior, camera publish state, microphone publish state, client rendering, subscriptions, LiveKit nodes, and app versions. Chi'llwood may run Owner/Admin Live Ops checks, dry-run incidents, and approved remediation workflows, but real production actions require owner/operator approval and must not fake health.",
        "Users should not interpret a failed join, blank feed, low relay bytes, black screen, audio-only failure, or ended room as proof of a legal or moderation decision. Technical incidents may be diagnosed separately from policy enforcement. Chi'llwood may preserve technical logs and incident records to troubleshoot reliability.",
      ]),
    ],
  }),
  policy({
  ...
  policy({
    ...
    sections: [
      section("Plain-English Summary", [
        "Chi'llwood may build or operate monetization surfaces such as Premium, subscriptions, ads, sponsorships, CTV, paid promotions, creator support, live events, replays, or future payout programs. But uploading content, publishing a channel, going live, receiving views, getting followers, being recommended, or paying for Premium does not automatically entitle a creator to money. Payment requires a separate written program, agreement, or product terms.",
      ...
      section("No Automatic Payment", [
        "Creators are not automatically paid for uploads, videos, thumbnails, lives, replays, chat, calls, watch parties, comments, discovery appearances, recommendations, promotions, public profiles, channels, or audience activity. Chi'llwood does not promise a minimum payout, advertising share, subscription share, sponsorship, bonus, tip, revenue split, or creator fund unless a specific monetization program or written contract says so.",
        "Views, watch time, likes, followers, comments, shares, room participation, replay availability, or public placement are not payment promises. Metrics may be delayed, estimated, unavailable, corrected, filtered, or invalidated for fraud, abuse, technical errors, privacy, rights disputes, or product changes. No creator should rely on unsupported metrics as income proof.",
      ]),
      ...
      section("No Guarantee of Audience or Revenue", [
        "Chi'llwood does not guarantee that creators will receive views, subscribers, followers, watch time, comments, likes, recommendations, sponsorships, ad fill, CPM, conversion, tips, sales, payout eligibility, or revenue. Algorithms, search, discovery, player placement, live rooms, recommendations, public channels, Premium surfaces, and ads may change at any time.",
