...
type StripeConnectAccountSkeletonPayload = {
...
type ParsePayloadResult = { value: StripeConnectAccountSkeletonPayload } | { error: Response };
...
const parseJsonPayload = async (req: Request): Promise<ParsePayloadResult> => {
  ...
  if (!rawBody.trim()) return { value: {} as StripeConnectAccountSkeletonPayload };
  ...
  try {
    ...
    return { value: parsed as StripeConnectAccountSkeletonPayload };
  } catch {
...
const requestedCreatorUserId = (payload: StripeConnectAccountSkeletonPayload) =>
  toText(payload.creator_user_id ?? payload.creatorUserId);

const hasClientProviderAccountId = (payload: StripeConnectAccountSkeletonPayload) =>
  !!toText(payload.provider_account_id ?? payload.providerAccountId);

const requestedAccountType = (payload: StripeConnectAccountSkeletonPayload) =>
...
Deno.serve(async (req) => {
  ...
  if (req.method !== "POST") {
    return json(405, { error: "method_not_allowed", message: "Use POST for Stripe Connect account skeleton requests." });
  }
  ...
  try {
    ...
    return json(200, {
      ...
      requestedAccountType: requestedAccountType(parsed.value),
      providerCall: false,
      providerWrite: false,
      ...
      message: "Stripe Connect account creation is skeleton-only in this foundation pass. No account was created.",
    });
  } catch (error) {
    return json(500, {
      error: "stripe_connect_account_skeleton_failed",
