-- Route existing creator-money purchase helpers to the finite App Store authority
-- without making client platform metadata authoritative for identity or access.
...
begin
  ...
  select * into v_product from public."monetization_products" where "product_key" = p_product_key limit 1;
  if v_product."id" is null then raise exception 'product_not_found'; end if;
  if v_product."status" in ('disabled', 'retired') then raise exception 'product_not_available'; end if;
  ...
  if coalesce(v_product."is_android_digital", false) is not true
    or v_product."provider" not in ('revenuecat_google_play', 'google_play', 'revenuecat') then
    raise exception 'android_digital_products_require_revenuecat_google_play';
  end if;
  if v_product."environment" <> 'sandbox' or v_product."status" <> 'sandbox' then raise exception 'sandbox_provider_mapping_required'; end if;
  if nullif(trim(coalesce(v_product."provider_product_id", '')), '') is null then raise exception 'provider_product_id_required'; end if;
  if coalesce((v_product."metadata"->>'sandbox_purchase_intents_enabled')::boolean, false) is not true then raise exception 'sandbox_purchase_intents_not_enabled'; end if;
  if not (public."has_platform_role"(array['owner'::text, 'operator'::text]) or public."has_active_beta_access"()
    or public."resolve_sandbox_monetization_tester"(v_user_id::text, v_actor_email)) then
  ...
  if p_source_type <> v_expected_source_type then raise exception 'source_type_mismatch'; end if;
  ...
  insert into public."money_purchase_intents" (
    ...
    v_user_id,v_product."id",v_product."product_key",v_product."product_type",v_product."provider",v_product."provider_product_id",
    p_source_type,p_source_id,nullif(p_metadata->>'creator_id','')::uuid,nullif(p_metadata->>'platform_id','')::uuid,
    'sandbox','pending',nullif(p_metadata->>'amount_minor','')::integer,lower(nullif(p_metadata->>'currency','')),
    'money_intent:'||v_user_id::text||':'||gen_random_uuid()::text,v_now+interval '15 minutes',
    jsonb_build_object('sandbox_only',true,'not_payable',true,'client_selected_payable_state',false,
      'source_policy_checked_by_product_lane',coalesce((v_product."metadata"->>'source_policy_checked')::boolean,false),
      'sandbox_tester_checked',true) || coalesce(p_metadata,'{}'::jsonb)
  ) returning * into v_intent;
  return public."money_purchase_intent_safe_row"(v_intent);
end;
$$;

-- Correct the canonical Paid Video source discriminator for the iOS resolver.
create or replace function public."ios_paid_video_offer_authority"(p_video_id uuid)
returns jsonb
language sql
stable
security definer
...
as $$
  select coalesce((
    select jsonb_build_object(
      ...
      'available', price."is_paid" and price."status" in ('sandbox','active')
    )
    from public."creator_content_prices" price
    where price."content_id" = p_video_id
      and price."content_type" in ('creator_video','video')
    order by price."updated_at" desc
    limit 1
  ), jsonb_build_object('available', false));
$$;
revoke all on function public."ios_paid_video_offer_authority"(uuid) from public, anon, authenticated;
grant execute on function public."ios_paid_video_offer_authority"(uuid) to service_role;
