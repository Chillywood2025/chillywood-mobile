...
comment on function public.search_public_people(text, integer) is
  'Public-safe Explore people search. Searches username/display name only, includes explicit public official Rachi when matched, excludes active staff/operator/system accounts, respects profile privacy/block policy, masks non-active media, and never returns email or private role fields.';
