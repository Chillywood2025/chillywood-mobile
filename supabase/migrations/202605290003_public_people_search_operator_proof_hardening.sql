create or replace function public.public_people_search_is_internal_account_candidate(
  p_user_id text,
  p_username text,
  p_display_name text
)
...
as $$
  select
    lower(coalesce(p_user_id, '')) <> 'platform_rachi_official'
    and (
      lower(coalesce(p_user_id, '')) like any (
        array['admin_%', 'moderator_%', 'owner_%', 'operator_%', 'proof_%', 'security_%', 'service_%', 'support_%', 'system_%', 'test_%']
      )
      or lower(coalesce(p_username, '')) like any (
        array['admin_%', 'moderator_%', 'owner_%', 'operator_%', 'proof_%', 'security_%', 'service_%', 'support_%', 'system_%', 'test_%']
      )
      or lower(coalesce(p_display_name, '')) like any (
        array[
          '%admin proof%',
          '%moderator proof%',
          '%owner proof%',
...
revoke all on function public.public_people_search_is_internal_account_candidate(text, text, text) from public;
grant execute on function public.public_people_search_is_internal_account_candidate(text, text, text) to anon;
grant execute on function public.public_people_search_is_internal_account_candidate(text, text, text) to authenticated;
grant execute on function public.public_people_search_is_internal_account_candidate(text, text, text) to service_role;

comment on function public.public_people_search_is_internal_account_candidate(text, text, text) is
  'Internal helper for public People discovery. Classifies owner/admin/operator/security/system/service/support/proof/test-operator account markers so Explore search can exclude them while allowing the explicit public Rachi official account.';
...
comment on function public.search_public_people(text, integer) is
  'Public-safe Explore people search. Searches username/display name only, includes explicit public official Rachi when matched, excludes active staff/operator/system accounts and proof/operator markers, respects profile privacy/block policy, masks non-active media, and never returns email or private role fields.';
