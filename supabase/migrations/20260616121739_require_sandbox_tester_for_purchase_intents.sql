-- Sandbox purchase intents must remain tester/internal only. Revoked testers
-- must lose the ability to start sandbox-only monetization purchases.
...
comment on function public."create_money_purchase_intent"(text, text, uuid, jsonb) is
  'Creates sandbox-only, not-payable purchase intents for active sandbox testers, beta/internal accounts, owners, or operators. It does not support Premium, physical merch, live payouts, live charges, arbitrary products, or revoked testers.';
