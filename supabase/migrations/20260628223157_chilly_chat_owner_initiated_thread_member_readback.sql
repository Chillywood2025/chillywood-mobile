-- Allow an explicitly invited direct-thread member to read an owner-created
-- Chi'lly Chat thread, without allowing normal users to create/open chats
-- targeting platform owners.
      ...
      or (
        ...
        and exists (
          ...
          where thread."thread_kind" = 'direct'
            and public.is_platform_owner_user(thread."created_by")
        )
      )
    )
  from actor;
...
comment on function public.can_access_chat_thread(uuid) is
  'Chat thread read/update gate. Members can read normal direct threads. Platform-owner threads stay hidden except to current owners or explicit members of owner-created direct threads, with account restriction and block checks preserved so incoming call banners resolve only valid readable direct threads.';
