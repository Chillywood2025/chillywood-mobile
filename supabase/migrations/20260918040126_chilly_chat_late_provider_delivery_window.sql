-- Extend only the authoritative ringing window so legitimate delayed APNs/FCM
-- presentation still leaves the recipient time to Answer. Accepted media, exact
-- account/thread/invite authority, terminal cleanup, and room access are unchanged.

set check_function_bodies = false;

alter table public."chat_call_invites"
  alter column "expires_at"
  set default (timezone('utc'::text, now()) + interval '90 seconds');
...
comment on function public."begin_chilly_chat_call"(uuid, text, text) is
  'Atomically begins a direct Chi''lly Chat call with a 90-second ringing authority window that tolerates bounded native-provider delivery delay while preserving exact accepted-room and terminal-cleanup authority.';
