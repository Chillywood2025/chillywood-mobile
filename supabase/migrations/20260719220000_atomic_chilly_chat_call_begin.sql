-- Serialize direct Chi'lly Chat call initiation per thread so simultaneous taps
-- cannot leave both participants acting as callers or overwrite the winning room.
...
comment on function public."begin_chilly_chat_call"(uuid, text, text) is
  'Atomically selects one ringing/active direct-thread call. Simultaneous reverse-direction starts reuse the winning invite and close only the losing candidate room; delivery remains a separate bounded dispatch.';
