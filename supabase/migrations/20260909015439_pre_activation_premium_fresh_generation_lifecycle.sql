...
alter table public."revenuecat_premium_transaction_authority"
  ...
  check (
    "product_change_projection_event_id" is null or (
      length("product_change_projection_event_id") between 1 and 512 and "product_change_projection_event_id" !~ '[[:cntrl:]]'
    )
  );
...
as $$
  select lower(trim(coalesce(p_provider,''))) in (
    ...
    ) and p_user_id is not null and lower(trim(coalesce(p_environment,'')))='sandbox' and nullif(trim(coalesce(p_original_transaction_id,'')),'') is not null and exists (
      select 1
      ...
      join public."provider_events" anchor
       ...
       and anchor."metadata"->>'unresolved_non_premium_quarantine_preserved'='true' and (
         (anchor."event_type"='INITIAL_PURCHASE' and anchor."metadata"->>'premium_fresh_authority_event_type'='INITIAL_PURCHASE') or (anchor."event_type"='RECONCILIATION'
           ...
           and anchor."metadata"->>'provider_snapshot_only'='true' and anchor."metadata"->>'provider_transaction_created'='false')
       )
        ...
        and transaction_authority."user_id"=p_user_id and transaction_authority."environment"='sandbox' and public."revenuecat_premium_fresh_authority_allowed_internal"(
          ...
          where quarantine."provider_scope"=transaction_authority."provider" and quarantine."user_id"=transaction_authority."user_id"
            and quarantine."environment_scope"=transaction_authority."environment" and quarantine."event_type"='TRANSFER' and quarantine."reason"=
              'terminal_identity_invalid:transfer_store_identity_missing_or_unsupported' and not exists (
              select 1
...
as $$
  select public."revenuecat_premium_post_quarantine_generation_internal"(
    ...
    ) and not public."is_account_access_restricted"(p_user_id::text) and exists (
      select 1
      from public."revenuecat_premium_transaction_authority" transaction_authority
      join public."provider_events" latest
        on latest."provider"=transaction_authority."provider" and latest."provider_event_id"=transaction_authority."latest_event_id"
      ...
      where transaction_authority."provider"=lower(trim(p_provider)) and transaction_authority."original_transaction_id"=p_original_transaction_id
        ...
        and latest."status"='ignored' and latest."event_type"='RENEWAL' and latest."metadata"->>'final_reason' in (
          'revenuecat_terminal_authority_quarantined',
          'premium_original_transaction_terminal_or_blocked'
        ) and coalesce(latest."metadata"->>'authority_granted','false')='false' and coalesce(latest."metadata"->>'money_action','false')='false' and exists (
          select 1
          ...
          where ignored_renewal."provider"=transaction_authority."provider" and ignored_renewal."user_id"=transaction_authority."user_id"
            ...
            and ignored_renewal."event_type"='RENEWAL' and ignored_renewal."metadata"->>'final_reason' in (
              'revenuecat_terminal_authority_quarantined',
              'premium_original_transaction_terminal_or_blocked'
            )
        ) and not exists (
          ...
          where intervening."provider"=transaction_authority."provider" and intervening."user_id"=transaction_authority."user_id"
            and intervening."environment"=transaction_authority."environment" and intervening."metadata"->>'original_transaction_id'
              ...
              intervening."status"='ignored' and intervening."event_type"='RENEWAL' and intervening."metadata"->>'premium_transaction_binding_id'
                =transaction_authority."id"::text and intervening."metadata"->>'final_reason' in (
                'revenuecat_terminal_authority_quarantined',
                'premium_original_transaction_terminal_or_blocked'
              ) and coalesce(intervening."metadata"->>'authority_granted','false')='false' and coalesce(intervening."metadata"->>'money_action','false')='false'
            )
        ) and exists (
          select 1
    ...
    where not exists (
      ...
      where resolution."quarantine_id"=quarantine."id"
        ...
        and quarantine."environment_scope"=p_environment and quarantine."event_type"='TRANSFER' and quarantine."reason"=
          'terminal_identity_invalid:transfer_store_identity_missing_or_unsupported' and exists (
          select 1
          from public."revenuecat_premium_transaction_authority" transaction_authority
          where transaction_authority."provider"=p_provider and transaction_authority."user_id"=p_user_id and transaction_authority."environment"=p_environment and (
              (transaction_authority."authority_state"='pending' and (
                (
                  transaction_authority."latest_event_type"='INITIAL_PURCHASE' and transaction_authority."first_event_id"
...
begin
  ...
  if v_provider in ('revenuecat_app_store','revenuecat_google_play') and p_user_id is not null and v_environment in ('sandbox','production')
  then
  ...
  order by event."created_at",event."id" limit 1 for update;
  ...
  then
    return jsonb_build_object(
      'status','processed','reason','premium_provider_event_already_processed',
      'eventType',v_event_type,'eventId',v_event_id,'userId',p_user_id,
      'productKey',v_existing_event."product_key",'providerEventId',v_existing_event."id",
      'accessGrantId',(select grant_row."id" from public."access_grants" grant_row
        where grant_row."provider_event_id"=v_existing_event."id" limit 1),
      'ledgerEventId',(select ledger."id" from public."money_access_ledger_events" ledger
        where ledger."provider_event_id"=v_existing_event."id" limit 1),
      'environment',v_environment,
      'entitlementActive',public."premium_subject_has_finite_authority_internal"(p_user_id::text),
      'duplicateEvent',true,
      'duplicateAccessGrant',exists(select 1 from public."access_grants" grant_row
        where grant_row."provider_event_id"=v_existing_event."id"),
      'duplicateLedgerEvent',exists(select 1 from public."money_access_ledger_events" ledger
  ...
  then
    if v_event_type='PRODUCT_CHANGE' then
      ...
      set "product_change_projection_event_id"=v_event_id,
          "updated_at"=v_now
  ...
  if v_stage_generation then
    if v_stage_product_change then
      update public."revenuecat_premium_transaction_authority" transaction_authority
      set "product_change_projection_event_id"=null,
          "updated_at"=v_now
    ...
    elsif not v_stage_product_change and exists (
      select 1
      from public."revenuecat_premium_transaction_authority" transaction_authority
...
begin
  ...
  select event.* into v_existing_event
  from public."provider_events" event
  where event."provider"='revenuecat_app_store' and event."idempotency_key"='RECONCILIATION:'||v_snapshot_id
  limit 1 for update;
  if v_existing_event."id" is not null then
    if v_binding."id" is null or v_existing_event."provider_event_id"<>v_snapshot_id or v_existing_event."event_type"<>'RECONCILIATION'
      or v_existing_event."user_id" is distinct from p_user_id or v_existing_event."product_id" is distinct from v_binding."current_product_id"
      ...
      or v_existing_event."metadata"->>'original_transaction_id'
        ...
        is distinct from trim(coalesce(p_provider_product_id,''))
    then
      raise exception 'premium_reconciliation_duplicate_identity_mismatch';
    end if;
    return jsonb_build_object(
      'status','duplicate_ignored','reason','premium_reconciliation_already_processed',
      'eventType','RECONCILIATION','eventId',v_snapshot_id,'userId',p_user_id,
      'environment','sandbox',
      'entitlementActive',public."premium_subject_has_finite_authority_internal"(p_user_id::text),
      'duplicateEvent',true,'liveMoneyAction',false,'providerTransactionCreated',false,
      'postQuarantineGenerationLifecycle',true
    );
  end if;
  ...
  if v_binding."id" is not null and v_binding."user_id"=p_user_id and v_binding."environment"='sandbox' and v_binding."authority_state" in ('active','retained')
    ...
    and trim(coalesce(p_provider_product_id,''))=v_binding."current_provider_product_id" and length(p_provider_product_id)<=512 and p_provider_product_id!~'[[:cntrl:]]'
    ...
    and p_observed_at is not null and p_observed_at<=v_now+interval '5 minutes' and p_observed_at>=p_starts_at and public."revenuecat_premium_post_quarantine_generation_internal"(
      ...
      where mapping."product_id"=v_binding."current_product_id" and mapping."concept"='premium' and mapping."platform"='ios'
        ...
        and mapping."unlocks_digital_access" and not mapping."grants_livekit_authority" and not mapping."creates_payable_balance"
    ) and (p_starts_at,3,v_snapshot_id collate "C")<=(
      v_binding."latest_occurred_at",v_binding."latest_event_rank",
      v_binding."latest_event_id" collate "C"
    )
  then
    return jsonb_build_object(
      'status','duplicate_ignored','reason','premium_reconciliation_snapshot_stale',
      'eventType','RECONCILIATION','eventId',v_snapshot_id,'userId',p_user_id,
      'environment','sandbox',
      'entitlementActive',public."premium_subject_has_finite_authority_internal"(p_user_id::text),
      'duplicateEvent',true,'liveMoneyAction',false,'providerTransactionCreated',false,
...
set search_path = ''
as $$
  select nullif(trim(coalesce(p_user_id,'')),'') is not null and not public."is_account_access_restricted"(p_user_id) and exists (
      select 1
      from public."user_entitlements" entitlement
      join public."access_grants" grant_row
       ...
       and grant_row."starts_at"=entitlement."starts_at" and grant_row."expires_at"=entitlement."expires_at"
      ...
      where entitlement."user_id"=p_user_id and entitlement."entitlement_key"='premium' and entitlement."source"='revenuecat'
        ...
        and grant_row."expires_at">timezone('utc'::text,now()) and (
          ...
          join public."monetization_products" later_product
            on later_product."id"=later_authority."product_id" and later_product."product_type"='premium_subscription'
