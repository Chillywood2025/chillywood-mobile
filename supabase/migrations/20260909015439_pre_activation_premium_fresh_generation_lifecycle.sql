...
create or replace function public."revenuecat_premium_post_quarantine_generation_internal"(
  p_provider text,
  p_user_id uuid,
  p_environment text,
...
set search_path = ''
as $$
  select lower(trim(coalesce(p_provider,''))) in (
    ...
    and exists (
      ...
      join public."provider_events" anchor
        on anchor."provider"=transaction_authority."provider"
       and anchor."provider_event_id"=transaction_authority."first_event_id"
       and anchor."raw_payload_hash"=transaction_authority."first_event_hash"
       and anchor."user_id"=transaction_authority."user_id"
       and anchor."environment"=transaction_authority."environment"
       and anchor."status"='processed'
       and anchor."metadata"->>'original_transaction_id'
         =transaction_authority."original_transaction_id"
       and anchor."metadata"->>'premium_transaction_binding_id'
         =transaction_authority."id"::text
       and anchor."metadata"->>'premium_fresh_authority_after_transfer_quarantine'='true'
       and anchor."metadata"->>'unresolved_non_premium_quarantine_preserved'='true'
       and (
         (anchor."event_type"='INITIAL_PURCHASE'
           and anchor."metadata"->>'premium_fresh_authority_event_type'='INITIAL_PURCHASE')
         or (anchor."event_type"='RECONCILIATION'
           and anchor."metadata"->>'premium_fresh_authority_event_type'='RECONCILIATION'
           and anchor."metadata"->>'reconciliation_source'='revenuecat_v2_current_customer'
           and anchor."metadata"->>'provider_snapshot_only'='true'
           and anchor."metadata"->>'provider_transaction_created'='false')
       )
      join public."monetization_products" product
        on product."id"=anchor."product_id"
       and product."product_type"='premium_subscription'
      where transaction_authority."provider"=lower(trim(p_provider))
        ...
        and public."revenuecat_premium_fresh_authority_allowed_internal"(
          transaction_authority."provider",transaction_authority."user_id",
          transaction_authority."environment",transaction_authority."original_transaction_id",
          anchor."metadata"->>'premium_fresh_authority_event_type',
          case when anchor."event_type"='RECONCILIATION'
            then anchor."created_at" else anchor."occurred_at" end
        )
        and exists (
          select 1
          from public."revenuecat_terminal_authority_quarantines" quarantine
          where quarantine."provider_scope"=transaction_authority."provider"
            and quarantine."user_id"=transaction_authority."user_id"
            and quarantine."environment_scope"=transaction_authority."environment"
...
revoke all on function public."revenuecat_premium_post_quarantine_generation_internal"(
  text,uuid,text,text
) from public,anon,authenticated,service_role;
comment on function public."revenuecat_premium_post_quarantine_generation_internal"(
  text,uuid,text,text
...
create or replace function public."revenuecat_premium_quarantine_bug_recoverable_internal"(
  p_provider text,
  p_user_id uuid,
  p_environment text,
  p_original_transaction_id text
...
set search_path = ''
as $$
  select public."revenuecat_premium_post_quarantine_generation_internal"(
      p_provider,p_user_id,p_environment,p_original_transaction_id
    )
    and not public."is_account_access_restricted"(p_user_id::text)
    and exists (
      select 1
      from public."revenuecat_premium_transaction_authority" transaction_authority
      join public."provider_events" latest
        on latest."provider"=transaction_authority."provider"
       and latest."provider_event_id"=transaction_authority."latest_event_id"
       and latest."raw_payload_hash"=transaction_authority."latest_event_hash"
       and latest."user_id"=transaction_authority."user_id"
       and latest."environment"=transaction_authority."environment"
       and latest."metadata"->>'original_transaction_id'
         =transaction_authority."original_transaction_id"
       and latest."metadata"->>'premium_transaction_binding_id'
         =transaction_authority."id"::text
      where transaction_authority."provider"=lower(trim(p_provider))
        and transaction_authority."original_transaction_id"=p_original_transaction_id
        and transaction_authority."user_id"=p_user_id
        and transaction_authority."environment"='sandbox'
        and transaction_authority."authority_state"='blocked'
        and latest."status"='ignored'
        and latest."event_type"='RENEWAL'
        and latest."metadata"->>'final_reason' in (
          'revenuecat_terminal_authority_quarantined',
          'premium_original_transaction_terminal_or_blocked'
        )
        and coalesce(latest."metadata"->>'authority_granted','false')='false'
        and coalesce(latest."metadata"->>'money_action','false')='false'
        and exists (
          select 1
          from public."provider_events" ignored_renewal
          where ignored_renewal."provider"=transaction_authority."provider"
            and ignored_renewal."user_id"=transaction_authority."user_id"
            and ignored_renewal."environment"=transaction_authority."environment"
            and ignored_renewal."metadata"->>'original_transaction_id'
              =transaction_authority."original_transaction_id"
            and ignored_renewal."metadata"->>'premium_transaction_binding_id'
              =transaction_authority."id"::text
            and ignored_renewal."provider_event_id"<>transaction_authority."first_event_id"
            and ignored_renewal."status"='ignored'
            and ignored_renewal."event_type"='RENEWAL'
            and ignored_renewal."metadata"->>'final_reason' in (
              'revenuecat_terminal_authority_quarantined',
              'premium_original_transaction_terminal_or_blocked'
            )
        )
        and not exists (
          select 1
          from public."provider_events" intervening
          where intervening."provider"=transaction_authority."provider"
            and intervening."user_id"=transaction_authority."user_id"
            and intervening."environment"=transaction_authority."environment"
            and intervening."metadata"->>'original_transaction_id'
              =transaction_authority."original_transaction_id"
            and intervening."provider_event_id"<>transaction_authority."first_event_id"
            and not (
              intervening."status"='ignored'
              and intervening."event_type"='RENEWAL'
              and intervening."metadata"->>'premium_transaction_binding_id'
                =transaction_authority."id"::text
              and intervening."metadata"->>'final_reason' in (
                'revenuecat_terminal_authority_quarantined',
                'premium_original_transaction_terminal_or_blocked'
              )
              and coalesce(intervening."metadata"->>'authority_granted','false')='false'
              and coalesce(intervening."metadata"->>'money_action','false')='false'
            )
        )
        and exists (
          select 1
          from public."user_entitlements" entitlement
          join public."provider_events" entitlement_event
            on entitlement_event."provider_event_id"
              =entitlement."metadata"->>'revenuecat_event_id'
           and entitlement_event."raw_payload_hash"
              =entitlement."metadata"->>'revenuecat_event_hash'
           and entitlement_event."provider"=transaction_authority."provider"
           and entitlement_event."user_id"=transaction_authority."user_id"
           and entitlement_event."environment"=transaction_authority."environment"
        ...
        and not exists (
          select 1
          from public."user_entitlements" production_entitlement
          where production_entitlement."user_id"=p_user_id::text
            and production_entitlement."entitlement_key"='premium'
            and production_entitlement."source"='revenuecat'
            and production_entitlement."metadata"->>'environment'='production'
        )
    );
$$;

revoke all on function public."revenuecat_premium_quarantine_bug_recoverable_internal"(
  text,uuid,text,text
) from public,anon,authenticated,service_role;
comment on function public."revenuecat_premium_quarantine_bug_recoverable_internal"(
  text,uuid,text,text
      ...
      and not (
        ...
        and exists (
          ...
          where transaction_authority."provider"=p_provider
            ...
            and (
              (
                transaction_authority."latest_event_type"='INITIAL_PURCHASE'
                and transaction_authority."first_event_id"
                  =transaction_authority."latest_event_id"
                and transaction_authority."first_event_hash"
                  =transaction_authority."latest_event_hash"
              ...
              or public."revenuecat_premium_post_quarantine_generation_internal"(
                transaction_authority."provider",transaction_authority."user_id",
                transaction_authority."environment",
                transaction_authority."original_transaction_id"
              )
            )
        )
...
) rename to "process_revenuecat_premium_event_atomic_pre_gen_lifecycle";

revoke all on function public."process_revenuecat_premium_event_atomic_pre_gen_lifecycle"(
  text,text,text,uuid,text,text,text,text,timestamptz,timestamptz,timestamptz,
  integer,text,text,text,text,text,uuid,uuid,text
) from public,anon,authenticated,service_role;
...
begin
  ...
  if v_binding."id" is not null
    ...
    and not exists (
      ...
      where event."provider" in (
        'revenuecat','revenuecat_app_store','revenuecat_google_play'
      ) and event."provider_event_id"=v_event_id
    )
    and v_binding."user_id"=p_user_id
    and v_binding."environment"=v_environment
    and v_environment='sandbox'
    and (
      v_candidate_at,
    ...
    and public."revenuecat_premium_post_quarantine_generation_internal"(
      v_provider,p_user_id,v_environment,v_original_transaction_id
    )
    and (
      v_binding."authority_state" in ('active','retained')
      or public."revenuecat_premium_quarantine_bug_recoverable_internal"(
        v_provider,p_user_id,v_environment,v_original_transaction_id
      )
    )
  then
  ...
  v_result:=public."process_revenuecat_premium_event_atomic_pre_gen_lifecycle"(
    p_provider,p_provider_event_id,p_event_type,p_user_id,p_provider_product_id,
    p_provider_base_plan_id,p_environment,p_entitlement_status,p_starts_at,p_expires_at,
    p_occurred_at,p_amount_minor,p_currency,p_raw_payload_hash,p_period_type,p_store,
  ...
  if v_stage_generation then
    if coalesce(v_result->>'status','')='processed' then
      update public."revenuecat_premium_transaction_authority" transaction_authority
      set "current_product_id"=event."product_id",
          "current_provider_product_id"=event."metadata"->>'provider_product_id',
          "current_provider_base_plan_id"=event."metadata"->>'provider_base_plan_id',
          "latest_event_id"=event."provider_event_id",
          "latest_event_hash"=event."raw_payload_hash",
          "latest_event_type"=event."event_type",
          "latest_occurred_at"=event."occurred_at",
          "latest_event_rank"=public."revenuecat_premium_authority_rank_internal"(
    ...
    if coalesce(v_result->>'status','')='processed' then
      ...
      set "metadata"=coalesce(event."metadata",'{}'::jsonb)||jsonb_build_object(
        'premium_post_quarantine_generation_lifecycle',true,
        'premium_post_quarantine_anchor_event_id',v_binding."first_event_id",
      ...
      v_result:=v_result||jsonb_build_object(
        'entitlementActive',
          public."premium_subject_has_finite_authority_internal"(p_user_id::text),
...
revoke all on function public."reconcile_revenuecat_premium_snapshot_pre_generation_lifecycle"(
  text,uuid,text,text,text,text,timestamptz,timestamptz,timestamptz,text
) from public,anon,authenticated,service_role;
...
begin
  ...
  if v_binding."id" is not null
    ...
    and v_binding."user_id"=p_user_id
    and v_binding."environment"='sandbox'
  ...
  v_result:=public."reconcile_revenuecat_premium_snapshot_pre_generation_lifecycle"(
      ...
      where entitlement."user_id"=p_user_id
        ...
        and (
          ...
          or (
            ...
            and provider_event."metadata"->>'reconciliation_source'
              ='revenuecat_v2_current_customer'
            and provider_event."metadata"->>'provider_snapshot_only'='true'
            and provider_event."metadata"->>'provider_transaction_created'='false'
            and public."revenuecat_premium_fresh_authority_allowed_internal"(
              provider_event."provider",provider_event."user_id",
              provider_event."environment",
              provider_event."metadata"->>'original_transaction_id',
              'RECONCILIATION',provider_event."created_at"
            )
          )
        )
        and not exists (
