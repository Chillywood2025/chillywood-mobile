-- A newly-created ordinary Live room is not ready until its exact authenticated
-- creator also has the canonical host membership. Establish that membership in
-- the same database transaction as room creation so Waiting Room -> Live Room
-- cannot race an empty membership table.
