...
comment on function public."get_admin_system_history_read_model"(text, integer) is
  'Admin-safe System history read model over immutable audit/event tables, including provider readiness and provider webhook evidence when backed. Casts every union column to public-safe scalar text/number fields and never returns secrets, provider keys, provider payload values, LiveKit tokens, raw room tokens, or metadata values.';
