...
values
  ...
  ('af200000-0000-4000-8000-000000000002',false,false),
  ('af200000-0000-4000-8000-000000000003',false,false),
...
as $$
  select public.process_revenuecat_premium_event_atomic(
    'revenuecat_google_play',p_event_id,p_event_type,p_user_id,
    product.provider_product_id,product.provider_base_plan_id,
    'sandbox',p_entitlement_status,p_starts_at,p_expires_at,p_occurred_at,
    999,'usd',p_hash,'NORMAL','google_play','android',null,product.id,
...
select lives_ok(
  $$insert into public.revenuecat_terminal_authority_quarantines(
      ...
      event_type,raw_payload_hash,reason,created_at
    ) values (
      'revenuecat_google_play','sandbox','af200000-0000-4000-8000-000000000001',
      'generation-lifecycle-quarantine','TRANSFER',repeat('1',64),
...
select is(
  pg_temp.apply_google_premium_lifecycle(
    'generation-lifecycle-initial','INITIAL_PURCHASE',
    'af200000-0000-4000-8000-000000000001','generation-lifecycle-original',
    'active',clock_timestamp()-interval '13 hours',clock_timestamp()+interval '30 days',
    clock_timestamp()-interval '12 hours',repeat('2',64)
  )->>'status',
  'processed',
  'a newer exact sandbox initial purchase admits the Premium generation'
);
select ok(
  public.revenuecat_premium_post_quarantine_generation_internal(
    'revenuecat_google_play','af200000-0000-4000-8000-000000000001',
    'sandbox','generation-lifecycle-original'
  ),
...
select ok(
  ...
  'the backend Premium resolver follows the renewed exact generation'
);
select is(
...
select is(
  (select metadata->>'premium_post_quarantine_generation_lifecycle'
   from public.provider_events
   where provider_event_id='generation-lifecycle-renewal'),
  'true',
...
select pg_temp.apply_google_premium_lifecycle(
    ...
    'af200000-0000-4000-8000-000000000001','generation-lifecycle-original',
    'expired',clock_timestamp()-interval '31 days',clock_timestamp()-interval '1 hour',
    clock_timestamp()-interval '1 hour',repeat('4',64)
  ) result;
select is(
  (select result->>'status' from expiration_result),
  'processed',
  'a provider-signed expiration still terminates the exact generation'
);
select ok(
  not public.premium_subject_has_finite_authority_internal(
    'af200000-0000-4000-8000-000000000001'
  ),
  'the expired generation no longer resolves as Premium'
);
select is(
  pg_temp.apply_google_premium_lifecycle(
    'generation-lifecycle-after-expiration','RENEWAL',
    'af200000-0000-4000-8000-000000000001','generation-lifecycle-original',
    'active',clock_timestamp()-interval '30 minutes',clock_timestamp()+interval '30 days',
    clock_timestamp(),repeat('5',64)
  )->>'reason',
...
select lives_ok(
  $$select public.quarantine_revenuecat_terminal_authority(
    ...
    'af200000-0000-4000-8000-000000000003','sandbox',repeat('6',64),
    'terminal_identity_invalid:transfer_store_identity_missing_or_unsupported'
  )$$,
...
select is(
  pg_temp.apply_google_premium_lifecycle(
    'recoverable-generation-initial','INITIAL_PURCHASE',
    'af200000-0000-4000-8000-000000000003','recoverable-generation-original',
    'active',clock_timestamp()-interval '1 minute',clock_timestamp()+interval '30 days',
    clock_timestamp(),repeat('7',64)
  )->>'status',
  'processed',
...
begin
  ...
  perform public.record_revenuecat_premium_ignored_internal(
    ...
    v_product.provider_base_plan_id,'sandbox',clock_timestamp(),repeat('8',64),v_product.id,
  ...
  set metadata=coalesce(event.metadata,'{}'::jsonb)||jsonb_build_object(
    'original_transaction_id','recoverable-generation-original',
    'premium_transaction_binding_id',v_binding_id,
    'authority_granted',false,'money_action',false
  ) where event.provider_event_id='recoverable-generation-rejected-renewal'
  returning event.* into strict v_event;
  update public.revenuecat_premium_transaction_authority authority
  set latest_event_id=v_event.provider_event_id,
      latest_event_hash=v_event.raw_payload_hash,
      latest_event_type='RENEWAL',latest_occurred_at=v_event.occurred_at,
      latest_event_rank=public.revenuecat_premium_authority_rank_internal('RENEWAL',false),
      authority_state='blocked',updated_at=clock_timestamp()
  where authority.id=v_binding_id;
...
select ok(
  public.revenuecat_premium_quarantine_bug_recoverable_internal(
    'revenuecat_google_play','af200000-0000-4000-8000-000000000003',
    'sandbox','recoverable-generation-original'
  ),
  'only the known ignored-renewal sequence is recoverable'
);
select is(
  pg_temp.apply_google_premium_lifecycle(
    'recoverable-generation-renewal','RENEWAL',
    'af200000-0000-4000-8000-000000000003','recoverable-generation-original',
    'active',clock_timestamp()-interval '1 minute',clock_timestamp()+interval '30 days',
...
select ok(
  ...
  'the recovered exact generation projects authoritative Premium'
);
select ok(
  not public.revenuecat_premium_quarantine_bug_recoverable_internal(
    'revenuecat_google_play','af200000-0000-4000-8000-000000000003',
    'sandbox','recoverable-generation-original'
  ),
...
select lives_ok(
  $$select public.quarantine_revenuecat_terminal_authority(
    ...
    'af200000-0000-4000-8000-000000000004','sandbox',repeat('a',64),
...
select is(
  pg_temp.apply_google_premium_lifecycle(
    'different-reason-generation-initial','INITIAL_PURCHASE',
    'af200000-0000-4000-8000-000000000004','different-reason-generation-original',
    'active',clock_timestamp()-interval '1 minute',clock_timestamp()+interval '30 days',
    clock_timestamp(),repeat('b',64)
  )->>'reason',
  'revenuecat_terminal_authority_quarantined',
  'the lifecycle repair cannot admit a different quarantine class'
);
select ok(
  not public.revenuecat_premium_post_quarantine_generation_internal(
    'revenuecat_google_play','af200000-0000-4000-8000-000000000004',
    'sandbox','different-reason-generation-original'
  ),
...
select lives_ok(
  $$select public.quarantine_revenuecat_terminal_authority(
    ...
    'af200000-0000-4000-8000-000000000005','sandbox',repeat('c',64),
    'terminal_identity_invalid:transfer_store_identity_missing_or_unsupported'
  )$$,
...
select is(
  (select public.reconcile_revenuecat_premium_snapshot_atomic(
    'restore-generation-first','af200000-0000-4000-8000-000000000005',
...
select ok(
  ...
  'the repeated exact Restore snapshot retains authoritative Premium'
);
select is(
  (select metadata->>'premium_post_quarantine_generation_lifecycle'
   from public.provider_events where provider_event_id='restore-generation-second'),
...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.revenuecat_premium_post_quarantine_generation_internal(text,uuid,text,text)',
    'EXECUTE'
  ),
...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.revenuecat_premium_quarantine_bug_recoverable_internal(text,uuid,text,text)',
    'EXECUTE'
  ),
...
select ok(
  ...
  'the prior Premium projector implementation is not directly service-callable'
);
select ok(
