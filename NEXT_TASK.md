...
- Media automation operator source/proof now exists for scale: `_lib/mediaAutomationController.ts`, `_lib/chillywoodAutonomyPolicy.ts`, `_lib/mediaAutomationDiscovery.ts`, `_lib/mediaAutomationBatchPolicy.ts`, `_lib/mediaAutomationBackfillPolicy.ts`, `_lib/mediaAutomationJobs.ts`, `_lib/mediaAutomationQueueProcessor.ts`, `_lib/mediaAutomationWorkerLoop.ts`, `scripts/media-automation-cli.mjs`, disabled systemd templates under `ops/media-automation/systemd/`, `docs/MEDIA_AUTOMATION_OPERATOR_RUNBOOK.md`, and proofs `proof:media-automation-controller`, `proof:media-automation-discovery`, `proof:media-automation-batch-policy`, `proof:media-automation-backfill-policy`, `proof:media-automation-queue-processor`, `proof:media-automation-scheduler-templates`, `proof:media-automation-cli`, `proof:media-automation-batch-planner`, and `proof:media-automation-worker-loop`. It is source/proof/templates only: no live daemon, cron, scheduler, queue processor, production media processing, production DB writes, broad backfill, or playback broadening happened.
- CLI auto-detect media automation is now the normal source/proof path and has its first production read-only linked-Supabase readback: `media-automation:discover`, `media-automation:plan-auto`, and `media-automation:dry-run-auto` discover eligible public-safe videos and calculate a safe adaptive batch size without manual source ids or manual batch-size selection. The 2026-07-09 read-only run scanned `27` candidates and found `eligible_needs_transcode=0`, `eligible_already_has_audited_hls=1`, `excluded_private=12`, `excluded_premium=9`, `excluded_unscanned=5`, and zero original/master, moderation-blocked, missing-source, unsupported, active-job, denied, or already-processed rows. Auto plan/dry-run calculated `batchSize=0`, `riskLevel=blocked`, reason `no_eligible_candidates`, no selected candidates, and no rollback scopes. `media-automation:run-auto` remains fail-closed unless a future confirmed run supplies `MEDIA_AUTOMATION_RUN_CONFIRM=I_UNDERSTAND_AUTO_DETECT_BATCH` and backup/restore, active-job, unsafe-row, dry-run, audit, rollback, fallback, and emergency-stop gates all pass. `media-automation:run-continuous-once` is source/proof-only and fail-closed unless a future lane supplies `MEDIA_AUTOMATION_CONTINUOUS_ONCE_CONFIRM=I_UNDERSTAND_ONE_CONTINUOUS_LIMITED_CYCLE`; `media-automation:report` is read-only. No daemon, cron, scheduler, queue processor, production media processing, production DB writes, broad backfill, or playback broadening happened.
- Catalog readiness automation is source/proofed and read-only: `_lib/mediaCatalogReadiness.ts`, `scripts/media-catalog-readiness-cli.mjs`, `docs/MEDIA_CATALOG_READINESS_RUNBOOK.md`, `npm run media-catalog:status`, `npm run media-catalog:readiness-plan`, `npm run media-catalog:scan-plan`, and `npm run proof:media-catalog-readiness` classify catalog rows before transcode. Linked readback scanned `27` rows and found `ready_for_transcode=0`, `already_audited_hls=1`, `needs_scan=5`, `private_excluded=12`, `premium_excluded=9`, and zero moderation-review, original/master, missing-source, unsupported, blocked, or denied rows. The five `needs_scan` rows are scan candidates only; they are not eligible for transcode until real scanner proof and moderation-safe readback promote them. The readiness CLI does not execute scans, does not mark media clean, does not write production rows, does not process media, and does not switch playback.
- Catalog scan automation is source/proofed and production-write-disabled: `_lib/mediaScanAutomation.ts`, `scripts/media-scan-cli.mjs`, `npm run media-scan:status`, `npm run media-scan:plan`, `npm run media-scan:dry-run`, `npm run media-scan:run-auto`, `npm run proof:media-scan-automation`, and `npm run proof:media-scan-auto-cycle` classify public scan candidates, prove ffprobe media-readability behavior, and keep private/Premium/already-audited rows out of scan plans. This scanner proof is media readability only, not malware scanning or content moderation. First full autonomous scan-cycle attempt selected all five public scan candidates and skipped private/Premium rows. Trusted scan download is now source-wired for S3/Hetzner-backed candidates through the existing backend `media-storage create_download_url` path using `MEDIA_SCAN_DOWNLOAD_ACCESS_TOKEN`, so the CLI does not need raw local service-role or S3 keys and never prints signed URLs. Current runtime stopped before mutation with `trusted_scan_download_access_missing` because that access token was not present; Supabase Storage-backed candidates still need the existing service-role scanner worker path or a future backend stream path. Future production clean writes must use trusted scanner/service-role authority plus scanner name/version/proof/readback and moderation-safe state. No scan write, transcode, or playback switch occurred.
...
2. First production CLI auto-detect transcode cycle remains Pass-No-op: linked read-only discovery found `0` `eligible_needs_transcode` candidates and `1` already-audited HLS source, so `plan-auto`, `dry-run-auto`, and `media-automation:report` correctly selected no transcode batch, no rollback scopes, and no work to run. First full autonomous readiness -> scan -> transcode cycle is Partial at the scan-download/write gate. Next autonomous step is to provide `MEDIA_SCAN_DOWNLOAD_ACCESS_TOKEN` as a Codex/environment secret for the existing `media-storage create_download_url` backend path, or run the existing service-role scanner worker for Supabase Storage-backed candidates; do not paste credentials into chat or docs. Then rerun `MEDIA_SCAN_AUTO_CONFIRM=I_UNDERSTAND_PUBLIC_SCAN_BATCH npm run media-scan:run-auto`, audit scan readback, and only if rows become `ready_for_transcode` continue to `media-automation:plan-auto` / `dry-run-auto` / bounded `run-auto`. Do not run transcode `run-auto` or `run-continuous-once` until scanner proof and moderation-safe readback create eligible public-safe candidates and every backup/restore/audit/rollback/kill-switch/output-validation gate passes.
