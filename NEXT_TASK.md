...
Player shared/fullscreen follow-up is repo-side complete, with the final gutter/bubble reuse correction pending OTA publication. `app/player/[id].tsx` keeps standalone fullscreen video in cover mode, lets shared Watch-Party playback enter fullscreen, overlays compact Share / Report / speed controls without a Watch-Party Live handoff toggle, auto-hides shared Player chrome after 5 seconds idle, and keeps tap-to-play on the existing shared playback tap handler. Shared fullscreen keeps the video in contain/letterbox mode like the requested reference, reuses the regular shared Player participant bubble renderer on the right, and narrows the same room comments composer into the left black gutter so it does not cover the video. It no longer reuses the full Watch-Party social panel, placeholder shared-player card, duplicate LiveKit media surface, or a separately invented fullscreen bubble. Prior EAS Update group `808798e4-1010-49f3-aa4b-d189ff9a8232` / Android update `019e9a3e-9a9a-7f2c-81ea-71faec45c9ba` was published on runtime `1.0.0`; proof path `/tmp/chillywood-player-shared-fullscreen-polish-proof-20260605/` captures standalone normal/fullscreen from the prior OTA. Admitted shared-player fullscreen visual proof still needs a live/admitted Party Room session after testers pick up the next OTA. This did not change media resolver logic, playback sync authority, LiveKit token issuer, host approval, route ownership, Party Room behavior, old-room handling, money state, Premium/content safety, or Owner/Admin authority.
