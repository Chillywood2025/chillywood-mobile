...
- Autonomous systems are now protected by a top-level source registry/contract: `_lib/autonomousSystemsRegistry.ts` plus `docs/AUTONOMOUS_SYSTEMS_SCOPE_REGISTRY.md`. Current approved systems are `media_automation`, `livekit_operator`, `money_flow_control`, `notification_delivery_operator`, `release_ota_operator`, `security_owner_operator`, `moderation_safety_operator`, `observability_runtime_operator`, `installed_product_qa_operator`, `platform_recovery_operator`, `privacy_compliance_operator`, `support_success_operator`, `search_ranking_integrity_operator`, `ads_sponsor_delivery_operator` foundation-only, and protected control-plane `owner_command_operator`. Notification, release, security-owner, moderation-safety, observability, Money Operator provider reliability, platform recovery, privacy compliance, support success, and search/ranking integrity run limited scheduled `watch_once` loops on `chillywood-prod-01`: notification every five minutes in `limited_scheduled_safe_recovery`, release every thirty minutes, security-owner every fifteen minutes, moderation-safety every ten minutes, observability every ten minutes, Money Operator provider reliability every ten minutes, platform recovery every thirty minutes, privacy compliance every six hours, support success every thirty minutes, and search/ranking integrity every thirty minutes in `limited_scheduled_probe`. `installed_product_qa_operator` now runs `limited_scheduled_probe` Firebase uploaded-artifact smoke on `chillywood-prod-01` with `schedulerStatus=chillywood-installed-qa-firebase-smoke.timer_daily_cost_capped`; the bounded service posted `scheduler=systemd_timer` matrix state rows and the daily timer is enabled/active. Systems 2-5 are now live deployed: remote DB/RLS migrations applied, Edge Functions ACTIVE, narrow token hashes stored only as Supabase function secrets, raw tokens stored only in root-owned host env files, timers enabled/active/fired, and latest audit rows show `scheduler=systemd_timer`, exact `operator_id`, `money_moved=false`, and `user_rights_changed=false`. `ads_sponsor_delivery_operator` remains `foundation_only_guarded`, `off`, `no_scheduler_foundation_only`, with no Edge Function, no scheduler, and no live writes. `owner_command_operator` remains manual/owner-invoked only with no scheduler, daemon, or worker. Safe writes are limited to status, review, finding, planning, draft, escalation, duplicate-detection, audit, learning, owner-command, and autonomous approval-request records. Every scheduled run writes scheduler/operator identity (`scheduler=systemd_timer`, exact `operator_id`, host source) and preserves `moneyMoved=false`, `userRightsChanged=false`, and `highRiskExecuted=false`. They cannot send broad push campaigns, bypass notification preferences, publish or roll back production OTA, submit app-store releases, mutate owner roles, mutate auth/RLS, rotate secrets, ban/restrict users, delete content/accounts/data, change user rights, move money, refund, grant Premium, change provider products, delete crash evidence, silence crash reporting, collect extra PII, mutate Remote Config/feature flags, change provider analytics config, fake installed proof, claim two-device proof without devices/lab, restore production, bypass legal hold, send legal/payment commitments, shadowban/secretly demote/boost, serve ads, run sponsor checkout, claim ad revenue, or silently pass installed route mismatches without the registered Level 3/4 approval path where applicable. The installed QA Firebase bucket bootstrap is a bounded Level 2 exception only for `gs://chillywood-installed-qa-testlab-results` in `chillywood-app` with seven-day lifecycle, public access prevention, uniform bucket-level access, and narrow bucket-level runner IAM; billing enablement, arbitrary buckets, Owner/Editor, project-wide Storage Admin, physical Firebase devices, broad crawls, over-cap runs, Play-installed/Premium/two-device proof claims remain forbidden. Future autonomous scope can be added only through explicit registry entries with approval level, read/write bounds, proof, guard, rollback/quarantine, kill-switch/fallback, and owner/admin approval requirements. Level 3/4 actions create owner/admin approval requests through the live owner/super-admin-backed path: `platform_role_memberships` authority, canonical `/admin` review, deployed `autonomous-approval-request`, approval/denial RPCs, audited request events, emergency-state checks, and trusted-operator preflight/execution markers. Migrations `20260711173119_autonomous_approval_requests` and `20260711185503_autonomous_approval_live_flow` are applied remotely, and `autonomous-approval-request` is ACTIVE. Trusted operator markers use `AUTONOMOUS_APPROVAL_REQUEST_TOKEN_SHA256` when configured or the existing server-side `OPS_APPROVAL_TOKEN` fallback. Owner Command Operator is the owner-judgment execution layer: it classifies owner commands, routes them to the proper existing autonomous systems, writes `owner_command_*` audit/proof/blocker rows, executes only safe Level 0/1/2 report/scoped-audit work, and creates approval requests as `owner_command_operator` for Level 3/4 instead of bypassing target operators. Live no-op approval proof passed with owner sign-in, request creation, pre-approval execution blocked, owner approval, preflight-required execution blocked, fresh preflight pass, no-op execution, second request denial, denied execution blocked, and system status readback. Rachi can recommend/request but cannot approve itself, autonomous operators can request approval through trusted paths but cannot approve, approval does not execute automatically, and execution requires fresh preflight, exact scope match, unexpired approval, active system state, Level 4 external confirmation where applicable, and audit.
- Full app authority/product behavior source audit is closed in `docs/FULL_APP_AUTHORITY_PRODUCT_BEHAVIOR_AUDIT.md` with `proof:full-app-authority-product-audit` and `guard:full-app-authority-product-audit`. It inventories all current `app/` route files and Supabase Edge Functions, classifies public, signed-in, creator, Premium, LiveKit, media/playback, admin/operator, owner/super_admin, moderator, legal/static, diagnostic, and money/provider surfaces, and ties high-risk controls back to the admin action registry, role-action matrix, owner command operator, autonomous registry, and approval model. Current source was published to production OTA and loaded by the Play-installed app, but installed role/device traversal remains Partial. `docs/release/FULL_APP_AUTHORITY_PRODUCT_BEHAVIOR_OTA_INSTALLED_TRAVERSAL_PROOF.md` records production runtime `1.0.0` update group `dfbfe7c9-33f0-4c11-96b1-c9bb311dbd3a`, Android update `019f58bd-8ae2-7c9e-b9fc-56f09dce62ba`, source commit `7487ba31694b9d3c8b889a6f21cee01d516e5b57`, Play-installed package `com.chillywood.mobile`, installer `com.android.vending`, versionCode `80`, channel `production`, embedded/emergency launch `false`, and seeded traversal counts `Pass=84`, `Human review=25`, `Blocked=3`, `Two-device required=4`, `Failures=0`. Do not claim installed Closed yet. Safe next action is a focused current-OTA installed proof for: normal `/chat` route marker, creator `/creator-monetization-setup` compatibility marker, restricted account `/chat` denial/account-status truth, a provider-backed Premium-active account traversal or approved provider-backed sandbox renewal, and two-device realtime assertions if closure is requested. No sideload, `adb install`, clear-data, manual Premium grant, money movement, provider mutation, auth/RLS mutation, enforcement, media processing, or secret exposure occurred.
- 2026-07-13 authority blocker follow-up supersedes the normal `/chat` and creator `/creator-monetization-setup` blockers. Production OTA group `8e158980-75d1-47ef-bd26-f3f9e564fdab`, Android update `019f58f4-55ea-7d55-b1b1-6ea85fdb1c56`, source commit `32e56c3abb5c046e853a9c2f3d978ba72af447f4`, loaded on Play-installed `R5CR120QCBF` with runtime `1.0.0`, channel `production`, embedded/emergency launch `false`. Normal `/chat` shows `chat-inbox-screen`; the creator compatibility route shows Platform Studio `Premium required` / `Manage Premium`. A later installed QA artifact showed broad Admin Search visible for `proof_moderator_001`; current source now gates Admin Search behind `canUseAdminSearch`, but installed moderator closure requires a future OTA/load/rerun. Installed authority proof remains Partial for exact prerequisites: an actually restricted approved proof account/fixture, a provider-backed Premium-active installed account/session or approved Google Play / RevenueCat sandbox renewal, moderator Admin Search rerun after the source gate fix, and two Play-installed devices for realtime.
- Installed Product QA Operator now records the autonomous-system gap behind those findings. Codex caught the current installed traversal blockers manually, and the autonomous system did not catch them before because `installed_product_qa_operator` did not exist. Current blockers are open QA findings until safe installed proof closes them. Premium fixture repair is provider-backed only, two-device proof requires two Play-installed devices or approved device lab, and Firebase uploaded-artifact scheduler coverage is active only for bounded route/no-crash smoke. The operator may create Owner Command requests for safe source/proof/testID fixes but cannot fake installed proof, manually grant Premium, sideload/install/clear data, move money, mutate roles/auth/RLS, enforce moderation, expose private evidence, or claim two-device closure with one device.
- 2026-07-13 installed QA live deployment follow-up is Partial only for physical/account proofs. `installed-product-qa-operator` is deployed/ACTIVE, token-gated, and writes safe QA rows only; the raw token is stored only in `/etc/chillywood/installed-product-qa-operator.env` on `chillywood-prod-01` as `root:root` mode `600`, and public client writes to installed QA tables are denied by RLS. Current source commit `f6ac19fe0c85f4e28db715315e5829043f9fb3ed` was published to production Android OTA group `cdfd42a5-7c78-4cd0-9673-1f451073aa16`, Android update `019f596f-1a87-76d8-abe3-14342c8d1cf6`; Play-installed `R5CR120QCBF` loaded it with runtime `1.0.0`, channel `production`, embedded/emergency launch `false`, package `com.chillywood.mobile`, installer `com.android.vending`, versionCode `80`. Live `watch_once` recorded the six current blockers with `source=play_installed` and `discovered_by=autonomous_operator`, and created Owner Command request `8a322bf5-79e9-4146-a3c6-5936c9ba5c09`. Targeted current-OTA traversal proved normal `/chat` and `/creator-monetization-setup` markers pass. Still open: restricted `/chat` requires a truly restricted approved fixture/account, Premium active-user proof requires a provider-backed active installed session or approved provider-backed sandbox renewal, focused moderator proof did not finish after persona switching stalled, and two-device realtime requires two Play-installed devices or approved device lab. The Firebase uploaded-artifact installed-QA timer is installed and active, but it does not close those Play-installed/account/two-device findings.
- Firebase Test Lab is cost-capped cheap mode for installed QA. Source includes `scripts/installed-qa-firebase-test-lab.mjs`, `qa/firebase-test-lab/README.md`, package scripts `installed-qa-operator:firebase-test-lab:*`, short aliases `installed-qa:firebase-test-plan` / `installed-qa:firebase-test-run`, source support for `firebase_test_lab_uploaded_artifact`, and Linux systemd templates under `ops/installed-product-qa-operator/systemd/`. Tier 0 source/backend/operator checks run without device lab; Tier 1 Firebase virtual smoke is daily at most, on OTA/source change, or on owner command when cost guard passes. Defaults: `$5/month`, `$0.25/run`, `FIREBASE_TEST_LAB_RESULTS_BUCKET=chillywood-installed-qa-testlab-results`, virtual-only, physical disabled, broad crawls disabled, two-device Firebase disabled, and unbounded/over-cap cost blocked. One owner-command Tier 1 virtual smoke completed as Firebase matrix `4612242345700782646` with worst-case `costEstimateUsd=0.09`, `billingRisk=low`, and local ledger write; this is uploaded-artifact proof only. The owner Mac is manual/admin only, not the long-term runner or runtime credential. `chillywood-prod-01` has Linux/systemd, non-interactive root/sudo, `/etc/chillywood/installed-product-qa-operator.env` as `root:root` mode `600`, `/var/lib/chillywood/installed-qa` as `root:root` mode `750`, `/opt/chillywood/current` on `origin/main`, Ubuntu `nodejs` `18.19.1`, `npm` `9.2.0`, Google Cloud SDK `575.0.1`, and `firebase-tools@15.23.0`. Owner-authorized Google Console provisioning created dedicated service account `installed-qa-testlab-runner@chillywood-app.iam.gserviceaccount.com`; its JSON key is stored only on the server as `root:root` mode `600`, and server gcloud can read the Test Lab virtual model/version catalog. A current-source release APK for commit `4061e2bda8c77dad174b72378db483acb6e906fe` is stored on the server with SHA-256 `6bcddf44e9d320d152742537339f3204a7295cd7868554b34146f82c81528b44`. The server cost guard now passes at `costEstimateUsd=0.09`, virtual-only, no physical, no broad crawl, no two-device. The exact dedicated results bucket bootstrap is now a standing bounded Level 2 policy for only `gs://chillywood-installed-qa-testlab-results` in `chillywood-app`, seven-day lifecycle cleanup, public access prevention, uniform bucket-level access, and bucket-level IAM for `installed-qa-testlab-runner@chillywood-app.iam.gserviceaccount.com`; billing enablement/linking remains owner-only and arbitrary buckets, Owner/Editor, project-wide Storage Admin, physical devices, broad crawl, over-cap run, Play-installed claim, Premium/Billing claim, and two-device claim remain forbidden. The dedicated bucket path is now active: `gs://chillywood-installed-qa-testlab-results` exists with seven-day lifecycle and bucket-level `roles/storage.objectCreator` for the dedicated runner. Server matrix `8384645686869695525` completed from `chillywood-prod-01` using that bucket and posted live traversal row `333817c9-b651-4e65-ab66-0d885952d2de` with `source=firebase_test_lab_uploaded_artifact`, `discovered_by=device_lab`, and `cost_estimate_usd=0.09`. The installed-QA runner now uses bounded async matrix start/poll, pending-matrix tracking, and systemd start-time bounds so it cannot wait indefinitely in `gcloud firebase test android run`. SSH to `87.99.145.160` recovered before VNC was needed; diagnostics found sshd active, disk/memory healthy, no reboot/reset/sshd restart performed, and older high load likely explained the prior banner timeout. The bounded service started matrix `4625992954862995833`, posted `scheduler=systemd_timer` audit rows for `matrix_started`, `matrix_timeout` with `POLL_HTTP_FAILED`, and duplicate-safe `matrix_pending`, exited cleanly, and left the daily timer enabled/active with `schedulerStatus=chillywood-installed-qa-firebase-smoke.timer_daily_cost_capped`. The current matrix state is pending/poll-failed rather than completed device proof. Firebase uploaded artifact is not Play-installed proof and cannot close Premium/RevenueCat/Google Play Billing or two-device LiveKit proof.
- Final autonomous coverage expansion live deployment is closed for systems 2-5. Platform Recovery, Privacy Compliance, Support Success, and Search/Ranking Integrity are real scoped-write guarded operators with remote DB/RLS, deployed token-gated Edge Functions, fired safe `watch_once` proof, and hardened systemd timers. Ads/sponsor delivery remains foundation-only until the provider, business, billing, brand-safety, and payout path is explicitly approved and proofed.
- Money Flow & Ledger Control Plane is now `scoped_write_capable_guarded` with a scheduled provider-health loop: `_lib/moneyFlowControl.ts`, `_lib/moneyExternalConfirmation.ts`, `supabase/functions/money-operator/index.ts`, `docs/MONEY_FLOW_CONTROL_RUNBOOK.md`, `proof:money-flow-control`, `proof:money-operator-write-scope`, `proof:money-external-confirmation`, `proof:money-operator-scheduler`, `guard:money-operator-scheduler`, and `guard:money-flow-control` classify money actions and block fake revenue, fake creator earnings, fake payable balances, fake paid status, fake transfer completion, manual Premium grants, payout release without provider confirmation, customer charges from foundation tables, and test-mode data being called production. Read-only reconciliation/reporting plus scoped safe reconciliation/status/review/audit writes can be autonomous through the Money Operator tables; `chillywood-money-operator-watch-once.timer` runs every ten minutes and calls only `watch_once` with `moneyMoved=false` and `highRiskExecuted=false`. Production money setup/mutation requires Level 3 owner/super-admin approval; real money movement requires Level 4 owner/super-admin approval plus external provider confirmation/readback. `/admin` Money Center shows Money Operator status and safe write visibility only. Do not move real money, create payouts/cashout, charge customers, manually grant Premium, change provider products/modes, or make foundation money rows payable without the required approval and provider confirmation.
- Provider webhook reliability is now a cross-provider Money Operator loop with a controlled Provider Access Broker foundation. Protected surfaces are `revenuecat_webhook_delivery`, `google_play_webhook_delivery`, `stripe_connect_webhook_delivery`, `stripe_merch_webhook_delivery`, `provider_readiness_audit`, `provider_delivery_error_rate`, `stale_provider_dashboard_integration_detection`, `duplicate_webhook_integration_detection`, `provider_access_broker`, `provider_dashboard_readback`, and `provider_test_delivery_status`. Package scripts now include `money-operator:provider-health`, `money-operator:watch-once`, `money-operator:report`, `money-operator:access-status`, `money-operator:provider-access-probe`, `money-operator:provider-dashboard-readback`, `money-operator:provider-test-plan`, `money-operator:provider-test-run`, `money-operator:provider-repair-request`, `proof:provider-webhook-reliability`, `proof:money-provider-reliability-loop`, `proof:provider-access-broker`, `proof:provider-dashboard-access-policy`, `guard:provider-webhook-reliability`, and `guard:provider-access-broker`. Provider Access Broker records available access modes, required credential names only, provider readback/test-delivery status, stale/duplicate dashboard findings, and repair approval requests without printing secrets or mutating provider dashboards. RevenueCat provider readback is closed: the read-only v2 Secret API key is stored as `REVENUECAT_SECRET_API_KEY` in trusted runtime secrets, Money Operator readback works, active integration count is `1`, endpoint host/path matches `bmkkhihfbmsnnmcqkoly.supabase.co/functions/v1/revenuecat-webhook`, and dashboard TEST returned HTTP `200` / `test_received` with `premiumGranted=false`, `liveMoneyAction=false`, and `moneyMoved=false`. Google Play direct webhook is readiness-only/stale when RevenueCat remains entitlement source of truth. Stripe Connect and Stripe merch webhook functions fail closed on invalid signatures. Error-rate classes are healthy/degraded/critical/outage/unknown; a 100% provider webhook error rate must record failed/blocked sync status plus a reconciliation finding, and dashboard mutation creates an approval request. Do not change provider dashboards, event selections, webhook signing config, products, live mode, Premium entitlements, payouts, charges, transfers, cashout, invoices, or payment links without the existing Level 3/4 approval path and external confirmation where required.
- Global LiveKit router eligibility is restored after the 2026-07-10/11 `no_eligible_livekit_server` incident. The blocker was not R2, object storage, Premium, or the Live tab button; the host heartbeat monitor was running but hit a missing Supabase function blob for `livekit-heartbeat-monitor`, leaving `chillywood-prod-01` stale in the router even though the host, Docker, Caddy, LiveKit, and `wss://live.chillywoodstream.com` were healthy. The operational repair redeployed the existing heartbeat monitor Edge Function and ran the legitimate host monitor path; router readback showed fresh `livekit-heartbeat-monitor` heartbeats under the 120-second cutoff and no fresh no-eligible blocker. Play-installed one-device proof on `R5CR120QCBF` renewed Premium through Google Play / RevenueCat sandbox, entered Live Stage, received a `live-stage` host/canPublish token contract, and rendered the local camera surface. Do not reopen R2/storage/Premium for this class of issue. Next LiveKit proof, if requested, should be a fresh two-device Watch-Party Live / Party Room sidecar token/render proof across all routed LiveKit surfaces, not another Live tab-only diagnosis.
- LiveKit Autonomous Operator is active as a limited scheduled safe-recovery loop through the hardened host timer `chillywood-livekit-operator-watch-once.timer` on `chillywood-prod-01`. It covers all LiveKit surfaces (`live-stage`, `watch-party-live`, Party Room live sidecar, `chat-call` where routed, `livekit-token`, router, heartbeat monitor, host-agent recommendation) with scoped operator tables, a token-gated `livekit-operator` Edge Function, CLI probes, proofs, policy guards, and a five-minute `watch_once` schedule. GitHub Actions remains blocked because the current GitHub OAuth token lacks `workflow` scope, Supabase CLI `2.75.0` exposes no scheduler command here, and Cloudflare Cron attachment was blocked by Cloudflare error `10063` requiring a Workers subdomain. Migration `20260711043323_livekit_autonomous_operator` is applied and `livekit-operator` is deployed. A fresh narrow operator token hash is stored in Supabase and the matching token is stored only in `/etc/chillywood/livekit-operator.env` on the host. Live manual and scheduled `watch_once` returned `healthy`, `eligible_server_available`, `eligibleServerCount=1`, planned `audit_only`, and executed no recovery; scheduled runs call the same `watch_once` action with `LIVEKIT_OPERATOR_ENABLE_SAFE_RECOVERY=true` and audit into `livekit_operator_events`. Safe recovery can run only Level 1/2 scoped actions such as legitimate heartbeat monitor/counter refresh paths and audited operator learning writes. Client render/token telemetry now captures bounded token `nbf` grace, future-`nbf` rejection, renderable-contract clearing, fallback roster shown/suppressed, surface mount, bubble grid, camera preparing/track, identity mismatch, and connection-state events. Authenticated app users may submit sanitized telemetry only; they cannot trigger recovery or direct health writes. Missing operator token denies with `401 operator_token_required`; unauthenticated render telemetry denies with `401 authenticated_user_required`. Do not loosen stale heartbeat cutoffs, write fake heartbeats, mark unhealthy servers healthy, broaden DB writes, auto-publish source OTA from telemetry, or treat Premium sandbox success as LiveKit proof. Watch-Party Live Shared Player fallback-bubble smoothing now keeps a stable LiveKit connecting shell for a short grace window instead of flashing the separate fallback roster while a valid renderable contract/token refresh is in progress.
- Play-installed OTA proof should now use the new release diagnostics card in Settings > App Info. The card exposes only non-secret release identity fields (`updateId`, `runtimeVersion`, channel, native app/build version, embedded/emergency launch, update created time, and latest safe update-check status) and a copy button with proof test IDs. Use `proof:release-diagnostics` plus the installed card to prove the exact OTA running in the Play release build; do not infer closure only from Expo `CheckCompleteUnavailable` when the installed update id is required.
- Hetzner Object Storage -> Cloudflare R2 private origin migration is closed for shutdown readiness by active-reference semantics, and the final shutdown/export packet is complete. Private R2 origin bucket `chillywood-media-origin` exists, trusted backend copier Edge Function `media-object-storage-migration` is deployed and token-gated, and R2 origin write config is present as Supabase secrets without printing values. Initial inventory found `31` Hetzner/S3 row refs (`22` distinct object refs); `16` existing distinct refs were copied/readback verified in R2 private origin, storage-metadata backup/restore closed at `backups/media-object-storage/storage-metadata-2026-07-10T21-26-24-644Z/`, and DB batch `media-object-storage-r2-2026-07-10T21-27-45-643Z` migrated `24` copied+verified row refs to `cloudflare_r2` / `chillywood-media-origin`. New creator-media uploads now target R2 private origin, Hetzner writes are disabled, and read fallback remains retained. The final stale-ref lane created private audit-only resolution metadata after backup `backups/media-object-storage/storage-metadata-2026-07-10T22-28-53-006Z/`; all `7` remaining raw `media_scan_jobs` Hetzner refs are resolved historical/stale only (`2` social attachment stale, `3` old proof/test orphan, `2` unsupported-provider stale), with `activeUnresolvedHetznerObjectRefs=0`, `unresolvedActiveRefs=0`, `unresolvedUnknownRefs=0`, and `hetznerObjectStorageShutdownReady=true`. Final export `hetzner-object-storage-shutdown-2026-07-10T22-59-48-461Z` is stored privately under `backups/hetzner-object-storage-shutdown/2026/07/10/hetzner-object-storage-shutdown-2026-07-10T22-59-48-461Z/` with readback checksum passed and no public playback bucket use. Raw scan-job history remains for audit and was not deleted, not marked migrated, and not replaced with fake R2 objects. Next object-storage step is owner-controlled fallback decision and shutdown execution for Hetzner Object Storage only; recommended Option A is 7-day read-only fallback retention before delete/cancel, while Option B is owner-approved immediate dashboard/provider shutdown after accepting no Hetzner fallback. Do not shut it down inside unrelated tasks. Do not shut down Hetzner LiveKit, `chillywood-prod-01`, or `live.chillywoodstream.com`.
...
- Premium tokenized CDN delivery is live for currently HD-capable audited public-safe sources through the protected proof Worker route. `_lib/mediaPremiumCdnToken.ts`, `_lib/mediaPlaybackCdnEligibility.ts`, `scripts/media-premium-hd-cli.mjs`, `npm run proof:media-premium-cdn-token`, `npm run proof:media-premium-hd-renditions`, and `npm run proof:media-premium-hd-resolver` prove short-lived user/source/rendition/path-scoped token claims, Premium entitlement requirement, wrong source/path/expiry denial, private/original/unscanned/moderation-blocked denial, free-user HD denial, and public 360p/480p unsigned behavior. Production has `5` protected Premium HD `media_renditions` rows: `4` source-supported 720p rows and `1` source-supported 1080p row, all under `playback/protected/premium/`, with unsigned public HD exposure `0`.
- Protected Premium HD CDN Worker route and installed Premium playback are Closed: Worker `chillywood-premium-media-access-proof` uses `premium-media.chillywoodstream.com` as the only active production Premium HD custom domain, binds private R2 bucket `chillywood-media-proof`, and allows only protected prefix `playback/protected/premium/`. The old `premium-media-proof.chillywoodstream.com` host is historical only and no longer serves protected Premium HD. `npm run proof:premium-cdn-worker` proves source allow/deny behavior, and `npm run proof:premium-cdn-worker-live` proves production host `premium-media.chillywoodstream.com` using harmless proof object `playback/protected/premium/proof/hello/720p/hello.txt` plus generated HD HLS objects return HTTP 200 only for valid short-lived Premium proof tokens while missing/expired/wrong-source/wrong-path/wrong-rendition/non-Premium/private/original/public-free requests are denied. The Worker is not routed through `media.chillywoodstream.com`; free 360p/480p playback remains public where eligible. Live backend token issuance is deployed as Supabase Edge Function `premium-media-playback-token`, source commit `f6f6bd9d2ce3ed7179876c7c7c9cdbaab1374198` is pushed, and Android EAS Update group `22916970-0161-4411-930a-3570eb5625fb` / update `019f4a0b-efff-71c9-bae5-9198bb001160` carries the app resolver integration. Final closeout on Play-installed `R5CR120QCBF` used Supabase user `4b5e7761-5bf1-4e18-9eb7-d6037a0eb32f` with a renewed Google Play / RevenueCat sandbox Premium entitlement and `monetization_has_active_premium=true` during proof; 1080p source `3de36e39-67e6-45ca-a12f-d5b1560473cb` and 720p source `4999b741-8854-4bc8-a2f0-45907b870db3` previously resolved through the historical proof host with `cloudflare_r2_premium_token`, `deliveryFormat=hls`, `fallbackUsed=false`, `tokenized=true`, and `protectedPlayback=true`; current token issuance uses `premium-media.chillywoodstream.com`, and Android logs showed ExoPlayer/decoder/active media playback.
- RCA update: the earlier Play-installed sandbox session used for Premium HD proof was not backend Premium-active; that was fixed by renewing the sandbox purchase. A second real issue was found after renewal: the Worker rewrote HLS child playlist/segment URIs as relative paths, causing native HLS clients to resolve them under the parent manifest directory and receive Worker `403`. The fix makes rewritten child URLs root-relative scoped protected paths. Do not change token security, hardcode Premium, or bypass RevenueCat in future work.
- Trusted backend migration schema is applied to production and now contains scoped audited rows from City Lights plus the bounded CLI auto-detect cycle: migration `supabase/migrations/20260709033207_trusted_media_transcode_renditions.sql` created server-owned `media_transcode_jobs` plus `media_renditions`; clients cannot write trusted ready/public-safe/path metadata, current row counts are `media_transcode_jobs=10`, `media_renditions=15`, `active_unfinished_jobs=0`, and `unsafe_cdn_rows=0`. Playback can use those rows only through audited-row rollout gates with signed-origin fallback available.
...
- Scheduled R2 logical backup/restore gate has a real manual runner and the latest post-HD production logical backup is complete, but no deployed schedule exists: `_lib/mediaRecoveryOperator.ts` defines scheduled backup policy, backup freshness, restore-drill freshness, retention, scheduler state, and continuous-worker backup gating. CLI operation is the only backup operation path; no GitHub Actions media-worker backup workflow, cron schedule, deployed scheduler, production worker, queue processor, or playback switch exists. `scripts/run-media-worker-logical-backup.mjs` is disabled/dry-run by default and can create/upload scoped `media_transcode_jobs` plus `media_renditions` backups only with explicit write-mode env, a private R2 backup bucket, and `MEDIA_BACKUP_R2_PREFIX=backups/media-worker/`. `MEDIA_BACKUP_EXPORT_MODE=auto|pg_dump|js` supports JS SELECT export fallback without local `pg_dump`/`psql`, and `MEDIA_BACKUP_DATABASE_SOURCE=linked` uses Supabase CLI linked read-only queries instead of requiring or printing a raw DB URL. Latest private prefix is `backups/media-worker/2026/07/10/media-worker-logical-20260710T024048-5de12265dded/` with row counts `media_transcode_jobs=10` and `media_renditions=15`; `backup:media-worker:verify-latest` and `backup:media-worker:restore-drill` should target that prefix. `npm run proof:media-worker-backup-runner` proves dry-run, missing-env fail-closed behavior, linked-source no-raw-DB-URL behavior, public bucket/domain denial, manifest/checksum generation, JS JSONL restore into disposable PGlite, and resolver-safe query. No cron schedule, GitHub Actions schedule, production worker, queue processor, PITR/billing change, or production playback switch happened.
...
- Media automation operator source/proof now exists for scale and CLI auto-detect bounded execution is active: `_lib/mediaAutomationController.ts`, `_lib/chillywoodAutonomyPolicy.ts`, `_lib/mediaAutomationDiscovery.ts`, `_lib/mediaAutomationBatchPolicy.ts`, `_lib/mediaAutomationBackfillPolicy.ts`, `_lib/mediaAutomationJobs.ts`, `_lib/mediaAutomationQueueProcessor.ts`, `_lib/mediaAutomationWorkerLoop.ts`, `scripts/media-automation-cli.mjs`, disabled systemd templates under `ops/media-automation/systemd/`, `docs/MEDIA_AUTOMATION_OPERATOR_RUNBOOK.md`, and proofs `proof:media-automation-controller`, `proof:media-automation-discovery`, `proof:media-automation-batch-policy`, `proof:media-automation-backfill-policy`, `proof:media-automation-queue-processor`, `proof:media-automation-scheduler-templates`, `proof:media-automation-cli`, `proof:media-automation-batch-planner`, and `proof:media-automation-worker-loop`. It is still CLI-only: no live daemon, cron, scheduler, queue processor, broad backfill, media processing, or playback broadening happened. A media readiness-only scheduled probe was not added because the current media scheduler templates are worker-oriented and not a separately proved no-processing readiness lane.
- CLI auto-detect media automation is now the normal bounded execution path for safe Level 0/1 media work. `media-automation:discover`, `media-automation:plan-auto`, and `media-automation:dry-run-auto` discover eligible public-safe videos and calculate a safe adaptive batch size without manual source ids or manual batch-size selection. After the trusted gateway scan proof, `MEDIA_AUTOMATION_RUN_CONFIRM=I_UNDERSTAND_AUTO_DETECT_BATCH npm run media-automation:run-auto` processed four safe scan-clean/moderation-safe public candidates one at a time, generated/uploaded 360p/480p HLS under `playback/public/auto/`, wrote/audited/promoted service-owned rows, and verified public HLS fetch/decode. Final linked discovery scans `27` candidates and finds `eligible_needs_transcode=0`, `eligible_already_has_audited_hls=5`, `excluded_private=12`, `excluded_premium=9`, `excluded_unsupported_format=1`, and zero unscanned, moderation-blocked, missing-source, active-job, denied, or already-processed rows. The one unsupported candidate is 320x180, below the current 360p minimum HLS ladder, and has only a scoped failed job marker with no uploaded objects or rendition rows.
- Catalog readiness automation is source/proofed and read-only for classification: `_lib/mediaCatalogReadiness.ts`, `scripts/media-catalog-readiness-cli.mjs`, `docs/MEDIA_CATALOG_READINESS_RUNBOOK.md`, `npm run media-catalog:status`, `npm run media-catalog:readiness-plan`, `npm run media-catalog:scan-plan`, and `npm run proof:media-catalog-readiness` classify catalog rows before transcode. Final linked readback scans `27` rows and finds `ready_for_transcode=0`, `already_audited_hls=5`, `needs_scan=0`, `private_excluded=12`, `premium_excluded=9`, `unsupported_format=1`, and zero moderation-review, original/master, missing-source, blocked, or denied rows. The readiness CLI does not execute scans, mark media clean, write production rows, process media, or switch playback.
- Catalog scan automation now has a trusted backend gateway and production scan proof: `supabase/functions/media-scan-private-access`, `_lib/mediaScanAutomation.ts`, `scripts/media-scan-cli.mjs`, `npm run media-scan:status`, `npm run media-scan:plan`, `npm run media-scan:dry-run`, `npm run media-scan:run-auto`, `npm run proof:media-scan-private-access`, `npm run proof:media-scan-automation`, and `npm run proof:media-scan-auto-cycle` classify public scan candidates, prove ffprobe media-readability behavior, and keep private/Premium/already-audited rows out of scan plans. The gateway uses a narrow scanner operator token with only `MEDIA_SCAN_OPERATOR_TOKEN_SHA256` stored server-side, denies missing/invalid tokens, denies private/Premium/moderation-blocked requests, supports both S3/Hetzner and Supabase Storage source objects through backend authority, streams bytes without returning signed URLs, and records scan results only after scanner name/version plus ffprobe proof. The raw proof token was not printed or committed. `MEDIA_SCAN_AUTO_CONFIRM=I_UNDERSTAND_PUBLIC_SCAN_BATCH npm run media-scan:run-auto` scanned all five public candidates, wrote trusted readability-clean results, skipped `12` private and `9` Premium rows, and left playback unchanged. This scanner proof is media readability only, not malware scanning or content moderation.
- Production HLS/transcoding is active only through trusted audited `media_renditions` eligibility: no production backend transcode queue/service worker is deployed, no trusted production `video_renditions` table write path is live for these assets, no broad playback migration happened, and signed-origin fallback remains available.
...
2. First full autonomous readiness -> scan -> transcode cycle is closed for all currently safe Level 0/1 work. Current linked readback after the cycle: `ready_for_transcode=0`, `already_audited_hls=5`, `needs_scan=0`, `private_excluded=12`, `premium_excluded=9`, `unsupported_format=1`; auto-discovery finds `eligible_needs_transcode=0` and `plan-auto` / `dry-run-auto` return `batchSize=0` with reason `no_eligible_candidates`. Next autonomous step is no-op unless new public scan-clean/moderation-safe candidates arrive or the owner approves a future low-resolution ladder such as 180p/240p. Do not run `run-continuous-once`, broad backfill, cron, scheduler, or daemon.
2a. Source-aware Premium HD generation is closed for the current HD-capable public-safe set: City Lights remains SD-only because the source is 854x480; three 1280x720 sources have protected 720p; one 720x1280 source has protected 720p plus 1080p. Backend/app token issuance is deployed and OTA-published; next Premium HD milestone is the Premium-active installed playback proof, not more backend token work or HD generation for the current catalog. Do not expose HD through `playback/public/`, and do not process private/Premium/original media.
...
5. Automation operator source/proof is available for future confirmed auto-detect batch work, but it is not deployed. Use `media-automation:discover`, `media-automation:plan-auto`, and `media-automation:dry-run-auto`; do not manually pick every video for normal operation. Do not make a permanent hardcoded City Lights path, run broad backfill, deploy a worker, add cron/scheduler, switch additional videos, or run an all-at-once production migration.
...
11. Do not expose protected Premium HD through unsigned public CDN. Installed-app Premium HD closure now requires only a fresh Premium-active Play-installed proof: active Premium user receives tokenized protected Worker HD, free/non-Premium user is denied or falls back to 360p/480p, and no token/signed URL is printed.
