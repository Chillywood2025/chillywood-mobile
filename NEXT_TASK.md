...
- Installed-device proof folder `/tmp/chillywood-profile-platform-visibility-device-proof-20260617-192905` captures Profile visibility controls and save/readback for Public, Private, Subscriber-only, restored to Public. The signed-in proof account was non-Premium and correctly hit the Platform Studio gate, so Platform visibility UI proof still needs an owner/operator/Premium session.
- Backend readback is migration-state-only in this environment: direct SQL readback was not available because Docker was not running for `db dump`, this Supabase CLI lacks `db query`, and no local DB/service-role env was present.
