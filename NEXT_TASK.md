...
- CLI auto-detect media automation is now the normal source/proof path: `media-automation:discover`, `media-automation:plan-auto`, and `media-automation:dry-run-auto` discover eligible public-safe videos and calculate a safe adaptive batch size without manual source ids or manual batch-size selection. `media-automation:run-auto` remains fail-closed unless a future confirmed run supplies `MEDIA_AUTOMATION_RUN_CONFIRM=I_UNDERSTAND_AUTO_DETECT_BATCH` and backup/restore, active-job, unsafe-row, dry-run, audit, rollback, fallback, and emergency-stop gates all pass. No daemon, cron, scheduler, queue processor, production media processing, production DB writes, broad backfill, or playback broadening happened.
