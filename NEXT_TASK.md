...
- Trusted backend migration path exists as docs, a draft SQL migration, and `npm run proof:media-rendition-migration-policy` only; draft migration `supabase/migrations/20260709033207_trusted_media_transcode_renditions.sql` is not applied, clients cannot write trusted ready/public-safe/path metadata, production DB writes are not live, and production playback remains unchanged.
- Trusted backend migration dry-run now passes static SQL validation plus runtime apply/RLS proof through `npm run proof:media-rendition-migration-dry-run` using an in-memory disposable `@electric-sql/pglite` Postgres runtime. It applies the draft migration, verifies tables/indexes/RLS/policies/grants, denies anon/authenticated trusted writes, allows service-role/worker writes, proves resolver-safe anon select, denies unsafe/original/Premium/private/non-public-prefix public-CDN rows, refuses production-looking DB URLs, and prints no secrets. Production migration remains unapplied.
...
3. If database migration proceeds beyond proof, get explicit owner approval and use a safe staging/shadow Supabase project or approved non-production DB URL only; do not use production DB URLs or production service-role keys.
...
6. Next real media milestone is explicit owner approval either to apply the trusted rendition metadata migration in staging/production or to build a real backend worker deployment plan, not production playback migration: trusted private-source download, ffprobe/ffmpeg, 360p/480p/720p/1080p outputs, master manifest, variant playlists, thumbnails, public-playback bucket upload after approval, production-safe cache proof, trusted rendition rows, and RLS/backend write proof.
7. Do not enable signed/token CDN playback or production playback switching until explicitly approved and proved.
8. Do not change app UX, Premium, LiveKit, Watch-Party, Live Stage, App Links, Chat/native, auth/RLS, billing, payout, or cashout behavior for this media lane.
