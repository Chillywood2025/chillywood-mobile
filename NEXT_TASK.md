...
- Admin Usage Metering Foundations 37-39 is pushed. `_lib/platformUsage.ts` reads existing backed Admin Usage data and local migration `supabase/migrations/202605070003_platform_usage_metering_foundation.sql` defines future metering/rollup tables, but that migration has not been applied remotely and generated database types were not edited.
- Admin Usage remains read-only. Live/watch-party rooms and uploads are DB estimates; storage is a metadata estimate from existing video/attachment rows; participant-minutes are a DB estimate from room membership rows; bandwidth remains `Not connected yet` unless future real metering events exist. Do not treat these values as billing, payout, revenue, cost, LiveKit invoice, or provider-log truth.
- Ledger Systems 4A-4D finance foundation is pushed. `_lib/platformFinance.ts` reads safe foundation counts and local migration `supabase/migrations/202605070004_platform_finance_ledger_foundation.sql` defines future finance/payout/network/sponsor/fraud tables, but that migration has not been applied remotely and generated database types were not edited.
...
2. Real AppLovin MAX readiness/integration planning:
   ...
   - no AdMob-only path
3. Admin Usage remote migration/type refresh and later writers:
   - apply the local usage metering migration only in a separately authorized Supabase pass
   - regenerate database types only from the approved schema after that pass
   - prove owner/operator/moderator RLS before relying on the new tables remotely
   - add bandwidth, participant-minute, or storage event writers only in separate scoped lanes
   - keep Admin Usage read-only and honest: DB estimate, Metadata estimate, or Not connected yet
4. Finance remote migration/type refresh and later writers:
   - apply the local finance ledger migration only in a separately authorized Supabase pass
   - regenerate database types only from the approved schema after that pass
...
- Admin Usage Metering Foundations 37-39 is pushed in `_lib/platformUsage.ts`, `app/admin.tsx`, and local migration `supabase/migrations/202605070003_platform_usage_metering_foundation.sql`. The migration is local-only until a future explicit Supabase apply/type-refresh pass. Admin Usage may show storage metadata estimates and participant-minute DB estimates, but bandwidth remains not connected unless future real metering rows exist. Do not show fake cost, fake billing, fake revenue, fake payouts, fake provider logs, fake storage billing, or fake LiveKit billing truth.
- Ledger Systems 4A-4D finance foundation is pushed in `_lib/platformFinance.ts`, `app/admin.tsx`, and local migration `supabase/migrations/202605070004_platform_finance_ledger_foundation.sql`. The migration is local-only until a future explicit Supabase apply/type-refresh pass. Admin finance panels may show safe foundation row counts only; no live money totals, payout execution, invoices, sponsor checkout, payout split execution, fraud enforcement, provider integrations, or fake revenue exists.
