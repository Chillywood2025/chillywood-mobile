...
- Hetzner Object Storage -> Cloudflare R2 private origin migration is Partial. Private R2 origin bucket `chillywood-media-origin` exists, trusted backend copier Edge Function `media-object-storage-migration` is deployed and token-gated, and R2 origin write config is present as Supabase secrets without printing values. Initial inventory found `31` Hetzner/S3 row refs (`22` distinct object refs); `16` existing distinct refs were copied/readback verified in R2 private origin, storage-metadata backup/restore closed at `backups/media-object-storage/storage-metadata-2026-07-10T21-26-24-644Z/`, and DB batch `media-object-storage-r2-2026-07-10T21-27-45-643Z` migrated `24` copied+verified row refs to `cloudflare_r2` / `chillywood-media-origin`. New creator-media uploads now target R2 private origin, Hetzner writes are disabled, and read fallback remains retained. Zero-ref audit still reports `7` Hetzner object-storage refs, all unresolved `media_scan_jobs` rows (`2` social attachment stale refs, `3` old proof/test stale refs, `2` unsupported-provider blockers), so Hetzner Object Storage is not shutdown-ready. Next object-storage step is an explicit cleanup/archive lane for those scan-job refs, followed by zero-ref audit. Do not shut down Hetzner Object Storage yet. Do not shut down Hetzner LiveKit, `chillywood-prod-01`, or `live.chillywoodstream.com`.
