...
- Current blockers before installed closure: Supabase has no FCM service-account secret (`FIREBASE_SERVICE_ACCOUNT_JSON`, `FIREBASE_SERVICE_ACCOUNT_JSON_BASE64`, `FCM_SERVICE_ACCOUNT_JSON`, or equivalent), so direct FCM active CallStyle sends cannot succeed yet; `R3CXA0DS5JV` was not visible over ADB after a non-destructive ADB server restart, so two-phone proof could not be rerun.
- Next exact steps: add the Firebase service-account JSON/base64 as a Supabase secret without committing or exposing it, reopen/load the latest OTA on the v77 devices, verify each device shows standard and native call push registration, recover R3 ADB visibility, then rerun locked-screen/background voice/video, native Answer/Decline, stale/expired call rejection, same-thread, normal in-app, and room-safe regressions on Google Play-installed v77 + verified OTA.
