...
- Protected Premium HD CDN Worker foundation exists in source/proof only: `workers/premium-media-access/worker.mjs` and `npm run proof:premium-cdn-worker` prove a Cloudflare Worker verifier for protected `playback/premium/` / `playback/protected/premium/` HLS paths, compact token signature validation, Premium entitlement claim, user/source/rendition/path/expiry scope checks, unsafe path denial, redacted logging, and mock R2 proxy behavior. The Worker is not deployed, HD media was not generated, and production playback did not change.
...
2a. Source-aware rendition ladder is now implemented for planning: 480p sources request 360p/480p, 720p sources can add 720p, and 1080p sources can add 720p/1080p. Premium token claims and the protected Worker verifier are source/proofed, but Premium HD remains blocked from current public R2/HLS execution until protected Premium output paths, deployed Worker routing, backend token issuance from live entitlement proof, HD generation proof, trusted-row audit, and installed playback proof are complete. Do not process media solely to test this lane.
