...
- CLI auto-detect media automation is now the normal source/proof path: `media-automation:discover`, `media-automation:plan-auto`, and `media-automation:dry-run-auto` discover eligible public-safe videos and calculate a safe adaptive batch size without manual source ids or manual batch-size selection. The original pre-scan read-only run had no eligible candidates; after the trusted gateway scan proof, linked readback scans `27` candidates and finds `eligible_needs_transcode=5`, `eligible_already_has_audited_hls=1`, `excluded_private=12`, `excluded_premium=9`, `excluded_unscanned=0`, and zero original/master, moderation-blocked, missing-source, unsupported, active-job, denied, or already-processed rows. Auto plan/dry-run calculates `batchSize=1`, `riskLevel=low`, one selected candidate, and scoped rollback. `media-automation:run-auto` remains fail-closed at `batch_execution_not_enabled_in_source_proof_build`; future execution still requires backup/restore, active-job, unsafe-row, dry-run, audit, rollback, fallback, emergency-stop, and safe bounded batch gates. `media-automation:run-continuous-once` remains source/proof-only and fail-closed; no daemon, cron, scheduler, queue processor, production transcode rows, broad backfill, or playback broadening happened.
- Catalog readiness automation is source/proofed and read-only for classification: `_lib/mediaCatalogReadiness.ts`, `scripts/media-catalog-readiness-cli.mjs`, `docs/MEDIA_CATALOG_READINESS_RUNBOOK.md`, `npm run media-catalog:status`, `npm run media-catalog:readiness-plan`, `npm run media-catalog:scan-plan`, and `npm run proof:media-catalog-readiness` classify catalog rows before transcode. After the trusted scanner gateway proof, linked readback scanned `27` rows and found `ready_for_transcode=5`, `already_audited_hls=1`, `needs_scan=0`, `private_excluded=12`, `premium_excluded=9`, and zero moderation-review, original/master, missing-source, unsupported, blocked, or denied rows. The readiness CLI does not execute scans, mark media clean, write production rows, process media, or switch playback.
...
2. First full autonomous readiness -> scan -> transcode cycle is Partial at the auto-transcode execution gate, not the scan gate. Current linked readback after scanning: `ready_for_transcode=5`, `already_audited_hls=1`, `needs_scan=0`, `private_excluded=12`, `premium_excluded=9`; auto-discovery finds `eligible_needs_transcode=5`, and `plan-auto` / `dry-run-auto` calculate a safe first batch size of `1`. `media-automation:run-auto` still fails closed with `batch_execution_not_enabled_in_source_proof_build`, so no transcode jobs, new renditions, R2/HLS outputs, or playback expansion happened. Next autonomous step is to enable/implement the bounded auto-transcode execution path for one safe batch inside existing backup/restore/audit/rollback/kill-switch/output-validation gates. Do not run `run-continuous-once`, broad backfill, cron, scheduler, or daemon.
