...
6. Next real media milestone is explicit owner approval for a controlled trusted-row pipeline or a real backend worker deployment plan, not production playback migration: trusted private-source download, ffprobe/ffmpeg, 360p/480p/720p/1080p outputs, master manifest, variant playlists, thumbnails, public-playback bucket upload after approval, production-safe cache proof, trusted rendition rows, and RLS/backend write proof.
