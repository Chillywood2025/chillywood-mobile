...
- Money Flow & Ledger Control Plane is now `foundation_readonly_guarded`: `_lib/moneyFlowControl.ts`, `docs/MONEY_FLOW_CONTROL_RUNBOOK.md`, `proof:money-flow-control`, and `guard:money-flow-control` classify money actions and block fake revenue, fake creator earnings, fake payable balances, fake paid status, fake transfer completion, manual Premium grants, payout release without provider confirmation, customer charges from foundation tables, and test-mode data being called production. Read-only reconciliation/reporting can be autonomous; production money setup/mutation requires Level 3 owner/super-admin approval; real money movement requires Level 4 owner/super-admin approval plus external provider confirmation/readback. `/admin` Money Center has a read-only Money Flow Control section only. Do not move real money, create payouts/cashout, charge customers, manually grant Premium, change provider products/modes, or make foundation money rows payable without the required approval and provider confirmation.
