...
- Trusted backend migration schema is applied to production as schema only: migration `supabase/migrations/20260709033207_trusted_media_transcode_renditions.sql` created server-owned `media_transcode_jobs` plus `media_renditions`; tables/indexes/RLS/policies/grants were read back, clients cannot write trusted ready/public-safe/path metadata, final row counts are zero, and production playback remains unchanged.
...
- The local worker proof does not connect to production DB, write production rows, upload R2 objects, deploy a worker, run a production queue processor, or switch playback.
- Backup/PITR gate is Blocked for production worker activation: Supabase readback on 2026-07-09 for project `bmkkhihfbmsnnmcqkoly` (`Chillywood2025's Project`, `us-west-2`, `ACTIVE_HEALTHY`) returned `pitr_enabled=false`, `walg_enabled=true`, `backups=[]`, and `physical_backup_data={}`. Billing add-on readback returned no selected add-ons and listed paid PITR variants `pitr_7` (`$100/month`), `pitr_14` (`$200/month`), and `pitr_28` (`$400/month`). Enabling PITR is a provider billing/add-on mutation that requires explicit owner approval. WAL-G alone is not enough for this worker-write/backfill gate without a verified restore window, latest backup metadata, or restore drill. No production worker writes or backfill while the gate is Blocked or Partial.
...
- R2-backed logical backup/restore readiness is closed for one-job proof only: `npm run proof:media-recovery-backup-restore` uploaded scoped logical backup artifacts for `media_transcode_jobs` and `media_renditions` to private R2 prefix `backups/media-worker/2026/07/09/media-worker-logical-20260709-one-job-readiness-0712c0fbc441/`, read them back with matching SHA-256 checksums, confirmed the public playback bucket and public media domain did not expose them, restored into disposable PGlite, and proved resolver-safe selection. `npm run proof:media-worker-rollback-drill` proved exact-batch/exact-prefix rollback and denied missing batch, broad prefix, private, Premium, and original/master rollback targets. This is logical backup/restore proof, not true PITR, and it does not unblock continuous automation.
...
3. If the next database lane proceeds, get explicit owner approval for a controlled worker/proof-row pipeline or production worker deployment plan; do not backfill existing videos, insert real media rows, or switch playback without a separate approval.
4. Do not enable public access on `chillywood-media-proof`.
5. Do not upload real production media or private/original/Premium media to the public-playback proof bucket; the only approved real-media copy there is the City Lights public-safe demo proof object.
6. Next real media milestone is explicit owner approval for a staged one-job worker activation lane or controlled trusted-row pipeline, not production playback migration.
7. The staged one-job lane can use the closed R2 logical backup/restore gate only with owner acceptance, operator lease, exact source allowlist, `max_jobs_per_run=1`, backfill disabled, pending-audit rows, auditor pass before resolver trust, auto-disable, quarantine on audit failure, and no production playback switch.
8. Continuous worker mode remains blocked until paid PITR or a proven scheduled backup/restore system is explicitly approved and verified.
9. Do not claim self-auditing replaces PITR for continuous production.
...
12. Do not change app UX, Premium, LiveKit, Watch-Party, Live Stage, App Links, Chat/native, auth/RLS, billing, payout, or cashout behavior for this media lane.
