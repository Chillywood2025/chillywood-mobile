...
Latest Stripe Connect payout-readiness proof: completed on June 4, 2026 from `27ad215` without activating payouts. Existing deployed functions `stripe-connect-account`, `stripe-connect-onboarding-link`, and `stripe-connect-account-sync` proved the sandbox readiness path with the upgraded proof account: the real Stripe test-mode Express account was reused, a sandbox onboarding link was created with the approved HTTPS Chi'llwood return/refresh origin, and Stripe account status was refreshed. Safe status is pending KYC / onboarding in progress, with charges disabled, payouts disabled, details not submitted, transfer capability inactive, and due requirements still present. Final readback kept app-level `live_money_enabled=off`, `payouts_enabled=off`, provider payouts-enabled accounts `0`, payout requests `0`, payable/paid creator payout ledger rows `0`, payable/paid money-access rows `0`, payout-readiness access grants `0`, Stripe Connect entitlements `0`, Stripe Connect Premium entitlements `0`, and active proof roles `0`. Proof path: `/tmp/chillywood-stripe-connect-payout-readiness-proof-20260604/`.

Latest Stripe merch runtime proof: one real Stripe test-mode physical-merch checkout is complete for `cw_merch_test_tee_sandbox`. The upgraded proof account authenticated, was temporarily reactivated as operator, created a sandbox Checkout Session, completed Stripe Checkout with a test card, and was revoked again. `stripe-merch-checkout` is ACTIVE version `4`; `stripe-merch-webhook` is ACTIVE version `5` and uses a dedicated `STRIPE_MERCH_WEBHOOK_SECRET` by server-side secret store only. The real `checkout.session.completed` event was resent to the merch endpoint with a valid Stripe signature and produced one processed `stripe_merch_events` row, one sandbox paid/processing merch order, and one merch order item. Final safety readback stayed clean: merch access grants `0`, merch orders with digital access `0`, Stripe/merch entitlements `0`, Stripe/merch Premium entitlements `0`, payable/paid money-access rows `0`, provider payout-enabled accounts `0`, active proof roles `0`, `live_money_enabled=off`, and `payouts_enabled=off`. Proof path: `/tmp/chillywood-stripe-merch-sandbox-checkout-proof-20260603/`.

Stripe remains physical merch and payout-readiness only. Android digital goods, including Premium, paid content, tickets, seats, tips, and event passes, remain Google Play/RevenueCat only. Production merch launch, public checkout, live payouts, cash-out, tax/legal/KYC approval, production money, and Data Safety finalization for public merch remain future explicit lanes.
...
Latest Money Center launch-polish lane: Creator Money Center and Owner/Admin Money Center now carry review-ready sandbox proof summaries without activating production money. New docs `docs/MONEY_CENTER_LAUNCH_REVIEW_PACKET.md` and `docs/MONEY_CENTER_UI_POLISH_PROOF.md` consolidate product proof, final counts, screenshot paths, reviewer posture, Data Safety posture, and remaining provider-tooling gaps. No migration was needed. Production live money, payouts, cash-out, public buy buttons, fake balances, Stripe Android digital checkout, LiveKit publish authority, host/speaker/mod/admin authority, and safety bypasses remain off/absent. Next release work should use those packets when preparing Play review and investor/operator readouts.

Latest sandbox money failure/event-pass closeout: duplicate/idempotency, admin revoke, expired-intent, and event-pass backing proof completed on June 4, 2026. Remote migrations `20260604015548`, `20260604015818`, and `20260604015941` are applied and dry-run aligned. Event pass now uses `creator_events` plus `has_event_pass_access`; real Google Play / RevenueCat sandbox purchase `cw_event_pass_sandbox_099` completed on Play-installed `R5CR120QCBF` after EAS Update group `581630d7-b67f-4159-99e0-43f76a5e1221` fixed the owner/operator proof route default. Final readback: provider_events `6`, purchase intents `8` with `6` consumed / `1` failed / `1` expired, access_grants `5` with `1` revoked, money_access_ledger_events `7`, payable/paid rows `0`, active proof roles `0`. Proof path: `/tmp/chillywood-money-failure-and-event-pass-proof-20260604/`. Remaining money follow-up is only provider-tooling proof for real refund/revoke and delayed-payment pending if Google Play/RevenueCat expose those test actions; do not fake them.

Latest Google Play sandbox item availability closeout: the prior non-Premium item-not-found blocker is fixed for `R5CR120QCBF`. The exact internal-test opt-in link was accepted, the EAS/internal install with `installer=null` was replaced by a Google Play install, and Play updated the app to versionCode `23` from EAS build `766b8015-cb3a-43ba-910d-fa442a45e9be` at commit `8219c23`. `/admin-money-sandbox-purchases` loaded on the Play-installed build, Creator tip `cw_creator_tip_sandbox_099` displayed the Google Play test-card dialog, and a real sandbox purchase completed. Backend readback: purchase intents `2` total with latest `creator_tip` intent consumed, provider_events `1`, access_grants `0`, money_access_ledger_events `1`, payable/paid money-access rows `0`, active proof roles `0`. The ledger row is `sandbox` / `sandbox_only` / `not_payable`. Proof path: `/tmp/chillywood-googleplay-item-availability-real-purchase-proof-20260603/`. Remaining money next lane: run real sandbox purchase and resolver proof for Watch-Party ticket, Live Watch-Party access/seat, paid content, and event pass; do not fake rows or activate production money.
...
Latest money-access architecture result: the shared provider-event -> product catalog -> access grant -> resolver -> not-payable ledger -> Money Center inspection path is repo-backed and remote-applied through migration `20260603165000_money_access_grants_product_catalog.sql`, helper `_lib/moneyAccessGrants.ts`, Owner/Admin Money Center readout counts, generated Supabase types, and `npm run guard:money-access-grants-policy`. The proof lane passed local migration lint, `supabase db push --dry-run`, remote `supabase db push`, post-apply dry-run, and `supabase gen types typescript --linked > supabase/database.types.ts`; `supabase migration list` is still unavailable in this shell because the generated CLI login role hits the known SASL auth failure. Premium remains sandbox-proved through RevenueCat/Google Play and backend `user_entitlements`, while source is closed by default with `PREMIUM_PURCHASE_SHELL_ON_HOLD = true` and `premiumPurchaseEnabled: false`. Paid content, Watch-Party Live tickets, Live Watch-Party access/seat passes, tips, merch, payouts, fake balances, fake payable ledger rows, cash-out, and Stripe Android digital checkout remain off/setup-only. Android signed-in proof is captured at `/tmp/chillywood-money-center-signed-in-proof-20260603/`: the proof account was temporarily upgraded to operator, Creator Money Center showed setup/readiness-only and not-payable states, Android EAS update group `5008f2c5-e002-40bd-8f6e-fcd1fa95e633` was published from current `main`, but installed Play/EAS versionCode `21` did not pick up the current Admin Money Center JS and only exposed Overview/Search. The temporary operator grant was revoked and post-revoke Admin denial was captured. Next money proof needs a fresh Play/EAS-signed build carrying current `main` or explicit local replacement approval before the separate real RevenueCat / Google Play sandbox ticket/seat product-event lane. Do not insert fake sales or fake ledger rows.

Latest real sandbox money preflight: `revenuecat-webhook` now mirrors real RevenueCat Premium events into the shared money-access tables and is deployed ACTIVE version `8`; this is backend readiness for the next real provider event, not a fake sale. Remote catalog readback shows Premium is the only mapped RevenueCat/Google Play product. Paid content, Watch-Party tickets, Live Watch-Party access/seat passes, tips, event pass, and merch are blocked by missing provider product ids/offering mappings. Remote safety switches are strict again: `live_money_enabled`, `payouts_enabled`, paid content, tips, merch, Watch-Party tickets, Live Watch-Party access, and Live Watch-Party seats are `off`; provider webhooks and RevenueCat/Google Play stay `sandbox_only`. EAS Update `c0bb32bb-3c7e-406e-a619-2e3e0eb536ed` from commit `4c3633e` did not make installed Play/EAS v21 load current Admin money visuals. EAS internal APK build `cc88ce26-6e94-4adb-9768-d0483c12505a` for versionCode `22` finished, was downloaded to `/tmp/chillywood-real-sandbox-digital-sales-proof-20260603/chillywood-v22-eas-production-apk.apk`, and was installed after explicitly uninstalling Play/EAS v21 because in-place install failed with signature mismatch. Current-main Admin Product Catalog / Provider Events / Access Grants / Ledger screenshots and sanitized detail proof are captured under `/tmp/chillywood-real-sandbox-digital-sales-proof-20260603/`. Next lane should create missing RevenueCat/Google Play product mappings first, then run only real sandbox purchase events for mapped products.

Latest RevenueCat/Google Play sandbox access-product update: the dynamic purchase-intent bridge is remote-applied, provider products are created, and real sandbox purchases are proved for creator tip plus the first four access products. Google Play has active one-time products for `cw_paid_content_access_sandbox_099`, `cw_watch_party_live_ticket_sandbox_099`, `cw_live_watch_party_access_sandbox_099`, `cw_live_watch_party_seat_sandbox_099`, `cw_creator_tip_sandbox_099`, and `cw_event_pass_sandbox_099`, each with purchase option `sandbox-099`. RevenueCat has matching published Play Store consumable products with no entitlements or offerings. Migration `20260603225500_sandbox_digital_product_mappings.sql` is remote-applied; migration `20260604011000_allow_sandbox_access_grants_in_resolvers.sql` lets real `sandbox_only` grants resolve for proof while keeping `canPublish:false`. Play-installed versionCode `23` proof completed Creator tip, Watch-Party ticket, Live Watch-Party access, Live Watch-Party seat, and paid content through Google Play's test card. Final readback: purchase intents `6`, consumed intents `5`, failed intents `1` from the old item-not-found attempt, provider_events `5`, access_grants `4`, money_access_ledger_events `5`, payable/paid money-access rows `0`, active proof roles `0`. Resolver proof passed buyer viewer-only access and non-buyer denial for ticket/access/seat, plus paid-content buyer access, non-buyer denial, and draft-content policy denial. Remaining proof: event-pass backing/resolver, refund/revoke, pending purchase, and duplicate webhook retry where provider tooling supports it. Do not insert fake sales.

Latest Premium follow-up: the owner screenshots showing `Premium Unavailable` / setup-needed copy were from the intentionally closed Premium purchase shell, and the later `The item you were attempting to purchase could not be found.` Google Play dialog was from a local sideloaded versionCode `8` install, not a Play-installed build. Current source reopens the subscription shell with `PREMIUM_PURCHASE_SHELL_ON_HOLD = false`; the old `premiumPurchaseEnabled: false` app-config value no longer blocks RevenueCat subscription purchase availability. This does not grant Premium and does not turn on live money, tickets/seats, tips, paid content, payouts, fake balances, cash-out, or Stripe Android digital checkout. Validation passed `npm run typecheck`, `npm run validate:runtime`, `npm run guard:premium-sandbox-policy`, and `npm run guard:creator-monetization-policy`. Commit `492cca3` is pushed, and EAS production update group `6d9b515e-c595-4173-8f88-ce7abb7adfb1` publishes the reopened shell for Android runtime `1.0.0`.
