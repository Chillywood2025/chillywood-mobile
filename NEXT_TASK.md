...
- Catalog scan automation is source/proofed and production-write-disabled: `_lib/mediaScanAutomation.ts`, `scripts/media-scan-cli.mjs`, `npm run media-scan:status`, `npm run media-scan:plan`, `npm run media-scan:dry-run`, and `npm run proof:media-scan-automation` classify public scan candidates, prove ffprobe media-readability behavior, and keep private/Premium/already-audited rows out of scan plans. This scanner proof is media readability only, not malware scanning or content moderation. `media-scan:run-one` requires `MEDIA_SCAN_RUN_ONE_CONFIRM=I_UNDERSTAND_PUBLIC_SCAN_ONE` and still fails closed in the source-proof build; future production clean writes must use trusted scanner/service-role authority plus scanner name/version/proof/readback and moderation-safe state. No scan write, transcode, or playback switch occurred.
...
2. First production CLI auto-detect cycle is Pass-No-op: linked read-only discovery found `0` `eligible_needs_transcode` candidates and `1` already-audited HLS source, so `plan-auto`, `dry-run-auto`, and `media-automation:report` correctly selected no batch, no rollback scopes, and no work to run. Catalog readiness now shows the next opportunity is `5` public `needs_scan` candidates. Use `npm run media-catalog:scan-plan` and `npm run media-scan:dry-run` to review them. Do not run `media-scan:run-one`, `run-auto`, or `run-continuous-once` until a trusted scanner write path is intentionally used, scanner proof and moderation-safe readback create eligible public-safe candidates, and every backup/restore/audit/rollback/kill-switch/output-validation gate passes.
