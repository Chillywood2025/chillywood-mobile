...
Wave 4 status:
- Verdict: Partial — runtime mutation proof now passes for the new backend controls using approved proof accounts. Password reset/auth email spam remains provider/operator proof pending and out of Wave 4.1 scope. Global blocked-user room-join and blocker-owned room seat-request behavior remains Partial because current product policy does not globally enforce audience blocks on those room surfaces.
- Commit: current Wave 4.1 runtime proof commit pending.
- Proof artifacts: `docs/WAVE4_ABUSE_RATE_LIMIT_PROOF.md`; `scripts/proof-wave4-abuse-rate-limits.mjs`; `scripts/proof-wave4-runtime-mutation-and-blocks.mjs`; static proof folder `/tmp/app-wave4-abuse-rate-limit-fix-proof-*`; runtime mutation/block proof folder `/tmp/app-wave4-runtime-mutation-block-proof-20260624132830`.
...
- Abuse cases tested:
  ...
  - Seat request spam: Pass for durable path. Runtime proof enforced durable Watch-Party seat-request marker throttling. LiveKit token issuance still keeps active camera/mic capped at 4 and downgrades unapproved/over-cap users to viewer/no-publish.
  - Room creation/join spam: Pass for creation cooldowns and join idempotency. Runtime proof enforced Watch-Party and communication-room creation cooldowns. Blocked-user room-join denial remains Partial because current policy does not globally block all room joins by audience block.
  ...
  - Blocked-user harassment prevention: Partial. Runtime proof passes for blocked chat write denial, blocked call dispatch/ring suppression, blocked creator-video comment denial, and chat/call/comment notification prevention by failed source action. Reports remain intentionally available for safety. Blocked-user room joins, blocker-owned room seat requests, seat-request notifications, and installed profile/platform route proof remain Partial.
- Code changes: added Wave 4.1 runtime proof harness; added a narrow comment-trigger type repair migration; fixed `media-storage` zero-byte upload validation; updated Wave 4 proof docs/status. No app UI, payment, Premium, LiveKit authority, participant cap, scan-gate, or media visibility weakening.
- Remaining blockers: provider proof for password-reset/auth email throttling; product decision and runtime proof for whether channel audience blocks should globally deny room joins and blocker-owned room seat requests; installed profile/platform blocked-route proof if required by a later lane.
