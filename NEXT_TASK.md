...
1. No immediate media task is required unless the owner wants another allowlisted one-job media worker run.
2. Use CLI checklist for any future allowlisted one-job media worker run. Checklist: `docs/MEDIA_WORKER_CLI_OPERATING_CHECKLIST.md`.
3. Do not tell future sessions to enable cron, scheduler, deployed worker service, queue processor, broad automation, broad backfill, or production playback migration.
4. Any additional one-job lane must use a fresh backup gate check, operator lease, exact source allowlist, `max_jobs_per_run=1`, backfill disabled, pending-audit rows, auditor pass before resolver trust, auto-disable, quarantine on audit failure, and no production playback switch.
5. Continuous worker mode remains blocked until paid PITR or a deployed/proved scheduled private backup plus recurring restore-drill system is explicitly approved and verified; the current scheduled backup gate has a manual runner and proof, but no deployed schedule.
6. Do not present self-auditing or R2 logical backup as PITR-equivalent for continuous production.
7. Do not enable paid PITR or other backup add-ons without explicit owner approval.
8. Do not enable signed/token CDN playback or production playback switching until explicitly approved and proved.
9. Do not upload real production media or private/original/Premium media to the public-playback proof bucket; the only approved real-media copy there is the City Lights public-safe demo proof object.
10. Do not enable public access on `chillywood-media-proof`.
11. Next real media milestone is explicit owner approval for an installed playback/resolver bridge proof or production worker deployment design, not production playback migration.
