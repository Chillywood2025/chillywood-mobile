...
Player shared/fullscreen follow-up is repo-side complete and OTA-published. `app/player/[id].tsx` now keeps standalone fullscreen video in cover mode instead of returning to black side letterboxing, lets shared Watch-Party playback enter fullscreen, overlays compact Share / Report / speed controls without a Watch-Party Live handoff toggle, auto-hides shared Player chrome after 5 seconds idle, and keeps tap-to-play on the existing shared playback tap handler. Shared fullscreen reuses the same room social panel and same room comments composer, with comments on the left and room/social panel on the right. The non-fullscreen shared dock has additional bottom room so the comments composer is not cut off without changing Player media size. EAS Update group `d32d4cdb-d39f-4815-a397-07648f0b694a` / Android update `019e9a29-c855-74d2-a2c2-3adab2aa1ea1` was published on runtime `1.0.0`; proof path `/tmp/chillywood-player-shared-fullscreen-polish-proof-20260605/` captures standalone normal/fullscreen from the prior OTA. The available route-backed Watch-Party fixture returned the expected unavailable state, so admitted shared-player fullscreen visual proof still needs a live/admitted Party Room session. This did not change media resolver logic, playback sync authority, LiveKit token issuer, host approval, route ownership, Party Room behavior, old-room handling, money state, Premium/content safety, or Owner/Admin authority.
