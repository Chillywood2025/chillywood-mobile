...
The Creator Media presentation pass now makes uploaded videos appear as modern content cards in Channel Settings and in the Profile/Channel creator area. Thumbnails are real only when `thumb_url` or `thumb_storage_path` is present; otherwise cards show a branded fallback preview rather than a broken or developer-looking record. Public videos can be route-shared with `chillywoodmobile://player/[id]?source=creator-video`; signed Storage URLs are never shared. The 2026-04-29 Profile/Channel clarity pass corrected the social/product split: Profile Posts now shows an honest personal-updates placeholder because text/status posts are not backed yet, and the Channel tab owns creator uploaded videos. Owner Profile still has a compact mobile header and owner upload composer, but the composer is explicitly a creator-video upload into Channel, not a fake personal post composer. Owner top actions are Edit Profile, Manage Channel, Upload Video, and Settings; public top actions are backed Follow/Following, Chi'lly Chat, View Channel, Share Profile, and Report where supported. Channel Settings remains the deeper management/edit/publish/unpublish/delete surface and now labels current access controls as Access Defaults instead of monetization. Creator-video likes, comments, saves/My List, engagement counts, generated thumbnails, duration metadata, text-only profile updates, profile feed comments, and comment media remain pending lanes and are intentionally not faked in the UI.

The large creator upload correction is now code-backed and the bucket migration is applied/proved. Profile and Channel Settings no longer reject 50 MiB-plus files locally; `_lib/creatorVideos.ts` treats the current creator upload lane as channel movie upload up to 5 GiB, and `supabase/migrations/202604280001_raise_creator_video_movie_upload_limit.sql` raises the `creator-videos` bucket intent to 5 GiB. `supabase migration list` aligns local/remote through `202604280001`, and linked remote bucket proof shows `file_size_limit = 5368709120` with the expected video MIME allowlist. The Supabase project Storage global file-size limit still needs dashboard/proof confirmation, and then the original 54.7 MB `Chillywoodtest.mp4` should be retried without compression. If movie uploads above the standard upload path become required, add a backed resumable/TUS or S3 multipart upload lane instead of faking success with compression.
...
The Admin UI modernization pass is code-backed and static-validated. `/admin` now presents a private Chi'llywood Operator Center with Reports, Content, Roles, Audit, and Rachi tabs; shorter summary cards; modern safety-report cards; compact/redacted identifiers; and a dedicated backend-protected Rachi / Official Account section. Rachi is managed there as Chi'llywood's public concierge presence only; Rachi is not Admin, does not grant platform roles, and does not replace operator authority. Owner/creator/channel controls remain in Channel Settings, while platform operator controls remain in Admin and stay gated by backend `platform_role_memberships`. Hide/Remove/Restore creator-video actions now require a confirmation modal, and Hide/Remove require a safety reason. Android visual proof of the redesigned tabs and confirmation modal is still pending.
...
2. Confirm the Supabase Storage global file-size limit is not still capped at 50 MB; the `creator-videos` bucket row is already applied/proved at 5 GiB.
3. Retry the owner-only Profile creator-video composer with the original 54.7 MB `Chillywoodtest.mp4`: Upload Video must expand inline on Profile, attach the local file through the picker, show the selected file/title/visibility state, upload through the existing creator-video backend path, refresh the Channel tab immediately, and keep public viewers from seeing the composer. The Posts tab must remain an honest personal-updates placeholder until a real personal-posts backend exists. If automation cannot select the file from Android DocumentsUI, use manual selection and mark it.
4. Repair or re-upload creator video `677650c2-4790-4298-8ddb-03ba7df5b424`, then rerun the creator-video Player source sanity and save screenshots/logs.
5. Include the smoothness hardening checks from `docs/PUBLIC_V1_SMOOTHNESS_HARDENING_AUDIT.md`: safe Home/Explore errors, My List empty/error actions, creator-video picker cancel/unsupported-file copy, creator-video load retry, Premium setup copy, and no token/signed URL logging.
6. Continue Android visual smoke for creator-video cards only where it remains unproved: thumbnail-present state if data exists, public/non-owner Profile/Channel view, and draft/hidden/removed blocked states. Owner no-thumbnail fallback and no Chi'llywood Originals/platform-title filler already have one-device proof.
7. Reopen `/channel-settings` as the creator owner and confirm uploaded video `f6a5c96a-812a-4132-8a2b-e3ecab51c983` appears in the Content list.
8. Confirm a draft video does not appear for public/non-owner viewers.
9. Publish the video and confirm it appears for public/non-owner viewers on Profile/Channel.
10. Confirm creator can edit metadata, publish/unpublish, and delete.
11. Confirm a non-owner cannot manage another creator's video or see the owner-only Profile composer.
12. Use the now-proved remote schema; do not rerun broad remote migration pushes unless `supabase migration list` shows drift.
13. Run two-device creator-video Watch-Party join proof if two phones are available: host starts room, second device joins by code, both resolve `source_type=creator_video`, no Live Stage route, no sample/platform fallback.
14. Confirm draft/private creator videos are blocked from public Watch-Party.
...
16. Include the 2026-04-28 non-room hardening in final route smoke: signup Sign In handoff, signed-out Chat inbox/thread Sign In handoffs, valid/invalid platform Player routes, valid/invalid creator-video Player routes, and no creator-video fallback from bare `/player/[id]`.
17. In a later live-layout proof, follow `docs/LIVE_WATCH_PARTY_LAYOUT_LOCK.md` and confirm Home Live Watch-Party routes to `/watch-party/live-stage/[partyId]`, Live First and Live Watch-Party share the same Chi'lly Party Members overlay behavior, the current user's own preview is not inside that grid, other live feeds render three across with two visible rows before scrolling, free/access-granted viewers can watch without becoming a camera tile, comments remain in their current visible placement, the overlay/comments dock auto-hides after 10 seconds on initial Live Room entry and after mode switches unless controls are locked or an active panel/input is open, and Chi’llyfects opens as foundation-only/no-camera-processing.
18. Do not pull Snap Camera Kit into Public v1. If Snap Camera Kit is revisited post-v1, follow `docs/CHILLYFECTS_SNAP_CAMERA_KIT_AUDIT.md`: Android-only, no production claim, keep layout/comments locked, and prove a second LiveKit device sees the processed feed before marking any Chi’llyfect as real.
19. In a later Party Room proof, follow `docs/LIVE_WATCH_PARTY_LAYOUT_LOCK.md` and confirm platform and creator Watch-Party Live remain content-first in `/watch-party/[partyId]`, with the shared source card at the top, host/viewer feed bubbles below it in a five-across/two-visible-row scroll grid, comments remain in their current visible placement, and no Live Stage route.
20. Confirm creator-video Report in Player writes a real `safety_reports` row with target type `creator_video`.
21. Keep the Admin role-alignment, temporary-operator proof, and Operator Center modernization in route smoke: signed-out users and local-helper/non-operator accounts must stay denied with no destructive controls, backend owner/operator roles can moderate safe creator videos, Rachi must appear only as the backend-protected official-account management section and never as Admin authority, and the Reports/Content/Roles/Audit/Rachi tabs plus Hide/Remove/Restore confirmation should render cleanly on Android.
22. Confirm hidden/removed creator videos stay absent from public Profile/Channel as well as unavailable in Player; Player hidden/unavailable was proved during temporary Admin proof.
23. Confirm active backend entitlement rows allow protected access while missing/expired/revoked rows block protected access.
24. Prove creator-video Storage API delete/remove behavior; direct SQL delete is intentionally blocked by Supabase's `storage.protect_delete()` trigger.
...
## Latest Admin UI Modernization Checkpoint

- Scope: code/static validation only; no Android proof was run in this pass.
- Result: `/admin` is now the Chi'llywood Operator Center with Reports, Content, Roles, Audit, and Rachi tabs; Rachi is restored as a backend-protected official-account management section without becoming Admin authority; owner/content-owner authority is separated from backend platform operator authority; local/dev helper state still cannot expose destructive controls; and creator-video Hide/Remove/Restore actions require confirmation with safety reason for Hide/Remove.
...
- owner Upload Video on Profile opens the inline creator-video upload composer instead of routing away
...
- creator Channels can accept files over 50 MB after the 5 GiB bucket/global Storage limit is applied and proved
...
- Admin UI presents a private Operator Center with a bounded Rachi / Official Account section, not a Rachi-as-Admin or Channel Owner surface
