...
Current status: the stale Brevo SMTP key failure was fixed by local key rotation and Supabase Auth SMTP patch/readback. Direct SMTP auth passes. The Play-installed app now has a dedicated Reset password request screen, app-origin reset submit shows success, and direct Brevo smoke email delivery/open proof passed. The remaining app-path issue was the hosted Supabase Auth email template link, not Brevo delivery: the branded email still used Supabase `{{ .ConfirmationURL }}`, which generated `redirect_to=https://chillywoodstream.com` in the user screenshot and landed on the public policy site. Hosted recovery/confirmation templates now use direct TokenHash app links (`chillywoodmobile://reset-password?...` and `chillywoodmobile://auth/callback?...`) and no longer use `{{ .ConfirmationURL }}` for those two flows. Brevo reports a new post-fix recovery email delivered to Gmail at `2026-06-10T21:40:44-05:00` with subject `Reset your Chi'llywood password`. Remaining gap is real inbox/device click-through proof from that newest message.

Current remaining auth email gap: forgot-password emails are now confirmed as accepted by Supabase Auth after SMTP key rotation, SMTP relay handoff is successful, the Brevo sender `Chi'llywood <no-reply@chillywoodstream.com>` is verified, the hosted Supabase Auth recovery template is branded, the exact current Brevo dashboard DNS records under `chillywood` are applied/resolving in Cloudflare DNS, and Brevo event readback shows delivered. Manual reset-link click-through is still unverified in this environment. See `docs/SUPABASE_AUTH_SMTP_CLI_CONFIGURE_PROOF.md` for the latest proof.

June 6 SMTP follow-up was completed with sender change to `no-reply@chillywoodstream.com` and sender name `Chi'llywood`. Patch/readback succeeded for project `bmkkhihfbmsnnmcqkoly`; `smtp_host=smtp-relay.brevo.com`, `smtp_port=587`, and existing Brevo credentials. `POST /auth/v1/recover` to `rob2037gn@gmail.com` returns `200`; exact Brevo DNS records requested by the dashboard (`chillywood`, `brevo1._domainkey.chillywood`, `brevo2._domainkey.chillywood`, `_dmarc.chillywood`) were added in Cloudflare and resolve publicly. Inbox visibility and reset-link click-through remain the final proof gap.
