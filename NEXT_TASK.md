...
- Premium signed/token CDN foundation exists in source/proof only: `_lib/mediaPremiumCdnToken.ts`, `_lib/mediaPlaybackCdnEligibility.ts`, and `npm run proof:media-premium-cdn-token` prove short-lived user/source/rendition/path-scoped token claims for future 720p/1080p HLS, Premium entitlement requirement, wrong source/path/expiry denial, and private/original/unscanned/moderation-blocked denial. No HD media was generated, no production rows were written, no playback switch occurred, and Premium HD still requires protected Premium output paths plus a tokenized URL signer before activation.
- Protected Premium HD CDN Worker proof route is deployed and proved: Worker `chillywood-premium-media-access-proof` is attached only to `premium-media-proof.chillywoodstream.com`, binds private R2 bucket `chillywood-media-proof`, and allows only protected prefix `playback/protected/premium/`. `npm run proof:premium-cdn-worker` proves source allow/deny behavior, and `npm run proof:premium-cdn-worker-live` proved harmless proof object `playback/protected/premium/proof/hello/720p/hello.txt` returned HTTP 200 only for a valid short-lived Premium proof token while missing/expired/wrong-source/wrong-path/wrong-rendition/non-Premium/private/original/public-free requests were denied. The Worker is not routed through `media.chillywoodstream.com`; HD media was not generated, Premium HD playback is not live, and production playback did not change.
- Trusted backend migration schema is applied to production and now contains scoped audited rows from City Lights plus the bounded CLI auto-detect cycle: migration `supabase/migrations/20260709033207_trusted_media_transcode_renditions.sql` created server-owned `media_transcode_jobs` plus `media_renditions`; clients cannot write trusted ready/public-safe/path metadata, current row counts are `media_transcode_jobs=6`, `media_renditions=10`, `active_unfinished_jobs=0`, and `unsafe_cdn_rows=0`. Playback can use those rows only through audited-row rollout gates with signed-origin fallback available.
...
2a. Source-aware rendition ladder is now implemented for planning: 480p sources request 360p/480p, 720p sources can add 720p, and 1080p sources can add 720p/1080p. Premium token claims and the protected Worker proof route are source/live-proofed, but Premium HD remains blocked from current public R2/HLS execution until protected Premium output paths for real HD assets, backend token issuance from live entitlement proof, HD generation proof, trusted-row audit, and installed playback proof are complete. Do not process media solely to test this lane.
