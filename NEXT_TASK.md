...
- Firebase Test Lab is cost-capped cheap mode for installed QA. Source includes `scripts/installed-qa-firebase-test-lab.mjs`, `qa/firebase-test-lab/README.md`, package scripts `installed-qa-operator:firebase-test-lab:*`, and `firebase_test_lab_uploaded_artifact` source support so the operator can record Firebase device-lab findings without calling them Play-installed proof. Tier 0 source/backend/operator checks run without device lab; Tier 1 Firebase virtual smoke is daily at most, on OTA/source change, or on owner command when cost guard passes. Defaults: `$5/month`, `$0.25/run`, virtual-only, physical disabled, broad crawls disabled, two-device Firebase disabled, and unbounded/over-cap cost blocked. The previous `$0`-only blocker is superseded by bounded cheap mode. One owner-command Tier 1 virtual smoke completed as Firebase matrix `4612242345700782646` with worst-case `costEstimateUsd=0.09`, `billingRisk=low`, and local ledger write; this is uploaded-artifact proof only. The local runner has Firebase/gcloud access but does not have `/etc/chillywood/installed-product-qa-operator.env`; `/etc/chillywood` is absent; `sudo -n true` requires a password; and `chillywood-prod-01` did not resolve from this host. The result was not posted live from this machine, no token/hash was generated, no non-interactive sudo rule was installed, and no Firebase scheduler is installed or claimed active. `device_lab_scheduler_pending` remains until a root-owned env/token and daily cost-capped timer are installed, fired, and audited. Firebase uploaded artifact is not Play-installed proof and cannot close Premium/RevenueCat/Google Play Billing or two-device LiveKit proof.
