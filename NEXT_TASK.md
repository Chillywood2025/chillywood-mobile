...
6. Next real media milestone is explicit owner approval for backup/PITR readiness and a staged worker activation lane, not production playback migration: choose paid PITR or another owner-approved restore path, verify restore window/latest backup metadata/restore drill, then proceed only after the gate is Closed.
7. After the backup/PITR gate is Closed, the later staged worker lane can cover trusted private-source download, ffprobe/ffmpeg, 360p/480p/720p/1080p outputs, master manifest, variant playlists, thumbnails, public-playback bucket upload after approval, production-safe cache proof, trusted rendition rows, and RLS/backend write proof.
8. If a one-job worker proof is requested before continuous mode, it must use operator lease, exact source allowlist, pending-audit rows, auditor pass before resolver trust, auto-disable, quarantine on audit failure, and no production playback switch.
