...
Admin Usage Writer V1A2 daily rollup foundation for creator-video upload/storage usage.

V1A2 should be scoped to rollups from already-backed usage rows only:

- read existing `usage_meter_events` rows for `event_type = video_uploaded`, `event_source = app_rpc`, and `usage_class = upload`
- read existing `platform_usage_metering_events` rows for `metric_key = storage_bytes` and `source_type = creator_video`
- upsert daily totals into `usage_daily_summaries`
...
- do not change creator upload behavior, storage implementation, Player, Watch-Party, Live Stage, Channel Studio layout, Public Channel, Profile, ads, RevenueCat, finance, RLS, migrations, generated database types, or Supabase remote state unless a later exact prompt scopes it
...
- Admin Usage Writer V1A0/V1A1 is pushed and runtime-proved for successful creator-video upload usage only. V1A0 adds secure idempotent RPC `record_creator_video_upload_usage(target_video_id text)`. V1A1 adds `_lib/platformUsage.ts` helper `recordCreatorVideoUploadUsage` and calls it from `_lib/creatorVideos.ts` only after the `videos` row insert succeeds. Runtime proof showed one `video_uploaded` usage row, one `storage_bytes` metadata row, and replay skipping without double-counting. Metering failure is non-fatal to upload success.
...
- Ledger Systems 4A-4D finance foundation is pushed. `_lib/platformFinance.ts` reads safe foundation counts and local migration `supabase/migrations/202605070005_platform_finance_ledger_foundation.sql` defines future finance/payout/network/sponsor/fraud tables, but that migration has not been applied remotely and generated database types do not include those finance tables yet.
...
3. Admin Usage later writers:
   ...
   - creator-video upload usage writer V1A0/V1A1 is pushed and runtime-proved for `video_uploaded` and `storage_bytes` metadata only after successful backed video row creation
   - next safe candidate is V1A2 daily rollup foundation from already-backed creator-video usage/storage rows into `usage_daily_summaries`
...
4. Finance remote migration/type refresh and later writers:
   - apply `supabase/migrations/202605070005_platform_finance_ledger_foundation.sql` only in a separately authorized Supabase pass
   - regenerate database types only from the approved schema after that pass
   - prove owner/operator RLS and fraud moderator visibility before relying on the new tables remotely
...
- Admin Usage Metering Foundations 37-39 plus Admin Usage Metering Foundation V1 remote/provider schema are pushed in `_lib/platformUsage.ts`, `app/admin.tsx`, migrations `supabase/migrations/202605070003_platform_usage_metering_foundation.sql` and `supabase/migrations/202605070004_admin_usage_metering_v1.sql`, and generated database types. Admin Usage Writer V1A0/V1A1 is pushed for successful creator-video upload usage only. Admin Usage may show storage metadata estimates, participant-minute DB estimates, schema-connected internal usage/provider import/reconciliation counts, and provider names as not connected, but bandwidth remains not connected unless future real metering rows exist. Do not show fake cost, fake billing, fake revenue, fake payouts, fake provider logs, fake provider bills, fake overages, fake storage billing, or fake LiveKit billing truth.
- Ledger Systems 4A-4D finance foundation is pushed in `_lib/platformFinance.ts`, `app/admin.tsx`, and local migration `supabase/migrations/202605070005_platform_finance_ledger_foundation.sql`. The migration is local-only until a future explicit Supabase apply/type-refresh pass. Admin finance panels may show safe foundation row counts only; no live money totals, payout execution, invoices, sponsor checkout, payout split execution, fraud enforcement, provider integrations, or fake revenue exists.
