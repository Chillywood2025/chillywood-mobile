...
Latest money-access architecture result: the shared provider-event -> product catalog -> access grant -> resolver -> not-payable ledger -> Money Center inspection path is repo-backed through migration `20260603165000_money_access_grants_product_catalog.sql`, helper `_lib/moneyAccessGrants.ts`, Owner/Admin Money Center readout counts, and `npm run guard:money-access-grants-policy`. Premium remains sandbox-proved through RevenueCat/Google Play and backend `user_entitlements`, while source is closed by default with `PREMIUM_PURCHASE_SHELL_ON_HOLD = true` and `premiumPurchaseEnabled: false`. Paid content, Watch-Party Live tickets, Live Watch-Party access/seat passes, tips, merch, payouts, fake balances, fake payable ledger rows, cash-out, and Stripe Android digital checkout remain off/setup-only. Next task: remote-apply/typegen the new migration when the linked Supabase workflow is available, then capture fresh Android Money Center proof if requested.
