...
5. Treat two-real-device transport plus canonical Party Room deep-link re-entry as already re-proved.
6. Bring a second distinct authenticated proof identity into the current safe workflow truth before retrying simultaneous participant proof on the two phones.
7. If the two-device proof still fails after that, isolate whether the blocker is account identity, client runtime support, room access, token usage, or simultaneous connection behavior.
...
- treat authenticated token issuance, one Android device-backed join, and two-device transport as already proved
- treat canonical Party Room deep-link re-entry on both phones as already proved
- add a second distinct proof account or already-valid second authenticated identity before retrying simultaneous room proof
- verify both devices reach the same canonical Party Room target and then the same authenticated Watch-Party Live join path under different identities
