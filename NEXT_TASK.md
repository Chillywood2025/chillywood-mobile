...
Latest RevenueCat/Google Play sandbox product mapping update: the dynamic purchase-intent backend bridge is remote-applied and provider products are now created. Google Play has active one-time products for `cw_paid_content_access_sandbox_099`, `cw_watch_party_live_ticket_sandbox_099`, `cw_live_watch_party_access_sandbox_099`, `cw_live_watch_party_seat_sandbox_099`, `cw_creator_tip_sandbox_099`, and `cw_event_pass_sandbox_099`, each with purchase option `sandbox-099`. RevenueCat has matching published Play Store consumable products with no entitlements, no offerings, and no transactions yet. Migration `20260603225500_sandbox_digital_product_mappings.sql` is remote-applied; Supabase connector readback confirmed six sandbox catalog rows with exact provider product IDs and `sandbox_purchase_intents_enabled=true`. `supabase/database.types.ts` was refreshed; post-apply `supabase db push --dry-run` hit the known generated `cli_login_postgres` SASL failure after apply, so remote verification used connector SQL readback. New route `/admin-money-sandbox-purchases` is owner/operator-only and can create a short-lived sandbox intent, fetch the RevenueCat non-subscription product, and launch the real Google Play sandbox purchase. Commit `8b27c42` is pushed, and EAS production update group `9757451f-7817-488e-9b10-fff68372fefd` published the route. On `R5CR120QCBF`, the route loaded, denied the non-operator proof session, exposed the sandbox-only/not-payable launcher only during a temporary operator reactivation, and then revoked that role. A real Creator tip purchase attempt created one sandbox purchase intent for `cw_creator_tip_sandbox_099`, but Google Play returned `The item you were attempting to purchase could not be found`; the intent was marked `failed` and unconsumed. Final readback: pending intents `0`, provider_events `0`, access_grants `0`, money_access_ledger_events `0`, payable/paid money-access rows `0`, active proof roles `0`. Remaining proof: resolve Google Play item availability for the signed/tester install, rerun real sandbox purchases, then capture RevenueCat events, webhook rows, access grants, sandbox/not-payable ledger rows, resolver checks, Creator Money Center, and Owner/Admin drilldowns. Do not insert fake sales.
