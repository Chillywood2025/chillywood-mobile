...
## Exact Next Task
Carry forward the still-valid proved baseline exactly as recorded, including the locked Party / Live / Profile / Chi'lly Chat semantics, the preserved Rachi official-account foundation, the closed fake-comment cleanup, the re-proved Live Waiting Room / Live Room / Live Stage path, the now-closed shared analytics sink fix in `app/_layout.tsx`, the now-re-proved auth/session path on the current owners, the now-closed Premium/store repo-proof lane, the tester/store-facing monetization hardening on `components/monetization/access-sheet.tsx`, `app/title/[id].tsx`, `app/player/[id].tsx`, `app/watch-party/index.tsx`, `app/watch-party/[partyId].tsx`, and `app/settings.tsx`, the Chi'lly Chat thread-call stale-session fix on `app/chat/[threadId].tsx`, the Chi'llywood-native internal invite path on `_lib/chat.ts`, `components/chat/internal-invite-sheet.tsx`, `app/watch-party/index.tsx`, `app/watch-party/[partyId].tsx`, and `app/watch-party/live-stage/[partyId].tsx`, the two-step `_lib/userData.ts` hardening that now both syncs signed-in member profiles into `public.user_profiles` and seeds first-run fallback identity from auth email / metadata instead of an unpredictable guest-only label, and the now-pushed call-lane runtime-proof cleanup in commit `c2394cc` that removes false dev-client call-surface error noise by downgrading render-only call instrumentation from `console.error(...)` to `console.log(...)`. Preserve the currently proved Chi'lly Chat call presentation truth exactly as it now stands on `main`: active thread video calls present as a full-page call surface from chat, exactly 2 participants use a true 50/50 split, and 3+ participants use a balanced full-page grid. Do not reopen the resolved room-flow, analytics, auth/session, monetization, Party/Live, or broader chat architecture owners unless a later regression proves a direct break. The next exact operational step is now the next honest two-real-phone Expo dev-client proof pass for the current Chi'lly Chat social path after this noise cleanup: verify internal invite search can find the other signed-in user, verify the invite lands in the correct direct thread, verify a fresh video call can start and join from `/chat/[threadId]`, then verify end/restart, leave/rejoin, and kill/reopen stale-state cleanup. After that runtime proof closes, return to the broader testing/store-readiness sequence already recorded.
...
6. Keep the pushed call-lane render-log cleanup in commit `c2394cc` closed and do not reopen those call owners unless a later regression proves a real runtime issue rather than dev-only noise.
7. Preserve the full-page Chi'lly Chat call presentation truth on `/chat/[threadId]` exactly as currently proved on `main`.
8. Narrow the next proof lane to two-real-phone Expo dev-client verification of internal invite plus thread video-call behavior.
9. Return to the broader testing/store-readiness closure sequence only after that focused social runtime proof completes.
...
- preserve the pushed call-lane render-log cleanup and the proved full-page chat-call presentation truth without reopening those owners for speculative UI churn
...
- preserve the pushed full-page Chi'lly Chat call presentation and the now-quiet render-only dev logging exactly as they now work
...
- reopen the pushed call-lane render-log cleanup or full-page call presentation owners unless a later regression proves a direct chat-call runtime bug
