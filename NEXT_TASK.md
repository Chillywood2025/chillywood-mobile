...
- Firebase Test Lab is cost-capped cheap mode for installed QA. Source includes `scripts/installed-qa-firebase-test-lab.mjs`, `qa/firebase-test-lab/README.md`, package scripts `installed-qa-operator:firebase-test-lab:*`, short aliases `installed-qa:firebase-test-plan` / `installed-qa:firebase-test-run`, source support for `firebase_test_lab_uploaded_artifact`, and Linux systemd templates under `ops/installed-product-qa-operator/systemd/`. Tier 0 source/backend/operator checks run without device lab; Tier 1 Firebase virtual smoke is daily at most, on OTA/source change, or on owner command when cost guard passes. Defaults: `$5/month`, `$0.25/run`, virtual-only, physical disabled, broad crawls disabled, two-device Firebase disabled, and unbounded/over-cap cost blocked. The previous `$0`-only blocker is superseded by bounded cheap mode. One owner-command Tier 1 virtual smoke completed as Firebase matrix `4612242345700782646` with worst-case `costEstimateUsd=0.09`, `billingRisk=low`, and local ledger write; this is uploaded-artifact proof only. Local CLI access exists on the owner Mac for manual runs (`node`, `npm`, `gcloud`, `firebase`, `npx`; gcloud project `chillywood-app`), but the owner Mac is not the long-term runner. Server audit on `chillywood-prod-01` showed Linux/systemd with non-interactive root/sudo available, `/etc/chillywood/installed-product-qa-operator.env` present as `root:root` mode `600` with non-secret Firebase cost-cap defaults, `FIREBASE_TEST_LAB_PROJECT=chillywood-app`, `GOOGLE_CLOUD_PROJECT=chillywood-app`, and `CHILLYWOOD_REPO_DIR=/opt/chillywood/current`, `/var/lib/chillywood/installed-qa` present as `root:root` mode `750`, and `/opt/chillywood/current` cloned from `origin/main`, with no secrets printed. Minimal server tooling is installed: Node `24.18.0`, npm `11.16.0`, Google Cloud SDK `575.0.1`, and `firebase-tools@15.23.0`. The server cost plan estimates `costEstimateUsd=0.09` under the cap, but blocks with `firebase_credentials_missing`; active gcloud credential count is `0`, Firebase/Test Lab API/catalog readback is unavailable, no APK/AAB artifact exists on the server, and no installed-QA timer/service is installed. Backend posting from the server works: a safe `Firebase Test Lab server access and artifact readiness` device finding was recorded, plus `watch_once` and `report` readback, all with `fakeProof=false`, `moneyMoved=false`, `userRightsChanged=false`, and `highRiskExecuted=false`. `device_lab_scheduler_pending` remains until server Google access and an approved artifact are ready, the cost guard can start a bounded matrix, and the systemd timer is installed, enabled, fired, and audited. Firebase uploaded artifact is not Play-installed proof and cannot close Premium/RevenueCat/Google Play Billing or two-device LiveKit proof.
