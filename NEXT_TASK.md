...
- Full app authority/product behavior source audit is closed in `docs/FULL_APP_AUTHORITY_PRODUCT_BEHAVIOR_AUDIT.md` with `proof:full-app-authority-product-audit` and `guard:full-app-authority-product-audit`. It inventories all current `app/` route files and Supabase Edge Functions, classifies public, signed-in, creator, Premium, LiveKit, media/playback, admin/operator, owner/super_admin, moderator, legal/static, diagnostic, and money/provider surfaces, and ties high-risk controls back to the admin action registry, role-action matrix, owner command operator, autonomous registry, and approval model. Installed proof remains pending for owner/super_admin, admin/operator, moderator, normal user/creator, Premium active user, and non-Premium user on the current commit; do not claim installed role/device behavior until that traversal is run. The 2026-07-12 installed preflight attempt is recorded in `docs/release/FULL_APP_AUTHORITY_PRODUCT_BEHAVIOR_INSTALLED_TRAVERSAL_ATTEMPT.md`: the attached Play-installed app was valid (`com.chillywood.mobile`, installer `com.android.vending`, versionCode `80`, runtime `1.0.0`, channel `production`, embedded/emergency launch false), but it was running update `019f529d-7e99-73e5-8f2d-c7d1c35d50a3` / group `8a8e9cee-bc6f-4b68-be52-8d7d80fdf212` from git commit `1cdb27ea1b0410ba8ac2aa840f1acfec6f7d6964`, older than source/guard commit `b1856059e4ce2acf23c43089a2795e23ef9c7927`. Because the task forbade publishing/rollback, sideload, `adb install`, and clear-data, the traversal was blocked before account login. Safe next action is to make a production OTA or Play-installed build available for the current source authority surface, then rerun the seeded installed role traversal.
