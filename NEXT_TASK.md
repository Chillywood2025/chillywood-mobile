...
## Exact Next Task
Complete the remaining Creator Media public/draft and owner/non-owner runtime checks, then run two-device creator-video Watch-Party join/rejoin proof if practical. The Live Layout and Effects foundation is now implemented but not runtime-proved; include it in the later live/two-device proof rather than treating this static pass as proof.
...
Route/owner audit hardening is now handled for the highest-priority findings: `app/modal.tsx` no longer renders template UI, `/communication` redirects to `/chat`, `app/lib/_supabase.ts` is only a compatibility export for the canonical Supabase client, and missing platform Player ids show an honest unavailable state instead of the first local/sample title. The new live layout/effects foundation keeps Home Live Watch-Party people-first in Live Stage, keeps Player Watch-Party Live content-first in Party Room, and marks non-Off camera effects as later real-processing work. Do not re-open those items unless validation or runtime proof shows a regression.
...
11. In a later live-layout proof, confirm Home Live Watch-Party routes to `/watch-party/live-stage/[partyId]`, shows host and participants together in Chi'lly Party Members, keeps comments visible, and opens Effects as foundation-only.
12. In a later Party Room proof, confirm platform and creator Watch-Party Live remain content-first in `/watch-party/[partyId]`, with the shared source card above participants and no Live Stage route.
