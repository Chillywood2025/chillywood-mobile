...
- Firebase Test Lab is zero-cost-first for installed QA. Source now includes `scripts/installed-qa-firebase-test-lab.mjs`, `qa/firebase-test-lab/README.md`, package scripts `installed-qa-operator:firebase-test-lab:*`, and `firebase_test_lab_uploaded_artifact` source support so the operator can record Firebase device-lab findings without calling them Play-installed proof. Current audit: Firebase config, `firebase` CLI, `gcloud` CLI, configured project, active gcloud credentials, Test Lab catalog access, and Android APK/AAB artifacts are present; Tool Results API and billing/free-quota readback are not fully confirmed, so a run cannot be guaranteed `$0`. The runner fails closed with `billingRisk=unknown` / `firebase_free_quota_unknown`; no Firebase matrix was started, no paid usage happened, no scheduler was installed, and `device_lab_scheduler_pending` remains. No paid Firebase run without owner approval is allowed. Firebase uploaded artifact is not Play-installed proof and cannot close Premium/RevenueCat/Google Play Billing or two-device LiveKit proof.
