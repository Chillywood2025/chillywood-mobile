...
Resolve the two-session live-room proof blocker, then Google Play Publishing Overview And Release Asset Closeout.
...
Signed-in route smoke proof is also captured separately on the Play-installed physical device because Firebase Test Lab signed-in coverage still needs safe credentials or an instrumentation/Robo-script path outside committed source. Proof path `/tmp/chillywood-signed-in-proof-20260605/`; doc `docs/android/SIGNED_IN_DEVICE_SMOKE_PROOF.md`; device `R5CR120QCBF`, package `com.chillywood.mobile`, installer `com.android.vending`, versionName `1.0.0`, versionCode `25`. Captures include signed-in Home, Settings, Profile, Watch-Party waiting-room/Premium gate, and route-backed Live seat gate. This closes the local signed-in route-smoke gap, but not the true Test Lab signed-in cloud gap or LiveKit two-session proof.

Firebase Test Lab signed-in cloud setup is ready for the owner to supply a safe non-admin test account outside git. Use `FIREBASE_TEST_LAB_SIGNIN_EMAIL` and `FIREBASE_TEST_LAB_SIGNIN_PASSWORD`, then run `npm run firebase:test-lab:signed-in-preflight` and one bounded `npm run firebase:test-lab:signed-in-build-robo`. The command/proof output redacts credentials and fails closed if they are missing. Do not commit passwords or claim LiveKit/purchase/Admin proof from this signed-in Robo route smoke.
