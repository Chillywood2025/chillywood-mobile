...
Full Interactive Surface QA Inventory And Device Proof has started in response to the need to check every tab, button, route, toggle, collapsible, field, and function. Static inventory is in `docs/FULL_INTERACTIVE_SURFACE_QA_INVENTORY.md`; current counts are `55` route files, `68` files with interactive markers, and `1180` marker hits. Tracking matrix: `docs/FULL_INTERACTIVE_SURFACE_QA_MATRIX.md`. Play-installed device `R5CR120QCBF` proved the current bottom navigation: Home, Explore, Live, and Library are bottom tabs; Profile is not a bottom tab on the tested Home screen and is reached through the top-right avatar/action. A temporary Maestro flow proved Home, Explore, Live, Library, and Home Settings navigation; proof path `/tmp/chillywood-full-interactive-surface-qa-20260605/`. Remaining work is the matrix rows: auth/signup, every tab-local action, Settings collapsibles/toggles, Player/comments, Platform/Profile/Studio, monetization/sandbox/Money Center, Owner/Admin, legal/support/report surfaces, and two-session live rooms.
