...
Player shared/fullscreen follow-up is repo-side complete, with one final shared-fullscreen rail correction pending OTA publication/device proof. `app/player/[id].tsx` keeps standalone fullscreen video in cover mode, lets shared Watch-Party playback enter fullscreen, overlays compact Share / Report / speed controls without a Watch-Party Live handoff toggle, auto-hides shared Player chrome after 5 seconds idle, and keeps tap-to-play on the existing shared playback tap handler. Shared fullscreen now keeps the video in contain/letterbox mode like the requested reference and uses persistent side rails: the real room comments composer on the left and display-only participant bubbles on the right. It no longer reuses the full Watch-Party social panel, placeholder shared-player card, or LiveKit media surface in fullscreen. The non-fullscreen shared dock has additional bottom room so the comments composer is not cut off without changing Player media size. Prior EAS Update group `d32d4cdb-d39f-4815-a397-07648f0b694a` / Android update `019e9a29-c855-74d2-a2c2-3adab2aa1ea1` was published on runtime `1.0.0`; proof path `/tmp/chillywood-player-shared-fullscreen-polish-proof-20260605/` captures standalone normal/fullscreen from the prior OTA. The final shared-fullscreen rail correction still needs OTA publication and admitted Party Room visual proof. This did not change media resolver logic, playback sync authority, LiveKit token issuer, host approval, route ownership, Party Room behavior, old-room handling, money state, Premium/content safety, or Owner/Admin authority.
