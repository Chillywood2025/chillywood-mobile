...
## Exact Next Task
Carry forward the still-valid proved baseline exactly as recorded, including the locked Party / Live / Profile / Chi'lly Chat semantics, the preserved Firebase runtime base on this branch, the repo-safe LiveKit mobile foundation, the deployed `livekit-token` Supabase Edge Function on project `bmkkhihfbmsnnmcqkoly`, the durable `verify_jwt = false` function config now recorded in `supabase/config.toml`, the re-proved `Party Room -> Watch-Party Live` player/title seam on real Android hardware, the re-proved `Live Room -> Continue to Live Stage` seam, and the new single-device local-camera publish proof on both canonical watch-party LiveKit surfaces. The exact next lane should stay narrow: get a second real participant/device into the same canonical rooms to prove remote subscriber rendering instead of local self-preview only, then decide whether the next deterministic expansion belongs in player/watch-together controls or in the canonical Chi'lly Chat `/chat/[threadId]` call owner. Do not reopen OVH/Hetzner or broader production-infrastructure decisions in that next lane.
...
3. Reopen only the existing canonical watch-party seams next: keep `Live Room -> Live Stage` and `Party Room -> Watch-Party Live` stable while proving remote subscriber rendering with a second participant.
...
- prove remote/subscribed video rendering with a second participant instead of only local self-publish
...
- at least one second-participant proof shows remote/subscribed video rendering instead of local self-preview only
