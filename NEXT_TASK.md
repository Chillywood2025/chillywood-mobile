...
Latest RevenueCat/Google Play sandbox product mapping update: the repo now has the dynamic purchase-intent backend bridge required before generic ticket/seat/content/tip products can safely unlock a specific room/content/creator. Migration `20260603190000_money_purchase_intents.sql` is remote-applied, linked typegen is refreshed, post-apply dry-run is up to date, and `revenuecat-webhook` is deployed ACTIVE version `9`. Dynamic non-Premium events now require a matching pending intent; missing/expired/consumed/mismatched/setup/production events are ignored without access grants or payable ledger rows. External dashboard setup remains the blocker: proposed product IDs are `cw_paid_content_access_sandbox_099`, `cw_watch_party_live_ticket_sandbox_099`, `cw_live_watch_party_access_sandbox_099`, `cw_live_watch_party_seat_sandbox_099`, `cw_creator_tip_sandbox_099`, and `cw_event_pass_sandbox_099`. Docs: `docs/REVENUECAT_GOOGLE_PLAY_SANDBOX_PRODUCT_SETUP.md` and `docs/SANDBOX_DIGITAL_SALES_PROOF_MATRIX.md`. The next lane should use owner-confirmed Google Play + RevenueCat products and a licensed tester to run real sandbox purchases; do not insert fake sales.
