...
Current spotted-fix lane should finish with validation and Play-installed/manual proof for owner Platform identity, owner-vs-viewer subscription/VIP route behavior, useful VIP/Subscriber areas, owner-only Platform content actions, and standalone Player fullscreen framing. The newest route-contract focus is to prove owner subscription/VIP management opens Platform Studio offer setup rather than the Premium purchase shell, owner self-purchase is blocked, true non-owner viewers/testers still see purchase CTAs where configured, and Premium remains only Chi'llywood Premium. Do not claim full Platform/Profile `public/private/subscriber_only` visibility until a later backed schema/product lane lands a single Platform-wide visibility gate and subscriber-only Profile behavior; followers/following remain public discovery/social relationships only.
