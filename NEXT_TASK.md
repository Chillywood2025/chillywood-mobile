...
Wave 5 status:
- Verdict: Partial — app/backend-controlled account scheduling/restoration, public Profile/Platform hiding for scheduled-deletion accounts, Admin/support privacy, DMCA privacy, Premium entitlement revoke behavior, and sandbox access-grant revoke behavior are proved. Permanent purge/de-identification, full deleted/deactivated private-feature denial, real provider refund execution, and installed visual proof remain pending.
- Commit: current Wave 5 proof/fix commit pending.
...
- Remaining blockers: permanent account purge/de-identification job/runbook proof; full disabled/deactivated account private-feature denial sweep across rooms, calls, chat, upload, comments, notifications, and LiveKit tokens; real provider refund execution path remains external/manual; installed Android account deletion/restore visual proof on a current Play/internal build; admin/operator suspend/deactivate support-action proof; Wave 4 password reset/auth email provider proof remains pending outside Wave 5.
