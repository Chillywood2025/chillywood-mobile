...
- Hetzner Object Storage -> Cloudflare R2 private origin migration is Partial. Private R2 origin bucket `chillywood-media-origin` exists and source support is staged behind fail-closed env gates. Trusted backend copier Edge Function `media-object-storage-migration` is deployed and token-gated; live inventory through the copier still shows `31` Hetzner/S3 object-storage reference rows (`22` distinct object refs). The prior missing R2 private-origin write config blocker is cleared with bucket-scoped R2 S3 credentials stored as Supabase function secrets. Copy now stops before DB metadata mutation on `source_object_head_failed_404` for the first selected legacy source row even after virtual-host and path-style legacy source addressing fallback. No media rows were written, no DB metadata was migrated, no Hetzner objects were deleted, and Hetzner fallback retained. Next object-storage step is to reconcile/restore the missing or stale legacy source object metadata, rerun copier copy/readback/checksum proof, then DB metadata migration, new-upload R2 proof, scanner/transcode R2 read proof, and `0 Hetzner object-storage` refs. Do not shut down Hetzner LiveKit, `chillywood-prod-01`, or `live.chillywoodstream.com`.
