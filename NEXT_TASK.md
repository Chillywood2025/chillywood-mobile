...
5. Treat `docs/hetzner-first-deployment-implementation-spec.md` and `infra/hetzner/*.example` as deployment-planning reference only; the current known Hetzner host facts, working `chillywood` deploy user, and SSH/bootstrap closeout now point to a narrow `Hetzner reverse proxy / TLS bootstrap prep` infra side-lane without changing the active next product chapter.
