...
Admin Usage Provider Imports V1D3 is pushed and proofed. The server-side `provider-usage-import` Supabase Edge Function is deployed, migration `202605070012_provider_usage_import_idempotency.sql` is applied, Admin Usage reads provider import status rows, missing auth returns `401`, and owner/operator dry-run proof returned `200` with `not_configured` for Cloudflare R2 and Hetzner Servers because provider secrets are not configured. The proof used the existing signed-in device session token in memory only and printed no token, credential, or provider secret. No mobile provider API calls, provider secrets, fake bandwidth, fake bills, overages, invoices, revenue, payouts, sponsor money, or Admin import buttons were added.
...
- Admin Usage Provider Imports V1D3 is pushed. It adds server-side Edge Function `provider-usage-import`, migration `supabase/migrations/202605070012_provider_usage_import_idempotency.sql`, provider import readouts in `_lib/platformUsage.ts`, and read-only Admin Usage provider rows in `app/admin.tsx`. Cloudflare R2 and Hetzner Servers can import provider facts only when server-side secrets are configured and an owner/operator invokes the function. Hetzner Object Storage, OVH Object Storage, and OVH Servers remain `Not connected yet` until later exact API lanes. No provider secrets are stored in repo or mobile app.
...
3. Admin Usage later writers/importers:
   ...
   - Provider Imports V1D3 is pushed for server-side Cloudflare R2 and Hetzner Servers import foundation only; current proof returns `not_configured` until provider secrets are set
   - Hetzner Object Storage, OVH Object Storage, and OVH Servers remain not connected until separate exact provider API lanes
