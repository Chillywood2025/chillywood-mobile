...
- Admin Usage Metering Foundations 37-39 is pushed. `_lib/platformUsage.ts` reads existing backed Admin Usage data, migration `supabase/migrations/202605070003_platform_usage_metering_foundation.sql` has been applied to the linked Supabase remote, and `supabase/database.types.ts` now includes `platform_usage_metering_events` and `platform_usage_daily_rollups`.
...
3. Admin Usage later writers:
   - usage remote migration/type refresh is complete for `202605070003_platform_usage_metering_foundation.sql`
   - add bandwidth, participant-minute, or storage event writers only in separate scoped lanes
   - prove writer behavior before relying on metering rows as real usage truth
...
- Admin Usage Metering Foundations 37-39 is pushed in `_lib/platformUsage.ts`, `app/admin.tsx`, and migration `supabase/migrations/202605070003_platform_usage_metering_foundation.sql`. The usage migration is applied remotely and generated database types include the usage metering tables. Admin Usage may show storage metadata estimates and participant-minute DB estimates, but bandwidth remains not connected unless future real metering rows exist. Do not show fake cost, fake billing, fake revenue, fake payouts, fake provider logs, fake storage billing, or fake LiveKit billing truth.
