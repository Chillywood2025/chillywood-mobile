...
## Exact Next Task
The active pass remains a narrow real-device verification follow-up. Repo truth is explicit: the earlier room-aware invite search fix still stands, the Device A runtime blocker is cleared, and the Party Room invite open-path bug is now also fixed narrowly and proved on the real host phone. The exact next step is to rerun the cross-device invite/thread handoff on the two real phones, then attempt fresh locked-room non-member denial only if a safe non-member identity is actually available, and only after that try full on-stage two-phone proof if the route path is safely reachable.
...
6. Keep Device A on the healthy current-bundle runtime and rerun the cross-device invite/thread handoff.
7. Attempt fresh locked-room non-member denial only if a safe non-member identity is actually available.
8. Attempt full on-stage two-phone proof only if the current route path is safely reachable after the rerun.
...
- the next pass focuses only on the now-cleared host-runtime recovery, the now-fixed Party Room invite open-path, the invite/thread rerun, fresh access-gate proof, and any directly related two-phone verification work
