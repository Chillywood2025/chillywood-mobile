...
2. Publish/load the fresh OTA containing the Party Room Shared Player test IDs, then re-prove the installed Party Room `Connecting…` / Open Shared Player progression using test-ID taps. Do not use the room code as a direct Player `partyId` workaround.
