...
## Exact Next Task
The next exact stage is now `behavior / polish batch 2` on `app/watch-party/index.tsx`. Repo truth is explicit: the first title/player fix landed cleanly, and the next highest-leverage behavior seam is waiting-room preview-state join/access error visibility.

## Current Plan
1. Treat the public profile private friendship boundary lane and the first title/player behavior batch as closed.
...
4. Fix the next real behavior mismatch now: when the waiting-room preview exists, join/access failures must still stay visible.
...
7. Keep the inbox error-visibility batch queued after the waiting-room fix.
8. Keep unrelated local dirt out of the checkpoint.

## Exact Next Batch
- keep the current truthful title/player social baseline as landed in batch 1
- make `app/watch-party/index.tsx` show real join/access failures even when a room preview is already on screen
...
- queue `app/chat/index.tsx` inbox error visibility as the third batch
...
- keep the second fix batch inside the waiting-room owner only
- keep unrelated local dirt out of the checkpoint
