...
- Protected Premium HD CDN Worker route is deployed and proved: Worker `chillywood-premium-media-access-proof` is attached only to `premium-media-proof.chillywoodstream.com`, binds private R2 bucket `chillywood-media-proof`, and allows only protected prefix `playback/protected/premium/`. `npm run proof:premium-cdn-worker` proves source allow/deny behavior, and `npm run proof:premium-cdn-worker-live` proved harmless proof object `playback/protected/premium/proof/hello/720p/hello.txt` plus generated HD HLS objects return HTTP 200 only for valid short-lived Premium proof tokens while missing/expired/wrong-source/wrong-path/wrong-rendition/non-Premium/private/original/public-free requests are denied. The Worker is not routed through `media.chillywoodstream.com`; free 360p/480p playback remains public where eligible. Live backend token issuance is deployed as Supabase Edge Function `premium-media-playback-token`, source commit `f6f6bd9d2ce3ed7179876c7c7c9cdbaab1374198` is pushed, and Android EAS Update group `22916970-0161-4411-930a-3570eb5625fb` / update `019f4a0b-efff-71c9-bae5-9198bb001160` carries the app resolver integration. Installed-app free/non-Premium fallback was proved on an HD-capable video with `tokenized=false` and `protectedPlayback=false`; installed Premium HD closure still needs a Premium-active installed session to prove `tokenized=true`.
- RCA update: the Play-installed sandbox session used for the Premium HD proof was not backend Premium-active. The installed replay refreshed Supabase user `4b5e7761-5bf1-4e18-9eb7-d6037a0eb32f`; that user's RevenueCat sandbox Premium entitlement had expired on `2026-07-08T17:58:59.352Z`, and `monetization_has_active_premium` returned `false` during the replay. Do not change token security, hardcode Premium, or bypass RevenueCat. The next Premium HD proof must first renew/create a current Google Play / RevenueCat sandbox Premium purchase for the installed logical user, or use a Play-installed session whose backend Premium RPC is true, then verify `premium-media-playback-token` returns a protected Worker URL and playback logs `tokenized=true`.
