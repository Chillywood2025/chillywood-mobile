...
- LiveKit Autonomous Operator is source/proofed and deployed fail-closed as the next operating layer for that routed stack. It covers all LiveKit surfaces (`live-stage`, `watch-party-live`, Party Room live sidecar, `chat-call` where routed, `livekit-token`, router, heartbeat monitor, host-agent recommendation) with scoped operator tables, a token-gated `livekit-operator` Edge Function, CLI probes, proofs, and policy guards. Migration `20260711043323_livekit_autonomous_operator` is applied and `livekit-operator` is ACTIVE; a one-time ephemeral operator token proved `health_snapshot` healthy, then the hash was reset to a disabled non-granting value. Before using autonomous recovery operationally, create/store a durable owner-held operator token outside git and rotate `LIVEKIT_OPERATOR_TOKEN_SHA256` to its hash. Safe autonomous recovery is limited to legitimate heartbeat monitor/counter refresh paths, known function-blob redeploy recommendations, scoped stale-assignment cleanup planning, affected-surface pause/reporting, audit writes, and learning-state updates. Do not loosen stale heartbeat cutoffs, write fake heartbeats, mark unhealthy servers healthy, broaden DB writes, or treat Premium sandbox success as LiveKit proof. Watch-Party Live Shared Player fallback-bubble smoothing now keeps a stable LiveKit connecting shell for a short grace window instead of flashing the separate fallback roster while a valid renderable contract/token refresh is in progress.
