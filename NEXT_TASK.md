...
## Exact Next Task
The next exact task is a narrow **audience chapter closeout audit** on `main`, scoped to `app/profile/[userId].tsx`, `app/channel-settings.tsx`, `_lib/channelAudience.ts`, `_lib/channelReadModels.ts`, `CURRENT_STATE.md`, `NEXT_TASK.md`, and `docs/audience-management-implementation-spec.md`. Use those files plus `docs/profile-channel-implementation-spec.md`, `PRODUCT_DOCTRINE.md`, and `ROADMAP.md` as governing truth. Do not widen back into implementation unless the closeout audit proves one final narrow audience seam is still justified.
...
3. Audit the current audience chapter owners and helpers to determine whether any meaningful audience seam still remains after the public visibility slice.
4. Choose exactly one next move: one final narrow audience batch only if it is clearly justified, or chapter closeout / next-chapter selection.
5. Preserve locked route truth for `/profile/[userId]`, `/channel-settings`, Chi'lly Chat, and all current room/live owners.

## Exact Next Batch
- audit the audience chapter after the landed public visibility pass
...
- identify whether any meaningful audience-management seam still remains after public visibility posture landed
...
- reopen `/channel-settings` workflow work unless the audit proves a regression or one final narrow seam
...
- any recommended final audience batch is grounded in current backed audience truth only
