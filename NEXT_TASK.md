...
## Exact Next Task
Carry forward the still-valid Stage 4, PostHog, auth/home, Settings/Logout, login-visual, and Phase 0 Rachi baseline exactly as already proved. Do not automatically reopen profile/channel completion or Chi'lly Chat completion; treat the current profile/channel and Chi'lly Chat state as good enough for now unless a later blocker proves otherwise. The next exact lane is now Phase 2: moderation/admin minimums after Rachi, while emulator/device-heavy proof stays deferred until the final integration phase unless a real blocker requires one targeted runtime check.
...
5. Complete moderation/admin minimums in the smallest durable way that builds safely on the protected Rachi foundation
...
- complete Phase 2 moderation/admin minimums after Rachi: report flow, role-aware permissions, minimum admin/moderation surfaces or foundations, and audit-minded platform action structure
- keep the next build phase scoped: moderation/admin minimums only, no broad profile/channel rewrites, no Chi'lly Chat completion sweep, no Home/content polish yet, and no release-hardening churn yet
...
- after Phase 2, continue in exact order: Phase 3 Home/content polish, Phase 4 release hardening, then Phase 5 final integration proof
...
- complete moderation/admin minimums in the smallest durable way that fits the current architecture
...
- broaden the next pass beyond moderation/admin minimums into Home/content polish or release-hardening work
...
- moderation/admin minimums are defined as the active next lane without reopening unrelated build work
