...
5. Next real media milestone is a separately approved real backend worker deployment plan or trusted `video_renditions` metadata migration design, not production playback migration: trusted private-source download, ffprobe/ffmpeg, 360p/480p/720p/1080p outputs, master manifest, variant playlists, thumbnails, public-playback bucket upload after approval, production-safe cache proof, trusted `video_renditions` rows, and RLS/backend write proof.
