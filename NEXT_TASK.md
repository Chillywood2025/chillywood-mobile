# NEXT TASK

## Exact Next Task
The active pass remains a narrow real-device verification follow-up. Repo truth is explicit: Device A runtime recovery is still good, the Party Room invite open-path bug is fixed, the cross-device invite/thread handoff is fully re-proved, fresh locked-room non-member denial is now proved honestly, and the only remaining verification gap is the true on-stage route. The exact next step is a narrow live-stage entry / handoff rerun: prove the host-side and viewer-side on-stage path cleanly on two real phones, using `app/watch-party/live-stage/[partyId].tsx` as the primary owner and only the smallest directly-related handoff owner if a route-local bug still blocks it.

## Current Plan
1. Preserve the current truthful product baseline exactly as landed across profile, watch-party/live, chat, title/player, settings/legal/support, channel settings, and bounded admin.
2. Carry forward the completed single-device proof on Home, profile, title, standalone player entry, watch-party/live owners, chat, channel settings, settings, support, and public legal surfaces.
3. Carry forward the real two-phone proof already gathered for Party Room host/viewer behavior, pre-stage Live Room host/viewer behavior, and room presence across both devices.
4. Carry forward the landed watch-party entry lane-switch fix without widening it into broader room/live redesign work.
5. Carry forward the landed room-aware invite search fix without widening it into broader chat or profile work.
6. Keep both proof phones on the healthy current-bundle runtime and carry forward the now-reproved invite/thread handoff.
7. Carry forward the now-proved fresh locked-room non-member denial and keep the locked-room denial copy honest on `/watch-party/[partyId]`.
8. Re-run the true on-stage path on two real phones and prove host/viewer stage entry on the canonical live-stage owner if that route can be reached cleanly.
9. Keep unrelated local dirt out of the checkpoint.

## Exact Next Batch
- keep the verified single-device baseline plus the now-landed two-phone watch-party entry fix intact
- keep the now-expanded room-aware invite search/match fix narrow and truthful
- keep Device A on the healthy current-bundle runtime
- carry forward the now-reproved cross-device invite/thread handoff on the two real phones
- keep the now-proved fresh locked-room denial intact without widening it into broader watch-party work
- keep the dev-only live-room invite autolaunch removed so host reruns read the real route first
- finish the remaining on-stage proof gap by re-testing the host-side and viewer-side live-stage path from a clean route-owned entry
- keep unrelated local dirt out of the checkpoint

## Scope
This next pass should:
- preserve all current canonical route owners and landed doctrine exactly as-is unless real verification proves a narrow fix is needed
- keep the current social baseline, monetization/access truth, and no-fake-comments/no-fake-ads doctrine intact
- use `docs/native-friend-graph-implementation-spec.md` as the implementation source of truth
- use `docs/native-social-engagement-foundation-spec.md` as the engagement-truth source of truth
- do not begin release hardening in this pass; only verify whether the current product baseline behaves honestly enough to close the lane cleanly
- keep `app/watch-party/live-stage/[partyId].tsx` as the primary remaining owner for the final verification follow-up
- keep unrelated local dirt out of the checkpoint

## Out Of Scope
Do not:
 - change route truth
 - reopen a closed chapter casually before the broader audit selects it
 - reopen Google Play readiness, OVH work, or database-move work
 - invent fake room powers, fake comments, fake social claims, fake ads, or fake entitlements
 - mix unrelated local dirt into the checkpoint

## Success Criteria
The next lane is successful when:
- one real device has already proved the key route-owned flows honestly and the public legal crash is fixed
- two real devices already prove the most important room host/viewer flows honestly, and the remaining claims stay precise about what is still partial
- the landed watch-party entry lane-switch fix stays verified on-device without changing room/live doctrine
- the next pass focuses only on the now-cleared host-runtime recovery, the now-fixed Party Room invite flow, the now-proved locked-room denial, and the last directly related on-stage proof blocker
- the repo truth clearly states that the broader verification lane is not fully closed yet, but the remainder is now a narrow live-stage entry / handoff proof follow-up rather than another full sweep
- no route drift, schema drift, fake room powers, or fake social claims are introduced
- the staged set stays task-pure
