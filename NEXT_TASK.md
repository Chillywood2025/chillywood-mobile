...
- The map records 11 P1 blockers and 10 P2 deferrals after the malware-scanning runtime proof. Scanner implementation and linked-Supabase runtime proof are closed; production deployment/monitoring is still an ops handoff.
...
- Malware scanning is now implemented and runtime-proved repo-side: new media scan metadata, `media_scan_jobs`, service-role scan RPCs, upload/update triggers, public-safe rendering gates, and a ClamAV worker live in the repo. Proof at `/tmp/chillywood-malware-scanner-runtime-proof-20260530/` scanned temporary private `dmca-evidence` objects against linked Supabase, read back `clean` and `malware_detected`, and cleaned up the proof rows/objects. Do not claim continuous production scanner coverage until the worker is deployed with server-side secrets, logging, alerts, and restart policy.
...
- Do not claim production malware scanning is continuously operational until the ClamAV worker is deployed in the selected runtime with monitoring. The implementation and linked-Supabase runtime proof exist; the next scanner work is production deployment/monitoring, not foundation.
