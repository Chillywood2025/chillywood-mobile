...
- Provider webhook reliability is now a cross-provider Money Operator loop. Protected surfaces are `revenuecat_webhook_delivery`, `google_play_webhook_delivery`, `stripe_connect_webhook_delivery`, `stripe_merch_webhook_delivery`, `provider_readiness_audit`, `provider_delivery_error_rate`, `stale_provider_dashboard_integration_detection`, and `duplicate_webhook_integration_detection`. Package scripts now include `money-operator:provider-health`, `money-operator:watch-once`, `money-operator:report`, `proof:provider-webhook-reliability`, `proof:money-provider-reliability-loop`, and `guard:provider-webhook-reliability`. RevenueCat is function-healthy and fail-closes missing/invalid shared auth, but dashboard valid TEST proof remains pending because Codex does not have provider dashboard access or webhook secret values; pending Level 3 approval request `a48f5fff-7c99-46ff-87e7-5bfbf2ee7bf5` asks the owner to review/repair RevenueCat dashboard webhook URL/shared auth/duplicate webhook state. Google Play direct webhook is readiness-only/stale because `GOOGLE_PLAY_WEBHOOK_SECRET` is absent and RevenueCat remains entitlement source of truth. Stripe Connect and Stripe merch webhook functions were redeployed from existing source and now fail closed on invalid signatures. Error-rate classes are healthy/degraded/critical/outage/unknown; a 100% provider webhook error rate must record failed/blocked sync status plus a reconciliation finding, and dashboard mutation creates an approval request. Do not change provider dashboards, event selections, webhook signing config, products, live mode, Premium entitlements, payouts, charges, transfers, cashout, invoices, or payment links without the existing Level 3/4 approval path and external confirmation where required.
