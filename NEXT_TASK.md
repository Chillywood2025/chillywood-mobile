...
2. First production CLI auto-detect planning is complete: linked read-only discovery found `0` `eligible_needs_transcode` candidates and `1` already-audited HLS source, so `plan-auto` and `dry-run-auto` correctly selected no batch. Do not run `run-auto` until future scans/moderation create eligible public-safe candidates and the owner/policy allows execution.
