...
Current spotted-fix lane should finish with validation and Play-installed/manual proof for owner Platform identity, owner-vs-viewer subscription/VIP route behavior, useful VIP/Subscriber areas, owner-only Platform content actions, and standalone Player fullscreen framing. The newest visible-UX focus is to prove owner Platform renders `Creator Offers` management, viewer/tester Platform renders `Support this Platform`, owner subscription/VIP management opens Platform Studio offer setup rather than the Premium purchase shell, owner self-purchase is blocked, true non-owner viewers/testers still see purchase CTAs where configured, and Premium remains only Chi'llywood Premium. Do not claim full Platform/Profile `public/private/subscriber_only` visibility until a later backed schema/product lane lands separate Profile and Platform access fields, server-side access resolvers, public read-path masking, and subscriber-only Profile behavior; followers/following remain public discovery/social relationships only.

Visibility implementation lane:

- Preserve current backed truth: Profile `everyone` is public, Profile `chilly_circle_only` is active Chi'lly Circle only, and Profile `private` is owner-only today; Platform `public_activity_visibility` plus follower/subscriber surface flags are audience posture, not a hard access gate.
- Local repo implementation now adds explicit Profile and Platform access values matching product language: `public`, `private`, and `subscriber_only`. Do not silently map old owner-only Profile `private` to the new Circle-or-subscriber meaning beyond the documented compatibility default.
- Local repo implementation adds `resolve_profile_visibility_access(...)` and `resolve_platform_visibility_access(...)` server-side/RPC helpers that combine owner/admin/operator, channel blocks, active Chi'lly Circle friendship, active creator subscription, signed-out state, and the selected visibility. Followers must not unlock private or subscriber-only access.
...
- Supabase migrations `20260617235547_profile_platform_access_visibility.sql` and `20260618000942_profile_access_visibility_rls_bridge.sql` are applied and post-apply dry-run reports the remote database is up to date. Before OTA/tester claim: run owner/viewer fixture proof and direct RPC/readback proof where service-role or SQL readback is available; local `db dump` was blocked by Docker not running.
- Required proof fixtures: owner, signed-out public viewer, follower-only viewer, active Chi'lly Circle member, active creator subscriber, blocked viewer, and non-subscriber/non-Circle viewer for both Profile and Platform.
...
- Paid Watch-Party Seats V1 is sandbox-proven for core room-ticket purchase, gate, paid entry, unpaid direct-link blocking, seat-limit enforcement, and Money Center RPC readback. It is not live-money sellable and still needs visual Money Center screenshot plus provider refund/revoke proof when safe tooling exists.
