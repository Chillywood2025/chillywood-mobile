...
- Hetzner Object Storage -> Cloudflare R2 private origin migration is Partial. Private R2 origin bucket `chillywood-media-origin` exists and source support is staged behind fail-closed env gates, but read-only inventory still shows `31` Hetzner/S3 object-storage reference rows (`22` distinct object refs). No media was processed, no media rows were written, no objects were copied, and Hetzner fallback retained. Next object-storage step is a trusted all-object backend copier plus R2 origin credentials, then copy/readback/checksum proof, DB metadata migration, new-upload R2 proof, scanner/transcode R2 read proof, and `0 Hetzner object-storage` refs. Do not shut down Hetzner LiveKit, `chillywood-prod-01`, or `live.chillywoodstream.com`.
