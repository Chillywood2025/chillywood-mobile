...
- Hetzner Object Storage -> Cloudflare R2 private origin migration is Partial. Private R2 origin bucket `chillywood-media-origin` exists and source support is staged behind fail-closed env gates. Trusted backend copier Edge Function `media-object-storage-migration` is deployed and token-gated; live inventory through the copier still shows `31` Hetzner/S3 object-storage reference rows (`22` distinct object refs). Copy stopped before mutation because the copier has no R2 private-origin write config secrets yet. No media was processed, no media rows were written, no objects were copied, and Hetzner fallback retained. Next object-storage step is to provision the scoped R2 private-origin write config for `chillywood-media-origin`, rerun copier copy/readback/checksum proof, then DB metadata migration, new-upload R2 proof, scanner/transcode R2 read proof, and `0 Hetzner object-storage` refs. Do not shut down Hetzner LiveKit, `chillywood-prod-01`, or `live.chillywoodstream.com`.
