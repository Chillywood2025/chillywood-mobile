...
Remaining Finance/Admin Lane Planning is recorded. Provider Billing Reconciliation Foundation is now pushed/deployed as backend-only reconciliation over existing usage rows, but provider billing API imports, test-mode payout transfer creation, production payout release, sponsor checkout test-mode, brand payment through Stripe, sponsor reporting/fraud integration, and fraud review queue all still require separate exact implementation prompts. Transfer creation requires provider readiness, KYC/tax readiness or safe test equivalent, fraud clear state, admin review, batch draft, immutable audit, idempotency, and explicit transfer-lane approval. Production payout release requires legal/accounting/provider approval and support/dispute readiness. Provider billing APIs must be server-side/idempotent/reconciled and must not send invoices or charge customers. Sponsor checkout/payment lanes must stay test-mode until review/disclosure/safety/fraud/audit proof and explicit product approval. Fraud review queue may not enforce restrictions, set risk scores, apply strikes, ban users, or pause payouts.
