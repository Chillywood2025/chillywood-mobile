...
Admin Usage Metering Foundations 37-39 is pushed. Current owners are `_lib/platformUsage.ts`, `app/admin.tsx` Usage readouts, migration `supabase/migrations/202605070003_platform_usage_metering_foundation.sql`, and generated table types in `supabase/database.types.ts`. The usage migration has been applied remotely and defines future `platform_usage_metering_events` and `platform_usage_daily_rollups` tables. Admin Usage must remain read-only until a future prompt explicitly scopes usage event writers and proof.
