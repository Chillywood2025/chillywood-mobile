...
- D7E/D7F follow-up is started as a Hetzner Object Storage HLS delivery lane. Server-side S3/Hetzner output secret names are present, but no proved public/custom HLS base URL is configured yet, so D7E public-safe playback remains blocked. A bounded exact-prefix public-read attempt temporarily applied a proof-prefix-only Hetzner bucket policy and a virtual-hosted `PUBLIC_HLS_BASE_URL`, but outside-LiveKit playlist fetch still returned Hetzner `403`; the temporary policy, temporary function, public base, proof room, and operator grant were cleaned up afterward. D7F may show foundation/readout states only and must not render playback until a real public-safe `.m3u8` and segment are proved outside LiveKit.
