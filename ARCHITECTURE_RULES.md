...
- D7B `room_broadcast_sessions` schema foundation plus the D7D test transition are pushed as admin/operator-owned foundation/test proof only; D7D real Egress ids/private HLS locations are allowed only for private `D7D_TEST_` proof rows, while D7F-approved public-safe rows require explicit server/admin approval, rights-safe public-free access, and the controlled spectator playback resolver
...
- D7C source update `8ba557b` is deployed and accepts existing `S3_*` output secret names as aliases for D7D `EGRESS_OUTPUT_*` readiness reporting, by name/status only and never values
...
- D7D private Egress start/status/stop proof passed on May 11, 2026 against a private `D7D_TEST_` room with one CLI proof participant. The proof called LiveKit `StartRoomCompositeEgress` and `StopEgress`, wrote a real private Egress id for the test row, completed cleanup, returned no HLS URL, enabled no public/spectator playback, created no full-room spectator token, and revoked the temporary proof-account operator grant after proof.
- D7E Hetzner Object Storage public-safe HLS delivery proof passed on May 12, 2026 with physical Android device `R5CR120QCBF` publishing real camera/mic tracks into a private proof LiveKit room. Supabase used Hetzner/S3 and LiveKit secrets server-side only; the temporary proof function returned no provider secret values. LiveKit Egress started and returned a real Egress id, Hetzner public playlist fetch reached `200` with a real `#EXTM3U` marker, and at least one HLS segment fetched outside LiveKit with `200` and `video/mp2t`. The proof exposed no full spectator LiveKit token, returned no HLS URL to app users, enabled no spectator playback, and left `PUBLIC_HLS_BASE_URL` unset. The temporary exact-prefix public-read policy, temporary proof function, nonce secret, local publisher token, proof room, and proof access were cleaned up afterward.
- D7F public-safe spectator HLS gate is implemented repo-side for eligible live public-safe rooms only. `spectator_hls_playback_records` is a server/admin-written read model; public reads expose only eligible live watch-only records and never raw provider URLs. `spectator-playback` owns the controlled app/admin resolver and playlist/segment proxy, `/spectate/[itemId]` renders playback only from that resolver, and private/proof/non-public/protected/title-rights-blocked/ticketed/Premium-full-room states remain blocked. D7F does not mint spectator LiveKit tokens, does not require `PUBLIC_HLS_BASE_URL`, and does not enable global public playback.

Forbidden outside the D7F eligible public-safe playback gate:
- global spectator playback
- raw HLS/Egress/CDN URL exposure to normal app users
...
Admin V1B2J is pushed as the remaining runtime-control enforcement. `live_first_enabled`, `live_watch_party_enabled`, and `watch_party_live_enabled` are enforced only through the central Premium live/watch-party helper layer before scoped Live First, Live Watch-Party hybrid, and Watch-Party Live entry flows; false controls show operations-paused copy and do not open Premium upsells, while Premium remains required when the controls are enabled. `max_upload_size_mb` is enforced before creator-video storage upload; default 5120 MB preserves the existing 5 GB limit. Admin may label these rows as enforced, but Admin Kill Switches must remain read-only until a separate backed write/toggle lane exists. `runtimeControls.ads_enabled` is still not the Ads Launch source of truth, and `premium_required_for_live` / `premium_required_for_watch_party` remain Premium helper truth, not runtime switches.
...
Sponsor Reporting / Fraud Integration Preflight Foundation is pushed and deployed as `sponsor-reporting-fraud-preflight`. Current owners are `supabase/functions/sponsor-reporting-fraud-preflight/index.ts` and `supabase/config.toml`. The function must keep `verify_jwt=false` only because it performs internal Supabase bearer auth; it must require owner/operator role, reject provider credentials, fake performance, revenue, payment, checkout, charge, risk-score, and enforcement instructions, read no provider secrets, make no provider calls, import no sponsor reports, create no fake sponsor performance or revenue, enforce no fraud action, restrict no creator, restrict no sponsor deal, activate no sponsor, charge no brand, release no creator payout, and return `preconditions_required` until backed reporting, disclosure/safety/fraud review, immutable audit, appeal/review path, and explicit product approval are proved. It may write immutable audit rows for authorized requested/blocked preflight attempts only.
