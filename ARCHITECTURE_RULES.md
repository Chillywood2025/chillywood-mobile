...
- D7E/D7F follow-up is started as a Hetzner Object Storage HLS delivery lane. Server-side S3/Hetzner output secret names are present, but no proved public/custom HLS base URL is configured yet, so D7E public-safe playback remains blocked. A bounded Supabase server-side storage probe confirmed Hetzner can return `200` for throwaway objects using either `x-amz-acl: public-read` or an exact-prefix public-read bucket policy, but real LiveKit Egress HLS delivery still is not proved: a no-publisher Egress probe started and returned an Egress id, yet signed playlist read returned `404` and public playlist fetch stayed `403`, so no `.m3u8` marker or segment was proved. The temporary policy, temporary function, public base/nonce, proof room, and operator/probe access were cleaned up afterward. D7F may show foundation/readout states only and must not render playback until a real public-safe `.m3u8` and segment are proved outside LiveKit.
