...
Stripe Connect Edge Function Skeletons (Payout S3B) is pushed source-only and not deployed. Current owners are `supabase/functions/stripe-connect-account/index.ts`, `supabase/functions/stripe-connect-onboarding-link/index.ts`, `supabase/functions/stripe-connect-account-sync/index.ts`, `supabase/functions/stripe-connect-webhook/index.ts`, and their `supabase/config.toml` entries. These functions return `not_configured`, use internal Supabase bearer auth validation where applicable, and do not read `STRIPE_*` env vars. `verify_jwt=false` is allowed here only because these skeletons use the repo's internal auth validation pattern. They must not grow Stripe SDK imports, Stripe keys, provider secrets, Stripe API calls, provider writes, webhook storage, connected-account creation, onboarding-link creation, transfers, payouts, checkout sessions, Connect Stripe UI, creator-facing payout setup buttons, fake payable balances, or live money actions without a separately scoped implementation and proof. Future real Stripe webhook processing must add signature verification and idempotency proof first, and future dangerous provider behavior must write immutable audit rows before/when it executes.
