...
- D7E/D7F follow-up is started as a Hetzner Object Storage HLS delivery lane. Server-side S3/Hetzner output secret names are present, but no proved public/custom HLS base URL is configured yet, so D7E public-safe playback remains blocked. D7F may show foundation/readout states only and must not render playback until a real public-safe `.m3u8` is proved outside LiveKit.
