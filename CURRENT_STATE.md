...
1. Admin V1B2 kill switch enforcement planning: choose the first real runtime control to enforce, prove the exact affected app surface reads `app_configurations.config.runtimeControls`, preserve existing permission gates, and keep Admin read-only unless a backed write path plus enforcement proof is scoped.
