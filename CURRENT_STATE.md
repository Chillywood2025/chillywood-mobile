...
1. Admin V1B2D kill switch enforcement audit/spec: map `profile_posting_enabled` first against backed Profile post creation submit only, then decide whether it is safe to implement. Preserve comments/replies, Chi'lly Circle, profile privacy, attachments, RLS, migrations, generated types, Supabase remote state, and Admin read-only boundaries unless a dedicated prompt scopes a real write-control lane.
