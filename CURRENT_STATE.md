...
1. Admin V1B2C kill switch enforcement audit/spec: choose the next real runtime control only after exact surface mapping, likely a comments/posting-related control, and preserve privacy, RLS, existing comment behavior, and Admin read-only boundaries unless a dedicated prompt scopes backed writes.
