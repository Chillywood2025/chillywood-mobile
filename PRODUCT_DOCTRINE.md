...
- Admin `runtimeControls.ads_enabled` is not currently the Ads Launch source of truth; Ads V1A/V1B/V1C read the Ads Launch config foundation (`ADS_LAUNCH_CONFIG_DEFAULTS` unless future config layering is explicitly implemented) and remain disabled by default
