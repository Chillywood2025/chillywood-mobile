...
- Ledger Systems 4A-4D finance foundation, Finance F2B/F2D/F2E payout-provider foundation, Network Billing Foundation, and Sponsor Checkout Foundation are pushed as schema/helper/Admin read-only foundation only. The finance, payout-provider, network billing, and sponsor checkout migrations are now applied to Supabase remote, generated database types include the foundation tables, Admin finance panels show read-only/foundation counts only, and no provider-backed creator ad revenue ledger, payout execution, sponsor deal execution, sponsor checkout, sponsor approval action, invoice action, live customer charge, payment link, fraud enforcement, or CTV revenue system is active.
...
- Admin finance foundation may show safe row counts for future finance ledger, payout ledger, payout-provider foundation, network billing, sponsor brands/deals/creatives/placements/disclosures/review logs/payment records/payout split records, and fraud holds now that the schema is applied and readable to platform roles. Counts are not live money totals and must not imply payout availability, payout-account readiness, invoice execution, sponsor checkout, brand payment, sponsor approval, payout split execution, fraud enforcement, provider reconciliation, fake sponsor revenue, or creator-facing balances.
