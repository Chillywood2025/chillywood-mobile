...
- Admin Usage Metering Foundation V1 remote/provider schema is pushed. Admin may show schema-connected/read-only counts for internal usage and provider-import/reconciliation foundation tables, but no app usage writers, provider API imports, provider billing truth, reconciliation jobs, overage calculation, customer invoices, bandwidth truth, or cost truth are active. Provider rows must stay `Not connected yet` until real imports exist.
