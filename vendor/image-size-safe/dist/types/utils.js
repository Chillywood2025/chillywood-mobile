...
function readBox(input, offset, endOffset) {
...
function findBox(input, boxName, offset, endOffset = input.length) {
    ...
    while (Number.isSafeInteger(currentOffset) &&
        currentOffset >= 0 &&
        currentOffset < endOffset) {
        const box = readBox(input, currentOffset, endOffset);
        if (!box)
            break;
        if (box.name === boxName)
            return box;
        const nextOffset = currentOffset + box.size;
