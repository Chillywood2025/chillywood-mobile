...
function decodeUriComponentUpstream(encodedURI) {
...
// This matches the application's external-navigation limit and bounds the
// number of replacement passes. Valid inputs still use the platform decoder;
// oversized or high-run malformed values remain encoded.
const maxMalformedComponentLength = 8192;
const maxMalformedPercentRuns = 32;
const unsafeMalformedReplacementToken = /%24%(?:26|27|60)/i;

module.exports = function decodeUriComponent(encodedURI) {
	if (typeof encodedURI !== 'string') {
		return decodeUriComponentUpstream(encodedURI);
	}

	try {
		return decodeURIComponent(encodedURI);
	} catch {
		// Continue only when the malformed component has bounded fallback work.
	}

	if (encodedURI.length > maxMalformedComponentLength) {
		return encodedURI;
	}

	const encodedRuns = encodedURI.match(multiMatcher);

	if (encodedRuns && encodedRuns.length > maxMalformedPercentRuns) {
		return encodedURI;
	}

	if (unsafeMalformedReplacementToken.test(encodedURI)) {
		return encodedURI;
	}
