...
This document remains the historical record for the self-hero and host-seat-card partial installed proof. It is superseded by the Closed v80 source-truth proof in `docs/release/GOOGLE_SIGNED_V80_LIVE_STAGE_SEAT_REQUEST_CONTRACT_AND_TRACK_IDENTITY_PROOF.md`, which proved room `NXQ4M2` on Google Play-installed v80 with both devices Premium-active through the approved sandbox path.
