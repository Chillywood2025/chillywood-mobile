...
June 28, 2026 source identity follow-up: cross-surface stale identity metadata is source-fixed but not installed-app Closed. Fresh remote profile must win over stale AsyncStorage, Settings/Profile/Chat must agree on the current handle, existing Chat inbox rows and Circle/Followers/Following/user-card/platform-role surfaces must not keep stale `@user230456` as primary identity, and v62+ Google Play internal proof is required before this can be actual-user Closed.
