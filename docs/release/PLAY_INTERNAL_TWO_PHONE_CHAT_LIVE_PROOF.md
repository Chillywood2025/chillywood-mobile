...
June 28, 2026 v60 Chi'lly Chat receiver banner thread-readback + responsive video layout follow-up: `docs/release/GOOGLE_SIGNED_V60_DIRECT_CHAT_CALL_PROOF.md` is Partial. EAS Build `8642fea7-b782-4c18-98c8-5805b6c7c20e` delivered versionCode `60` to Google Play internal testing and both physical phones updated through Google Play with installer `com.android.vending`. Visible Chat search and direct-thread open/create for `user230455` now work after authenticated RPC ambiguity fixes. A live receiver readback migration fixed the installed blocker where tapping the app-wide banner opened `This Chi'lly Chat thread could not be found.`; after the fix, the receiver tapped the banner and both phones showed `2 in call`. Source now adds a shared responsive layout foundation and fixes the video layout issue where the bottom feed could be cut off by controls and participant metadata blocked too much of the feed. Direct Chat video call layout adapts by dimensions and safe area, but fullscreen video fit is not Closed until proved on installed app and iOS/tablet/foldable proof remains Pending unless tested. Two-phone call proof remains Partial because installed v60 recorded a false `Missed voice call` after the joined call ended, the cleanup/responsive video layout source fixes are not installed in Google Play yet, and video/background/decline/missed cleanup matrices remain incomplete.

June 28, 2026 v61 Chi'lly Chat responsive video proof: `docs/release/GOOGLE_SIGNED_V60_DIRECT_CHAT_CALL_PROOF.md` records Google-signed Play internal versionCode `61` installed on both physical Android phones from `com.android.vending`. EAS Build `bc2e9532-6a1e-4174-a153-679345c6ef20` included commit `70b276c336b1164a674a8ae51b421e0a039d0d35`; EAS Submit `36c7bae7-4181-4c67-ac46-75070f76142f` submitted to Google Play internal only. Android two-phone installed responsive video layout proof passed for owner -> user Direct Chat video: local/remote video on both phones, `2 in call`, bottom feed not cut off, controls below video and safe-area aware, compact participant edge metadata, Back to Thread, End Call, no visible false missed-call text after joined video calls, and repeated call after end with a new room. iOS/tablet/foldable proof remains Pending unless tested; background push/ringing and decline/missed/background cleanup remain Partial.

June 28, 2026 identity follow-up: cross-surface stale identity metadata is source-fixed but installed-app proof remains Partial. Play-installed versionCode `63` reached fresh surfaces showing `user230455` / `@user230455`, but a separate existing Chat row still displayed `Proof Normal` / `@user230456` and opened to the same stale header. Fresh remote profile must win over stale AsyncStorage, Settings/Profile/Chat must agree on the current handle, and existing Chat inbox rows plus Circle/Followers/Following/user-card/platform-role surfaces must not keep stale `@user230456` as primary identity before this can be actual-user Closed.
...
- Latest source fixes are present in the repo:
  ...
  - Direct Chat fullscreen video layout reserves bottom control/safe-area space, uses compact edge metadata, and now has a shared responsive foundation for phone-size/safe-area adaptation.
