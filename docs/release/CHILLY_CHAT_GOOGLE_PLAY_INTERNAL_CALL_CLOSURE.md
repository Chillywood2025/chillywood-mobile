...
Root cause: the current direct-thread screen made call/status surfaces visually heavier than the actual message thread when recent call events existed.

Files changed: `app/chat/[threadId].tsx`.

Screenshots reviewed: Pic 1 is the intended message-first thread direction; Pic 2 is the call/status-heavy drift that this source change corrects.

Thread message list path reviewed: real message history remains rendered in `chat-thread-messages-scroll`.

Call event rendering path reviewed: call events stay in-thread but now appear as lightweight recent-call timeline rows inside the message area, not large pre-message cards.

Thread status card reviewed: the card is reduced to compact message-thread context.

Composer path reviewed: composer, attachments, and Send remain visible and usable.
...
June 28, 2026 v60 receiver banner thread-readback + responsive video layout follow-up: `docs/release/GOOGLE_SIGNED_V60_DIRECT_CHAT_CALL_PROOF.md` is Partial. Both attached phones updated through Google Play internal to versionCode `60` with installer `com.android.vending`, and no logout, uninstall, reinstall, sideload, or clear-data happened. Fresh `user230455` search and visible direct-thread open succeeded after targeted authenticated RPC ambiguity fixes. A live receiver readback migration fixed the installed blocker where tapping the receiver banner opened `This Chi'lly Chat thread could not be found.`; after the fix, R5 tapped the real incoming banner and both phones showed `2 in call`. Source now adds a shared responsive layout foundation and fixes the observed video layout issue where the lower feed could be cut off by bottom controls and a dark participant card covered too much of the feed. Video tiles must adapt to phone size instead of hard-coded device hacks, but fullscreen video fit is not Closed until proved on installed app. Full actual-user call closure remains Partial because installed v60 recorded a false `Missed voice call` after the joined call ended, the cleanup/responsive video layout source fixes are not installed in Google Play yet, iOS/tablet/foldable proof remains Pending unless tested, and video, background push, same-thread, decline/missed, and full cleanup proof remain incomplete.

June 28, 2026 v61 Google-signed responsive video call proof: `docs/release/GOOGLE_SIGNED_V60_DIRECT_CHAT_CALL_PROOF.md` now records Android two-phone installed responsive video layout proof as Closed for the owner -> user Direct Chat video path. EAS Build `bc2e9532-6a1e-4174-a153-679345c6ef20` and EAS Submit `36c7bae7-4181-4c67-ac46-75070f76142f` delivered versionCode `61`, versionName `1.0.0`, commit `70b276c336b1164a674a8ae51b421e0a039d0d35` to Google Play internal only. Both attached phones updated through Google Play with installer `com.android.vending` and no logout, uninstall, reinstall, sideload, or clear-data. Android two-phone installed proof passed for `2 in call`, local/remote video on both phones, no bottom feed cutoff, no bottom control overlap, compact participant metadata, Back to Thread, End Call, no visible false missed-call text after joined video calls, and repeated call after end using a new room. Full call closure remains Partial because background push/ringing, decline/missed/background cleanup, user -> owner direction, stale existing inbox metadata, and iOS/tablet/foldable proof remain incomplete.

June 28 source follow-up for cross-surface identity: source now fixes the stale identity path where Settings/Profile/Chat search could show `@user230455` while an existing Chat inbox row still showed stale `@user230456`. One user identity must render consistently across profile, chat, search, circle, followers, and following. Fresh remote profile must win over stale AsyncStorage. Settings/Profile/Chat must agree on the current handle. Circle/Followers/Following must not keep stale handle metadata as primary identity. Existing inbox rows must not show stale participant metadata as primary identity. Stale @user230456 is not Closed if it still appears as the primary inbox, circle, follower, following, or user-card identity. Platform/owner/admin/moderator/creator surfaces must use the same fresh profile identity source as Chat/Profile/Search/Circle/Followers; role badges may show role/status, but they must not cause stale handle/name/avatar metadata to win. Source fixed is not installed-app proof, and Google Play internal install is not enough without actual user flow proof; v62+ Google Play internal proof is required before this identity lane can be actual-user Closed.
