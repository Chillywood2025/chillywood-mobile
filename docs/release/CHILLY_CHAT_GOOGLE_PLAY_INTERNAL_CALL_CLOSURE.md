...
June 28, 2026 v60 follow-up: `docs/release/GOOGLE_SIGNED_V60_DIRECT_CHAT_CALL_PROOF.md` is Partial. Both attached phones updated through Google Play internal to versionCode `60` with installer `com.android.vending`, and no logout, uninstall, reinstall, sideload, or clear-data happened. Fresh `user230455` search and visible direct-thread open succeeded after targeted authenticated RPC ambiguity fixes, and a voice call showed an app-wide incoming banner on the receiver while elsewhere in app. Full actual-user call closure remains Partial because tapping the receiver banner opened `This Chi'lly Chat thread could not be found.` and the caller stayed at `1 in call`; video, background push, same-thread, and full cleanup proof remain incomplete.
