...
| Direct Chat fullscreen video lower feed can sit under bottom controls and participant metadata can cover too much video. | Fixed now / installed proof pending | Current source reserves safe-area bottom control space, flexes tiles inside the available stage, and uses compact edge metadata; installed two-phone proof remains required. |
