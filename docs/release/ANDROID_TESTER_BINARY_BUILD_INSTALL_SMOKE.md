...
Every visible surface active wiring audit: Closed. Testers should verify the active route/status/tester-safe flows from Google Play internal/closed testing after the next approved build/update includes this lane. No visible clickable dead buttons are allowed. Nothing visible should be hidden or disabled. Every visible control works, routes correctly, opens a setup/status/resolution flow, opens a support/review flow, or starts a tester-safe flow.
