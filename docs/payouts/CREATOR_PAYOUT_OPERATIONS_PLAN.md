# Creator Payout Operations Plan
...
A creator payout flow should check account standing, accepted terms, Stripe Connect onboarding, KYC status, tax status, provider disabled reasons, requirements currently due, fraud holds, refund reserve, chargeback exposure, minimum payout threshold, payout schedule, and Owner/Admin approval. A creator who fails any requirement should see a clear readiness blocker, not a payout button.

## Operations Flow

Operations should reconcile provider events, purchase intents, ledger rows, refunds, chargebacks, fees, holds, and payout records before any payout batch. Payout batches should require review, evidence, and audit. Provider transfers and payouts should be created only in a future approved live lane, never from sandbox readiness docs.

## Creator Communication

Creators should see that payouts are not active until production approval. After activation, they should see plain-language eligibility, schedule, minimum threshold, holds, reserves, tax status, and support path. They should not see fake balances or sandbox earnings.
