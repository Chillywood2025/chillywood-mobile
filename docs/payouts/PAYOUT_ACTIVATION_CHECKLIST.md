...
This checklist defines the evidence required before Chi'llwood can activate production payout execution.
...
Confirm Stripe Connect live platform approval, live account configuration, webhook live endpoint verification, connected account status requirements, KYC complete criteria, tax complete criteria, disabled_reason handling, requirements_currently_due handling, provider payout capability, and provider dashboard access. Test-mode readiness is not enough.

## Policy Requirements

Confirm legal approval, tax approval, payout terms acceptance, fraud/risk holds, support playbook, refund and chargeback reserve, negative balance handling, dispute workflow, creator statements, privacy and Data Safety review, and Owner approval. Each requirement needs evidence and responsible owner.

## Technical Requirements

Confirm live_money_enabled review, payouts_enabled review, cashout_enabled review, kill switch availability, no single-click activation, rollback plan, audit logging, admin permission boundaries, no Stripe Android digital checkout, and route/LiveKit unchanged. Activation should be staged and reversible.
