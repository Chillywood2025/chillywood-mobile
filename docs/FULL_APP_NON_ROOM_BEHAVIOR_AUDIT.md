...
Remote Supabase proof update: migration `202604290001_public_v1_social_basics.sql` is applied to the linked project, and sanitized PostgREST proof confirms `profile_posts` owner create/delete, public clean reads, draft/hidden filtering, and `profile_post` report target support. New local migration `202604290002_profile_post_engagement.sql` adds Profile post comments, likes, and `profile_post_comment` reports but still needs remote application/proof. Android UI proof for the Profile Posts flow and keyboard visibility remains pending.
...
Engagement truth: creator-video Report is backed in Player, public creator-video Share uses the app route/deep link, standalone creator-video text comments are backed through `creator_video_comments`, and text-only Profile updates are backed through `profile_posts`. Profile post text comments and single likes/counts are now locally backed through `profile_post_comments` and `profile_post_likes`, with remote application pending. Creator-video likes/saves/counts, media posts/comments, nested replies, richer reactions, and full Friends are not backed or shown. Title-only engagement remains title-only.
...
| --- | --- | --- |
| Profiles/channel | `user_profiles`, `profile_posts`, `profile_post_comments`, `profile_post_likes`, profile helpers, channel read models | Route code uses profile/channel helpers plus text-only Profile posts, comments, likes/counts, and share; runtime owner/public proof and remote engagement migration proof pending. |
