...
Presentation update: creator videos now use a shared media-first card in Channel Settings and feed-style post cards on Profile/Channel Posts. The media card/post shows real thumbnails from `thumb_url` / `thumb_storage_path` when available, a branded fallback preview when not, Play overlay, visibility and moderation badges where appropriate, file size/date metadata where present, and owner controls only in owner surfaces or management handoffs.
