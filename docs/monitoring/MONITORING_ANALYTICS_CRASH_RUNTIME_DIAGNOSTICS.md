...
Audit log integrity and privileged action evidence governance: Closed for current repo-side monitoring alignment. Monitoring, analytics, crash, and runtime diagnostics must not copy raw audit logs, private evidence, private chat bodies, call content, tokens, signed URLs, raw IPs, provider secrets, payment credentials, tax IDs, bank details, or private reporter identity into logs, analytics, crash reports, support diagnostics, or proof artifacts.
