...
- Do not run live billing/store, ad-provider, or dashboard setup actions during repo audits.
...
Creator monetization systems foundation is now live-schema installed through remote migration `202605140011_creator_monetization_systems_foundation.sql` and repo-backed in `_lib/creatorMonetization.ts`, Admin readouts, Platform Studio Money Center, public Platform readiness copy, and Player resolver integration. Post-apply proof shows the linked Chi'llywood project has `202605140011` local/remote aligned, dry-run up to date, linked lint clean, all live-money flags default off, and anon direct money-table writes denied. The foundation covers creator pricing/access-control records, product/order records, tip records, append-only earnings ledger, payout request records, webhook idempotency, audit logs, fail-closed RPCs, Money Center readiness sections, public Platform no-checkout commerce/readiness state, and Player paid-content resolver checks before creator-video playback signing. `_lib/paymentRailPolicy.ts`, `supabase/functions/_shared/payment-rail-policy.ts`, and `guard:payment-rail-policy` now make the payment rails explicit before provider setup: Premium is Google Play/RevenueCat only; Android in-app digital creator paid content requires Google Play Billing or an approved Play billing/external-offers path; Android creator tips use Google Play/RevenueCat; physical merch/products can use Stripe or a commerce provider only after provider/tax/shipping/refund proof; payout/cash-out remains Stripe Connect-only when later enabled. This is not live money: paid creator checkout, tips, merch checkout, cash-out, payout execution, production Stripe Connect, and real paid-content access grants remain external/provider/legal/security proof blockers. Do not configure Stripe production transfers, Stripe checkout, Google Play paid-content products, provider webhooks, cash-out, or payouts from this checklist unless a later exact lane proves provider policy, legal/tax/accounting, fraud/DMCA holds, refunds/chargebacks, idempotency, immutable audit, Owner approval, and secret storage.

## RevenueCat / Google Play Premium Credentials

Google Cloud project `chillywood-app` has Android Publisher API, Play Developer Reporting API, Pub/Sub API, and Cloud Monitoring API enabled for RevenueCat/Play support. Service account `chillywood-revenuecat-play@chillywood-app.iam.gserviceaccount.com` exists, and the service account JSON is stored outside the repo at `/Users/loverslane/secrets/chillywood/revenuecat-google-play-service-account.json`; do not move it into the repo or print its private key. Project IAM for that service account is scoped to `roles/pubsub.editor` and `roles/monitoring.viewer`; no Owner, Editor, Admin, Service Account Token Creator, broad Firebase, or client-side secret grant is required from this lane. Play Console shows the service account as active per operator report, but gcloud cannot fully verify app-level Play Console permissions. RevenueCat API proof using a secret key stored outside repo confirmed project `projc5629a24`, Play Store app `appd24db94dd8` for `com.chillywood.mobile`, and one Android production public SDK key. Canonical entitlement `premium` and offering `premium` now exist in RevenueCat, and the RevenueCat Android public SDK key is configured in EAS production env. EAS production AAB build `aedded50-3818-4d7b-8bcf-9bb9c2d36513` completed for versionCode `2` and was manually uploaded to Google Play Internal Testing as `Internal test v2 - RevenueCat Premium setup`. EAS production build `96e60082-00b1-48dc-843b-132ce82a7710` completed for versionCode `3` and EAS submitted it to the internal track only as submission `920c452f-1b9f-401c-94c9-616801684939`. Android Publisher API package visibility for `com.chillywood.mobile` returns `200`; after the payments profile appeared registered, the API created/verified subscription `premium_subscription`, monthly base plan `monthly` is `ACTIVE`, billing period is `P1M`, and the US price is `$9.99/month`. RevenueCat product `premium_subscription:monthly` (`prod38dd089721`) exists and is attached to entitlement `premium`; offering `premium` is current; package `$rc_monthly` (`pkge6abeb5695a`) exists under offering `premium`; and package `$rc_monthly` is attached to `premium_subscription:monthly` with eligibility `all`. Physical-device proof on `R3CXA0DS5JV` installed versionCode `3` from Google Play, loaded the offer, completed a no-charge Google Play test purchase, proved RevenueCat active entitlement, restored purchases, canceled through Google Play, waited for test-cycle expiry, and proved entitlement removal plus in-app Premium blocking. RevenueCat credential-health/attention status is not exposed by the available API response. Remaining external setup is final release smoke and Play listing/Data Safety/account-deletion acceptance.

May 19, 2026 Live Stage / Live Watch-Party / Live-First media-contract hardening is repo-backed and keeps the approved room UX/layout unchanged. Approved Seats remains enforced by the backend LiveKit token authority, fallback/avatar/presence is not counted as media success, old/stale rooms remain guarded, and new static guards cover canonical route ownership, layout markers, participant visibility, comments/reactions/composer markers, token grants, muted/unseated downgrade, and reconnect/fallback behavior. No new AAB is required for this hardening lane because it did not change native packages, signing, build profiles, schema, migrations, public APIs, Premium gates, Stripe/payouts/live money, ads, RevenueCat/Google Play, or Hetzner/HLS.

May 19, 2026 Watch-Party Live Player shared-playback hardening is repo-backed and keeps the approved Player room UX/layout unchanged. Shared playback mutation is host-only, viewers stay synced and retain non-playback room interactions, and Player native video promise rejections are caught/logged safely instead of surfacing the dev-runtime unhandled promise toast. `guard:watch-party-livekit` covers the playback authority lock and promise handling. No external dashboard action, function deploy, native build, package/signing change, schema/migration, public API change, Premium gate change, Stripe/payout/live-money action, ad-provider action, RevenueCat/Google Play action, or Hetzner/HLS change is required. No new AAB is required for this JS/runtime hardening lane.
...
| --- | --- | --- | --- | --- | --- | --- |
| Google Play Billing product | `react-native-purchases` and `react-native-purchases-ui` are installed. `_lib/revenuecat.ts`, `_lib/monetization.ts`, `_lib/premiumEntitlements.ts`, `_lib/premiumWatchPartyAccess.ts`, and `app/subscribe.tsx` own Premium billing UX and entitlement checks. The app does not grant Premium from local-only state. All full live/watch-party access is Premium in app-side gates. Play subscription `premium_subscription` exists with active monthly base plan `monthly`, billing period `P1M`, and US price `$9.99/month`; RevenueCat offering `premium` is current with `$rc_monthly` attached to `premium_subscription:monthly`. | Keep Play subscription status testing-ready/active for internal-test purchase proof. | Product owner plus Play Console/RevenueCat operator. | Internal build purchase starts, product loads, purchase completes, backend entitlement becomes active, and protected live/watch-party routes unlock only after trusted entitlement truth. | External Setup Pending | Run license-tester purchase proof with full Live First, Live Watch-Party, and Watch-Party Live. |
| RevenueCat configuration | Runtime config supports `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY_DEV`, `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY`, and `EXPO_PUBLIC_REVENUECAT_IOS_PUBLIC_SDK_KEY`. Monetization targets include `premium_subscription`, `premium_live_access`, and `premium_watch_party_access`; later paid content targets remain non-v1. The app prefers RevenueCat offering `premium` and can fall back for `premium_subscription` only to a RevenueCat current/default/first offering with purchasable packages, but entitlement truth still owns access. RevenueCat API proof confirms the Android app, Android production public SDK key, entitlement `premium`, offering `premium`, product `premium_subscription:monthly`, entitlement attachment, current offering status, and `$rc_monthly` package attachment; EAS production env contains `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY`, and key-injected local validation reports RevenueCat configured. Physical-device internal-test proof confirmed `/subscribe` loads the configured Premium offer, the no-charge Google Play test purchase activates RevenueCat `premium`, Restore works, cancellation/test-cycle expiry removes active entitlements, and missing entitlement blocks Premium again. | Preserve the RevenueCat/Play mapping and rerun proof only if product/offering/env changes. | Billing owner. | `/subscribe` shows a valid RevenueCat Premium offer, purchase and restore call RevenueCat, active entitlement is reflected by account-owned truth, and missing/expired/revoked states remain blocked. | Premium Proof Passed / Release Smoke Pending | Continue final release smoke; do not weaken entitlement checks or use local Premium flags. |
| Billing restore path | `/subscribe` exposes restore/manage actions through the existing RevenueCat owner and shows honest failure copy when unavailable. | Store account restore is configured through RevenueCat/Google Play and passed same-session attached-device proof. Reinstall/clear-state restore remains optional release smoke. | Billing owner. | Restore button proof on the Play-installed internal-test build returned `Purchases restored.` and kept RevenueCat entitlement active; no receipt tokens were stored. | Restore Proof Passed / Reinstall Smoke Pending | Preserve restore behavior and rerun after any RevenueCat, Play product, or auth-session changes. |
| Ad provider setup | Ads Launch Foundation V1A/V1B/V1C is pushed in repo as provider-neutral, no-SDK, no-real-rendering infrastructure. Current owners are `_lib/ads/adConfig.ts`, `_lib/ads/adEligibility.ts`, `_lib/ads/adProvider.ts`, `_lib/ads/providers/placeholder.ts`, `_lib/ads/adSession.ts`, `hooks/useAdEligibility.ts`, `hooks/useActiveBrowsingTime.ts`, `components/ads/NativeAdSlot.tsx`, `components/ads/InterstitialController.tsx`, Home placement in `app/(tabs)/index.tsx`, root interstitial controller mount in `app/_layout.tsx`, and read-only/foundation Admin Ads status in `app/admin.tsx`. Default `ads_enabled` is false, default provider is `placeholder`, the placeholder provider reports not connected without SDK calls, and Home native/feed plus placeholder interstitial controller foundation stay hidden in normal runtime until real backed ad enablement exists. | Future external AppLovin MAX account/app/ad-unit setup is required before real ads. Unity LevelPlay / Unity Ads may be added later through AppLovin MAX. Do not create an AdMob-only path. Keep real providers behind the Chi'llywood provider-neutral wrapper. | Product owner plus ad-ops/release owner. | After a dedicated future AppLovin integration lane, release proof must show no direct screen-level SDK calls, no committed provider secrets/ad IDs beyond approved public/native config, Premium users see zero ads, caps are enforced, and forbidden contexts suppress ads. Ads V1C placeholder interstitial helper proof passed before real SDK integration. | Later / External Setup Pending | Do not run AppLovin dashboard setup from repo-control passes. Next app-side ad lane is real AppLovin MAX readiness/integration planning only after account/app/ad units are ready; no real SDK or IDs should be added before that explicit lane. |
...
| Account deletion policy | `app/account-deletion.tsx` exists; Settings links to deletion support, and the public web URL candidate is live at `https://chillywoodstream.com/account-deletion`. It is an honest request/help flow, not fake automated deletion. The route now includes request status, identity verification, Profile/Channel/upload effects, chat/room/social effects, retained-record categories, subscription caveats, timing placeholders, and backend-runbook requirements. The live URL redirects with HTTP/2 308 to `/account-deletion/` and then returns HTTP/2 200 with real static content. | Confirm process owner, response SLA, backend deletion/export policy, and Play Console account deletion declaration/acceptance. | Product owner plus support/legal owner. | URL is public, opens outside the app, explains delete request flow, and Play Console accepts it. | Live URL Candidate / Play Acceptance Pending | Follow the Lane 2 runbook below and `docs/ACCOUNT_LEGAL_DATA_SAFETY_RUNBOOK.md`; enter `https://chillywoodstream.com/account-deletion` in Play Console when ready, and do not build destructive deletion until the backend retention plan is approved. |
| Legal pages and support | Expanded app routes exist for Privacy, Terms, Community Guidelines, Creator Rules, Copyright/DMCA, formal Copyright Report, Support, and account deletion. `legal/policies.mjs` is now the canonical production policy bundle for the 12 full policy pages: Privacy, Terms, Community Guidelines, Creator Rules, Copyright/DMCA, Support & Account Help, Account Deletion, Premium Terms, Live/Watch-Party/Chat Rules, Law Enforcement, Moderation/Appeals, and Creator Monetization. `public-site/legal-site/` builds 18 static pages from that bundle, including the hosted public copyright notice form at `/copyright-report` and compatibility aliases, using only confirmed `support@chillywoodstream.com` for public contact. Cloudflare Pages project `chillywood-legal` redeployed on May 22, 2026; custom domain `chillywoodstream.com` is active. Apex proof returned HTTP 200 after redirect for `/copyright-report/`; May 21 proof remains valid for `/privacy/`, `/creator-monetization/`, `/live-rules/`, `/terms/`, `/account-deletion/`, `/copyright/`, and `/support/` unless later deployment regresses them. Admin Canary legal/DMCA readiness ran from the physical Android owner device release build and passed with `65 pass / 0 manual / 0 fail` after Legal Intake / Legal Evidence closeout. No LiveKit fallback was reached; Privacy mentions LiveKit only as policy text. `https://chillywoodstream.com/support` and `https://chillywoodstream.com/copyright-report` are live. DMCA designated agent public contact posting and U.S. Copyright Office registration are recorded complete under registration `DMCA-1072720`; the copyright page lists Chi'llywood Copyright Agent / Chi'llywood, `support@chillywoodstream.com`, 9316 S Kimbark, Chicago, IL 60619, and phone `3124879454`. Backed/Admin/public DMCA operational tooling is live-proof closed for public notice submission, private evidence attachment upload/access denial with private evidence scan queueing and pending worker proof, formal notice intake, invalid notice rejection, uploader counter-notice self-service/other-user denial, owner/operator Admin case visibility/actions, supported content hide/restore, counter-notice recording/deadlines, court-action restore blocking, content action history, copyright strikes, repeat-infringer review, DMCA audit history, RLS/private-data denial, generic report compatibility, and proof-content cleanup. Owner/Admin Legal Intake and Legal Evidence are live-proof closed for one top-level Legal tab, request list/create/open/status/timeline/evidence linkage, owner normal no-reason work, scoped Admin preview/export/hold enforcement, ungranted Admin/moderator/viewer denial, target-matrix coverage, and proof grant cleanup. Automated email ingestion is not configured; manual support/admin intake is the live path. `app.config.ts` has privacy/terms/account deletion/public copyright-report fallbacks, supports `EXPO_PUBLIC_SUPPORT_EMAIL`, and falls back to `support@chillywoodstream.com`. Cloudflare Email Routing for `chillywoodstream.com` is enabled/ready, the support destination is verified, and an enabled forwarding rule routes `support@chillywoodstream.com` to the support inbox destination. A real support receipt proof message with subject `Chi'llywood Support Receipt Proof - 2026-05-13` was sent to `support@chillywoodstream.com`, and the operator confirmed it arrived in the destination inbox; May 17, 2026 signed-in in-app Support feedback also wrote a verified backend queue row, while signed-out Support required Sign In. No private email contents, screenshots, headers, exports, or credentials are committed. Public DNS now proves Cloudflare MX, SPF `v=spf1 include:_spf.mx.cloudflare.net ~all`, and DMARC `v=DMARC1; p=none; rua=mailto:support@chillywoodstream.com; adkim=r; aspf=r; fo=1`. DKIM remains pending until a real outbound mail provider for `@chillywoodstream.com` provides DKIM selector records; no fake DKIM record was added. | Attorney/legal review, outbound email automation if required, production malware scanner worker proof, support process/SLA approval, Play Console Data Safety/account deletion acceptance, and DKIM after outbound mail provider setup. | Legal/support owner plus domain/hosting owner. | Release build links open expected destinations, final apex public URLs are reachable without login, legal copy is approved, DMCA/reporting proof remains clean without private data leaks, and DMCA/account-deletion store requirements are accepted where applicable. | Apex Legal URLs Live / Public DMCA URL And Form Live / Legal Canary Passed / Legal Intake And Evidence Live-Proof Closed / DMCA Agent Registered / DMCA Tooling Live-Proof Closed / Support Inbox Receipt Proved / DMARC Baseline Proved / Runtime Support Proof Passed / Attorney Review Pending / Play Acceptance Pending | Complete attorney review and enter/verify the live legal/support/account deletion/copyright-report URLs in Play Console where requested. Keep the DMCA agent registration/contact current, configure outbound mail provider DKIM before authenticated `@chillywoodstream.com` sending, and do not claim Google Play account deletion compliance until Play accepts the URL. |
...
| Supabase remote migrations and RLS | Migrations define `videos`, `creator-videos` storage policies and movie-size bucket intent, `safety_reports`, `user_entitlements`, `billing_events`, Watch-Party source fields, tightened room RLS, private legal acceptance storage, and platform staff roles. H1B2 applied `202605070001_user_account_legal_acceptances.sql` and `202605070002_harden_user_account_legal_acceptance_grants.sql`; generated database types were regenerated; anon reads to `user_account_legal_acceptances` return permission denied. May 17, 2026 disposable signup proof wrote one `public_v1_h1b` legal acceptance row, confirmed owner read, confirmed anon denial, and confirmed another authenticated user saw zero rows. Staff-role migrations `202605140008_platform_staff_role_management.sql` and `202605140009_platform_staff_role_management_null_actor_guard.sql` are applied remotely. They bootstrap Owner `rob2008gn@gmail.com`, Admin `chillywood92@gmail.com` as internal `operator`, add staff audit, add server-side grant/revoke RPCs with last-owner protection, and fix a NULL actor-role permission fallthrough found during live proof. Live SQL/RPC proof closed Owner/Admin/Moderator/regular boundaries, revoked-role blocking, direct role-write denial, and grant/revoke audit rows. | Re-run safe remote lint/schema proof before launch, then run focused remote API/RLS proof for creator media, storage, reports, entitlements, admin writes, and Watch-Party rooms. | Backend/Supabase owner. | Live RLS proof shows owner/non-owner/public/admin/moderator/anon behavior exactly matches Public v1 access rules, creator movie upload is not blocked at 50 MB, and private legal acceptance rows remain owner-only. | Partial / Legal Acceptance Proof Passed | Staff-role and legal-acceptance proof are closed; continue remaining focused non-destructive live RLS proof with sanitized artifacts. |
| Supabase storage buckets | Migration creates a private `creator-videos` bucket with owner write/delete, public-or-owner read policy intent, and a 5 GiB per-bucket upload limit. Direct SQL delete is intentionally blocked by Supabase storage protection, so Storage API proof remains separate. | Verify the project Storage global file-size limit is at least 5 GiB, public access is policy-mediated, and Storage API owner delete/remove works. | Backend/Supabase owner. | Owner uploads/removes own object, non-owner cannot overwrite/delete, public reads only public clean videos, draft/hidden/removed sources do not leak, and the original 54.7 MB `Chi'llywoodtest.mp4` uploads without compression. | Proof Pending | Confirm the Supabase Storage global limit, then run Storage API proof against remote project using real owner/non-owner sessions, not service-role-only SQL. |
| Supabase Edge Functions | `supabase/functions/livekit-token/index.ts` exists and `supabase/config.toml` disables automatic JWT verification for that function because the function validates bearer auth internally. Stripe Connect test-mode functions `stripe-connect-account`, `stripe-connect-onboarding-link`, `stripe-connect-account-sync`, `stripe-connect-webhook`, and `stripe-connect-transfer-sync` are deployed as backend-only/test-mode functions with internal auth where applicable. Payout Review Queue + Batch Draft Workflow Foundation remains database/Admin read-only/draft-only. `docs/LIVEKIT_PRODUCTION_READINESS_RUNBOOK.md` maps the token request/denial proof. | Verify function secrets: `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`, `LIVEKIT_URL`. Stripe proof secrets `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` are configured by name/digest only; never put either value in React Native or repo docs/logs. | Backend/LiveKit/finance owner. | Authenticated app request receives server URL and participant token; signed-out/malformed requests are rejected; no secrets appear in logs. For Stripe, account/link/sync/transfer-sync require auth, webhook requires signature verification, transfer sync is admin/operator-only and accepts only local provider transfer record ids, account creation/reuse, onboarding-link creation, account sync, signed webhook handling, and duplicate-event idempotency have test-mode provider proof. `stripe-connect-account`, `stripe-connect-onboarding-link`, and `stripe-connect-account-sync` are ACTIVE version 32; `stripe-connect-webhook` is ACTIVE version 34. No-auth account setup returns `401`, unsigned webhook POST returns `400 invalid_signature`, authenticated app-test proof created a real Stripe test connected account, created a short-lived `connect.stripe.com` onboarding link, synced non-payable readiness, and proved provider/KYC/tax blockers plus cash-out fee math. Provider proof must not create transfers, payouts, checkout, or live money actions. Payout review/batch foundations must remain read-only/draft-only until fraud review, KYC/tax, Owner approval, and immutable audit requirements are separately proved. | Partial / Stripe Preproduction Readiness Proved | Verify LiveKit secrets in Supabase dashboard, then run token request/denial proof from a release-like build. For Stripe, rotate the exposed test API secret in the Stripe Dashboard and keep backend proof non-production: Platform Studio may open backend test-mode Connect setup, but no transfer creation, payouts, payout review approval, batch processing, fake paid status, or checkout is allowed before separately approved payout execution lanes. |
...
| Android release build and signing | `eas.json` has development, preview, and production profiles; production builds Android `app-bundle`, sets `cli.appVersionSource: "remote"`, and uses production `autoIncrement: true` for EAS-managed Android build numbers. `app.json` includes package `com.chillywood.mobile`, EAS project id, Expo Updates URL, icon/splash assets, and camera/microphone permissions. Google Play Android developer/package verification is complete for `com.chillywood.mobile`; Play Console accepted ADI verification and verified selected signing fingerprint `2B:A7:84:1B:70:6B:E8:69:E8:0F:EA:DA:43:8D:14:33:34:D6:E8:70:99:63:FA:B9:63:94:07:4E:9D:D0:95:43`. The ADI APK is archive proof only. First production AAB `aedded50-3818-4d7b-8bcf-9bb9c2d36513` / versionCode `2` was manually uploaded to Internal Testing, and production build `96e60082-00b1-48dc-843b-132ce82a7710` completed for versionCode `3` after the Admin-login safety fix. EAS submitted versionCode `3` to the internal track only as submission `920c452f-1b9f-401c-94c9-616801684939`; versionCode `3` installed from Google Play on physical device `R3CXA0DS5JV` and Premium purchase/restore/expiry proof passed. | Confirm EAS Android production release credentials/signing ownership, Play App Signing posture, remote Android build number state, production env values, release track, and final preview/production build approval. | Release manager. | Current `main` builds successfully with production profile, Play internal testing has accepted versionCode `2`, versionCode `3` is uploaded/submitted to Internal Testing and Play-installed on a physical device, and release route smoke/log audit pass. | Package Verification Complete / Internal Test Purchase Proof Passed / Release Smoke Pending | Run final release smoke from Internal Testing. Do not treat ADI verification as production release proof. |
...
- Do not configure paid creator videos, subscriber-only videos, tips, coins, live payouts, ads, or paid title access. The Platform Studio payout dashboard foundation is read-only and must not be configured or described as active payouts; Admin Payouts review/batch/transfer-sync foundations are read-only/draft/status-import only and must not be described as approval, processing, transfer creation, retry, Mark Paid, payable balance, fake paid status, or active payout support; Stripe Connect test-mode backend functions are deployed for backend proof only, have backend secret names configured, and must not be described as active Stripe payout support.
...
1. Google Play Console app/package verification is complete for package `com.chillywood.mobile`; keep this as the baseline before subscription setup.
2. Upload an internal or closed-test Android build first if Play Console will not allow subscription creation yet. VersionCode `2` is manually uploaded to Internal Testing, versionCode `3` is submitted to the internal track, and package visibility returns `200`; subscription `premium_subscription` now exists with active monthly base plan `monthly` at `$9.99/month`.
...
10. Import or add the Google subscription product/base plan in RevenueCat.
    - For newer Google subscription products, RevenueCat may show the product identifier as `<subscription_id>:<base-plan-id>`, for example `premium_subscription:monthly-autorenewing`.
...
- The current in-app report sheet supports a generic copyright category and links to the formal copyright report route. Formal notice intake, hosted public DMCA form submission, private evidence attachment upload/access denial, and uploader-facing counter-notice self-service are backed and live-proof closed; production malware scanner worker proof and outbound email automation remain pending.

Manual actions before marking Done:

1. Legal/support owner approves the canonical `legal/policies.mjs` bundle: Privacy, Terms, Community Guidelines, Creator Rules, Copyright/DMCA, Support & Account Help, Account Deletion, Premium Terms, Live/Watch-Party/Chat Rules, Law Enforcement, Moderation/Appeals, and Creator Monetization.
2. Product/support owner finalizes support email/URL, support inbox owner, and response SLA.
3. Legal owner keeps the published DMCA designated-agent/contact details and U.S. Copyright Office registration current; registration `DMCA-1072720` is recorded complete, backed/Admin workflow proof passed, and attorney review remains pending.
...
- Google Play Android developer/package verification for `com.chillywood.mobile` is complete; Play Console accepted ADI verification and verified selected signing fingerprint `2B:A7:84:1B:70:6B:E8:69:E8:0F:EA:DA:43:8D:14:33:34:D6:E8:70:99:63:FA:B9:63:94:07:4E:9D:D0:95:43`. The ADI APK is archive proof only and should not be treated as a production release artifact.
...
4. Google Play package ownership verification is complete. Still confirm Play App Signing / upload-key posture before production AAB/internal-testing upload.
...
- `supabase projects list` showed project ref `bmkkhihfbmsnnmcqkoly`, name `Chi'llywood2025's Project`, region West US (Oregon).
...
- `supabase functions list --project-ref bmkkhihfbmsnnmcqkoly` showed `livekit-token` ACTIVE, version 55 after the May 18 live mic authority deploy; no-auth smoke returned `401 missing_authorization`, and authenticated token proof remains pending on local proof-account/service-role env vars.
...
- The app has bundled public legal/support routes for Privacy, Terms, Account Deletion, Community Guidelines, Creator Rules, Copyright/DMCA, and Support; the public web policy surface is generated from `legal/policies.mjs` as 17 Cloudflare Pages routes covering the full 12-policy bundle plus aliases.
...
- Later-phase truth remains excluded from store copy: native game streaming, paid creator videos, subscriber-only media, tips, coins, live payouts, ads, VIPs, comment media uploads, advanced Platform Studio, and automatic transcoding. The pushed Platform Studio payout dashboard is foundation-only readiness/ledger visibility and Stripe Connect test-mode backend/transfer-sync functions are backend proof infrastructure only; none must be marketed as Withdraw, Cash Out, Stripe Connect, KYC/tax, earnings, payable balance, onboarding, transfer creation, Mark Paid, Retry Transfer, payouts, or active payout support.
- The signup route shows Terms/Privacy/Community Guidelines acceptance copy plus the 18+ confirmation, and H1B2 legal acceptance write/read privacy proof passed with a disposable signup account on May 17, 2026.
...
5. Product/legal owner confirms final Privacy, Terms, Account Deletion, Community Guidelines, Creator Rules, Copyright/DMCA, Support, Premium Terms, Live Rules, Law Enforcement, Moderation/Appeals, and Creator Monetization URLs.
...
- Bundled legal/support/account-deletion routes exist, and the expanded May 21 public policy bundle is live from Cloudflare Pages on `chillywoodstream.com`.
- Firebase packages/plugins/helpers exist, and Firebase Crashlytics/Performance proof prep now has a dedicated runbook.
- Supabase migrations for creator media, billing entitlements, moderation, storage policy intent, creator movie upload size, Watch-Party source/RLS, and H1B2 private legal acceptance storage exist; remote migration history is aligned through `202605070002`.
...
- Final Play Store listing, Data Safety, account deletion entry acceptance, and release-route smoke. Google Play subscription product `premium_subscription`, monthly $9.99 base plan, RevenueCat entitlement/offering/product attachment, no-charge internal-test purchase, restore, active entitlement, cancellation/expiry, and missing-entitlement Premium blocking are proved.
...
- Final attorney review plus Play Console/Data Safety/account deletion acceptance for the live legal/support/DMCA/account deletion policy set.
...
1. Google Play app record/package verification is complete for `com.chillywood.mobile`; keep it as the anchor for the remaining setup.
2. RevenueCat product, entitlement, offering, and internal-test Premium purchase/restore/expiry proof are complete; preserve this setup.
3. Configure any remaining EAS env values for final release smoke.
4. Prove Firebase Crashlytics/Performance.
5. Prove Supabase RLS/API/storage.
6. Prove LiveKit production/two-device behavior.
7. Complete Play Store listing, Data Safety, and content rating.
8. Finalize legal/DMCA/account deletion approvals.
9. Run final release smoke from the internal-testing build.

## Do Not Mark Done Until

- Play Console subscription, RevenueCat offering, internal-test purchase/restore, active-entitlement, and expiry-blocking proof exists.
