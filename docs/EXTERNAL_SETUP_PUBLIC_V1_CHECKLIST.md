...
| Legal pages and support | Expanded app routes exist for Privacy, Terms, Community Guidelines, Creator Rules, Copyright/DMCA, Support, and account deletion. `docs/legal/` contains drafted launch policy documents for Terms, Privacy, Creator Rules, Community Guidelines, Copyright/DMCA, copyright takedown workflow, account deletion, refunds/subscriptions, creator payouts, fraud/forfeiture, sponsor disclosure, banned content, moderation/reporting, and launch status. `public-site/legal-site/` builds a static public URL surface for `/terms`, `/privacy`, `/creator-rules`, `/community-guidelines`, `/copyright`, `/account-deletion`, `/refunds`, `/sponsor-disclosure`, `/banned-content`, `/moderation`, and `/support`, using only confirmed `support@chillywoodstream.com` for public contact. Cloudflare Pages project `chillywood-legal` is deployed and `https://chillywood-legal.pages.dev/*` returned HTTP 200 for all required paths. The preferred apex custom domain `chillywoodstream.com` is attached to the Pages project but remains pending because public DNS for the apex does not resolve and the current Wrangler OAuth session receives `403` from the Cloudflare DNS Records API. `app.config.ts` has privacy/terms/account deletion fallbacks, supports `EXPO_PUBLIC_SUPPORT_EMAIL`, and falls back to `support@chillywoodstream.com`. Cloudflare Email Routing for `chillywoodstream.com` is enabled/ready, the support destination is verified, and an enabled forwarding rule routes `support@chillywoodstream.com` to the support inbox destination. | Attorney/legal review, complete Cloudflare apex DNS/custom-domain activation, curl/browser proof for each `https://chillywoodstream.com/*` URL, support inbox receipt proof, support email/SLA, DMCA designated agent/contact process, DMCA agent public posting, Copyright Office registration if safe-harbor readiness will be claimed, and support inbox ownership. | Legal/support owner plus domain/hosting owner. | Release build links open expected destinations, final apex public URLs are reachable without login, `support@chillywoodstream.com` receives a test request through the verified inbox route, legal copy is approved, and DMCA/account-deletion store requirements are accepted where applicable. | Cloudflare Pages Proved / Apex DNS Pending / Inbox Receipt Proof Pending / Attorney Review Pending | In Cloudflare DNS, add the apex record for `chillywoodstream.com` to route to `chillywood-legal.pages.dev` or rerun with a token that has DNS Records permissions, wait for Pages custom-domain status to become active, prove all required apex URLs, send a real test message to `support@chillywoodstream.com`, and complete attorney/DMCA setup before entering legal URLs in Play Console. Do not claim DMCA safe-harbor completion until the agent is publicly posted and registered. |
