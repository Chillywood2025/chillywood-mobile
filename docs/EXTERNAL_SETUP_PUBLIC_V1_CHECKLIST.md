...
| Legal pages and support | Expanded app routes exist for Privacy, Terms, Community Guidelines, Creator Rules, Copyright/DMCA, formal Copyright Report, Support, and account deletion. `legal/policies.mjs` is now the canonical production policy bundle for the 12 full policy pages: Privacy, Terms, Community Guidelines, Creator Rules, Copyright/DMCA, Support & Account Help, Account Deletion, Premium Terms, Live/Watch-Party/Chat Rules, Law Enforcement, Moderation/Appeals, and Creator Monetization. `public-site/legal-site/` builds 17 static pages from that bundle, including compatibility aliases, using only confirmed `support@chillywoodstream.com` for public contact. Cloudflare Pages project `chillywood-legal` redeployed on May 21, 2026; custom domain `chillywoodstream.com` is active. Sampled apex proof returned HTTP 200 after redirect for `/privacy/`, `/creator-monetization/`, and `/live-rules/`; earlier core proof remains valid for `/terms/`, `/account-deletion/`, `/copyright/`, and `/support/`. Admin Canary legal readiness ran from the physical Android owner device and passed with `33 pass / 0 manual / 0 fail`. No LiveKit fallback was reached; Privacy mentions LiveKit only as policy text. `https://chillywoodstream.com/support` is live. DMCA designated agent public contact posting and U.S. Copyright Office registration are recorded complete under registration `DMCA-1072720`; the copyright page lists Chi'llywood Copyright Agent / Chi'llywood, `support@chillywoodstream.com`, 9316 S Kimbark, Chicago, IL 60619, and phone `3124879454`. Backed/Admin DMCA operational tooling is live-proof closed for formal notice intake, invalid notice rejection, owner/operator Admin case visibility/actions, supported profile-post and creator-video hide/restore, public hidden/restored visibility, counter-notice recording/deadlines, court-action restore blocking, content action history, copyright strikes, repeat-infringer review, DMCA audit history, RLS/private-data denial, generic report compatibility, and proof-content cleanup. `app.config.ts` has privacy/terms/account deletion fallbacks, supports `EXPO_PUBLIC_SUPPORT_EMAIL`, and falls back to `support@chillywoodstream.com`. Cloudflare Email Routing for `chillywoodstream.com` is enabled/ready, the support destination is verified, and an enabled forwarding rule routes `support@chillywoodstream.com` to the support inbox destination. A real support receipt proof message with subject `Chi'llywood Support Receipt Proof - 2026-05-13` was sent to `support@chillywoodstream.com`, and the operator confirmed it arrived in the destination inbox; May 17, 2026 signed-in in-app Support feedback also wrote a verified backend queue row, while signed-out Support required Sign In. No private email contents, screenshots, headers, exports, or credentials are committed. Public DNS now proves Cloudflare MX, SPF `v=spf1 include:_spf.mx.cloudflare.net ~all`, and DMARC `v=DMARC1; p=none; rua=mailto:support@chillywoodstream.com; adkim=r; aspf=r; fo=1`. DKIM remains pending until a real outbound mail provider for `@chillywoodstream.com` provides DKIM selector records; no fake DKIM record was added. | Attorney/legal review, uploader-facing counter-notice route if required, outbound email automation if required, support process/SLA approval, Play Console Data Safety/account deletion acceptance, and DKIM after outbound mail provider setup. | Legal/support owner plus domain/hosting owner. | Release build links open expected destinations, final apex public URLs are reachable without login, legal copy is approved, DMCA/reporting proof remains clean without private data leaks, and DMCA/account-deletion store requirements are accepted where applicable. | Apex Legal URLs Live / Legal Canary Passed / DMCA Agent Registered / DMCA Tooling Live-Proof Closed / Support Inbox Receipt Proved / DMARC Baseline Proved / Runtime Support Proof Passed / Attorney Review Pending / Play Acceptance Pending | Complete attorney review and enter/verify the live legal/support/account deletion URLs in Play Console. Keep the DMCA agent registration/contact current, configure outbound mail provider DKIM before authenticated `@chillywoodstream.com` sending, and do not claim Google Play account deletion compliance until Play accepts the URL. |
...
- The current in-app report sheet supports a generic copyright category and links to the formal copyright report route. Formal notice intake is backed and live-proof closed; direct uploader-facing counter-notice intake and outbound email automation remain pending.
...
- External Privacy, Terms, Creator Rules, Community Guidelines, Copyright/DMCA, Support, Account Deletion, Premium Terms, Live Rules, Law Enforcement, Moderation/Appeals, and Creator Monetization URLs open from a non-authenticated browser where used.
