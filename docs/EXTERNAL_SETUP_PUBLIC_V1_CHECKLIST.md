...
| RevenueCat configuration | Runtime config supports `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY_DEV`, `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY`, and `EXPO_PUBLIC_REVENUECAT_IOS_PUBLIC_SDK_KEY`. Monetization targets include `premium_subscription`, `premium_live_access`, and `premium_watch_party_access`; later paid content targets remain non-v1. The app prefers RevenueCat offering `premium` and can fall back for `premium_subscription` only to a RevenueCat current/default/first offering with purchasable packages, but entitlement truth still owns access. RevenueCat API proof confirms the Android app, Android production public SDK key, entitlement `premium`, and offering `premium`; EAS production env contains `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY`, and key-injected local validation reports RevenueCat configured. The first AAB is uploaded and Play package visibility works, but product `premium_subscription` and product attachments remain blocked until the real Play subscription/base plan is created and importable. | Configure/connect the real Google Play subscription product/base plan to RevenueCat product `premium_subscription` or the exact Google subscription/base-plan identifier required by RevenueCat. | Billing owner. | `/subscribe` shows a valid RevenueCat Premium offer, purchase and restore call RevenueCat, active entitlement is reflected by account-owned truth, and missing/expired/revoked states remain blocked. | External Setup Pending | Create the Play subscription/base plan, attach/import the real Play subscription product to entitlement/offering `premium`, and verify the offering loads in `/subscribe` without weakening entitlement checks. |
...
2. Upload an internal or closed-test Android build first if Play Console will not allow subscription creation yet. VersionCode `2` is now uploaded to Internal Testing and package visibility returns `200`; subscription creation is still blocked because `premium_subscription` is missing and the Play monetization setup probe returns `403 permission denied`.
