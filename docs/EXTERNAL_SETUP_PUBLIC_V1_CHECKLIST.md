...
| --- | --- | --- | --- | --- | --- | --- |
| Google Play Billing product | `react-native-purchases` and `react-native-purchases-ui` are installed. `_lib/revenuecat.ts`, `_lib/monetization.ts`, `_lib/premiumEntitlements.ts`, `_lib/premiumWatchPartyAccess.ts`, and `app/subscribe.tsx` own Premium billing UX and entitlement checks. The app does not grant Premium from local-only state. All full live/watch-party access is Premium in app-side gates. Play subscription `premium_subscription` exists with active monthly base plan `monthly`, billing period `P1M`, and US price `$9.99/month`. | Connect the real Play product to the RevenueCat offering package and keep Play subscription status testing-ready/active for internal-test purchase proof. | Product owner plus Play Console/RevenueCat operator. | Internal build purchase starts, product loads, purchase completes, backend entitlement becomes active, and protected live/watch-party routes unlock only after trusted entitlement truth. | External Setup Pending | Attach `premium_subscription:monthly` to RevenueCat offering `premium`, then run license-tester purchase proof with full Live First, Live Watch-Party, and Watch-Party Live. |
| RevenueCat configuration | Runtime config supports `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY_DEV`, `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY`, and `EXPO_PUBLIC_REVENUECAT_IOS_PUBLIC_SDK_KEY`. Monetization targets include `premium_subscription`, `premium_live_access`, and `premium_watch_party_access`; later paid content targets remain non-v1. The app prefers RevenueCat offering `premium` and can fall back for `premium_subscription` only to a RevenueCat current/default/first offering with purchasable packages, but entitlement truth still owns access. RevenueCat API proof confirms the Android app, Android production public SDK key, entitlement `premium`, offering `premium`, product `premium_subscription:monthly`, and entitlement attachment; EAS production env contains `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY`, and key-injected local validation reports RevenueCat configured. Product/package attachment to offering `premium` remains blocked from CLI because the available API key lacks package read/write scopes. | Attach product `premium_subscription:monthly` to a Monthly package in offering `premium` and make offering `premium` current only after the package is valid. | Billing owner. | `/subscribe` shows a valid RevenueCat Premium offer, purchase and restore call RevenueCat, active entitlement is reflected by account-owned truth, and missing/expired/revoked states remain blocked. | External Setup Pending | Complete the RevenueCat offering package attachment and verify the offering loads in `/subscribe` without weakening entitlement checks. |
...
- Google Play subscription product `premium_subscription` with monthly $9.99 base plan, plus RevenueCat product attachment to entitlement/offering `premium`.
