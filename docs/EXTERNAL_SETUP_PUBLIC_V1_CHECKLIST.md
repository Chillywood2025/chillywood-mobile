...
2. Upload an internal or closed-test Android build first if Play Console will not allow subscription creation yet. VersionCode `2` is manually uploaded to Internal Testing, versionCode `3` is submitted to the internal track, and package visibility returns `200`; subscription creation is still blocked because `premium_subscription` is missing and Google Play requires the developer payments profile to be registered first.
