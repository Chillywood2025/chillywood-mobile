...
| Ad provider setup | Ads Launch Foundation V1A is pushed in repo as provider-neutral, no-SDK, no-real-rendering infrastructure. Current owners are `_lib/ads/adConfig.ts`, `_lib/ads/adEligibility.ts`, `_lib/ads/adProvider.ts`, `_lib/ads/providers/placeholder.ts`, `_lib/ads/adSession.ts`, `hooks/useAdEligibility.ts`, `hooks/useActiveBrowsingTime.ts`, and read-only/foundation Admin Ads status in `app/admin.tsx`. Default `ads_enabled` is false, default provider is `placeholder`, and the placeholder provider reports not connected without SDK calls. | Future external AppLovin MAX account/app/ad-unit setup is required before real ads. Unity LevelPlay / Unity Ads may be added later through AppLovin MAX. Do not create an AdMob-only path. Keep real providers behind the Chi'llywood provider-neutral wrapper. | Product owner plus ad-ops/release owner. | After a dedicated future AppLovin integration lane, release proof must show no direct screen-level SDK calls, no committed provider secrets/ad IDs beyond approved public/native config, Premium users see zero ads, caps are enforced, and forbidden contexts suppress ads. | Later / External Setup Pending | Do not run AppLovin dashboard setup from repo-control passes. Next app-side lane is Ads V1B Native/feed placeholder placement with no real SDK or IDs; real AppLovin setup waits until account/app/ad units are ready. |
