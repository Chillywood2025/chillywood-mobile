...
| Android release build and signing | `eas.json` has development, preview, and production profiles; production builds Android `app-bundle`, sets `cli.appVersionSource: "remote"`, and uses production `autoIncrement: true` for EAS-managed Android build numbers. `app.json` includes package `com.chillywood.mobile`, EAS project id, Expo Updates URL, icon/splash assets, and camera/microphone permissions. Google Play Android developer/package verification is complete for `com.chillywood.mobile`; Play Console accepted ADI verification and verified selected signing fingerprint `2B:A7:84:1B:70:6B:E8:69:E8:0F:EA:DA:43:8D:14:33:34:D6:E8:70:99:63:FA:B9:63:94:07:4E:9D:D0:95:43`. The ADI APK is archive proof only. | Confirm EAS Android production release credentials/signing ownership, Play App Signing posture, remote Android build number state, production env values, release track, and final preview/production build approval. | Release manager. | Current `main` builds successfully with preview and production profiles; the remote EAS Android build number is initialized and higher than any Play-uploaded `versionCode`; production AAB installs or reaches Play internal testing; release route smoke and log audit pass. | Package Verification Complete / External Setup Pending | Follow `docs/ANDROID_RELEASE_EAS_RUNBOOK.md`: verify production release credentials, initialize/confirm remote EAS Android build number, configure EAS production env, then run preview/production builds after runtime proof lanes are green. Do not treat ADI verification as production AAB/internal-testing proof. |
