...
| Supabase remote migrations and RLS | Migrations define `videos`, `creator-videos` storage policies and movie-size bucket intent, `safety_reports`, `user_entitlements`, `billing_events`, Watch-Party source fields, tightened room RLS, private legal acceptance storage, and platform staff roles. H1B2 applied `202605070001_user_account_legal_acceptances.sql` and `202605070002_harden_user_account_legal_acceptance_grants.sql`; generated database types were regenerated; anon reads to `user_account_legal_acceptances` return permission denied. Staff-role migration `202605140008_platform_staff_role_management.sql` is repo-side ready and must be applied before live Owner/Admin/Moderator truth is considered closed. It bootstraps Owner `rob2008gn@gmail.com`, Admin `chillywood92@gmail.com` as internal `operator`, adds staff audit, and adds server-side grant/revoke RPCs with last-owner protection. Linked DB lint was attempted previously but blocked by remote login password auth. | Re-run safe remote lint/schema proof before launch, then run focused remote API/RLS proof for creator media, storage, reports, entitlements, admin writes/staff RPCs, Watch-Party rooms, legal acceptance insert/read, and anon denial. | Backend/Supabase owner. | Live RLS proof shows owner/non-owner/public/admin/moderator/anon behavior exactly matches Public v1 access rules, creator movie upload is not blocked at 50 MB, and legal acceptance rows are owner-only after a real signup write proof. | Proof Pending | Configure approved remote DB login path, apply/prove `202605140008`, run `supabase db lint --linked --schema public --fail-on none`, then run focused non-destructive live RLS proof with sanitized artifacts. |
...
- Later-phase truth remains excluded from store copy: native game streaming, paid creator videos, subscriber-only media, tips, coins, live payouts, ads, VIPs, comment media uploads, advanced creator studio, and automatic transcoding. The pushed Channel Studio payout dashboard is foundation-only readiness/ledger visibility and Stripe Connect test-mode backend/transfer-sync functions are backend proof infrastructure only; none must be marketed as Withdraw, Cash Out, Stripe Connect, KYC/tax, earnings, payable balance, onboarding, transfer creation, Mark Paid, Retry Transfer, payouts, or active payout support.
