...
| Ad provider setup | Ads Launch Foundation V1A/V1B is pushed in repo as provider-neutral, no-SDK, no-real-rendering infrastructure. Current owners are `_lib/ads/adConfig.ts`, `_lib/ads/adEligibility.ts`, `_lib/ads/adProvider.ts`, `_lib/ads/providers/placeholder.ts`, `_lib/ads/adSession.ts`, `hooks/useAdEligibility.ts`, `hooks/useActiveBrowsingTime.ts`, `components/ads/NativeAdSlot.tsx`, Home placement in `app/(tabs)/index.tsx`, and read-only/foundation Admin Ads status in `app/admin.tsx`. Default `ads_enabled` is false, default provider is `placeholder`, the placeholder provider reports not connected without SDK calls, and Home native/feed placeholder foundation stays hidden in normal runtime until real backed ad enablement exists. | Future external AppLovin MAX account/app/ad-unit setup is required before real ads. Unity LevelPlay / Unity Ads may be added later through AppLovin MAX. Do not create an AdMob-only path. Keep real providers behind the Chi'llywood provider-neutral wrapper. | Product owner plus ad-ops/release owner. | After a dedicated future AppLovin integration lane, release proof must show no direct screen-level SDK calls, no committed provider secrets/ad IDs beyond approved public/native config, Premium users see zero ads, caps are enforced, and forbidden contexts suppress ads. Ads V1C placeholder interstitial proof should happen before real SDK integration. | Later / External Setup Pending | Do not run AppLovin dashboard setup from repo-control passes. Next app-side lane is Ads V1C placeholder interstitial controller with no real SDK or IDs; real AppLovin setup waits until account/app/ad units are ready. |
...
| Supabase remote migrations and RLS | Migrations define `videos`, `creator-videos` storage policies and movie-size bucket intent, `safety_reports`, `user_entitlements`, `billing_events`, Watch-Party source fields, and tightened room RLS. Migration `202604280001` raises the `creator-videos` bucket file-size limit to 5 GiB and is applied remotely. H1B1 adds local pending migration `202605070001_user_account_legal_acceptances.sql` for private age/terms/privacy acceptance storage; it is not applied remotely yet and generated database types were not regenerated. Linked DB lint was attempted previously but blocked by remote login password auth. | Re-run safe remote lint/schema proof before launch, then run focused remote API/RLS proof for creator media, storage, reports, entitlements, admin writes, Watch-Party rooms, legal acceptance storage, and anon denial. | Backend/Supabase owner. | Live RLS proof shows owner/non-owner/public/admin/anon behavior exactly matches Public v1 access rules, creator movie upload is not blocked at 50 MB, and legal acceptance rows are owner-only if H1B2 applies that schema. | Proof Pending | Configure approved remote DB login path, run `supabase db lint --linked --schema public --fail-on none`, and handle H1B2 migration/typegen/runtime wiring only through a separately authorized prompt with sanitized proof. |
...
- Supabase migrations for creator media, billing entitlements, moderation, storage policy intent, creator movie upload size, Watch-Party source/RLS, and H1B1 private legal acceptance foundation exist; remote migration history is aligned through `202604280001`, while `202605070001_user_account_legal_acceptances.sql` is local pending and not applied remotely yet.
