...
| Supabase Edge Functions | `supabase/functions/livekit-token/index.ts` exists and `supabase/config.toml` disables automatic JWT verification for that function because the function validates bearer auth internally. Stripe Connect test-mode functions `stripe-connect-account`, `stripe-connect-onboarding-link`, `stripe-connect-account-sync`, and `stripe-connect-webhook` are deployed ACTIVE version 1 as backend-only/test-mode functions with internal auth where applicable. `docs/LIVEKIT_PRODUCTION_READINESS_RUNBOOK.md` maps the token request/denial proof. | Verify function secrets: `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`, `LIVEKIT_URL`. For Stripe proof later, set backend-only `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET`; never put either in React Native or repo docs/logs. | Backend/LiveKit/finance owner. | Authenticated app request receives server URL and participant token; signed-out/malformed requests are rejected; no secrets appear in logs. For Stripe, account/link/sync require auth, webhook requires signature verification, and missing Stripe secrets return `not_configured`. | Partial / Proof Pending | Verify LiveKit secrets in Supabase dashboard, then run token request/denial proof from a release-like build. Configure Stripe test-mode secrets only when ready, then rerun S3C provider proof; no Connect Stripe UI, transfers, payouts, or checkout before that proof. |
