...
| Android push provider credentials | D9 repo/backend/app implementation is deployed: remote migration `202605120003` is applied, `notification-device-tokens` and `notification-dispatch` are deployed, physical Android token registration succeeded on device `R5CR120QCBF` with safe fingerprint `c1ab24247f5d`, all five notification/activity trigger classes created backed in-app rows, blocked states stayed silent/skipped, and delivery attempts were logged. Real push send failed at Expo provider boundary with `InvalidCredentials`. The May 12 EAS credential rerun for production profile `com.chillywood.mobile` showed Push Notifications FCM Legacy: none assigned, and Push Notifications FCM V1: none assigned. | Configure Android FCM V1 credentials for Expo/EAS project `@chillywood2025/chillywood-mobile` / package `com.chillywood.mobile` using Firebase project `chillywood-app`. Store provider credentials only in Expo/EAS/Firebase or an approved server-side secret store; do not commit service account JSON or private keys. | Release manager plus Firebase/Expo owner. | Backend-sent Android push reaches a real physical device, tapping the notification opens the safe deep link, delivery attempt status is `sent`, and private/proof/protected/title-rights-blocked/ticketed/Premium-full-room/non-recipient/blocked-user cases do not notify or leak. | External Provider Blocked | Upload/configure FCM V1 push credentials for the Expo project, build/install a release-like Android build, then rerun D9 real-device receipt/tap proof. iOS/APNs can remain later. |
