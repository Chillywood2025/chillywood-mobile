...
| Supabase Edge Functions | `supabase/functions/livekit-token/index.ts` exists and `supabase/config.toml` disables automatic JWT verification for that function because the function validates bearer auth internally. Stripe Connect test-mode functions `stripe-connect-account`, `stripe-connect-onboarding-link`, `stripe-connect-account-sync`, and `stripe-connect-webhook` are deployed ACTIVE version 1 as backend-only/test-mode functions with internal auth where applicable. Payout Review Queue + Batch Draft Workflow Foundation is database/Admin read-only only and does not add Edge Function payout processing. `docs/LIVEKIT_PRODUCTION_READINESS_RUNBOOK.md` maps the token request/denial proof. | Verify function secrets: `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`, `LIVEKIT_URL`. Stripe proof secrets `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` are configured by name/digest only; never put either value in React Native or repo docs/logs. | Backend/LiveKit/finance owner. | Authenticated app request receives server URL and participant token; signed-out/malformed requests are rejected; no secrets appear in logs. For Stripe, account/link/sync require auth, webhook requires signature verification, and provider proof must not create transfers, payouts, checkout, or live money actions. Payout review/batch foundations must remain read-only/draft-only until provider proof, fraud review, KYC/tax, admin approval, and immutable audit requirements are separately proved. | Partial / Proof Pending | Verify LiveKit secrets in Supabase dashboard, then run token request/denial proof from a release-like build. Rerun S3C provider proof now that Stripe test-mode secret names are configured; no Connect Stripe UI, transfers, payouts, payout review approval, batch processing, or checkout before that proof. |
...
- Do not configure paid creator videos, subscriber-only videos, tips, coins, live payouts, ads, or paid title access. The Channel Studio payout dashboard foundation is read-only and must not be configured or described as active payouts; Admin Payouts review/batch foundation is read-only/draft-only and must not be described as approval, processing, transfer, payable balance, or active payout support; Stripe Connect test-mode backend functions are deployed for backend proof only, now have backend secret names configured, and must not be described as active Stripe payout support.
...
- Later-phase truth remains excluded from store copy: native game streaming, paid creator videos, subscriber-only media, tips, coins, live payouts, ads, VIPs, comment media uploads, advanced creator studio, and automatic transcoding. The pushed Channel Studio payout dashboard is foundation-only readiness/ledger visibility and Stripe Connect test-mode backend functions are backend proof infrastructure only; neither must be marketed as Withdraw, Cash Out, Stripe Connect, KYC/tax, earnings, payable balance, onboarding, transfers, payouts, or active payout support.
