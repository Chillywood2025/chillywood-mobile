...
Public Channel now lives on `/channel/[userId]` as the viewer-facing mini streaming/network home. Profile remains the personal/social identity route at `/profile/[userId]`. Owner controls live in Channel Studio on `/channel-studio`, with `/channel-settings` preserved as compatibility.
...
2. Quick actions: public viewers get backed Follow/Following, Chi'lly Chat, View Channel, Share Profile, and Report where supported; Profile View Channel routes to `/channel/[userId]`; owners keep Channel Studio, upload, and settings handoffs where backed.
...
Channel Studio owns owner-only channel operations: Home dashboard, Content, Live, Audience, Insights, Brand, creator upload/manage handoffs, backed audience actions, and backed channel defaults. Channel Studio must not be exposed to non-owners.
...
Public viewers must not see owner controls, Channel Studio controls, upload, edit, publish, unpublish, delete, audience management, insights/analytics, drafts, private/hidden/removed videos, or admin controls.
...
- Channel Studio Content tab and `/channel-settings` compatibility
