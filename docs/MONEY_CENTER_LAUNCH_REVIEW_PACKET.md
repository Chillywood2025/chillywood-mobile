...
Internal tester sandbox addendum: `docs/INTERNAL_TESTER_SANDBOX_PURCHASE_MODE.md` documents the bounded tester-only purchase mode and Owner/Admin controls. Public/default purchase surfaces remain closed; approved testers can run clearly labeled sandbox purchases and Stripe physical merch sandbox checkout without activating production money, payouts, cash-out, payable balances, or public buy buttons. Owner/Admin Money Center shows `Internal Sandbox Testing` status and tester-tool routing. Payout readiness remains read-only with no request, simulation, cash-out, withdrawal, transfer, or payout activation.

Latest Stripe Connect payout-readiness proof: `docs/STRIPE_CONNECT_SANDBOX_PAYOUT_PROOF.md`. Proof path: `/tmp/chillywood-stripe-connect-payout-readiness-proof-20260604/`. Existing test-mode Connect functions reused a real Stripe Express connected account, created a sandbox onboarding link with an HTTPS Chi'llywood return/refresh origin, and refreshed the account into `pending_kyc` / `onboarding_in_progress` with `charges_enabled=false` and `payouts_enabled=false`. App-level `payouts_enabled` and `live_money_enabled` remain off; no cash-out, withdrawal, transfer, payable balance, digital access grant, RevenueCat/Premium entitlement, or payout simulation was created.

## Current Truth

- Production live money: off.
- Public/default Premium purchase shell: closed.
- Approved internal tester sandbox mode: available for sandbox proof only.
