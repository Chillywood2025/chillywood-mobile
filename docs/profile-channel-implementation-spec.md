...
2026-05-06 current-route note: this older implementation spec is superseded where it says Profile and Channel share one public route or where it treats `/channel-settings` as the primary studio route. Current truth is Profile = `/profile/[userId]`, public Platform = `/channel/[userId]`, owner Platform Studio = `/channel-studio`, and `/channel-settings` compatibility must remain.
...
2026-05-25 social attachment note: the current Profile/social attachment UX uses a shared action sheet for backed social attachment entry points. Profile post/comment Attach, Chi'lly Chat Attach, creator-video comment Attach, Watch-Party room comment Attach, and Live Stage comment Attach all offer Photos and Files only. Photos opens the phone's native gallery through `expo-image-picker`; Files keeps the existing `expo-document-picker` path. Platform Studio must not appear as a social attachment-sheet option; creator-content handoff stays in Profile owner actions and creator-content copy. The sheet does not change legal evidence uploads, Brand Studio media uploads, Platform Studio creator-video upload, storage/RLS, runtime controls, LiveKit, Premium, monetization, or public Platform routing.

2026-05-26 profile appearance/actions note: personal Profile photo/background now belong to Profile, not Platform Brand Studio. Owner avatar tap and long-press open an Edit Profile Photo sheet; Profile Settings has Profile Appearance rows for Profile Photo, Profile Background, and Preview Profile. Profile media uploads use the phone gallery with safe crop/edit level, Fill/Fit/Center metadata, owner-only profile writes, and the profile-only `profile-media` bucket from remote-applied migration `202605260001_profile_appearance_media.sql`. Viewer avatar tap and long-press open a Profile Actions sheet with View Profile Photo, Chi'lly Chat, View Platform, Block User, Report User, and Share Profile where backed. Block User requires sign-in and confirmation, refuses self-block, writes through existing block helpers, refreshes relationship state, and blocked Chi'lly Chat must not create a direct thread. Locked/private/blocked shells must not render private Profile media, and raw storage paths must never be shown. Android `R5CR120QCBF` current-build startup proof after lazy image-picker loading is stored at `/tmp/chillywood-profile-avatar-actions-proof-20260526/`; full picker visual proof still needs a current native build if the installed dev-client predates `expo-image-picker`. Profile media moderation and cleanup automation remain future work; do not imply Brand Studio or Platform hero/background behavior changed.

2026-05-26 owner-controlled profile media status note: Profile photo/background uploads publish immediately after validation and do not default into manual approval or `pending_review`. Remote-applied migration `202605260002_profile_media_status_policy.sql` adds Profile media statuses `active`, `user_removed`, `flagged`, and `admin_removed`; public Profile RPC rendering masks avatar/background URLs unless the corresponding status is `active`. Viewer Profile Actions can report Profile Photo or Profile Background as `profile_media` with `profileMediaKind` context and no raw URL/storage key. Admin report target actions can hide (`flagged`), remove (`admin_removed`), or restore (`active`) reported Profile media while preserving storage evidence. Generic Profile saves must not write media status so stale local cache cannot undo moderation. This is abuse/report/admin-removal support, not a Brand Studio change and not a manual approval queue.
...
2026-05-29 proof closeout note: `/tmp/chillywood-admin-search-audit-denial-spectator-profile-proof-20260529/` re-proves the compact owner Profile Photo sheet and captures Android DocumentsUI showing a safe Chi'llywood image asset. The picker did not return that selected safe file to the app during automation, and the lane did not have an owner-session backend read-back path for active media status, so avatar/background save, active read-back, remove, and fallback proof remain open. Do not use private gallery photos to close this; the next pass needs a reliable app-owned/safe asset selection path plus owner/read-back proof.

2026-05-31 profile media reliability note: Profile media upload now mirrors the hardened Brand Studio Android upload path. The selected image is staged from Android `content://` to cache when needed, uploaded through Supabase Storage REST with signed-in bearer auth and SDK fallback, and verified through a short signed read-back before the Profile row is updated. The picker explicitly uses the modern non-legacy photo-library surface with native edit enabled, square/oval crop for Profile Photo, and a 4:1 banner crop for Profile Background. Profile/Settings close the action sheet before opening the native picker or remove confirmation. This closes the obvious code-side reliability gap, but current-build Android save/read-back/remove/fallback proof with safe non-private images still needs to run.

2026-05-31 Platform draft preview note: Brand Studio pending-review assets still do not render on public Platform. Platform Studio now offers `Preview Brand Draft`, which routes the signed-in owner to `/channel/[userId]?preview=brand-draft` and renders saved Brand Studio media for visual checking without owner controls. Non-owners and signed-out users still get the public-safe Platform renderer, and public `Preview Platform` remains the visitor view.

2026-05-29 Home/Profile cleanup note: Home, Explore, Live, and Library expose top Profile/avatar and Settings access while Profile remains hidden from bottom navigation. Own Profile, public Profile/Platform, Platform Studio, Admin, Player/detail, and room surfaces should use route-local controls rather than duplicate global buttons. Profile feed empty states are now role-aware: owner `No posts yet` can focus the composer through `Create Post`; viewer `No public posts yet` stays read-only. The feed empty state must not show `Your feed is ready when you are`, a creator-video Upload CTA, or a random Platform CTA because Platform is already available through top Profile actions. Android proof lives at `/tmp/chillywood-home-profile-cleanup-proof-20260529/`.
...
`/profile/[userId]` is Chi'llywood's personal/social identity surface. `/channel/[userId]` is the Public Platform viewer surface.
...
- public Platform preview/presentation stays on `/channel/[userId]`
...
### 7.2 Public Platform Hero Model
...
### Stage 1 - Profile And Public Platform Route Split
Goal:
- preserve personal/social Profile on `/profile/[userId]`
- preserve public Platform on `/channel/[userId]`
...
- `/channel/[userId]` reads as a Public Platform viewer surface
...
- self Profile and own public Platform render without collapsing into settings
- other-user Profile and public Platform render without owner leakage
...
| Bucket | Items |
| --- | --- |
| Current doctrine / should build now | Personal/social Profile on `/profile/[userId]`; public Platform on `/channel/[userId]`; owner Platform Studio on `/channel-studio`; `/channel-settings` compatibility; Platform Studio tabs Home, Content, Live, Audience, Insights, Brand; public Platform sections Hero, Featured, Latest Uploads, Live & Upcoming, About; `Chi'lly Chat` as the communication entry; locked `Watch-Party Live` and `Live Watch-Party` semantics; official Rachi behavior on canonical profile/chat/channel surfaces. |
| Near-term but not current route truth | Deeper backed Platform Studio controls, richer owner stats only when backed, stronger curated public Platform shelves only when backed, and audience summary refinement under existing routes. |
