...
Object-storage migration note: Hetzner Object Storage -> R2 private origin is Partial. Private bucket `chillywood-media-origin` exists and trusted copier `media-object-storage-migration` is deployed, but production still has `31` Hetzner/S3 object-storage reference rows and `22` distinct object refs. The missing backend R2 private-origin write config blocker is cleared; reconciliation found `16` existing distinct refs copied/readback verified in R2 private origin, `5` `missing_404` refs, `1` unsupported ref, `9` duplicate row refs, and no permission denials. No media rows were written, no DB metadata was migrated, no Hetzner objects were deleted, and Hetzner fallback retained because unresolved refs plus the affected storage-metadata backup/restore gate remain open. Automation must not treat Hetzner Object Storage as removable until `0 Hetzner object-storage` refs, copy/readback/checksum proof, DB metadata migration, new-upload R2 proof, and scanner/transcode R2 read proof close. This does not cover Hetzner LiveKit; do not shut down Hetzner LiveKit.
