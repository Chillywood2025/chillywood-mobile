...
Object-storage migration note: Hetzner Object Storage -> R2 private origin is Partial. Private bucket `chillywood-media-origin` exists, but production still has `31` Hetzner/S3 object-storage reference rows and `22` distinct object refs. No media was processed, no media rows were written, no objects were copied, and Hetzner fallback retained. Automation must not treat Hetzner Object Storage as removable until `0 Hetzner object-storage` refs, copy/readback/checksum proof, DB metadata migration, new-upload R2 proof, and scanner/transcode R2 read proof close. This does not cover Hetzner LiveKit; do not shut down Hetzner LiveKit.
