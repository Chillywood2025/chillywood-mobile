...
Object-storage migration note: Hetzner Object Storage -> R2 private origin is Partial. Private bucket `chillywood-media-origin` exists, trusted copier `media-object-storage-migration` is deployed, `16` existing distinct refs were copied/readback verified, storage-metadata backup/restore passed, and DB batch `media-object-storage-r2-2026-07-10T21-27-45-643Z` migrated `24` copied+verified row refs to R2 private-origin metadata. New creator-media uploads now target R2 private origin with Hetzner writes disabled and read fallback retained. Automation must not treat Hetzner Object Storage as removable because zero-ref audit still reports `7` unresolved `media_scan_jobs` refs (`5` missing-object stale rows and `2` unsupported-provider blockers). This does not cover Hetzner LiveKit; do not shut down Hetzner LiveKit.
