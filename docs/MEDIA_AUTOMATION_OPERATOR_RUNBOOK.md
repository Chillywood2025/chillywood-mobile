...
Object-storage migration note: Hetzner Object Storage -> R2 private origin is shutdown-ready by active-reference semantics. Private bucket `chillywood-media-origin` exists, trusted copier `media-object-storage-migration` is deployed, `16` existing distinct refs were copied/readback verified, storage-metadata backup/restore passed, and DB batch `media-object-storage-r2-2026-07-10T21-27-45-643Z` migrated `24` copied+verified row refs to R2 private-origin metadata. New creator-media uploads now target R2 private origin with Hetzner writes disabled and read fallback retained. Automation must still keep Hetzner read fallback during the owner-controlled retention window; final audit reports `7` raw `media_scan_jobs` historical refs, all resolved stale with `activeUnresolvedHetznerObjectRefs=0`. This does not cover Hetzner LiveKit; do not shut down Hetzner LiveKit.
