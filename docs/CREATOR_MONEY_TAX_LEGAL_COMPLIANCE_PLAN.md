...
Creator-money tax/legal/compliance plan: Partial. The plan remains complete enough for owner/legal/tax review and lists each provider field, recommended value, owner-confirmation requirement, and Codex proceed/stop rule. Product creation is partial because five one-time Draft records were created/imported with approved values while the Channel Subscription base-plan save remains blocked by Google Play.

Creator-money product creation: Partial. Browser execution on 2026-06-25 created five one-time Google Play Draft records with approved product IDs, public names/descriptions, Google Play-valid hyphenated purchase-option IDs, United States-only availability, approved launch prices, and the default `Digital app sales` tax category. The one-time Draft records are `cw_creator_tip_099` / `tip-099` / USD 0.99, `cw_paid_content_access_099` / `paid-video-099` / USD 0.99, `cw_watch_party_ticket_099` / `ticket-099` / USD 0.99, `cw_vip_pass_499` / `vip-499` / USD 4.99, and `cw_event_pass_099` / `event-099` / USD 0.99. RevenueCat imported those five products as Draft consumables with no entitlement attachment and no Premium mapping. Google Play already has subscription product record `cw_channel_subscription_monthly_499` / `Creator Channel Subscription`, but it has `0` active base plans and cannot be purchased. Focused blocker investigation found the Google Play `Base plan ID` field remains invalid before Save on both stale and clean Add base plan forms for the approved `monthly` value and other valid-format probes, so the `monthly` base plan remains missing. Channel Subscription base plan: Blocked.
...
| Purchase option | Standard `Buy` / repeat-purchasable style for Tips; exact-access one-time product for others | Yes: `tip-099`, `paid-video-099`, `ticket-099`, `vip-499`, `event-099` | Yes for these exact values only | No for approved values; yes for any change | Billing/access behavior | Indirect | High | Proceed only for exact approved hyphenated values; stop on any unapproved value or public activation. |
...
| Country/region beyond United States | Launch scope is US-only first. | Five one-time products exist with approved US-only setup. Channel Subscription focused blocker investigation stopped before reconfiguring region because the `Base plan ID` field was already invalid before Save. | Owner/operator must keep exact US-only scoping on any future retry. |
| Publishing/review/activation control | Could make products publicly available or submit provider review. | Five one-time products were saved as Draft only; Channel Subscription base plan still failed to save. | Owner must authorize any publish/activate/review submission separately. |
...
| Provider | Result |
| --- | --- |
| Google Play one-time products | Product creation is partial. Browser execution created five production one-time Draft records with approved metadata, hyphenated purchase-option IDs, United States-only availability, and approved USD prices. Codex did not activate a product, did not save all-region availability, did not submit a product, and did not edit the `Digital app sales` tax category. |
| Google Play Channel Subscription | Product record `cw_channel_subscription_monthly_499` exists. The focused blocker investigation confirmed `monthly` is plain ASCII, but the `Base plan ID` field remains invalid before Save on stale and clean forms. Valid-format non-saving probes also stayed invalid, so the base plan remains missing. |
| RevenueCat | Product catalog access was verified. The five production-labeled one-time products are imported as Draft consumables with no entitlement attachment and no Premium mapping. `cw_channel_subscription_monthly_499:monthly` remains absent because the Google Play base plan does not exist. |
