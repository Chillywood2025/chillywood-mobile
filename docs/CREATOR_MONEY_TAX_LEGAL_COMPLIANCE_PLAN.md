...
Creator-money tax/legal/compliance plan: Partial. The plan remains complete enough for owner/legal/tax review and lists each provider field, recommended value, owner-confirmation requirement, and Codex proceed/stop rule. Product creation is blocked by provider form validation, not by app activation.

Creator-money product creation: Blocked. Google Play already has subscription product record `cw_channel_subscription_monthly_499` / `Creator Channel Subscription`, but it has `0` active base plans and cannot be purchased. The five one-time products are not created. Browser execution on 2026-06-25 confirmed the one-time product form accepts the approved Product ID, Name, Description, existing icon preview, and default `Digital app sales` tax category strongly enough to reach the Availability and pricing step without selecting Age rating. Creation then stopped because Google Play rejected the owner-approved purchase-option IDs `tip_099`, `paid_video_099`, `ticket_099`, `vip_499`, and `event_099`: that field accepts lowercase letters, numbers, and hyphens, but not underscores. The Channel Subscription base-plan draft was scoped to United States only, auto-renewing Monthly, USD 4.99, default 7-day grace period, and automatic account hold, but Google Play returned `Your changes couldn't be saved`; the `monthly` base plan remains missing.
...
| Purchase option | Standard `Buy` / repeat-purchasable style for Tips; exact-access one-time product for others | Approved IDs were provided but rejected by Google Play because underscores are invalid | No final save until owner approves Google Play-valid IDs | Yes for replacement IDs | Billing/access behavior | Indirect | High | Stop until owner approves lowercase-letter/number/hyphen purchase-option IDs. |
...
| Country/region beyond United States | Launch scope is US-only first. | One-time product pricing did not reach a safe save because the purchase-option ID was invalid. Channel Subscription base-plan draft was narrowed to United States only before save, but Google Play returned `Your changes couldn't be saved`. | Owner/operator must keep exact US-only scoping on retry. |
| Publishing/review/activation control | Could make products publicly available or submit provider review. | Channel subscription base plan not created; one-time product flow stopped on invalid purchase-option ID and no product was saved. | Owner must authorize any publish/activate/review submission separately. |
...
| One-time purchase-option IDs | Owner-approved IDs `tip_099`, `paid_video_099`, `ticket_099`, `vip_499`, and `event_099` are currently blocked because Google Play rejects underscores | Owner must approve Google Play-valid replacements using lowercase letters, numbers, and hyphens only before Codex retries product creation. |
...
| Provider | Result |
| --- | --- |
| Google Play one-time products | Product creation remains blocked. Browser execution confirmed the five production one-time IDs are absent from the catalog. The create form accepted approved first-step metadata for `cw_creator_tip_099` and allowed progression without selecting Age rating, but Google Play rejected owner-approved purchase-option ID `tip_099` because the field accepts lowercase letters, numbers, and hyphens only. The other owner-approved IDs have the same underscore problem. Codex did not save a one-time product, did not save all-region availability, did not submit a product, and did not edit the `Digital app sales` tax category. |
| Google Play Channel Subscription | Product record `cw_channel_subscription_monthly_499` exists. The base-plan draft used `monthly`, auto-renewing Monthly, United States only, USD 4.99, default 7-day grace period, and automatic account hold. Google Play returned `Your changes couldn't be saved`, so the base plan remains missing. |
...
3. Approve Google Play-valid replacement `Purchase option ID` values for each one-time product. The previously approved IDs `tip_099`, `paid_video_099`, `ticket_099`, `vip_499`, and `event_099` are blocked because Google Play allows lowercase letters, numbers, and hyphens only.
...
7. Retry the `monthly` base plan for `cw_channel_subscription_monthly_499`; the approved US-only, auto-renewing monthly, USD 4.99 draft failed with `Your changes couldn't be saved`.
