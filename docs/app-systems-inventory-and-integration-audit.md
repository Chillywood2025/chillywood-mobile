...
| 4 | Profile / public Platform | `app/profile/[userId].tsx`, `app/channel/[userId].tsx` | `_lib/userData.ts`, `_lib/accessEntitlements.ts`, `_lib/chat.ts`, `_lib/friendGraph.ts`, `_lib/liveEvents.ts`, `_lib/notifications.ts`, `_lib/monetization.ts`, `_lib/moderation.ts`, `_lib/officialAccounts.ts`, `_lib/watchParty.ts`, `_lib/channelAudience.ts` | Canonical personal/social Profile, public Platform media/network presentation, chat handoff, public-safe live/event context | `PARTIALLY_REAL` | `shared` | self vs public, signed-in vs signed-out, official vs member, Chi'lly Circle/privacy state, channel owner vs non-owner, blocked/unavailable, public videos present vs empty |
| 5 | Platform Studio / creator control | `app/channel-studio/index.tsx`, `app/channel-settings.tsx` compatibility | `_lib/accessEntitlements.ts`, `_lib/channelReadModels.ts`, `_lib/channelAudience.ts`, `_lib/monetization.ts`, `_lib/liveEvents.ts`, `_lib/notifications.ts`, `_lib/userData.ts`, `_lib/betaProgram.tsx` | Canonical creator control center for dashboard Home, Content, Live, Audience, Insights, Brand, access defaults, audience summaries/actions, creator permissions, event editing | `PARTIALLY_REAL` | `creator-facing` | signed-in vs signed-out, beta allowed vs blocked, owner vs unavailable, access summary variants, audience action status, creator permission granted vs blocked |
...
| 14 | Audience / follower / subscriber | `app/channel-studio/index.tsx`, `app/channel-settings.tsx`, public-safe posture on `app/channel/[userId].tsx` | `_lib/channelAudience.ts`, `_lib/channelReadModels.ts`, `_lib/userData.ts` | Creator audience summaries/actions for followers, subscribers, requests, blocked audience, plus public-safe backed Channel stats | `PARTIALLY_REAL` | `creator-facing` | owner vs operator vs unavailable, follower vs subscriber vs request vs blocked, public visibility enabled vs disabled |
| 15 | Scheduled events / reminders | `app/channel/[userId].tsx`, `app/channel-studio/index.tsx`, `app/channel-settings.tsx` | `_lib/liveEvents.ts`, `_lib/notifications.ts` | Creator event records, public Live & Upcoming summaries, replay/reminder readiness, reminder enrollment counts where backed | `PARTIALLY_REAL` | `shared` | draft vs scheduled vs live_now vs ended, replay available vs pending vs expired, reminder ready vs not ready, signed-in vs signed-out enrollment |
...
- Exact owner files: `app/profile/[userId].tsx`, `app/channel/[userId].tsx`, `app/channel-studio/index.tsx`, `app/channel-settings.tsx`, `_lib/liveEvents.ts`, `_lib/notifications.ts`, `_lib/accessEntitlements.ts`, `app/watch-party/index.tsx`
...
- Exact owner files: `app/(tabs)/index.tsx`, `app/(tabs)/explore.tsx`, `app/(tabs)/my-list.tsx`, `app/title/[id].tsx`, `_lib/userData.ts`, `_lib/supabase.ts`
