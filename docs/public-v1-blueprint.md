...
- Rachi remains the official seeded guide, first Chi'lly Circle connection, Originals, and moderation-ready presence on canonical Profile, public Platform, Home, and Admin. It is not a Chi'lly Chat starter, private-chat monitor, or normal user.
...
- Current business decision: all full live/watch-party access becomes Premium in a later gate update; free full LiveKit access should not be preserved.
- Launch is planned as 18+.
- Free users see ads at launch, Premium users see zero ads, and ads must follow the AppLovin MAX direction and documented caps before rollout.
- Creator paid content, tips, payout scale, Game Live, Game Watch-Party, and full creator-platform depth remain outside Public v1 unless already stable and intentionally gated. Ads are planned for free launch access but must be built through the separate Ads launch foundation, AppLovin MAX direction, admin controls, caps, and Premium no-ads enforcement.
...
| Basic Chi'lly Chat / Simple Direct Messaging | `app/chat/index.tsx`, `app/chat/[threadId].tsx`, plus profile-to-thread handoff from `app/profile/[userId].tsx` | Provide persistent off-room communication that stays native to Chi'llywood. | Inbox works, direct thread works, profile-to-thread handoff works, direct-message-first behavior stays intact, and Rachi is excluded from Chat. | Group threads, translation, productivity tooling, deep community messaging, replacing room-native interaction, private-chat monitoring. | Thread/member/message data, call-room primitives where already approved, moderation/report, notifications. | `/chat` and `/chat/[threadId]` open cleanly, direct messaging works, profile-to-thread handoff works, Rachi does not appear as a Chat starter/helper thread, and room semantics are not taken over by messaging. |
...
| Preserved Rachi Official Presence | `app/profile/[userId].tsx`, `app/channel/[userId].tsx`, `app/chilly-circle.tsx`, `app/(tabs)/index.tsx`, `app/admin.tsx`, `_lib/officialAccounts.ts`, `_lib/userData.ts` | Preserve trusted official platform presence inside canonical Profile, public Platform, Chi'lly Circle, Home, and Admin. | Rachi remains official, protected, canonical, visible as the first pinned Chi'lly Circle official connection, able to publish backed official updates, and able to surface real public-safe Rachi-owned Originals. | Separate support app, mascot-style drift, unofficial helper personas, hidden admin-only alternate route, private-chat surveillance, Rachi Chat starter/helper entry, fake Rachi activity. | Official-account identity protections, platform-role backing, audited admin posting, official markers, creator-video public-safe visibility. | Rachi Profile, Rachi Platform, pinned Chi'lly Circle connection, and Home Rachi updates/Originals render as clearly official, protected, and canonical, with no doctrine drift into private-chat monitoring, Chat helper threads, or fake social stats. |
...
- Rachi stays out of Chi'lly Chat and remains first in Chi'lly Circle
...
- moderation/report
- notifications
- preserved direct-thread architecture
- Proof required before ship:
- `/chat` and `/chat/[threadId]` open cleanly
- profile-to-thread handoff works
- Rachi does not appear as a Chat starter/helper thread
...
- Owner route/file(s) or owning surface: `app/profile/[userId].tsx`, `app/chilly-circle.tsx`, `app/channel/[userId].tsx`, `_lib/officialAccounts.ts`, `_lib/userData.ts`
- Product purpose: preserve trusted official onboarding/moderation-ready presence inside canonical Profile, public Platform, and first Chi'lly Circle placement
- Minimum shippable behavior:
- Rachi remains official, protected, canonical, and visibly platform-owned
- Rachi stays discoverable through Profile, Platform, Home, and first Chi'lly Circle placement
- Chi'lly Chat excludes Rachi starter/helper threads
- Explicitly out of scope:
...
- direct-thread continuity for normal user conversations
- official markers and identity data
- Proof required before ship:
- Rachi profile, Platform, Home, and first Chi'lly Circle placement render as clearly official and protected
- no Rachi Chat starter/helper thread appears
...
4. Keep Premium subscription gating as the active subscription monetization lane and prepare the next live-gate update so all full live/watch-party access becomes Premium.
5. Move into Admin Command Center V1, ads foundation, age gate, upload/content lifecycle polish, and security/compliance/moderation only with exact scoped prompts.
6. Add payout onboarding groundwork after creator-tool foundations, not before.
7. Build ads through the AppLovin MAX direction with admin on/off, session/daily caps, and Premium no-ads enforcement; do not build AdMob-only ads.
