...
- Rachi remains the official seeded guide, Chi'lly Circle connection, Originals, and moderation-ready presence on canonical Profile, public Platform, Home, Admin, and opt-in Rachi Help surfaces. It is not a private-chat monitor or normal user.
...
| Preserved Rachi Official Presence | `app/profile/[userId].tsx`, `app/channel/[userId].tsx`, `app/chilly-circle.tsx`, `app/(tabs)/index.tsx`, `app/admin.tsx`, `app/chat/index.tsx`, `app/chat/[threadId].tsx`, `_lib/officialAccounts.ts`, `_lib/userData.ts` | Preserve trusted official platform presence inside canonical Profile, public Platform, Chi'lly Circle, Home, Admin, and opt-in Rachi Help surfaces. | Rachi remains official, protected, canonical, visible as the first pinned Chi'lly Circle official connection, able to publish backed official updates, and able to surface real public-safe Rachi-owned Originals. | Separate support app, mascot-style drift, unofficial helper personas, hidden admin-only alternate route, private-chat surveillance, fake Rachi activity. | Official-account identity protections, platform-role backing, audited admin posting, direct-thread continuity, official markers, creator-video public-safe visibility. | Rachi Profile, Rachi Platform, pinned Chi'lly Circle connection, Home Rachi updates/Originals, and Rachi Help render as clearly official, protected, and canonical, with no doctrine drift into private-chat monitoring or fake social stats. |
...
- opt-in Rachi Help presence remains intact
