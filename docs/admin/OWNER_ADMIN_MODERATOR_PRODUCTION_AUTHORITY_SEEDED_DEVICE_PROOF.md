...
Proof truth audit: `docs/release/OWNER_ADMIN_MODERATOR_PROOF_TRUTH_AUDIT.md` is the governing classification for this lane. This document's Closed language covers controlled seeded installed traversal, static policy guards, app-backed RPC/backend proofs, and provider-safe governance. It must not be read as full actual-user production staff proof for every Owner/Admin/Moderator operation. Diagnostic/backend proof is not actual-user proof. Service-role/bootstrap proof is not role-authority proof. Owner RPC staff grant path is the authority proof where applicable. Provider dashboard MFA/access remains owner-confirmation-required unless sanitized evidence exists.
