...
This runbook defines how Owner/Admin should handle money kill switches.
...
Relevant switches include live_money_enabled, payouts_enabled, cashout_enabled, production_merch_enabled, production_paid_content_enabled, production_tickets_enabled, production_seats_enabled, production_event_passes_enabled, production_tips_enabled, provider webhooks, sandbox tester mode, and related rail-specific controls. All production switches default off until future activation.

## Activation Discipline

A switch should not be flipped without matching legal, tax, provider, support, fraud, Data Safety, and Owner approval. If activation is approved, the operator should capture pre-change readback, execute the narrow change, capture post-change readback, verify user-facing copy, and keep rollback ready. No switch should bypass code guards.

## Emergency Shutdown

If fraud, provider failure, support incident, safety issue, legal concern, tax issue, chargeback spike, or fulfillment failure occurs, Owner/Admin should disable the affected production rail, preserve evidence, notify support, update user copy where needed, and document audit reason. Turning off a rail should not delete records or hide evidence.
