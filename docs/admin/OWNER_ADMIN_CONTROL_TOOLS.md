...
- Network/IP proof is not currently attached to Owner Security or Admin audit rows. The approved direction is `docs/admin/SECURITY_CONTEXT_IP_AUDIT.md`: capture request context only in backend/Edge code, link high-risk audit/security events through a future `security_context_id`, show masked network proof only in owner/admin surfaces, and never expose raw IP in public UI.
