...
- `admin-owner-controls` is deployed as ACTIVE version 21 on Supabase project `bmkkhihfbmsnnmcqkoly`; `admin-legal-evidence` is ACTIVE version 7.
...
- May 22, 2026 local / May 23 UTC Owner Security hardening deployed `admin-owner-controls` version 21 and migration `202605230001_owner_security_center_hardening.sql`. Owner Security now separates owner identity, telemetry connection, current-device trust, and emergency-action status; device/grant revokes require Owner authority, backend-trusted current device, reason, exact confirmation, and security audit rows. Physical Android non-owner proof on `R5CR120QCBF` denied `/admin` and hid Owner Security labels; backend non-owner proof returned zero visible owner-security rows and `403 staff_role_required` for security overview/revoke. A signed-in owner session on `R5CR120QCBF` proved current-device trust, Emergency Actions availability only while trusted, exact `UNTRUST DEVICE` plus reason enforcement, successful current-device untrust/revert, and immutable Supabase `security_audit_events` rows for `owner_device_trusted` and `owner_device_revoked` with the recorded proof reason.
