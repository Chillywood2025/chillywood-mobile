...
Status: App Content/listing saved; signed internal/closed draft uploads complete; production review blocked by closed-testing requirements

Final store/release readiness and Play submission packet alignment is consolidated in `docs/release/FINAL_STORE_RELEASE_READINESS_PLAY_SUBMISSION_PACKET.md`. Use that packet as the current release handoff: this lane did not submit the app to production, did not mutate Google Play/RevenueCat/Stripe/provider dashboards, keeps Premium public purchase OFF, keeps Premium monthly public purchase as a separate owner-approved proof lane, keeps Premium annual and Creator Channel Subscription Google Play base-plan provider-blocked, keeps creator-money/live-money/payouts/Stripe/merch/refund automation OFF/manual, and leaves final Play Console acceptance owner/store external.

June 25, 2026 legal/privacy/Data Safety final alignment: use `docs/legal/LEGAL_PRIVACY_DATA_SAFETY_FINAL_ALIGNMENT.md` as the latest repo-side consistency check before entering or updating Play Console fields. Use `docs/monitoring/MONITORING_ANALYTICS_CRASH_RUNTIME_DIAGNOSTICS.md` for the current monitoring, analytics, crash, and runtime diagnostics truth. Do not claim attorney legal advice, full Play acceptance, Premium annual live status, creator-money live status, automatic refunds, payouts, Stripe Connect, merch checkout, payable balances, money movement, immediate hard deletion, or a full in-app appeal center. Account deletion uses scheduled deletion, restore window where supported, and controlled purge/de-identification. Appeals use support/escalation workflow in V1. Provider refunds remain manual/external.

This document tells the owner/operator what to enter in Google Play Console based on current repo evidence. App Content and store listing fields have now been saved where Play allowed, and signed internal/closed draft uploads are complete. It does not claim Google production review acceptance, legal approval, DKIM verification, support staffing, or production monetization.

June 1, 2026 external acceptance tracker update: `docs/google-play/EXTERNAL_ACCEPTANCE_TRACKER.md` now records final per-section status and proof requirements. App Content fields were saved/actioned, contact details/category were saved/published, store listing assets/copy were saved, EAS signed v14 was submitted to internal testing, and EAS signed v15 was submitted to closed alpha as a draft release. Current device proof is Play-installed versionCode `13`; current local repo AAB remains debug-signed and must not be uploaded; Premium remains ready-but-closed with purchase shell closed by default; and live money/tickets/tips/paid content/payouts remain off. Production review remains blocked by Play's closed-testing requirement for at least 12 opted-in testers and the required 14-day closed test.
...
| Store listing | Short description | `Upload, watch together, go live, and build your creator Platform.` | Saved Play listing proof | High | Keep honest unless owner/legal revises | Store listing screenshot | Yes | Yes |
| Store listing | Full description | Current Play description saved with public flows, Premium-gated creator tools, safety/support/legal paths, and money-off posture | Saved Play listing proof | High | Keep honest unless owner/legal revises | Store listing screenshot | Yes | Yes |
| Store listing | Category | Entertainment | Store settings proof | High | Keep unless owner changes positioning | Console screenshot | No | Yes |
...
| Store listing | App icon | Uploaded generated 512x512 Play listing icon | `/tmp/chillywood-google-play-release-v14-20260601/store-assets/play-listing-icon-512.png` | High | Replace only if owner approves alternate brand asset | Asset upload proof | Brand review | Yes |
| Store listing | Feature graphic | Uploaded generated 1024x500 feature graphic | `/tmp/chillywood-google-play-release-v14-20260601/store-assets/feature-graphic-1024x500.png` | High | Replace only if owner approves alternate graphic | Asset upload proof | Brand/legal copy review | Yes |
| Store listing | Phone screenshots | Uploaded sanitized phone screenshots | `/tmp/chillywood-google-play-release-v14-20260601/store-assets/screenshots/phone/` | High | Recapture only if app UI materially changes | Uploaded screenshot proof | Legal/content review | Yes |
| Store listing | Tablet screenshots | Uploaded same sanitized screenshots for 7-inch and 10-inch tablet sections | Store listing proof | Medium | Replace with tablet-specific captures later if product requires | Console screenshot if used | No | No unless targeting/tablet policy requires |
...
| Data Safety | Data deletion mechanism | Yes, in-app Delete Account schedules deletion with a 30-day restore window, and the web page explains the process. Do not claim permanent purge completion before backend processing is proved. | Account deletion URL packet and Android proof | High | Enter web URL and in-app path | Data deletion screenshot | Yes | Yes |
...
| Release | App bundle upload | EAS signed v14 submitted to internal; EAS signed v15 submitted to closed alpha as draft. Local Gradle AAB remains debug-signed and must not be uploaded. | `/tmp/chillywood-google-play-release-v14-20260601/`, release checklist | High | Use EAS signed artifacts for future uploads; complete closed-testing requirements before production review | AAB upload/pre-launch proof | No | Yes |
| Closed testing | Track | Closed alpha v15 draft uploaded; production access requires at least 12 opted-in testers and the required 14-day closed test | Public V1 blocker map, release checklist | High | Add/confirm testers, keep closed test running, then return to Production access/Publishing overview | Track/tester screenshot | No | Yes before public |
...
| Posts/comments/replies | Yes if user posts | Public/participants depending surface; service providers | UGC/social engagement, moderation | Optional | Delete/hide/scheduled account deletion subject to retention | Moderation and RLS gates | Profile social docs/code | High |
| Creator videos/uploads | Yes if user uploads | Public only after publish/public-safe gates; service providers | Creator Platform content, Player playback | Optional | Unpublish/delete/scheduled account deletion with legal/moderation retention | Scan/status/public-safe gates | Creator media and scanner docs | High |
| Photos/videos/files/attachments | Yes when selected/uploaded | Public only where user posts/publishes; service providers | Profile posts, creator uploads, evidence/support where used | Optional | Owner removal/scheduled account deletion where backed; legal evidence may be retained | MIME/size/scan/status gates where configured | Picker packages, scanner docs | High |
...
| Support/account deletion records | Yes if submitted/scheduled | Support operators/service providers/legal where required | Support, account management, compliance | Optional unless requesting help/deletion | Schedule/restore records may be retained for legal/security/audit | Support workflow, no secrets in docs | Account/legal runbooks | High |
...
| Is deletion automatic? | In-app deletion is self-service scheduled deletion with a 30-day restore window. Permanent purge/de-identification after the deadline remains backend/legal operations with retention exceptions. |
...
- Enter account deletion URL and scheduled deletion / 30-day restore wording.
