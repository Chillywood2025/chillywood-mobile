...
| Release | App bundle upload | Fresh current-HEAD AAB required before final upload. Existing AAB proof is evidence only and should not be treated as current final release candidate. | `android/app/build/outputs/bundle/release/app-release.aab`, release proof | High | Rebuild final AAB and upload to closed testing first | AAB upload/pre-launch proof | No | Yes |
...
| Existing AAB proof artifact is not a fresh current-HEAD upload candidate. | Uploading stale artifact could miss newer code/runtime changes. | Rebuild final AAB after this docs lane before Play upload. |
