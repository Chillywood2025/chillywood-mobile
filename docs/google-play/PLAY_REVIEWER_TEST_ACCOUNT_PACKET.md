...
June 4, 2026 internal tester sandbox purchase mode note: `docs/INTERNAL_TESTER_SANDBOX_PURCHASE_MODE.md` adds a bounded tester-only sandbox mode. Public/default users still see Premium and digital purchases unavailable. Approved testers may see clearly labeled Google Play / RevenueCat sandbox purchase surfaces with `Sandbox test`, `Not production`, and `Not payable` copy. Do not describe this as production commerce, public live Premium, public paid content, public tickets/seats/tips/event passes, payouts, cash-out, or Stripe Android digital checkout.
