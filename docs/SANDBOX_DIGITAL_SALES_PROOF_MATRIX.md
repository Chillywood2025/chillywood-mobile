...
| Real non-Premium sandbox purchases | Creator tip, Watch-Party Live Seat Pass, Live Watch-Party access pass, Live Watch-Party seat pass, paid content access, and event pass completed through Google Play test card / RevenueCat sandbox and webhook processing |
...
| `merch_physical_good` | `cw_merch_test_tee_sandbox` | Stripe physical goods sandbox | Not part of Android digital intents | Passed: real Stripe test-mode Checkout completed for physical merch | Passed: signed Stripe merch webhook processed `checkout.session.completed` | Correctly none; merch creates no digital access | Order/readiness only; one sandbox paid/processing merch order, not payable | Physical merch separate / sandbox checkout proved; no payable balance or cash-out | Sanitized merch order/event readback captured | Production merch approval, fulfillment, refund/return, support, and Data Safety review remain future |
...
- Admin revoke proof changed one sandbox Watch-Party Seat Pass grant to `revoked`, appended one sandbox `reversed` ledger row, and resolver access became `allowed=false`.
...
- Stripe physical merch checkout proof: `/tmp/chillywood-stripe-merch-sandbox-checkout-proof-20260603/`
- Stripe Connect payout-readiness proof: `/tmp/chillywood-stripe-connect-payout-readiness-proof-20260604/`
