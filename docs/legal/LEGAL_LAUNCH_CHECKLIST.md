...
- Supabase Auth custom SMTP is configured by Management API for project `bmkkhihfbmsnnmcqkoly`: host `smtp-relay.brevo.com`, sender name `Chi'llywood`, verified sender email `no-reply@chillywoodstream.com`; recovery dispatch proof returned `200`. Future recovery proof and Play Console App Access must use a disposable non-admin inbox, not the owner's personal/internal tester inbox.
