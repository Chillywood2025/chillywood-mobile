...
| Copyright report flow | Implemented / hosted public URL live / live backed proof passed / attorney review pending | `app/copyright-report.tsx` and hosted `https://chillywoodstream.com/copyright-report` collect formal notice fields and submit to `dmca_cases` through `submit_dmca_notice`; generic report sheet and Settings Legal & Support link copyright users to the formal route. Admin manual intake is backed by `admin_dmca_create_case`. Safe proof passed with invalid-notice rejection, public form case creation, private evidence attachment upload/access denial, Admin list/detail visibility, supported content hide/restore, filters/search, test-only proof-case hygiene, and access denial for normal users. Automated malware scanning is not configured, so uploaded evidence is marked pending manual review. |
