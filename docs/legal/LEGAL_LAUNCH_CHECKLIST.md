...
- Supabase Auth custom SMTP is configured by Management API for project `bmkkhihfbmsnnmcqkoly`: host `smtp-relay.brevo.com`, sender name `Chi’llwood`, verified sender email `no-reply@chillywoodstream.com`; `POST /auth/v1/recover` for `rob2037gn@gmail.com` returns `200`.
