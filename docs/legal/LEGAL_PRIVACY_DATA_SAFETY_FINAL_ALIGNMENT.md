...
- Monitoring, analytics, crash, and runtime diagnostics alignment is documented in `docs/monitoring/MONITORING_ANALYTICS_CRASH_RUNTIME_DIAGNOSTICS.md`; owner must confirm final SDK/provider collection settings before Play submission.
...
| Monitoring/diagnostics readiness | Yes | `docs/monitoring/MONITORING_ANALYTICS_CRASH_RUNTIME_DIAGNOSTICS.md` | Root error boundary, runtime unavailable screen, Firebase helpers | Repo-side closed; external SDK/provider confirmation pending | No public proof/debug/internal wording added | Yes, Firebase Analytics/Crashlytics/Performance status, Sentry/PostHog disabled status, sanitized diagnostics, support/admin privacy, and incident checklist are aligned | Owner confirms final Firebase collection settings and release log audit before Play submission |
...
| Monitoring/runtime diagnostics | Root error boundary, runtime unavailable screen, Firebase diagnostics helpers, and support-safe diagnostics are documented. | Declare diagnostics/analytics/performance if enabled; do not underclaim SDK collection. | User-facing runtime errors use safe copy; support/admin diagnostics are scoped and privacy-safe. |
