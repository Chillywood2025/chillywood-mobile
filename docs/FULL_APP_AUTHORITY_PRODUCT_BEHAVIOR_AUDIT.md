...
Installed Product QA has a cost-capped Firebase Test Lab foundation as of 2026-07-13. The source path can record `firebase_test_lab_uploaded_artifact` findings with `device_lab_provider=firebase_test_lab`, `costEstimateUsd`, `maxAllowedCostUsd`, `monthlyBudgetUsd`, `monthlySpentEstimateUsd`, `billingRisk`, `quotaMode`, `deviceType`, and `runReason`, but Firebase uploaded artifact is not Play-installed proof. The default cheap mode is virtual-device smoke only, `$5/month`, `$0.25/run`, daily at most or on OTA/source change/owner command, with physical devices, broad crawls, two-device Firebase, and over-cap runs blocked unless explicitly approved. One owner-command Tier 1 virtual smoke completed as Firebase matrix `4612242345700782646` with worst-case `costEstimateUsd=0.09`, `billingRisk=low`, and local ledger write. Firebase cannot close Premium/RevenueCat/Google Play Billing or two-device LiveKit proof unless those exact provider/device flows are separately and actually proved. The owner Mac is not the long-term runner; `chillywood-prod-01` is the daily systemd runner through `ops/installed-product-qa-operator/systemd/`. Server Node/npm/gcloud/Firebase tooling, dedicated runner credentials, Test Lab catalog readback, and the APK artifact are present. The exact dedicated results bucket bootstrap is now a standing bounded Level 2 policy for `gs://chillywood-installed-qa-testlab-results`; the bucket exists with seven-day lifecycle and bucket-level `roles/storage.objectCreator` for the dedicated runner. Server Firebase matrix `8384645686869695525` completed under the cap and posted live traversal row `333817c9-b651-4e65-ab66-0d885952d2de`. The runner now uses bounded async matrix start/poll, pending-matrix tracking, and systemd start timeout bounds so it cannot wait indefinitely inside `gcloud firebase test android run`. Current scheduler proof started matrix `4625992954862995833`, posted audited `scheduler=systemd_timer` `matrix_started`, `matrix_timeout` / `POLL_HTTP_FAILED`, and duplicate-safe `matrix_pending` rows, exited cleanly, and left the daily timer enabled/active. That current matrix is pending/poll-failed, not a completed device result.

User Report Router is now a scoped support-success lane for user-submitted bugs, fixes, support issues, safety concerns, and repeated product problems. Authenticated users can submit sanitized reports through `user-report-intake`; clients cannot choose the routed system, write clusters, or create Owner Commands directly. `support_success_operator` classifies and clusters reports, dedupes same-user repeats, routes safety/enforcement reports to `moderation_safety_operator`, routes product/system findings to the owning autonomous operator, and creates a routed finding or Owner Command when three unique users report the same bug/fix within seven days. Critical safety, security, privacy, payment, billing, payout, or provider issues can escalate immediately for review. Reports never directly move money, grant Premium, mutate auth/RLS, enforce moderation, publish/rollback OTA, change LiveKit routing, change R2/media behavior, or activate ads/sponsors.
...
| `user-report-intake` | user report intake/router | authenticated user + service backend | live source | 2 | Sanitized report intake/classification only; clients cannot choose routed system or create Owner Commands directly. |
...
- Earlier focused moderator installed proof passed for exact moderation/admin entry with broad Admin Search, owner approvals, direct enforcement, and private evidence absent, but the later installed QA targeted artifact showed `admin-search-panel` / `admin-user-search-input` visible for `proof_moderator_001`. Source now gates Admin Search behind `canUseAdminSearch` (owner/super_admin/operator or explicit `admin.user.search` / `user_lookup` permission), but installed moderator closure remains pending until that source fix is OTA-loaded and rerun.
...
Installed Product QA live deployment follow-up on 2026-07-13: current source commit `f6ac19fe0c85f4e28db715315e5829043f9fb3ed` was published to production Android OTA group `cdfd42a5-7c78-4cd0-9673-1f451073aa16`, Android update `019f596f-1a87-76d8-abe3-14342c8d1cf6`, and the Play-installed app loaded it with runtime `1.0.0`, channel `production`, embedded launch `false`, emergency launch `false`. The deployed `installed-product-qa-operator` then ran live `watch_once` and recorded the current route, role, account-fixture, moderator-review, and device/two-device blockers with `source=play_installed` and `discovered_by=autonomous_operator`, plus a safe Owner Command request. A targeted installed rerun on this OTA reproved normal `/chat` and `/creator-monetization-setup` as passing markers. Follow-up artifact `/tmp/app-full-seeded-one-device-role-traversal-rerun-20260713063737` returned `Pass=18`, `Human review=4`, `Blocked=1`, `Two-device required=4`, `Failures=0`; it kept restricted `/chat`, Premium-active proof, moderator Admin Search boundary, and two-device realtime open. Firebase Test Lab server smoke later completed from `chillywood-prod-01` as uploaded-artifact matrix `8384645686869695525` and posted live row `333817c9-b651-4e65-ab66-0d885952d2de`. The daily scheduler is now active after bounded start/poll proof: matrix `4625992954862995833` posted `scheduler=systemd_timer` matrix state rows and did not start duplicates, but that current matrix remains pending/poll-failed rather than completed device proof.
