...
External provider product setup is no longer the blocker for the six first-pass sandbox products. Google Play and RevenueCat products exist, but no real non-Premium sandbox purchase has run yet. The remaining blocker is shipping/installing a signed build or EAS update with `/admin-money-sandbox-purchases`, then using a licensed tester to complete real Google Play sandbox purchases. No fake provider event, fake sale row, fake access grant, fake ledger row, fake balance, payout, or cash-out was inserted.
