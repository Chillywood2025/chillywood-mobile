...
External provider setup is still the blocker for non-Premium digital sale proof. No fake provider event, fake sale row, fake access grant, fake ledger row, fake balance, payout, or cash-out was inserted.
...
| `paid_content_access` | `cw_paid_content_access_sandbox_099` | Not verified from this repo lane | One-time/consumable pattern appropriate to repeat testing | Missing / external setup required | Not verified | Missing / external setup required | Usually none; access comes from Chi'llwood intent/grant | Webhook endpoint exists | No | Provider product and offering missing | Create Play product, import to RevenueCat, attach package, map catalog row to sandbox product |
| `watch_party_live_ticket` | `cw_watch_party_live_ticket_sandbox_099` | Not verified | One-time/consumable ticket pattern | Missing / external setup required | Not verified | Missing / external setup required | Usually none; ticket comes from Chi'llwood intent/grant | Webhook endpoint exists | No | Provider product and offering missing | Create product, package, and catalog mapping; prove ticket grants viewing only |
| `live_watch_party_access_pass` | `cw_live_watch_party_access_sandbox_099` | Not verified | One-time/consumable pass pattern | Missing / external setup required | Not verified | Missing / external setup required | Usually none | Webhook endpoint exists | No | Provider product and offering missing | Create product, package, and catalog mapping; prove viewer/listener entry only |
| `live_watch_party_seat_pass` | `cw_live_watch_party_seat_sandbox_099` | Not verified | One-time/consumable seat eligibility pattern | Missing / external setup required | Not verified | Missing / external setup required | Usually none | Webhook endpoint exists | No | Provider product and offering missing | Create product, package, and catalog mapping; prove host approval still controls speaker role |
| `creator_tip` | `cw_creator_tip_sandbox_099` | Not verified | Consumable support product | Missing / external setup required | Not verified | Missing / external setup required | None; ledger only unless a `creator_tip_record` readout is intentionally added | Webhook endpoint exists | No | Provider product and offering missing | Create consumable product/package; prove sandbox ledger stays `not_payable` and no payout appears |
| `event_pass` | `cw_event_pass_sandbox_099` | Not verified | One-time/consumable event pass pattern | Missing / external setup required | Not verified | Missing / external setup required | Usually none | Webhook endpoint exists | No | Event model/provider mapping not backed yet | Define event resolver/model, then create product/package and catalog mapping |
