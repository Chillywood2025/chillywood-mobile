...
Updated: 2026-05-06 for pushed Public Platform and Platform Studio route truth
...
- Signed-in non-owner proof used the available authenticated account viewing the official Rachi Profile because no true second-account credential was available. The viewer saw no Platform Studio, Preview Platform, Settings, delete controls, draft/reported badges, composer, or Attach; View Platform opened public Platform, not Studio. Current product truth now keeps Rachi out of Chi'lly Chat and pinned first in Chi'lly Circle.
...
- Android visual proof for the compact Profile Photo action sheet is closed at `/tmp/chillywood-profile-photo-picker-proof-20260529/`: owner tap and long-press open the sheet, the first sheet has `Change Photo`/`Cancel` only when no photo exists, the native picker focus opens through Android DocumentsUI, and Profile Background remains a separate compact sheet. Safe-asset save/read-back/removal, viewer Profile Actions sheet, block confirmation, signed-out handoffs, and no-edit viewer/signed-out state still need fixture proof. The repo now lazy-loads `expo-image-picker` so older installed dev-clients do not crash on app startup, but choosing images requires a current native build if the installed client predates the module.
- May 29 proof closeout follow-up at `/tmp/chillywood-admin-search-audit-denial-spectator-profile-proof-20260529/` captures a safe Chi'llywood image visible inside Android DocumentsUI, but DocumentsUI did not return the selected file to the app during automation. No avatar/background save, active backend read-back, remove, or fallback proof is claimed from that pass, and no private gallery image should be used to force the proof.
- May 31 code-side hardening updates `_lib/profileMedia.ts` to stage Android `content://` picks to cache, upload Profile media through Supabase Storage REST with SDK fallback, and verify a signed read-back before saving Profile fields. The native picker now explicitly stays non-legacy/gallery-first with editing enabled, square/oval Profile Photo crop, and wide 4:1 Profile Background crop. Sheets close before picker/remove flows. Current-build Android save/read-back/remove/fallback proof is still required before the runtime gap is closed.
- May 31 Brand Studio preview clarification adds an owner-only `Preview Brand Draft` route for saved/pending Platform visuals. Public `Preview Platform` and public viewer routes still hide draft/pending/unsafe Platform branding.
...
- Profile remains connected to Channel through View Platform, but `/profile/[userId]` must not collapse into one vague Profile/Channel feed.
...
- Public Platform must not show owner controls, drafts, private videos, or unpublished content to non-owners.
...
- Public top actions are backed Follow/Following, Chi'lly Chat, View Platform, Share Profile, and Report where supported.
...
- Public Platform owns viewer-facing creator uploaded videos; owner surfaces use `CreatorVideoCard` where current code provides it and open `/player/[id]?source=creator-video`.
...
- Older audit language said Channel had no standalone public URL; current truth is `/channel/[userId]` exists and Profile View Platform routes there.
...
1. Owner entry point from Platform Studio (`/channel-studio`, with `/channel-settings` compatibility), with a clear `Upload Video` or `Add Video` action and an owner-only public Platform empty-state CTA where backed.
...
- open their public Profile or public Platform route
...
- Canonical `/channel/[userId]` public Platform route.
