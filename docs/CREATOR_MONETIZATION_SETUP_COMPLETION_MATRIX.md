...
This lane completes the creator monetization setup proof that began with `/creator-monetization-setup`; current source treats that route as compatibility-only and lands creators in Money Center Offers setup. The result is not production monetization. It is a sandbox/internal-test setup matrix over already-proved payment rails.
...
- Creators can use Money Center setup controls for Tips, Paid Video, Watch-Party Seat Pass, Channel Subscription, VIP, and Event Pass.
...
- Access grants: paid content, live access pass, live seat pass, and event pass have sandbox-only grants; Watch-Party Seat Pass grant exists and is currently revoked from prior admin-revoke proof; creator tip intentionally has no access grant.
- Money access ledger: sandbox/not-payable purchase rows exist for all six Android digital products; Watch-Party Seat Pass also has a sandbox reversed revoke row.
