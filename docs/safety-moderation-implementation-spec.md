...
- Stripe Connect Test-Mode Backend Functions are deployed backend-only/test-mode and do not change safety enforcement, fraud holds, payout release, creator UI, or live money behavior; real provider proof is blocked until backend Stripe secrets are configured, and future payout release still requires fraud review plus immutable audit rows before dangerous actions
