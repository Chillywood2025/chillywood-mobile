...
2026-05-06 current-route note: creator/channel safety summaries belong in owner Platform Studio on `/channel-studio`; `/channel-settings` remains compatibility. Public Platform on `/channel/[userId]` may open public-safe report intake but must not expose moderation/admin controls.
...
| `/channel/[userId]` | `app/channel/[userId].tsx` | Canonical public Platform route. May open honest public-safe report intake, but must not expose owner/admin controls. |
...
- `/channel/[userId]` remains the public Platform route and must not expose owner/admin safety controls.
...
- Sponsor Disclosure / Moderation Foundation is Admin Sponsors read-only/foundation only: disclosure and safe product/scam review rows can be counted for future sponsor review readiness, but they do not enforce sponsorship policy against users, remove content, restrict creators, approve sponsors, activate placements, create checkout, charge brands, or pay creators
...
- audit row edit/delete/clear controls, payout review approval/release, payout batch processing, transfer creation, or dangerous-action audit execution unless those systems are separately backed
