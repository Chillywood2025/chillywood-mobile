...
June 1, 2026 backend entitlement sync update: the Play-installed RevenueCat/Google sandbox purchase and restore proof passed, and `revenuecat-webhook` now contains a real verified-event-to-`user_entitlements` sync path for Premium. The server-side RevenueCat webhook secret and RevenueCat dashboard webhook integration are now configured; dashboard `TEST` delivery is received as `test_received` with no Premium grant and no live-money action. Sanitized backend readback still shows zero Premium rows because the post-setup device restore says Premium is not active, so Money Center must continue to show setup/readiness posture for Premium backend sync. RevenueCat client UI or dashboard test delivery is not enough to unlock creator tools or mark production Premium live.
...
- Backend `user_entitlements` purchase sync is implemented in `revenuecat-webhook`, and the RevenueCat/Supabase webhook secret path is configured. It remains unproved by a real active Premium event because the post-setup restore says Premium is not active. Do not claim Premium creator-tool unlock until sanitized backend readback/webhook sync proves the row.
