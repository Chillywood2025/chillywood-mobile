...
June 11, 2026 Paid Watch-Party Seats / Room Tickets V1 status: repo-side implemented and Supabase-applied, but not Play/internal sandbox-proven yet. It uses RevenueCat / Google Play dynamic sandbox product `watch_party_live_ticket_sandbox_099` / `cw_watch_party_live_ticket_sandbox_099`, not Stripe Tips. Room tickets are digital room access and must stay separate from Premium, Tips, Paid Videos, VIP, subscriptions, and Paid Events. Party Waiting Room checks ticket access before routing to Party Room, and Party Room re-checks before membership/session/presence setup; paid Watch-Party tickets must route to Party Waiting Room -> Party Room, never Live Stage. Play/internal v38 and v40 proved the app installed from Google Play and exposed setup/UI blockers; the backend blockers are fixed and remote-applied, fresh v40 room lookup works, and the remaining fan-gate proof needs a new Play/internal build with the inline preview `Room ticket required` / `Buy Room Ticket` state. Sandbox rows are not payable, live money remains off, and tickets do not grant LiveKit publish authority, host authority, payout, cash-out, withdrawal, transfer, Premium, paid video access, VIP, subscription, or event access. Dedicated proof doc: `docs/PAID_WATCH_PARTY_SEATS_V1_END_TO_END_PROOF.md`.
