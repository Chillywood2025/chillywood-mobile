...
June 13, 2026 Channel Subscriptions V1 status: implemented, Supabase-applied, webhook-deployed, and Play/internal sandbox-proven for purchase, Money Center visual readback, authenticated non-subscriber denial, and effective-access stale-row safety. Creator channel subscriptions are recurring Android digital access and use RevenueCat / Google Play subscriptions, not Stripe Tips. Google Play product `channel_subscription_sandbox_monthly_499` has active monthly base plan `monthly`; RevenueCat product `channel_subscription_sandbox_monthly_499:monthly` is published and attached to entitlement `creator_channel_subscription`. Play/internal v51 on `R5CR120QCBF` proved package `com.chillywood.mobile`, installer `com.android.vending`, and versionCode `51`; after a cold app restart Google Play Billing opened, the sandbox subscription purchase completed, provider event `9dabc47f-61f7-49f7-a169-3adb0ebbac30` processed, and Supabase created subscription `436f2acc-ec46-4977-ba51-958452ea2f2e`, paid/not-payable transaction `e49cddea-cd6d-4097-b70c-a07abaa24823`, and sandbox access grant `1a5492fe-c135-435e-878c-5e21a7638322`. The subscribed fan saw `SUBSCRIBED` on `/channel-subscription/[creatorId]`; follow-up proof showed the exact transaction in Money Center as `$4.99 channel subscription`, `Paid`, `Sandbox`, and `payout status: not_payable`, and showed authenticated non-subscriber access blocked. Channel subscriptions unlock only subscriber state for that creator channel and do not include Chi'llwood Premium, VIP, Paid Videos, Paid Watch-Party tickets, Paid Events, Tips, LiveKit authority, payout access, cash-out, withdrawal, transfer, platform-wide badge/status, or other creators' channels. Live money remains off, sandbox rows are `not_payable`, and Money Center must not present subscription revenue as withdrawable. Lifecycle handling is implemented and deployed for `RENEWAL`, `CANCELLATION`, `EXPIRATION`, `BILLING_ISSUE`, `UNCANCELLATION`, `PRODUCT_CHANGE`, `REFUND`, `REVOCATION`, and `SUBSCRIPTION_PAUSED`; historical ignored lifecycle rows were not manually rewritten. Google Play accepted a sandbox refund with entitlement removal for exact order `GPA.3353-3923-8017-31040..4`, but RevenueCat did not emit a fresh signed webhook during the proof window, so cancellation/expiration/revoke proof still requires a fresh or safely replayed signed RevenueCat event. Until then, subscriber routes and Money Center readback use effective access safety: stale `status=active` rows with expired provider periods do not unlock access or read as current active subscriber access. Dedicated proof doc: `docs/CHANNEL_SUBSCRIPTIONS_V1_END_TO_END_PROOF.md`.
...
June 13, 2026 Channel Subscriptions V1 Android proof update: the provider catalog blocker is closed. Google Play product `channel_subscription_sandbox_monthly_499` has active base plan `monthly`, RevenueCat product `channel_subscription_sandbox_monthly_499:monthly` is attached to entitlement `creator_channel_subscription`, and Play/internal v51 completed one sandbox subscription purchase. Provider event `9dabc47f-61f7-49f7-a169-3adb0ebbac30` created subscription `436f2acc-ec46-4977-ba51-958452ea2f2e`, paid/not-payable transaction `e49cddea-cd6d-4097-b70c-a07abaa24823`, and sandbox access grant `1a5492fe-c135-435e-878c-5e21a7638322`. Follow-up proof captured Money Center visual transaction readback and authenticated non-subscriber route denial. Channel subscriptions remain sandbox-only and unlock only one creator-channel subscriber state. Money Center must label subscription rows as sandbox/not_payable until live money is explicitly approved. Lifecycle handling is now deployed; a Google Play sandbox refund/revoke attempt was accepted, but RevenueCat did not deliver a fresh signed event during this proof window. Effective access fallback is the safety source until provider lifecycle delivery is proven: stale active rows with expired provider periods do not unlock the route and are not presented as current active subscriber access.
