...
June 5, 2026 creator setup flow update: `docs/CREATOR_MONETIZATION_IN_APP_FLOWS.md` records the in-app setup layer for approved creator/internal tester sandbox monetization. Platform Studio now links to `/creator-monetization-setup`, where creators choose approved sandbox tiers only and bind a real source UUID. Migrations `20260605000610_creator_monetization_in_app_setup_flows.sql` and `20260605002000_bound_creator_monetization_setup_access.sql` add `creator_monetization_configs` plus safe creator/admin RPCs, then require owner/operator or active beta/internal tester access for saves. Config rows are sandbox-only/not-payable and cannot activate production money, payouts, payable balance, cash-out, withdrawal, transfer, Stripe Android digital checkout, fake sales, LiveKit publish, host/speaker/mod/admin authority, or safety bypass.
