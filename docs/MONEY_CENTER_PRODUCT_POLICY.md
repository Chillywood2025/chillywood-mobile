...
June 1, 2026 backend entitlement sync update: the Play-installed RevenueCat/Google sandbox purchase and restore proof passed, and `revenuecat-webhook` now contains a real verified-event-to-`user_entitlements` sync path for Premium. The deployed endpoint still fails closed because `REVENUECAT_WEBHOOK_SECRET` is not configured server-side; sanitized backend readback shows zero Premium rows, and Platform Studio still denies with `Premium required`. Money Center must therefore continue to show setup/readiness posture for Premium backend sync. RevenueCat client UI being active is not enough to unlock creator tools or mark production Premium live.

June 1, 2026 RevenueCat/Google sandbox purchase proof update: the earlier restore-only blocker was superseded by the Play-installed internal-test v12 sandbox purchase/restore proof. Money Center still stayed setup/readiness-only: no backend entitlement update, payout, balance, ticket, seat, tip, paid-content unlock, live-money action, or Stripe Android checkout was created. Watch-Party Live and Live Watch-Party / Live Stage keep Premium access gates, but ticket/seat monetization remains `watch_party_seats_enabled=off` and setup-only with no buy button unless a future Play/RevenueCat-backed product is proved.
...
- Play-installed v12, signed-in app state, RevenueCat mapping, sandbox purchase, and sandbox restore are now proved. The bounded proof used EAS update group `b678522a-8734-49a1-a582-f2bc6743c756` to open only the Premium shell, then closed the shell again with update group `82f7e7fd-d213-4f50-9c5d-6e6a328884db`; current source keeps `PREMIUM_PURCHASE_SHELL_ON_HOLD = true`. Do not mark production Premium active from local config, provider mapping, signed-in screenshots, Play install proof, sandbox screenshots, or setup-only screenshots.
- Backend `user_entitlements` purchase sync is implemented in `revenuecat-webhook` but remains unproved by a real provider event because the server-side RevenueCat webhook secret is missing. Platform Studio still denied after the sandbox purchase because creator-tool gates trust backend entitlement rows. Do not claim Premium creator-tool unlock until sanitized backend readback/webhook sync proves the row.
