...
| --- | --- | --- | --- |
| Support email | `EXPO_PUBLIC_SUPPORT_EMAIL` is runtime-configured; legal pages and default runtime fallback use `support@chillywoodstream.com`. Cloudflare Email Routing is enabled for `chillywoodstream.com`, the destination is verified, the forwarding rule is enabled, and operator-confirmed inbox receipt proof passed. Public DNS proves Cloudflare MX, SPF `v=spf1 include:_spf.mx.cloudflare.net ~all`, and DMARC `v=DMARC1; p=none; rua=mailto:support@chillywoodstream.com; adkim=r; aspf=r; fo=1`; DKIM remains pending until a real outbound provider issues selector records. | Inbox Receipt Proof Passed / DMARC Baseline Proved / Support SLA Pending | Confirm support SLA before entering it in Play Console; configure outbound-provider DKIM before sending authenticated `@chillywoodstream.com` mail. |
| Website URL | `https://chillywoodstream.com` is live on Cloudflare Pages for the public legal/support surface | Live Legal Site / Marketing Approval Pending | Use only approved public/support URLs in Play Console and add any separate marketing site only after approval. |
| Privacy Policy URL | `https://chillywoodstream.com/privacy` redirects to `/privacy/` and returns HTTP/2 200 | Live URL / Attorney Review Pending | Legal owner approves final Privacy URL and Play owner enters it. |
| Terms URL | `https://chillywoodstream.com/terms` redirects to `/terms/` and returns HTTP/2 200 | Live URL / Attorney Review Pending | Legal owner approves final Terms URL. |
| Account Deletion URL | `https://chillywoodstream.com/account-deletion` redirects to `/account-deletion/` and returns HTTP/2 200 | Live URL Candidate / Play Acceptance Pending | Confirm deletion process/SLA and enter final URL in Play Console; do not claim Play acceptance until accepted. |
...
| Copyright / DMCA URL | `https://chillywoodstream.com/copyright` redirects to `/copyright/` and returns HTTP/2 200; registration `DMCA-1072720` is recorded complete | Live URL / DMCA Agent Registered / Attorney Review Pending | Keep the DMCA agent contact current and complete attorney review before broad legal compliance claims. |
...
| Account deletion/data deletion | Settings Delete Account schedules deletion with a 30-day restore window; `/account-deletion` documents deletion/retention categories, uploaded-content handling, subscriptions, restore, and backend permanent-processing caveats | External Setup Pending | Final permanent purge process/SLA and Play Console acceptance. |
