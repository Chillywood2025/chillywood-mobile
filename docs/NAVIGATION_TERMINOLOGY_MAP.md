...
- Continue Watching hero eligibility requires real playback progress: position at or above `HOME_CONTINUE_MIN_POSITION_MILLIS` (`10_000` ms), and when duration is known the ratio must be below `HOME_CONTINUE_COMPLETION_THRESHOLD` (`0.94`). The selector also requires an available title row that is not unpublished/draft/scheduled/archived/deleted/private/restricted/paid, sorts by the merged watch-progress last-watched timestamp, and shows only the latest eligible item. Finished, not-started, saved/favorite, unavailable, and broad discovery titles are not eligible Home hero content.
