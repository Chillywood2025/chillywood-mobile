...
Minimal server tooling is installed: Node `24.18.0` and npm `11.16.0` from the official OpenJS `node` snap, Google Cloud SDK `575.0.1` from the official `google-cloud-cli` snap, and pinned `firebase-tools@15.23.0` through npm. The systemd wrapper exports `/snap/bin:/usr/local/bin:/usr/bin:/bin` so snap-installed tools are visible under systemd. Server Firebase readiness remains Partial: active gcloud credential count is `0`, Firebase/Test Lab API/catalog readback fails closed, no APK/AAB artifact exists under `/opt/chillywood/current` or `/var/lib/chillywood`, and no `chillywood-installed-qa-firebase-smoke.timer` or service is installed. `npm run installed-qa:firebase-test-plan` on the server estimates `costEstimateUsd=0.09` under the `$0.25` per-run cap, but blocks with `firebase_credentials_missing`; artifact readiness is also missing. Therefore the server-side Firebase scheduler remains blocked by server Google access plus approved artifact readiness, and `schedulerStatus=device_lab_scheduler_pending` remains truthful.
...
Local CLI access exists on the owner Mac for manual runs: `node`, `npm`, `gcloud`, `firebase`, and `npx` are present and the configured gcloud project is `chillywood-app`. That does not make the Mac the scheduler. The long-term scheduled path now has equivalent server CLI tooling, but still requires noninteractive server Google access and an approved server APK/AAB artifact path.
