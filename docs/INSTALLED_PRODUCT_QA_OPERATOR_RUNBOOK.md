...
Local CLI access exists on the owner Mac for manual runs: `node`, `npm`, `gcloud`, `firebase`, and `npx` are present and the configured gcloud project is `chillywood-app`. That does not make the Mac the scheduler. The long-term scheduled path still requires equivalent noninteractive Firebase/gcloud readiness on `chillywood-prod-01`, or a server-local container/wrapper path with credentials stored only in the approved host secret store.
