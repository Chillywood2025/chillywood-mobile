...
The scheduler is pending until the systemd Firebase smoke service completes cleanly. Do not claim a scheduled installed QA loop active until a safe installed device automation path or approved device lab/timer proof exists.
...
The timer is daily only with ten-minute randomized delay. No every-30-minute Firebase device schedule is allowed. The service is hardened with `NoNewPrivileges=true`, `ProtectSystem=strict`, `ProtectHome=true`, `PrivateTmp=true`, `RestrictSUIDSGID=true`, `LockPersonality=true`, `TimeoutStartSec=20min`, `RuntimeMaxSec=20min`, `KillMode=control-group`, `Restart=no`, and an empty `CapabilityBoundingSet`. It does not use a service-role key and must not print token values.

Do not enable the timer until the systemd service can complete cleanly. The runner now uses bounded matrix lifecycle behavior: start asynchronously with `--async`, capture the matrix id, poll Test Lab with a max wait, record `matrix_pending`/`matrix_timeout` if the matrix is still running, and avoid a duplicate start while a pending matrix exists. Billing enablement/linking remains owner-only. Do not store sudo/admin passwords. Do not enable `NOPASSWD: ALL`; if a non-root future runner ever needs sudo, use only a narrow one-time `visudo`-validated rule for the exact installed-QA scheduler/env commands.

Scheduler closure requires server SSH access, a live run, and backend audit row showing `scheduler=systemd_timer`, `operator_id=installed_product_qa_operator`, `device_lab_provider=firebase_test_lab`, `proofSource=firebase_test_lab_uploaded_artifact`, `costEstimateUsd <= 0.25`, `fakeProof=false`, `moneyMoved=false`, `userRightsChanged=false`, `highRiskExecuted=false`, and `secretsLogged=false`. Until then, scheduler remains `firebase_scheduler_service_completion_blocked`: the device-lab path and dedicated bucket exist, but the systemd Firebase smoke service must exit cleanly after posting timer audit. The latest server-access blocker is `ssh_banner_timeout`: ICMP and TCP/22 to `87.99.145.160` succeed, but SSH never receives a banner; Hetzner reports the host running, provider VNC console is available, and recent read-only metrics show CPU saturation plus high disk read I/O. Do not reboot, reset, or restart sshd without owner approval.
