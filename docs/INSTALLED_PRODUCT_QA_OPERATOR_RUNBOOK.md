...
Do not enable the timer until the systemd service can complete cleanly. Current blocker: `gcloud firebase test android run` can upload to the dedicated bucket and manual smoke/posting works, but the proof service waited indefinitely during result collection and was stopped; the timer is installed but disabled. Billing enablement/linking remains owner-only. Do not store sudo/admin passwords. Do not enable `NOPASSWD: ALL`; if a non-root future runner ever needs sudo, use only a narrow one-time `visudo`-validated rule for the exact installed-QA scheduler/env commands.
