...
Minimal server tooling is installed: Ubuntu package `nodejs` `18.19.1`, `npm` `9.2.0`, Google Cloud SDK `575.0.1` from Google's signed apt repository, and pinned `firebase-tools@15.23.0` through npm. Snap-installed Node/gcloud remain present but are not used by the hardened service because snap confinement is incompatible with the empty `CapabilityBoundingSet`. On 2026-07-13 the owner-authorized Google Console session created dedicated service account `installed-qa-testlab-runner@chillywood-app.iam.gserviceaccount.com` with Test Lab-scoped project roles `Firebase Test Lab Admin`, `Firebase Analytics Viewer`, and upload-only `Storage Object Creator`. Its JSON key is stored only on `chillywood-prod-01` at `/var/lib/chillywood/firebase-test-lab/service-account.json` as `root:root` mode `600`; the owner personal Google session is not the runtime credential. The server gcloud config under `/var/lib/chillywood/firebase-test-lab/gcloud-config` activates that service account and can read the Firebase Test Lab virtual model/version catalog. A current-source release APK for commit `4061e2bda8c77dad174b72378db483acb6e906fe` is stored at `/var/lib/chillywood/firebase-test-lab/artifacts/chillywood-4061e2bda8c7-release.apk` with SHA-256 `6bcddf44e9d320d152742537339f3204a7295cd7868554b34146f82c81528b44`. `npm run installed-qa:firebase-test-plan` on the server now passes with `costEstimateUsd=0.09`, `billingRisk=low`, virtual-only device type, no broad crawl, no physical device, no two-device claim, and `resultsBucket=chillywood-installed-qa-testlab-results`.

Server Firebase bucket/bootstrap and bounded scheduler closure are complete for the uploaded-artifact Tier 1 lane. Google Cloud billing readback is active for project `chillywood-app`. The exact owned results-bucket bootstrap is now a standing bounded Level 2 policy for `installed_product_qa_operator`, limited to `gs://chillywood-installed-qa-testlab-results` in project `chillywood-app`, seven-day lifecycle cleanup, public access prevention, uniform bucket-level access, and bucket-level IAM for `installed-qa-testlab-runner@chillywood-app.iam.gserviceaccount.com`. The bucket exists with lifecycle delete after seven days and bucket-level `roles/storage.objectCreator` for the dedicated runner. It does not allow billing enablement, arbitrary bucket creation, Owner/Editor, project-wide Storage Admin, or arbitrary IAM mutation. Manual server Firebase smoke completed as matrix `8384645686869695525`, used `source=firebase_test_lab_uploaded_artifact`, `results_bucket=chillywood-installed-qa-testlab-results`, `cost_estimate_usd=0.09`, and posted live traversal row `333817c9-b651-4e65-ab66-0d885952d2de` after the Firebase source-classification migrations and Edge Function redeploy. The bounded systemd runner then started matrix `4625992954862995833`, posted live `scheduler=systemd_timer` rows `ff81956d-94e3-49e9-8c80-fae2c12b0dd8` (`matrix_started`), `1dc00369-b5ca-4289-92bc-daf5bae00222` (`matrix_timeout` with `POLL_HTTP_FAILED`), and `282fb154-101c-402b-9539-d3fb8080de51` (`matrix_pending` without starting a duplicate). The service exited cleanly, the daily timer is enabled/active, and `schedulerStatus=chillywood-installed-qa-firebase-smoke.timer_daily_cost_capped`. The current matrix state is pending/poll-failed, not a completed device result.

Approval request `c3e89c38-8ede-4598-86c2-967de0c17d64` was marked `superseded`, not executed, because its old allowed write scope did not authorize bucket creation or bucket IAM. The standing bounded Level 2 policy now replaces that per-request approval for only the exact Firebase Test Lab results bucket bootstrap.
...
The daily Firebase uploaded-artifact smoke scheduler is active on `chillywood-prod-01`. It is not Play-installed proof and does not close Premium Billing, push/camera/mic, or two-device LiveKit; those still require physical Play-installed devices or an explicitly approved coordinated lab.
...
The dedicated results-bucket bootstrap is pre-approved only as the exact bounded action `firebase_test_lab_results_bucket_bootstrap`: create/manage `gs://chillywood-installed-qa-testlab-results` in `chillywood-app`, configure uniform bucket-level access, public access prevention, seven-day lifecycle deletion, bucket-level `roles/storage.objectCreator` for the dedicated runner service account, and `roles/storage.objectViewer` only if result readback requires it. Billing enablement/linking remains owner-only and outside this Level 2 policy. Arbitrary buckets, broad IAM, project-wide Storage Admin, Owner/Editor grants, physical devices, broad crawls, over-cap runs, Play-installed claims, Premium/Billing claims, and two-device LiveKit claims remain forbidden.
...
Local CLI access exists on the owner Mac for manual runs and admin provisioning: `node`, `npm`, `gcloud`, `firebase`, and `npx` are present and the configured gcloud project is `chillywood-app`. That does not make the Mac the scheduler or runtime credential. The long-term scheduled path runs from `chillywood-prod-01` with equivalent server CLI tooling, a dedicated service-account runtime credential, a current-source server APK, the dedicated results bucket, and the daily cost-capped systemd timer.
...
The service reads only `/etc/chillywood/installed-product-qa-operator.env`, requires `CHILLYWOOD_REPO_DIR`, calls `installed-qa:firebase-test-plan` before bounded `installed-qa:firebase-test-run`, posts to `installed_product_qa_operator`, and then runs `installed-qa-operator:report`. It sets `FIREBASE_TEST_LAB_MODE=cost_capped`, `FIREBASE_TEST_LAB_MONTHLY_CAP_USD=5`, `FIREBASE_TEST_LAB_PER_RUN_CAP_USD=0.25`, `FIREBASE_TEST_LAB_ALLOW_VIRTUAL=true`, `FIREBASE_TEST_LAB_ALLOW_PHYSICAL=false`, `FIREBASE_TEST_LAB_MAX_SCHEDULED_RUNS_PER_DAY=1`, `FIREBASE_TEST_LAB_ALLOW_BROAD_CRAWL=false`, `FIREBASE_TEST_LAB_ALLOW_TWO_DEVICE=false`, `FIREBASE_TEST_LAB_RUN_REASON=daily_scheduled`, `FIREBASE_TEST_LAB_REPORT_TO_OPERATOR=true`, `FIREBASE_TEST_LAB_RESULTS_BUCKET=chillywood-installed-qa-testlab-results`, `FIREBASE_TEST_LAB_MAX_WAIT_SECONDS=900`, `FIREBASE_TEST_LAB_POLL_INTERVAL_SECONDS=30`, `FIREBASE_TEST_LAB_ALLOW_PENDING_MATRIX=true`, and `FIREBASE_TEST_LAB_MAX_ACTIVE_MATRICES=1`. It uses `/var/lib/chillywood/installed-qa/firebase-budget-ledger.jsonl` plus `/var/lib/chillywood/installed-qa/firebase-pending-matrix.json` for persistent budget and pending-matrix state.

The timer is daily only with ten-minute randomized delay. No every-30-minute Firebase device schedule is allowed. The service is hardened with `NoNewPrivileges=true`, `ProtectSystem=strict`, `ProtectHome=true`, `PrivateTmp=true`, `RestrictSUIDSGID=true`, `LockPersonality=true`, `TimeoutStartSec=20min`, `RuntimeMaxSec=20min`, `KillMode=control-group`, `Restart=no`, and an empty `CapabilityBoundingSet`. For the current `Type=oneshot` unit, `TimeoutStartSec=20min` is the effective bound on the start command; `RuntimeMaxSec=20min` remains present in the template. It does not use a service-role key and must not print token values.

The runner uses bounded matrix lifecycle behavior: start asynchronously with `--async`, capture the matrix id, poll Test Lab with a max wait, record `matrix_pending`/`matrix_timeout` if the matrix is still running or poll readback fails, and avoid a duplicate start while a pending matrix exists. Billing enablement/linking remains owner-only. Do not store sudo/admin passwords. Do not enable `NOPASSWD: ALL`; if a non-root future runner ever needs sudo, use only a narrow one-time `visudo`-validated rule for the exact installed-QA scheduler/env commands.

Scheduler closure proof on 2026-07-13 restored SSH without VNC or reboot, synced `/opt/chillywood/current` to `origin/main`, installed the bounded service/timer templates, and ran the service manually. Backend audit rows show `scheduler=systemd_timer`, `operator_id=installed_product_qa_operator`, `device_lab_provider=firebase_test_lab`, `proofSource=firebase_test_lab_uploaded_artifact`, `costEstimateUsd=0.09`, `fakeProof=false`, `moneyMoved=false`, `userRightsChanged=false`, `highRiskExecuted=false`, and `secretsLogged=false`. The current pending matrix `4625992954862995833` has `POLL_HTTP_FAILED` after bounded polling; this is a tracked matrix state, not a fake success. The daily timer is enabled and active on `chillywood-prod-01`.
