...
- Protected Premium HD is live only for audited protected rows: `5` HD rows (`4` 720p, `1` 1080p) are stored under `playback/protected/premium/`, served through `premium-media-proof.chillywoodstream.com` with valid Premium tokens, and denied to free/missing/wrong tokens. HD must not be copied to `playback/public/`, and installed app Premium HD proof is still a separate future milestone.
