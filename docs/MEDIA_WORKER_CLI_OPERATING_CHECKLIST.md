...
Object-storage migration status: Hetzner Object Storage -> R2 private origin is Partial. `chillywood-media-origin` exists as the private R2 origin target and trusted copier `media-object-storage-migration` is deployed, but production still has `31` Hetzner/S3 object-storage reference rows and `22` distinct object refs. The missing backend R2 private-origin write config blocker is cleared, but copy is blocked before DB metadata mutation by `source_object_head_failed_404` on the first selected legacy source row. No media rows were written, no DB metadata was migrated, no Hetzner objects were deleted, and Hetzner fallback retained. Do not shut down Hetzner Object Storage until the zero-reference audit closes. Do not shut down Hetzner LiveKit.
