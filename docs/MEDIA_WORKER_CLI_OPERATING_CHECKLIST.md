...
Object-storage migration status: Hetzner Object Storage -> R2 private origin is shutdown-ready by active-reference semantics. `chillywood-media-origin` exists as the private R2 origin target, trusted copier `media-object-storage-migration` is deployed, `16` existing distinct refs were copied/readback verified, and DB batch `media-object-storage-r2-2026-07-10T21-27-45-643Z` migrated `24` copied+verified row refs after storage-metadata backup/restore passed. New creator-media uploads now target R2 private origin with Hetzner writes disabled and read fallback retained. Final audit reports `7` raw `media_scan_jobs` history refs, all resolved stale with `activeUnresolvedHetznerObjectRefs=0`; Object Storage shutdown is owner-controlled after fallback retention/export. Do not shut down Hetzner LiveKit.
...
- Protected Premium HD is live only for audited protected rows: `5` HD rows (`4` 720p, `1` 1080p) are stored under `playback/protected/premium/`, served through production host `premium-media.chillywoodstream.com` with valid Premium tokens, and denied to free/missing/wrong tokens. The old `premium-media-proof.chillywoodstream.com` host is historical only and is not an active fallback route. HD must not be copied to `playback/public/`.
