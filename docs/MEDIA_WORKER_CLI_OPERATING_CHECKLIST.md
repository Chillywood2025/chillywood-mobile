...
- Latest manual private R2 backup prefix: `backups/media-worker/2026/07/10/media-worker-logical-20260710T003002-02eab4b4c5cd/`.
- Current production media-worker rows: `media_transcode_jobs=6` and `media_renditions=10`, with `active_unfinished_jobs=0` and `unsafe_cdn_rows=0`. City Lights plus four auto-processed public-safe sources have audited HLS rows; one low-resolution source has only a failed unsupported job marker.
