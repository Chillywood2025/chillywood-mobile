...
- Stripe Connect Edge Function Skeletons are not entitlement, payout activation, or creator setup UI. They are source-only/not-deployed backend skeletons that return `not_configured`; they add no Connect Stripe button, onboarding link, provider write, transfer, payout, checkout session, or live money movement.
