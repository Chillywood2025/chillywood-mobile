...
| Premium monthly purchase | Tester-safe only where Play internal/licensed tester/provider setup supports it | Yes, status/restore/tester flow | No live_money switch, but owner proof required | Owner-approved purchase proof pending | Separate owner-approved proof lane | Active monthly tester/status flow | Premium monthly provider path verified separately | Visible CTA opens purchase/status path; `premiumPurchaseEnabled=false` remains | Partial |
| Premium annual | Status/resolution only | Yes, provider-blocked status | No | Yes | Future annual base plan and mapping | Active provider-blocked status | Google Play base-plan blocker documented | Visible annual control opens resolution/status flow | Blocked |
| Creator tips | Tester/status only | Yes, sandbox/status flow where backed | Yes | Product readiness future | Creator-money activation lane | Active tip status/tester flow | Creator-money OFF proof referenced | Visible controls open tester/status flow; no settlement | Closed for off-state |
| Paid creator video | Tester/status only | Yes, sandbox/status flow where backed | Yes | Provider/product readiness future | Creator-money/live-money gates | Active paid-video status/tester flow | Content takedown/access guard referenced | Visible unlock action opens tester/status flow while money off | Closed for off-state |
| Paid Watch-Party Seat Pass | Tester/status only | Yes, sandbox/status flow where backed | Yes | Provider/product readiness future | Creator-money/live-money gates | Active Seat Pass status/tester flow | Live/Watch-Party guard referenced | Visible Seat Pass setup/purchase opens tester/status flow while money off | Closed for off-state |
| Channel Subscription | Status/resolution only | Yes, provider-blocked status | Yes | Yes | Future base plan and mapping | Active provider-blocked status | Provider-blocked docs referenced | Visible subscription controls open status/resolution flow | Blocked |
| VIP | Tester/status only | Yes, sandbox/status flow where backed | Yes | Provider/product readiness future | Creator-money/live-money gates | Active VIP status/tester flow | Creator-money OFF docs referenced | Visible controls open tester/status flow while money off | Closed for off-state |
| Paid event | Tester/status only | Yes, sandbox/status flow where backed | Yes | Provider/product readiness future | Creator-money/live-money gates | Active paid-event status/tester flow | Event/report docs referenced | Visible controls open tester/status flow while money off | Closed for off-state |
| Payouts | Readiness/status only | Yes, readiness/status flow | Yes | Future payout readiness | Owner/First Owner future lane | Active payout readiness/status | Money governance proof referenced | Visible controls open readiness/status flow; payout movement off | Blocked |
| Stripe Connect | Readiness/status only | Yes, readiness/status flow | Yes | Future Stripe lane | Owner/First Owner future lane | Active provider readiness/status | Money governance proof referenced | Visible controls open readiness/status flow; production Connect off | Blocked |
| Merch checkout | Readiness/status only | Yes, readiness/status flow | Yes | Future Stripe/merch lane | Owner/First Owner future lane | Active merch readiness/status | Money governance proof referenced | Visible controls open readiness/status flow; checkout off | Blocked |
...
User-facing monetization proof copy was replaced with public-safe active status copy. Paid creator video and Watch-Party Seat Pass buttons now open tester-safe purchase/status/readiness flows while live money and paid checkout runtime switches stay OFF unless a separate approved lane enables them. Provider product identifiers are no longer shown in that public monetization status card.
