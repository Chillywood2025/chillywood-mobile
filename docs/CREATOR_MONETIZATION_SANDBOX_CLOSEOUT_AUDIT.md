...
| `watch_party_tickets_enabled` | `sandbox_only` | Watch-Party Seat Pass proof only. |
| `watch_party_seats_enabled` | `sandbox_only` | Watch-Party seat/Seat Pass proof only. |
...
| Paid Watch-Party Seats V1 | RevenueCat / Google Play sandbox product `cw_watch_party_live_ticket_sandbox_099` | Sandbox-proven for Seat Pass purchase, waiting-room gate, Party Room direct-link gate, paid entry, second unpaid denial, seat-limit, RPC Money Center readback, and June 20 exact-target purchase/readback closeout | Active room Seat Pass only for the purchased room | Readback passed as sandbox/not-payable | Provider/test-account maintenance only after June 20 closeout | Sandbox only; not live | `docs/PAID_WATCH_PARTY_SEATS_V1_END_TO_END_PROOF.md` |
...
- Paid Watch-Party Seat Passes unlock only the linked Party Waiting Room and Party Room.
...
Premium remains a Chi'llywood platform subscription. It does not automatically unlock creator paid videos, room Seat Passs, paid events, channel subscriptions, or VIP passes.

VIP and Channel Subscription status do not unlock each other and do not unlock Paid Videos, Paid Watch-Party Seat Passes, or Paid Events.
