...
| Static public legal/support site | `public-site/legal-site/`; build with `npm run legal-site:build`; output `public-site/legal-site/site/`; Cloudflare Pages project `chillywood-legal` | Settings keeps bundled fallbacks and may prefer configured public URLs | Public web surface only | Canonical public policy text now comes from `legal/policies.mjs`. The May 21 build generates 17 static pages: the 12 full legal/policy pages plus compatibility aliases. Cloudflare Pages redeployed the expanded site on May 21, 2026; custom domain `chillywoodstream.com` is active. Sampled apex proof returned HTTP 200 after redirect for `/privacy/`, `/creator-monetization/`, and `/live-rules/`; earlier core proof remains valid for `/terms/`, `/account-deletion/`, `/copyright/`, and `/support/`. Admin Canary legal readiness passed with `33 pass / 0 manual / 0 fail`. | Use the live URLs for store entries only after attorney approval; Play Console Data Safety/account deletion acceptance remains pending |
...
- `public-site/legal-site/` now builds 17 static pages from canonical policy truth: the 12 full legal/policy pages plus compatibility aliases. Cloudflare Pages redeployed the expanded site on May 21, 2026, and Admin Canary legal readiness passed with `33 pass / 0 manual / 0 fail`.
...
- Public legal/support URLs are live on `https://chillywoodstream.com/*`; the May 21 expanded policy deployment proved sampled current routes `/privacy/`, `/creator-monetization/`, and `/live-rules/` returned HTTP 200 after redirect, while earlier core proof remains valid for `/terms/`, `/account-deletion/`, `/copyright/`, and `/support/`. No LiveKit fallback was reached, and Privacy mentions LiveKit only as policy text.
...
| Expanded static legal site | `public-site/legal-site/site/`, `https://chillywood-legal.pages.dev`, and `https://chillywoodstream.com` | May 21 Cloudflare Pages deployment generated 17 pages from `legal/policies.mjs`; apex sampled proof passed for `/privacy/`, `/creator-monetization/`, and `/live-rules/`, and earlier core proof remains valid for `/terms/`, `/account-deletion/`, `/copyright/`, and `/support/` | Apex Legal URLs Live / Legal Canary Passed / Store Acceptance Pending | Use `https://chillywoodstream.com/account-deletion` as the live Google Play account deletion URL candidate and enter/verify required legal/support URLs in Play Console after attorney approval. |
...
- `public-site/legal-site/` generates 17 static public pages for the complete requested URL surface; the May 21 Cloudflare Pages deployment is live on `chillywoodstream.com`, and Admin Canary legal readiness passed with `33 pass / 0 manual / 0 fail`.
