...
The launch classification remains one P0 external blocker for Play/Data Safety/account-deletion/legal acceptance. Support/DKIM remain P1 operational blockers before broad public launch. After the May 30 follow-up, malware scanning is repo-side implemented, production-deployed, runtime-proved, and Admin-reviewable.

## May 30, 2026 Google Play Execution Package

The owner/operator Play Console execution package is now repo-side complete:
...
Public legal/support URL proof returned HTTP 200 after redirects for Privacy, Terms, Account Deletion, Copyright, Copyright Report, Support, Moderation Policy, Community Guidelines, and Creator Rules. Android proof on `R5CR120QCBF` captured Settings Legal and Support, Privacy, Terms, Account Deletion, Copyright Report, and Moderation Policy. The direct Support deep link did not resolve during this proof, so the May 29 release screenshot set remains the current visual Support route reference until a later route smoke recaptures it.

This closes the repo-side Play Console evidence packaging. It does not complete Play Console Data Safety, account deletion URL acceptance, content rating, store listing submission, app access reviewer setup, legal/attorney approval, DKIM verification, account deletion fulfillment, or support staffing. The P0 remains external.

May 30 scanner update: production malware scanning is now implemented, runtime-proved, deployed on `chillywood-prod-01`, and visible through sanitized Admin review. Production proof lives at `/tmp/chillywood-malware-scanner-production-proof-20260530/`.
...
June 25, 2026 account restriction update: Account restriction and appeals operations are closed for current production policy and existing backed enforcement. Reports do not auto-suspend or auto-ban. Suspension/deactivation/restore require exact scope, reason, target, and audit. First Owner cannot be suspended, deactivated, deleted, restored, or restricted by Admin/Moderator. Moderator cannot perform account-wide suspension/restoration by default. Restricted users fail closed for private app features where backed. Premium entitlement may remain provider-side, but app access fails closed for restricted users. Paid-access and payment history are preserved. Provider refunds remain manual/external, payouts and money movement remain disabled, and appeals use support/escalation workflow in V1 without exposing reporter identity or private evidence.
...
| Privacy Policy | `app/privacy.tsx`; runtime env `EXPO_PUBLIC_PRIVACY_POLICY_URL`; public URL `https://chillywoodstream.com/privacy` | Yes; Settings opens configured external URL first, otherwise bundled `/privacy` | Yes | Expanded route exists with account, Profile, Platform, uploads, chat, room, billing, diagnostics, provider, retention, deletion, and safety/legal data categories; `https://chillywoodstream.com/privacy` redirects with HTTP/2 308 to `/privacy/` and then returns HTTP/2 200; legal review still pending | Final Privacy Policy copy must be attorney-approved and entered in Play Console |
| Terms of Service | `app/terms.tsx`; runtime env `EXPO_PUBLIC_TERMS_OF_SERVICE_URL`; public URL `https://chillywoodstream.com/terms` | Yes; Settings opens configured external URL first, otherwise bundled `/terms` | Yes | Expanded route exists with acceptance, Profile/Platform, UGC, creator upload, Watch-Party/Live/Chat, Premium, moderation, liability/indemnification, and public-readable dispute/support language; `https://chillywoodstream.com/terms` redirects with HTTP/2 308 to `/terms/` and then returns HTTP/2 200; legal review still pending | Final Terms copy must be attorney-approved and available without login |
| Account Deletion | `app/settings.tsx`; `_lib/accountDeletionRequests.ts`; migration `20260603014500_self_service_account_deletion_30_day_restore.sql`; public URL candidate `https://chillywoodstream.com/account-deletion` | Yes; Settings > Account actions exposes Delete Account and Read Deletion Policy | Yes; Support points signed-in users to Settings Delete Account and keeps policy/help paths available | Backend-backed Delete Account schedules deletion immediately, hides scheduled accounts from public Profile/People search where backed, signs users out, and supports Restore Account for 30 days. `https://chillywoodstream.com/account-deletion` redirects with HTTP/2 308 to `/account-deletion/` and then returns HTTP/2 200; Play acceptance remains pending | Enter/keep the live URL in Play Console and wait for account deletion acceptance before claiming Google Play compliance |
...
| Creator Rules | `app/creator-rules.tsx` | Yes | Yes | Bundled public route covers creator ownership, upload rights, Platform identity, files/links, live rules, monetization/payout inactive status, sponsorship disclosures, and enforcement; legal/content policy review pending | Public hosted URL is recommended before Play listing submission |
| Copyright / DMCA | `app/copyright.tsx`, `app/copyright-report.tsx`, `app/counter-notice.tsx`, `_lib/dmca.ts`, `app/admin.tsx`, `supabase/migrations/202605140001_dmca_operational_tooling.sql`, `supabase/migrations/202605140005_dmca_restore_eligibility_court_action_guard.sql`, `supabase/migrations/202605140006_dmca_restore_action_court_action_guard.sql`, `supabase/migrations/202605220001_dmca_public_pipeline_closeout.sql`, `supabase/migrations/202605220002_dmca_attachments_uploader_counter_notice.sql`, `supabase/migrations/20260530191115_media_malware_scanning_pipeline.sql`, `supabase/migrations/20260530193203_fix_dmca_scan_enqueue_loop.sql` | Yes | Yes | Expanded bundled route covers takedown notices, required notice information, possible actions, counter-notice workflow, repeat infringer policy, false-notice warning, and designated agent contact for registration `DMCA-1072720`; hosted public notice intake, private evidence attachments, formal notice intake, uploader self-service counter-notice for own cases, Admin case tooling, counter-notice recording/deadlines, court-action restore blocking, strikes, repeat-infringer review, audit history, RLS/private-data denial, safe proof-content cleanup, repo-side malware scan queueing, production scanner proof, and DMCA enqueue-loop protection are backed; attorney review remains pending | Public hosted URL is live; evidence files queue `pending_scan` / `clamav` metadata and the production worker proof shows clean/EICAR outcomes. Outbound email automation if required and attorney review should be completed before broader compliance claims |
| Report Abuse / Safety Contact | `components/safety/report-sheet.tsx`, `components/profile/profile-media-sheets.tsx`, `_lib/moderation.ts`, `app/player/[id].tsx`, `app/admin.tsx`, Support route, report migrations `202605220005`, `202605220006`, Profile media policy migration `202605260002` | Contextual report actions, not one global Settings route | Support can collect help requests | Safety report intake, Admin report queue reads, Mark Reviewed, Dismiss, Escalate, target Hide/Remove/Restore, immutable report audit rows, Profile Photo/Profile Background report actions, and Profile media `flagged` / `admin_removed` / `active` public masking are repo-backed. Generic `copyright` reports route/link to formal DMCA intake; DMCA end-to-end proof passed. | Play listing/support materials should explain report-abuse path, copyright-report limits, support contact, and that support/moderation review is human-operated. Attorney approval, ops SLA, outbound email/DKIM, and optional disposable-fixture visual report lifecycle proof remain; scanner integration is P2/future unless Play/legal review requires it. |

May 30 malware-scanning implementation note: repo-side scan queueing, scan metadata, public-safe gates, a ClamAV worker, production deployment, benign/EICAR runtime proof, fail-closed public gates, and sanitized Admin scanner review are complete. Future scanner work is monitoring/SLO or signed-delivery hardening, not a launch-foundation blocker.
...
- Terms now say users are responsible for what they upload, stream, post, message, share, or otherwise make available, and the page now includes detailed Profile/Platform, creator upload, Watch-Party, Live, Chat, Premium, moderation, suspension, and feature-availability sections.
- Privacy now maps account/Profile/Platform, creator-uploaded media, file picker, camera/microphone, chat/messages, Watch-Party/Live participation, billing/entitlement, support/report/moderation, diagnostics/crash/performance, provider, sharing, retention, deletion, and minor-safety posture.
...
- DMCA designated agent public contact posting and U.S. Copyright Office registration are recorded complete under registration `DMCA-1072720`; hosted public DMCA notice intake, private evidence attachments with scan metadata, formal notice intake, owner/operator Admin case tooling, uploader self-service counter-notices for own cases, Admin-recorded counter-notices/deadlines, court-action restore blocking, strike tracking, repeat-infringer review, audit history, malware scan queueing/production proof, and RLS/private-data denial are backed and live-proof closed. Support inbox receipt proof for `support@chillywoodstream.com` passed by operator-confirmed destination-inbox receipt, and domain email DNS now proves Cloudflare MX, SPF `v=spf1 include:_spf.mx.cloudflare.net ~all`, and DMARC `v=DMARC1; p=none; rua=mailto:support@chillywoodstream.com; adkim=r; aspf=r; fo=1`. Attorney review, outbound email automation, and DKIM after real outbound-provider setup remain pending.
- General report lifecycle and Profile media moderation are now repo-side backed: contextual report creation, Admin queue reads, status changes, target hide/remove/restore, immutable audit rows, Profile Photo/Profile Background report actions, and non-active Profile media public masking are covered by code/migration proof. The remaining report/moderation gaps are external operations and optional safe-fixture visual proof, not missing app-code buttons.
...
- Account Deletion now says some records may be retained for legal, fraud, security, billing, moderation, chargeback, dispute, copyright, or compliance reasons, and that Profile/Platform/upload deletion handling needs final legal/backend approval.
...
Current decision: Public v1 uses backend-backed self-service account deletion scheduling. The app can claim Delete Account schedules deletion with a 30-day restore window and hides scheduled accounts from public Profile/People search where backed. It must not claim automated permanent purge/de-identification has completed until the backend processing job/runbook is built and proved.
...
- `app/settings.tsx` has `Delete Account`, `Restore Account`, and `Read Deletion Policy` inside Account actions.
...
- The account deletion backend now schedules deletion, stores `delete_after` and `restore_deadline`, allows restore during the 30-day window, and hides scheduled accounts from public Profile/People search where backed.
- The account deletion policy names Profile, Platform, uploaded videos, Chi'lly Chat messages, Watch-Party/Live room records, billing/subscription records, and moderation/report records as surfaces that may be reviewed during deletion.
- Permanent Profile, Platform, uploaded video, storage object, thumbnail, comment, message, report, room, and metadata purge/de-identification still needs legal/backend processing approval before claiming completion.

What still needs legal/backend/manual work:

- Define the permanent backend deletion/de-identification job/runbook after `delete_after` for Supabase auth, `user_profiles`, channel/audience rows, `videos`, creator-video storage objects, chat rows, watch-party memberships/rooms, `user_entitlements`, `billing_events`, `safety_reports`, support rows, notifications/reminders, and logs/backups.
...
- Confirm permanent purge timing/SLA and support/legal owner after the 30-day restore window.
...
Security/IP note: `docs/admin/SECURITY_CONTEXT_IP_AUDIT.md` records the current repo decision for network proof. Chi'llywood should treat IP/network request metadata as restricted security, fraud-prevention, abuse-prevention, account-protection, live-cost-protection, audit-integrity, payout/revenue-integrity, and legal-compliance evidence. The current repo avoids app-owned raw IP columns and has no public IP display. Backend support is remote-applied through the restricted `security_request_context` path with hashed/masked context and nullable `security_context_id` links for admin/owner/LiveKit audit events plus restricted DMCA, Reports, media-storage, Live Ops, Live Cost Guard, payout/network/fraud, and staff audit/event tables; migrations through `202605230005` are applied so metadata-carried context ids compare uuid actor truth correctly and signed trusted proxy proof state is tracked. Direct `x-forwarded-for`, `x-real-ip`, `forwarded`, `x-client-ip`, and `cf-connecting-ip` are not trusted. Verified network proof requires signed masked/hash proof from the deployed Cloudflare Worker `chillywood-network-proof-proxy` at `https://network-proof.chillywoodstream.com`, using `CHILLYWOOD_NETWORK_PROOF_SECRET`; app runtime now defaults proof-required Supabase Edge calls, LiveKit token calls, and media-storage function calls through that proxy while Auth/REST/storage remain on Supabase. Owner Security owner-session proof displayed masked `/24` prefix, approximate location/ISP, signed-proxy proof state, and shortened context id without raw IP. Owner Security and Audit Explorer may show masked network proof only through approved backend summaries for authorized owner/admin contexts; public UI must not show raw IP, and final retention durations remain a legal/product decision.

| Data category | Repo evidence | Collected? | Purpose | Shared with third parties/services? | Optional or required | Deletion scheduling and retention coverage | Play Console note |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Account info: email, user id, sign-in identity | Supabase auth, `useSession`, Settings signed-in identity, RevenueCat identity sync | Yes | Account creation, login, access control, support, entitlement/account linking | Supabase; RevenueCat when billing is configured; Firebase identity for diagnostics/analytics helpers if enabled | Required for signed-in features | Covered by Delete Account scheduling and permanent deletion/de-identification subject to retained records | Enter account info / identifiers as collected. Confirm exact Play labels manually. |
| Profile and Platform info | `user_profiles`, `app/profile/[userId].tsx`, `app/channel/[userId].tsx`, `app/channel-studio/index.tsx`, `app/channel-settings.tsx` compatibility | Yes | Public identity, public Platform surface, owner Platform Studio display/defaults | Supabase; visible to other users according to profile/platform visibility | Optional beyond basic account, but required for public Profile/Platform use | Scheduled accounts are hidden from public Profile/People search where backed; permanent deletion/de-identification remains subject to retention rules | Enter user-provided profile/content data where applicable. |
| Creator-uploaded videos/media | `expo-document-picker`, `_lib/creatorVideos.ts`, `videos`, `creator-videos` storage | Yes when user uploads | Creator Platform media, playback, Player, Watch-Party source | Supabase storage/database; visible to other users when public | Optional | Covered by deletion/unpublish/de-identification request, with legal/safety retention exceptions | Enter photos/videos or files/media as user-provided content if Play form asks. |
| Photos/videos selected through picker | `expo-document-picker` for upload; no broad camera-roll import found | Yes when user chooses a file | Upload creator video to Platform Studio or attach media to Profile posts where supported | Supabase storage after upload; local picker selection not globally scanned by app evidence | Optional | Uploaded media covered by deletion; local device files are not deleted by app | Do not imply broad photo library collection beyond selected upload files. |
...
- Creator Rules route exists and covers creator ownership, rights clearance, Platform identity, upload/live rules, inactive payouts, sponsor disclosure, and enforcement.
...
- Profile Photo and Profile Background reports exist through Profile Actions when active media is present.
- Admin/operator moderation surface exists in `app/admin.tsx`.
- Safety reports use `safety_reports`; report status/action RPCs and immutable audit rows are backed.
- Creator video/profile post/comment/social attachment target actions use `moderation_status`; Profile media target actions use `profile_avatar_media_status` / `profile_background_media_status`.
- Hidden/removed creator videos and non-active Profile media are intended not to appear publicly or play publicly/render publicly through the public-safe read paths.

Still proof-pending:

- Optional disposable-fixture visual proof that a normal user submits a general `safety_reports` row and an admin completes status/action review without touching real user content.
- Non-admin Android panel denial for the newest Admin Reports/Legal presentation remains a fixture/session task; server-side role/RLS denial is already the governing security proof.
...
| Copyright / DMCA | `https://chillywoodstream.com/copyright` and `https://chillywoodstream.com/copyright-report` | HTTP/2 308 to trailing-slash routes, then HTTP/2 200 | Live URLs / Public DMCA Form Live / DMCA Agent Registered / DMCA Tooling Live-Proved / Support Inbox Proved / Scanner Proved / Attorney Review Pending | Copyright page lists designated agent contact and registration `DMCA-1072720`; hosted public notice intake, private evidence attachments with scan queueing and production worker proof, formal notice intake, uploader self-service counter-notices for own cases, Admin case tooling, Admin-recorded counter-notices, strike tracking, repeat-infringer review, audit history, supported content hide/restore, and court-action restore blocking are backed and live-proof closed. Support inbox receipt proof for `support@chillywoodstream.com` passed. Attorney review and outbound email automation remain pending. |
...
- DMCA designated agent registration/contact maintenance; registration `DMCA-1072720` is recorded complete, backed/Admin/public DMCA tooling is live-proof closed, private evidence attachment/uploader counter-notice proof passed, scanner production proof passed, support inbox receipt proof passed, and domain MX/SPF/DMARC baseline is proved; attorney review, outbound email automation, and DKIM after real outbound-provider setup remain pending.
