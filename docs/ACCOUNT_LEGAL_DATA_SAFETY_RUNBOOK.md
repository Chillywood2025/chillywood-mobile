...
- repo-side malware scanning is implemented; production worker deployment/runtime proof is still pending and must not be claimed until captured;
...
The launch classification remains one P0 external blocker for Play/Data Safety/account-deletion/legal acceptance. Support/DKIM remain P1 operational blockers before broad public launch; malware scanning is repo-side implemented and remains P1 for production worker deployment/runtime proof.
...
May 30 malware-scanning implementation note: repo-side scan queueing, scan metadata, public-safe gates, and a ClamAV worker are implemented. Production worker deployment/runtime proof is still pending and is the next scanner lane before broad public UGC launch claims.
...
- DMCA designated agent registration/contact maintenance; registration `DMCA-1072720` is recorded complete, backed/Admin/public DMCA tooling is live-proof closed, private evidence attachment/uploader counter-notice proof passed, support inbox receipt proof passed, and domain MX/SPF/DMARC baseline is proved; attorney review, outbound email automation, and DKIM after real outbound-provider setup remain pending. Scanner integration is P2/future unless Play/legal review requires it.
