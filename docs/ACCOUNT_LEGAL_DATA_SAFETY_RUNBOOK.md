...
| Support | `app/support.tsx`, `components/system/support-screen.tsx`; runtime env `EXPO_PUBLIC_SUPPORT_EMAIL`; fallback `support@chillywoodstream.com` | Settings links to support/account help surfaces indirectly through legal/account controls | Yes | Support route exists with support categories, account/billing/upload/safety/copyright/deletion guidance, emergency disclaimer, signed-in feedback handoff, and pending SLA language; public support email is `support@chillywoodstream.com`, Cloudflare routing is configured, and inbox receipt proof passed by operator-confirmed destination-inbox receipt for a real test message | Public support URL and inbox receipt are proved; support ownership/SLA and legal approval must still be confirmed |
| Static public legal/support site | `public-site/legal-site/`; build with `npm run legal-site:build`; output `public-site/legal-site/site/`; Cloudflare Pages project `chillywood-legal` | Not rewired in app by this docs-only update | Public web surface only | Generates `/terms`, `/privacy`, `/creator-rules`, `/community-guidelines`, `/copyright`, `/account-deletion`, `/refunds`, `/sponsor-disclosure`, `/banned-content`, `/moderation`, and `/support` from repo policy truth and confirmed support contact. `https://chillywood-legal.pages.dev/*` returned HTTP 200 for all required paths. Cloudflare Pages custom domain `chillywoodstream.com` is active; `https://chillywoodstream.com/` returns HTTP/2 200, and `/terms`, `/privacy`, `/account-deletion`, `/copyright`, and `/support` return HTTP/2 200 after trailing-slash redirect with real static content. No LiveKit fallback was reached; Privacy mentions LiveKit only as policy text. | Use the live URLs for store entries only after legal approval; Play Console Data Safety/account deletion acceptance remains pending |
...
| Copyright / DMCA | `app/copyright.tsx`, `app/copyright-report.tsx`, `_lib/dmca.ts`, `app/admin.tsx`, `supabase/migrations/202605140001_dmca_operational_tooling.sql`, `supabase/migrations/202605140005_dmca_restore_eligibility_court_action_guard.sql`, `supabase/migrations/202605140006_dmca_restore_action_court_action_guard.sql` | Yes | Yes | Expanded bundled route covers takedown notices, required notice information, possible actions, counter-notice workflow, repeat infringer policy, false-notice warning, and designated agent contact for registration `DMCA-1072720`; formal notice intake, Admin case tooling, counter-notice recording/deadlines, court-action restore blocking, strikes, repeat-infringer review, audit history, RLS/private-data denial, and safe proof-content cleanup are backed and live-proof closed; attorney review remains pending | Public hosted URL is live; uploader-facing counter-notice route if required, outbound email automation if required, and attorney review should be completed before broader compliance claims |
...
- DMCA designated agent public contact posting and U.S. Copyright Office registration are recorded complete under registration `DMCA-1072720`; formal notice intake, owner/operator Admin case tooling, Admin-recorded counter-notices/deadlines, court-action restore blocking, strike tracking, repeat-infringer review, audit history, and RLS/private-data denial are backed and live-proof closed. Support inbox receipt proof for `support@chillywoodstream.com` passed by operator-confirmed destination-inbox receipt. Attorney review, uploader-facing counter-notice submission if required, and outbound email automation remain pending.
...
- Public legal/support URLs are live on `https://chillywoodstream.com/*`; Terms, Privacy, Account Deletion, Copyright, and Support returned HTTP/2 200 after trailing-slash redirect. No LiveKit fallback was reached, and Privacy mentions LiveKit only as policy text.
...
- Live end-to-end backed/Admin DMCA proof passed with safe reporter/uploader/admin/viewer accounts and supported profile-post/comment/creator-video content; uploader-facing counter-notice submission and outbound email automation remain pending unless handled manually through Support/Admin.
...
| Copyright / DMCA | `https://chillywoodstream.com/copyright` | HTTP/2 308 to `/copyright/`, then HTTP/2 200 | Live URL / DMCA Agent Registered / DMCA Tooling Live-Proved / Support Inbox Proved / Attorney Review Pending | Copyright page lists designated agent contact and registration `DMCA-1072720`; formal notice intake, Admin case tooling, Admin-recorded counter-notices, strike tracking, repeat-infringer review, audit history, supported content hide/restore, and court-action restore blocking are backed and live-proof closed. Support inbox receipt proof for `support@chillywoodstream.com` passed. Attorney review, direct uploader-facing counter-notice submission, and outbound email automation remain pending. |
| Support | `https://chillywoodstream.com/support` and `support@chillywoodstream.com` | HTTP/2 308 to `/support/`, then HTTP/2 200; Cloudflare destination/routing is configured; operator-confirmed destination-inbox receipt passed | Live URL / Inbox Receipt Proof Passed | Real test message subject `Chi'llywood Support Receipt Proof - 2026-05-13` was sent to `support@chillywoodstream.com` and confirmed received. No screenshots, raw headers, inbox exports, private contents, sender private details, credentials, tokens, or email secrets are committed. Public MX and SPF are present; no public DMARC TXT record was returned, so DMARC/DKIM hardening remains an email-deliverability follow-up. |
| Expanded static legal site | `public-site/legal-site/site/`, `https://chillywood-legal.pages.dev`, and `https://chillywoodstream.com` | Cloudflare Pages hostname proved for all required public routes; apex custom domain is active and key legal/support paths return HTTP/2 200 after trailing-slash redirect | Apex Legal URLs Live / Store Acceptance Pending | Use `https://chillywoodstream.com/account-deletion` as the live Google Play account deletion URL candidate and enter/verify required legal/support URLs in Play Console after attorney approval. |
...
- `public-site/legal-site/` generates static public pages for the complete requested URL surface; Cloudflare Pages deployment is live on `chillywoodstream.com`.
...
- Draft UGC responsibility, platform moderation rights, user-content license, DMCA/copyright, repeat-infringer, limitation/disclaimer, deletion-retention, refund/subscription, payout inactive, fraud/forfeiture, sponsor disclosure, banned content, and moderation workflow language is documented in app or `docs/legal/` and marked pending attorney/legal approval where repo-only.
...
- DMCA designated agent registration/contact maintenance; registration `DMCA-1072720` is recorded complete, backed/Admin DMCA tooling is live-proof closed, and support inbox receipt proof passed; attorney review, uploader-facing counter-notice route, and outbound email automation remain pending.
- Final account deletion support process/SLA and support ownership approval; public support URL and verified `support@chillywoodstream.com` inbox receipt are proved.
- Final backend deletion/de-identification and retention runbook.
