...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Premium | `premium_subscription` | `premium_subscription` | Subscription | Monthly active; annual blocked | `monthly`, monthly auto-renewing, United States; annual `annual` save rejected by Google Play | Yes for monthly; annual blocked | Resolve annual base-plan save blocker, then keep Premium purchase switch OFF until an approved Premium-only activation lane. |
...
| Watch-Party Seat Pass | `cw_watch_party_live_ticket_sandbox_099` | `cw_watch_party_live_ticket_sandbox_099` | One-time product, `Buy` purchase option | Active | Not applicable | Yes | Dashboard product is sandbox-labeled and says not real money; owner must approve using it or create a production-labeled product before activation. |
...
| --- | --- | --- | --- | --- | --- | --- |
| Premium | `premium_subscription:monthly`; annual target `premium_subscription:annual` | Published Play Store product `premium_subscription:monthly`; annual missing because Google Play annual base plan did not save | `premium` entitlement associated only to the Premium monthly product | `premium` offering with `$rc_monthly`; no `$rc_annual` package visible | Monthly yes; annual blocked | Resolve Google Play annual base plan, import/map annual to `premium`, keep monthly intact, then run final Premium smoke. |
...
| Watch-Party Seat Pass | `cw_watch_party_live_ticket_sandbox_099` | Published Play Store product | Not applicable; dashboard shows no associated entitlements | Not applicable for direct one-time product flow | Yes | Sandbox-labeled product; owner approval or replacement production product required before activation. |
...
| Watch-Party Seat Pass | `cw_watch_party_live_ticket_sandbox_099` | Google Play and RevenueCat match | Exact `watch_party_live_ticket` for one room/target | Does not map to Premium. |
...
| Watch-Party Seat Pass | Sandbox setup / production off | `watch_party_tickets_enabled: "sandbox_only"` | Verified locally |
...
- Premium monthly base plan/subscription ready: Yes, base plan `monthly` is visible and active at `$9.99/month`.
- Premium annual base plan/subscription ready: No. Annual target `annual` at `$99.99/year` is blocked because Google Play returned `Your changes couldn't be saved` after approved values were entered.
...
- Offering/package ready: Monthly yes, offering `premium` contains package `$rc_monthly` for the Premium monthly product. Annual no, because no `premium_subscription:annual` / `$rc_annual` mapping exists.
- App config match: Yes.
- Premium can remain OFF until owner activation: Yes, `premiumPurchaseEnabled=false`.
- Owner action remaining before turning Premium on: resolve the Google Play annual base-plan save blocker or approve a different annual base-plan path, import/map annual in RevenueCat, then approve a Premium-only activation lane, final-check pricing/availability/support ownership, run Play-installed smoke, and keep rollback ready.
...
| Watch-Party Seat Pass | Yes, configured sandbox-labeled product | One-time product | Yes, product imported; no entitlement | Yes, exact room/target | Yes | Yes | Owner must approve using sandbox-labeled product or create production-labeled product | Exact room Seat Pass, other-room denial, no LiveKit authority, revoke/readback |
...
| Watch-Party Seat Pass | Decide whether to use the configured sandbox-labeled one-time product or create a production-labeled product; confirm pricing/type; approve activation; run controlled same-room proof later. |
