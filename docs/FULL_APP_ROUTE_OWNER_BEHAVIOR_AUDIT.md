...
- Channel is the creator's mini streaming platform on `/channel/[userId]`.
...
- Creator uploaded videos now belong in public Channel media sections and the Platform Studio Content panel, not the personal Posts lane.
...
| Route | Owner file | System | What it owns | Must not own | Access | Backend dependencies | Enters | Exits | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Profile `/profile/[userId]` | `app/profile/[userId].tsx` | Profile social identity | Public identity, personal/social profile, owner/public profile actions, profile posts, Chi'lly Chat handoff, View Channel handoff | Public Channel implementation, Platform Studio management, Player playback, admin moderation writes, platform title discovery | Signed-in normal app route; public viewer vs owner behavior is user-id based | `user_profiles`, profile posts/social tables, audience summaries, safety reports | Home, Chat, Party Room, Live Stage, public Channel | `/channel/[userId]`, `/channel-studio` for owner, `/player/[id]?source=creator-video`, `/chat`, `/chat/[threadId]` | V1 core |
| Public Channel `/channel/[userId]` | `app/channel/[userId].tsx` | Public Channel | Viewer-facing mini streaming/network route with hero, backed Channel Pulse stats, Featured, Latest Uploads, Live & Upcoming, About, Follow/Share/Report/View Profile, owner-only Open Platform Studio | Owner upload/edit/publish/unpublish/delete, drafts/private/unpublished videos, audience management, insights/analytics, admin controls, Profile privacy controls | Public/viewer-facing route with owner-vs-non-owner UI separation and existing block/unavailable behavior | `user_profiles`, public clean creator videos, creator events, channel audience summary/block truth | Profile View Channel, Platform Studio Preview Channel, deep link | `/profile/[userId]`, `/channel-studio` owner-only, `/player/[id]?source=creator-video` | V1 core; visual polish pushed and proof-passed |
| Platform Studio `/channel-studio` | `app/channel-studio/index.tsx`, `app/channel-settings.tsx` implementation/compatibility | Owner Platform Studio | Owner-only creator operating system with Home, Content, Live, Audience, Insights, Brand, backed creator upload/manage, audience actions, event/default summaries, and Preview Channel | Public viewing experience, Player shell, global admin moderation queue, billing/payout/ad systems | Signed-in owner only | `user_profiles`, `creator_permissions`, `videos`, creator video storage, `creator_events`, `channel_*`, `safety_reports`, admin summaries | Owner profile/settings, `/channel-settings` compatibility | `/channel/[ownUserId]`, `/player/[id]?source=creator-video` | V1 core |
| Channel Settings compatibility `/channel-settings` | `app/channel-settings.tsx` | Compatibility route for Platform Studio | Keeps older owner links resolving to Platform Studio | New public Channel route, platform admin, Player playback | Signed-in owner behavior follows Platform Studio implementation | Same as Platform Studio | Existing links/settings/profile | `/channel-studio` experience, `/channel/[ownUserId]` preview | Compatibility preserved |
...
| `/channel/[userId]` | Yes, already pushed | Public Channel | First-class viewer-facing Channel route | Preserve it. Do not remove, route around, or leak owner controls/drafts/private content. |
