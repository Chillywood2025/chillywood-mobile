...
- Public Platform is the creator's viewer-facing surface on `/channel/[userId]`.
...
- Profile owns personal/social identity and handoffs, while Platform Studio owns deeper creator upload/manage controls.
...
7. The user-approved Live Stage and Watch-Party Live major layout structure is now locked in `docs/LIVE_WATCH_PARTY_LAYOUT_LOCK.md`. Visible comments are part of that lock and must stay in their current placement unless the user explicitly asks for a comments redesign.
8. Public v1 still needs deferred runtime proof: two-device creator-video Watch-Party, live layout/Chi’llyfects proof, draft/private blocked state, signed-out/non-premium blocked state, and final route smoke.
...
- Creator uploaded videos now belong in public Platform media sections and the Platform Studio Content panel, not the personal Posts lane.
- Platform Studio owns deeper creator video management and current Access Defaults. It should not present ad/sponsorship/Premium-playback cards as Channel owner controls.
...
| Route | Owner file | System | What it owns | Must not own | Access | Backend dependencies | Enters | Exits | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Profile `/profile/[userId]` | `app/profile/[userId].tsx` | Profile social identity | Public identity, personal/social profile, owner/public profile actions, profile posts, Chi'lly Chat handoff, View Platform handoff | Public Platform implementation, Platform Studio management, Player playback, admin moderation writes, platform title discovery | Signed-in normal app route; public viewer vs owner behavior is user-id based | `user_profiles`, profile posts/social tables, audience summaries, safety reports | Home, Chat, Party Room, Live Stage, public Platform | `/channel/[userId]`, `/channel-studio` for owner, `/player/[id]?source=creator-video`, `/chat`, `/chat/[threadId]` | V1 core |
| Public Platform `/channel/[userId]` | `app/channel/[userId].tsx` | Public Platform | Viewer-facing creator route with hero, backed Channel Pulse stats, Featured, Latest Uploads, Live & Upcoming, About, Follow/Share/Report/View Profile, owner-only Open Platform Studio | Owner upload/edit/publish/unpublish/delete, drafts/private/unpublished videos, audience management, insights/analytics, admin controls, Profile privacy controls | Public/viewer-facing route with owner-vs-non-owner UI separation and existing block/unavailable behavior | `user_profiles`, public clean creator videos, creator events, channel audience summary/block truth | Profile View Platform, Platform Studio Preview Platform, deep link | `/profile/[userId]`, `/channel-studio` owner-only, `/player/[id]?source=creator-video` | V1 core; visual polish pushed and proof-passed |
| Platform Studio `/channel-studio` | `app/channel-studio/index.tsx`, `app/channel-settings.tsx` implementation/compatibility | Owner Platform Studio | Owner-only creator operating system with Home, Content, Live, Audience, Insights, Brand, backed creator upload/manage, audience actions, event/default summaries, and Preview Platform | Public viewing experience, Player shell, global admin moderation queue, billing/payout/ad systems | Signed-in owner only | `user_profiles`, `creator_permissions`, `videos`, creator video storage, `creator_events`, `channel_*`, `safety_reports`, admin summaries | Owner profile/settings, `/channel-settings` compatibility | `/channel/[ownUserId]`, `/player/[id]?source=creator-video` | V1 core |
| Channel Settings compatibility `/channel-settings` | `app/channel-settings.tsx` | Compatibility route for Platform Studio | Keeps older owner links resolving to Platform Studio | New public Platform route, platform admin, Player playback | Signed-in owner behavior follows Platform Studio implementation | Same as Platform Studio | Existing links/settings/profile | `/channel-studio` experience, `/channel/[ownUserId]` preview | Compatibility preserved |
...
| Party Room `/watch-party/[partyId]` | `app/watch-party/[partyId].tsx` | Shared video Watch-Party room | Content-first shared title/creator-video room, membership, chat/reactions, watch together, room state, shared source card, visible text comments in the approved placement, Chi’llyfects foundation handoff, Player playback | Live Stage media rendering except explicit live room CTA, upload management, real AR/Chi’llyfects processing, menu-only comments replacement | Signed-in active beta; room access resolved before join | `watch_party_rooms`, `watch_party_room_memberships`, messages, sync events, source resolver, access entitlements | Waiting room/deep link | `/player/[id]`, `/player/[id]?source=creator-video`, `/watch-party/live-stage/[partyId]`, profile/chat | V1 core; content-first layout implemented and layout-locked, runtime proof pending |
| Live Stage `/watch-party/live-stage/[partyId]` | `app/watch-party/live-stage/[partyId].tsx`, `components/watch-party-live/livekit-stage-media-surface.tsx` | Live Watch-Party / LiveKit | People-first live camera stage, Live First/Live Watch-Party mode, LiveKit join contract, participant media, Party Members grid, host badge, visible live-stage comments in the approved dock, live-stage reporting, Chi’llyfects foundation UI | Creator video party player, platform title source model, real AR/Chi’llyfects processing, menu-only comments replacement | Signed-in active beta; room access resolved before join | `watch_party_rooms`, memberships, LiveKit token contract, safety reports | Home Live waiting room, live room CTA, profile linked live | `/watch-party/[partyId]`, profile, chat, `/watch-party?mode=live` | V1 core; separate from creator-video Watch-Party, layout-locked, proof pending |
...
| Profile | `app/profile/[userId].tsx` | `_lib/userData.ts`, `_lib/channelReadModels.ts`, `_lib/creatorVideos.ts` for fast-post upload | `user_profiles`, profile/social tables, audience summary tables | Working | Required | Deeper profile/social later | Deep channel management/edit/delete | Owner/public route proof |
| Public Platform | `app/channel/[userId].tsx` | `_lib/channelAudience.ts`, `_lib/creatorVideos.ts`, `_lib/liveEvents.ts`, `_lib/userData.ts` | `user_profiles`, public `videos`, `creator_events`, `channel_followers`, `channel_audience_blocks` | Built and visually polished | Required | Advanced shelves/programming later | Owner management actions | Owner/non-owner public route proof |
| Platform Studio | `app/channel-studio/index.tsx`, `app/channel-settings.tsx` | `_lib/channelReadModels.ts`, `_lib/channelAudience.ts`, `_lib/creatorVideos.ts` | `user_profiles`, `videos`, `creator_events`, `channel_followers`, `channel_subscribers`, `channel_audience_requests`, `channel_audience_blocks` | Built with dashboard-first Home | Required | Advanced creator ops later | Public viewing route | Owner/manage proof |
| Creator Media | `app/channel-settings.tsx`, `app/profile/[userId].tsx`, `app/player/[id].tsx` | `_lib/creatorVideos.ts`, `_lib/watchPartyContentSources.ts` | `videos`, `creator-videos` storage/RLS | Upload/play/watch-party linking built | Required | Advanced Platform Studio, transcoding later | Platform title creation | Draft/public/non-owner/two-device proof |
...
| Watch-Party Live / Party Room | `app/watch-party/index.tsx`, `app/watch-party/[partyId].tsx`, `app/player/[id].tsx` | `_lib/watchParty.ts`, `_lib/watchPartyContentSources.ts`, `_lib/accessEntitlements.ts`, `_lib/roomRules.ts`, `_lib/liveEffects.ts` | `watch_party_rooms`, memberships, messages, sync events | Platform and creator-video source model plus content-first layout built; Chi’llyfects UI is foundation-only | Required | Larger sync/multidevice hardening, real Chi’llyfects processing later | Live Stage camera ownership | Platform/creator start, join, rejoin, access gates, content-first layout |
| Live Watch-Party / Live Stage / LiveKit | `app/watch-party/index.tsx`, `app/watch-party/live-stage/[partyId].tsx` | LiveKit helpers under `_lib/livekit/*`, `_lib/watchParty.ts`, `_lib/liveEffects.ts` | `watch_party_rooms`, memberships, LiveKit token backend/config | Working from prior proof; people-first layout/Chi’llyfects foundation added | Required | Native game streaming and real camera Chi’llyfects later | Creator-video party source | Live First/Live Watch-Party proof plus layout/Chi’llyfects proof |
...
| Admin | `app/admin.tsx` | `_lib/moderation.ts`, `_lib/appConfig.ts`, `_lib/userData.ts` | `platform_role_memberships`, `app_configurations`, `titles`, `creator_permissions`, `safety_reports`, `videos` | Working foundation; backend `operator` is only the internal Admin alias | Required | Rich admin workbench later | Public user controls | Non-admin denial and admin action proof |
...
| `/channel/[userId]` | Yes, already pushed | Public Platform | First-class viewer-facing creator route | Preserve it. Do not remove, route around, or leak owner controls/drafts/private content. |
| `/creator-media` or `/creator-dashboard` | No | Platform Studio/Creator Media | Could reduce Platform Studio density later | Do not add now. Platform Studio is the v1 management owner. |
...
1. Add a public Platform alias only if user testing shows Profile/Channel is not clear enough.
...
Anything involving full Audience Role Roster, native game streaming, paid creator media, tips, coins, payouts, ads, advanced Platform Studio, or comment media upload remains post-v1 unless a separate product decision changes scope.
