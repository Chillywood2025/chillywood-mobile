...
- Profile owns personal/social identity and handoffs, while Channel Studio owns deeper creator upload/manage controls.
...
- Creator uploaded videos now belong in public Channel media sections and the Channel Studio Content panel, not the personal Posts lane.
- Channel Studio owns deeper creator video management and current Access Defaults. It should not present ad/sponsorship/Premium-playback cards as Channel owner controls.
...
| Route | Owner file | System | What it owns | Must not own | Access | Backend dependencies | Enters | Exits | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Profile `/profile/[userId]` | `app/profile/[userId].tsx` | Profile social identity | Public identity, personal/social profile, owner/public profile actions, profile posts, Chi'lly Chat handoff, View Channel handoff | Public Channel implementation, Channel Studio management, Player playback, admin moderation writes, platform title discovery | Signed-in normal app route; public viewer vs owner behavior is user-id based | `user_profiles`, profile posts/social tables, audience summaries, safety reports | Home, Chat, Party Room, Live Stage, public Channel | `/channel/[userId]`, `/channel-studio` for owner, `/player/[id]?source=creator-video`, `/chat`, `/chat/[threadId]` | V1 core |
| Public Channel `/channel/[userId]` | `app/channel/[userId].tsx` | Public Channel | Viewer-facing mini streaming/network route with hero, backed Channel Pulse stats, Featured, Latest Uploads, Live & Upcoming, About, Follow/Share/Report/View Profile, owner-only Open Channel Studio | Owner upload/edit/publish/unpublish/delete, drafts/private/unpublished videos, audience management, insights/analytics, admin controls, Profile privacy controls | Public/viewer-facing route with owner-vs-non-owner UI separation and existing block/unavailable behavior | `user_profiles`, public clean creator videos, creator events, channel audience summary/block truth | Profile View Channel, Studio Preview Channel, deep link | `/profile/[userId]`, `/channel-studio` owner-only, `/player/[id]?source=creator-video` | V1 core; visual polish pushed and proof-passed |
| Channel Studio `/channel-studio` | `app/channel-studio/index.tsx`, `app/channel-settings.tsx` implementation/compatibility | Owner Channel Studio | Owner-only creator operating system with Home, Content, Live, Audience, Insights, Brand, backed creator upload/manage, audience actions, event/default summaries, and Preview Channel | Public viewing experience, Player shell, global admin moderation queue, billing/payout/ad systems | Signed-in owner only | `user_profiles`, `creator_permissions`, `videos`, creator video storage, `creator_events`, `channel_*`, `safety_reports`, admin summaries | Owner profile/settings, `/channel-settings` compatibility | `/channel/[ownUserId]`, `/player/[id]?source=creator-video` | V1 core |
| Channel Settings compatibility `/channel-settings` | `app/channel-settings.tsx` | Compatibility route for Channel Studio | Keeps older owner links resolving to Channel Studio | New public Channel route, platform admin, Player playback | Signed-in owner behavior follows Studio implementation | Same as Channel Studio | Existing links/settings/profile | `/channel-studio` experience, `/channel/[ownUserId]` preview | Compatibility preserved |
...
| Channel Studio | `app/channel-studio/index.tsx`, `app/channel-settings.tsx` | `_lib/channelReadModels.ts`, `_lib/channelAudience.ts`, `_lib/creatorVideos.ts` | `user_profiles`, `videos`, `creator_events`, `channel_followers`, `channel_subscribers`, `channel_audience_requests`, `channel_audience_blocks` | Built with dashboard-first Home | Required | Advanced creator ops later | Public viewing route | Owner/manage proof |
...
| `/creator-media` or `/creator-dashboard` | No | Channel Studio/Creator Media | Could reduce Channel Studio density later | Do not add now. Channel Studio is the v1 management owner. |
