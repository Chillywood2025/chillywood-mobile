...
6. The Live Layout and Effects System is now documented in `docs/LIVE_LAYOUT_AND_EFFECTS_SYSTEM.md`. Live Stage remains people-first with a visible Chi'lly Party Members grid; Party Room remains content-first with a shared source card; effects are foundation-only unless a real camera processor is built.
...
8. Public v1 still needs deferred runtime proof: two-device creator-video Watch-Party, live layout/effects proof, draft/private blocked state, signed-out/non-premium blocked state, and final route smoke.
...
| Watch-Party Live / Party Room | `app/watch-party/index.tsx`, `app/watch-party/[partyId].tsx`, `app/player/[id].tsx` | `_lib/watchParty.ts`, `_lib/watchPartyContentSources.ts`, `_lib/accessEntitlements.ts`, `_lib/roomRules.ts`, `_lib/liveEffects.ts` | `watch_party_rooms`, memberships, messages, sync events | Platform and creator-video source model plus content-first layout built | Required | Larger sync/multidevice hardening, real effects later | Live Stage camera ownership | Platform/creator start, join, rejoin, access gates, content-first layout |
| Live Watch-Party / Live Stage / LiveKit | `app/watch-party/index.tsx`, `app/watch-party/live-stage/[partyId].tsx` | LiveKit helpers under `_lib/livekit/*`, `_lib/watchParty.ts`, `_lib/liveEffects.ts` | `watch_party_rooms`, memberships, LiveKit token backend/config | Working from prior proof; people-first layout/effects foundation added | Required | Native game streaming and real camera effects later | Creator-video party source | Live First/Live Watch-Party proof plus layout/effects proof |
