...
Object-storage migration note: the scanner gateway now has a source-prepared `cloudflare_r2` private-origin read path for future migrated public-safe scan/transcode candidates under `chillywood-media-origin`, and trusted copier `media-object-storage-migration` is deployed, but the Hetzner Object Storage migration is still Partial. Copy is blocked before mutation by missing backend R2 private-origin write config. It does not move DB metadata, does not make Hetzner Object Storage ready for shutdown, and keeps Hetzner fallback retained; do not shut down Hetzner LiveKit.
