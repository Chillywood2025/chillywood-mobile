...
Final camera/reaction source hardening keeps the Shared Player layout locked while making that proof path observable: received reactions create a visible local `Reaction` room event plus a longer receiver toast, local reaction attribution uses the sender participant first, and approved speaker bubbles without an identity-matched camera track show `Camera preparing`. Installed proof is still required after OTA before this checklist can move Watch-Party Live to Closed.
