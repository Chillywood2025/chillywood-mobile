...
| Watch-Party Live Shared Player installed closure | Partial | Active launch blocker for this lane | Latest photos invalidated the prior foreground-bubble carry-forward. Current source hardening keeps the real LiveKit bubble surface mounted during post-approval authority refresh, keeps downgraded viewer/no-publish contracts out of publish-ready state, adds bounded speaker/canPublish token retries, and suppresses the competing top-video participant-name preview during Shared Player. Full validation, OTA, and installed proof are still required; do not count source proof as installed closure. |
