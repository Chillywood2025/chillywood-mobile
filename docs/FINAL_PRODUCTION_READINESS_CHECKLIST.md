...
Settings/Bell Activity ownership correction: source fixed. Settings no longer renders the notification Activity list. The bell icon/tray owns Activity records, timestamps, read state, dismiss, and routing. Settings remains for notification preferences, device push registration, Register Device, Refresh, and call sound. Source fixed is not installed-app proof until delivered by OTA or a later Google Play internal build.
