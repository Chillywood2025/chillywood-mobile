...
Cross-surface stale identity metadata fix: Partial after Play-installed versionCode `63` proof. One user identity must render consistently across profile, chat, search, circle, followers, and following. Fresh remote profile must win over stale AsyncStorage. Settings/Profile/Chat must agree on the current handle. Circle/Followers/Following must not keep stale handle metadata as primary identity. Existing inbox rows must not show stale participant metadata as primary identity. Platform/owner/admin/moderator/creator surfaces now prefer the same fresh profile identity source where available, while role badges remain separate from handle/name/avatar identity. Fresh Profile, fresh Chat inbox/filter row, and fresh direct-thread header showed `user230455` / `@user230455`, but a separate existing Chat row still displayed `Proof Normal` / `@user230456` and opened to the same stale header. Stale `@user230456` cannot be considered Closed across inbox, Circle, Followers, Following, shared user cards, and role surfaces until that old row is resolved or proved to be a distinct non-stale account/thread.
