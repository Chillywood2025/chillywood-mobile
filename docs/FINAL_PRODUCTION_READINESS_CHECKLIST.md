...
Watch-Party Live sidecar/shared-player now has source-truth coverage for the app-controlled failure modes found in the real-media proof without changing the locked fullscreen layout. Source commit `833520e5ba6fad86160b93df1da45cd510b1c433` is on `origin/main`; Android EAS Update production runtime `1.0.0` published group `0d23919f-bedb-40b4-9428-6550bdfd765c`, Android update `019f3da7-dcd6-7732-a757-cb36b1bd7c61`. R3 visibility was recovered over wireless ADB/mDNS, and both devices read back Play-installed v80. Fresh room `KFUFYC` on OTA group `9a6ce961-1632-4488-a189-98403ea72b78` proved the previous post-approval roster/bubble blocker is resolved: both devices showed host + viewer bubbles after approval, no self-only/no-bubble collapse, no duplicate self, host remained host, viewer became speaker, and R5 showed an identity-safe viewer camera bubble/feed or seated participant surface. The custom fullscreen row remains the same left comments rail, center shared player/video surface, and right portrait-reused LiveKit bubble rail. Guard coverage rejects replacing the right rail with fake participants, removing the rails, or falling back to normal native fullscreen for shared playback.

The source repair adds a canonical Watch-Party Live Premium access key helper shared by Party Room handoff and Player entry, real/non-fixture media classification for installed proof honesty, strict LiveKit token-contract authority matching before host/speaker publish-ready UI, request-versioned camera-seat requests with X-close duplicate suppression and `Not now` clear semantics, sheet/review-card-first pending approval instead of competing inline pending controls, identity-matched participant-specific bubble tracks, and helper-backed proof importing the route's real Watch-Party Live source-truth module. The latest source follow-up also restores visible regular Shared Player comments at the bottom, adds Android shared-video render recovery, treats durable membership as the base roster, drops remote LiveKit identities that collide with the current device identity, uses durable participant id for `You`, renders membership placeholders while LiveKit bubbles are syncing instead of hiding active members, and changes the social preview line to a neutral room count instead of a host/viewer label list. Source commit `cb546f06d65f5bb05ec1c66cccb79ce47ee7039a` is on `origin/main`; Android EAS Update production runtime `1.0.0` published group `9a6ce961-1632-4488-a189-98403ea72b78`, Android update `019f3ff9-39f4-714e-91b9-c9a5de71ba61`.

Installed real-media rerun: a safe public Home media card, `Chi'llywood City Lights`, now classifies as `real-media` with remote playback URL present and no proof/fixture wording. In room `38M7L3`, R5 hosted from that card and R3 joined the same Party Room; both opened the same Shared Player. Actual video frames appeared on both devices after host playback started; R3 no longer stayed on a black surface with only `Synced · Playing`. Regular Shared Player comments were visible at the bottom, R3 showed `Synced · Controls locked` during viewer locked-control proof, fullscreen preserved the locked left comments / center video / right LiveKit bubble rails and rendered actual video after host restart, and R3 returned to Party Room. No `Live feed unavailable` / `Live video is temporarily unavailable` alert appeared. Public production remains No-Go for Watch-Party Live sidecar until Google Play-installed proof closes the remaining viewer camera request / host approval packet with matching LiveKit speaker/canPublish authority and an approved identity-safe viewer camera feed. This lane does not change Premium entitlement logic, billing/provider setup, Watch-Party Party Room behavior, Android App Links, LiveKit routing/heartbeat/cutoffs, Chi'lly Chat/native calls, auth/RLS, live money, payout, or cashout.

Remaining camera request / host approval packet: artifact folder `/tmp/google-play-internal-v80-watch-party-live-camera-request-approval-proof-20260707-171433/`. Both devices were visible and Play-installed v80, backend LiveKit health passed with one eligible server and no fresh no-eligible blocker, and both devices renewed/read back Premium active through the approved Google Play / RevenueCat sandbox path after normal sandbox expiry. The stale room `38M7L3` returned `Room not found`, so fresh room `EJPK7C` was created from the strict real-media `Chi'llywood City Lights` Player path. R3 joined the same Party Room/Shared Player. The viewer self audience bubble/card was visible, but tapping it did not send or persist a camera request, did not show a pending request state, and R5 never received a request badge/review card/approval surface. Viewer comment/reaction send also remained unproved because the attempted lower-control interaction escaped to Android share/intent UI and was backed out. Classification: playback, media classification, regular comments visibility, locked viewer playback controls, fullscreen rails, return-to-room, Premium, Party Room, and backend LiveKit health remain proved; public production still requires a source fix and installed proof for Shared Player camera-request/comment-reaction reachability and host approval.

Current source follow-up: the Shared Player reachability gap is source-fixed, validation-clean, pushed, and OTA-published while preserving the locked layout. Source commit `7ec03c5e4fc716a65fad633db1a593906c2012c3` is on `origin/main`; Android EAS Update production runtime `1.0.0` published group `d6828e44-5e61-4329-9721-d4106a97909f`, Android update `019f3eea-b092-7f99-9833-723a9ed710a3`. Regular Shared Player now has an explicit viewer `Request Camera` button with pending/error states, bubble taps route through the same versioned request path, host request review controls have stable proof targets, regular comment input/send has stable proof targets, and the regular reaction button broadcasts room reactions. Guard/proof coverage rejects Android Share in request/comment/reaction paths and keeps these controls outside the host-only playback lock. Public production still requires installed closure of the remaining two-phone camera request / host approval packet on this OTA.

Latest request-control follow-up: installed room `ZWZ2KP` proved the new testID controls were visible, but tapping explicit `Request Camera` or the self bubble did not create `Request pending` because the request path recomputed requester identity from a stale party-user/ref path. The regular comments dock also clipped input/send below the viewport. Source commit `ad1611b9810299a2ffb98cfdefc15d193a2869e2` passes the visible participant id into the versioned request path and uses compact regular dock comments so input/send are reachable; fullscreen rails remain unchanged. Android EAS Update production runtime `1.0.0` published group `6f83ca92-a7c8-41f5-b4d3-864998e2823e`, Android update `019f3f06-a76b-7dea-8ad7-f5db9374533b`. Public production still requires installed closure on this latest OTA.
...
Current installed follow-up: room `NZP627` on the keyboard-height OTA proved the regular lower dock and comment composer path. R3 was recovered through its previously paired private-LAN wireless ADB endpoint, R5 stayed USB-visible, R5 renewed Premium through the approved Google Play / RevenueCat sandbox path after normal sandbox expiry, and backend LiveKit health stayed green. R3 focused the visible regular Shared Player comment input, the input/Send row moved above Android's keyboard, R3 sent `proof`, and R5 saw `Guest: proof` in Room Comments. R3 tapped `Request Camera`, R5 received a stable `CAMERA REQUEST` card and approved it, and post-approval roster convergence held: R5 showed `You` as Host plus `chillywood92` as Speaker, while R3 showed self as Speaker plus `user230455` as Host. No device showed only self, no duplicate self bubble appeared, and host/viewer roles did not mix. Public production remains No-Go for Watch-Party Live until a fresh installed packet proves the final approved camera/reaction path: R5 must render the approved viewer camera feed under the correct identity, or show explicit honest camera-preparing/publish-ready copy while waiting for that remote track, and R3's reaction must produce an observable host-side reaction/energy marker.

Final camera/reaction source hardening keeps the Shared Player layout locked while making that proof path observable: source commit `7d92e8a07bff084916969e665b5ee0ea89ee83fb` is on `origin/main`; Android EAS Update production runtime `1.0.0` published group `c4edaad0-0565-4548-8848-9ef6450a5a4f`, Android update `019f4247-cb7f-7bbd-b19e-80cf06b2ae1b`. Received reactions create a visible local `Reaction` room event plus a longer receiver toast, local reaction attribution uses the sender participant first, and approved speaker bubbles without an identity-matched camera track show `Camera preparing`. Installed proof is still required after OTA before this checklist can move Watch-Party Live to Closed.

Latest foreground media-bubble correction: source commit `f9882fc3fcbd23e7765d17f432c47a78ed056f5c` is on `origin/main`; Android EAS Update production runtime `1.0.0` published group `ecd5dfde-3650-40e9-a3c9-8431ed8806ec`, Android update `019f42a7-167d-70bf-a8f1-fea8d81469c3`. The remaining visible issue after roster convergence was that both phones could show correct host/viewer bubbles without camera feeds. The source now keeps Watch-Party LiveKit connected while Android AppState is foreground `active` instead of disabling media on transient foreground `blur` events, makes publish-capable no-track bubbles show `Camera preparing` before role labels, and scrolls the lower dock to the comment composer while the keyboard is open. User installed confirmation reported the behavior works now. Public production still requires artifacted OTA-pickup proof if this group is used as final closure evidence; no fullscreen layout, Premium, Party Room source/room semantics, LiveKit routing/heartbeat/cutoff/server registry, Android App Links, Chat/native, auth/RLS, billing/provider, payout, or cashout behavior changed.

## July 7, 2026 Live Stage Source-Truth Follow-Up

Live Stage host/viewer UX is Closed for the app-controlled installed Google Play v80 proof lane. The stricter source/guard repair is committed, pushed, OTA-published, and installed-proved: request close suppression is versioned, duplicate pending broadcasts cannot reopen an X-closed sheet, downgraded LiveKit speaker-desired contracts are not publish-ready, pending approval is sheet-only, participant-specific tiles require identity-matched tracks, and the proof imports the real Live Stage helper module instead of a duplicate model. Installed room `NXQ4M2` proved the host/viewer presentation, X close, `Not now`, `Bring on stage`, viewer publish-state readback, identity-safe tile behavior, and Stage / `2 in room` with both phones Premium-active through the approved Google Play / RevenueCat sandbox path. This preserves Premium gates, LiveKit routing/heartbeat/cutoffs, Watch-Party Party Room behavior, Android App Links, Chi'lly Chat/native calls, auth/RLS, billing/provider state, live money, payout, and cashout boundaries. Public production still requires the remaining Watch-Party Live Shared Player post-approval roster/bubble/feed closure and broader LiveKit load/reconnect/cellular/TURN/metrics hardening.
...
- X close is request-version safe; duplicate pending broadcasts do not reopen the sheet.
- `Not now` clears/declines the current request while keeping the participant visible.
- Pending approval is sheet-only; participant cards do not render inline pending approve/dismiss controls.
- A speaker-desired participant is not publish-ready until the LiveKit contract matches speaker/canPublish authority.
- Participant-specific video uses only identity-matched LiveKit tracks; missing tracks render a safe fallback, not another participant's feed.
- The source proof imports the real helper module that the component uses.
...
- Primary IPv4 endpoint: `Blocked: no` (address intentionally omitted from this checklist)
...
Governing doc: `docs/release/GOOGLE_SIGNED_V79_CREATOR_MONEY_POLISH_PASS.md`. Artifact folder: `/tmp/google-play-internal-v79-creator-money-polish-pass-20260705-063948/`; installed closure subfolder: `/tmp/google-play-internal-v79-creator-money-polish-pass-20260705-063948/installed-proof-closure-20260705-123246/`.
...
Installed proof found and fixed one real Paid Video locked-state regression. Follow-up source commit `ddb07f332010ace899284df6f01bd6065dc28e21` corrected locked-player chrome in `app/player/[id].tsx`; EAS Update production Android runtime `1.0.0` published group `24c53b6a-2ee5-469e-a3cd-9eccc5904375`, Android update `019f3364-40aa-7ea7-baff-9ea606e87b11`, commit `ddb07f332010ace899284df6f01bd6065dc28e21`. Both `R5CR120QCBF` and `R3CXA0DS5JV` read back Google Play-installed v79 from `com.android.vending`. Installed proof closed: clean locked Paid Video poster/paywall with `Unlock Video` and no player chrome bleed-through; unavailable Seat Pass with no strong enabled purchase CTA; Money Center safe-area/header spacing; creator `Watch-Party Seat Passes` wording; room tray user-facing Activity copy; and fresh concise buyer/creator notification rows with timestamps, dismiss controls, and route smoke checks. The proof account display-name typo remains external proof-account data and was not mutated. Full validation passed after the source regression fix, and final proof-state diff checks passed. No live money, payouts, cashout, payable balances, provider activation/mutation, Premium entitlement change, Money Center architecture refactor, native call change, auth/RLS weakening, Play production submission, sideload, `adb install`, logout, clear data, uninstall, or reinstall happened.
...
| Watch-Party Live Shared Player installed closure | Partial | Active launch blocker for this lane | Latest photos invalidated the prior foreground-bubble carry-forward. Source commit `4127cb10aad7007c57a30e9baee1a9088b02ad52` and Android OTA group `996c2593-7df5-4826-8c39-cfc3810de91c` / update `019f42d8-93ad-72c5-806a-9ff679c28202` keep the real LiveKit bubble surface mounted during post-approval authority refresh, keep downgraded viewer/no-publish contracts out of publish-ready state, add bounded speaker/canPublish token retries, and suppress the competing top-video participant-name preview during Shared Player. Installed proof is still required; do not count source proof as installed closure. |
