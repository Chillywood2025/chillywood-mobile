...
Current source follow-up: the Shared Player reachability gap is source-fixed and validation-clean while preserving the locked layout. Regular Shared Player now has an explicit viewer `Request Camera` button with pending/error states, bubble taps route through the same versioned request path, host request review controls have stable proof targets, regular comment input/send has stable proof targets, and the regular reaction button broadcasts room reactions. Guard/proof coverage rejects Android Share in request/comment/reaction paths and keeps these controls outside the host-only playback lock. Public production still requires a fresh OTA from aligned source and installed closure of the remaining two-phone camera request / host approval packet.
