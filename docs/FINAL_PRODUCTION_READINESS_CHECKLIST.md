...
Status: Source-repaired, guard/proof upgraded, installed proof Partial on regular Shared Player comment-composer keyboard reachability.
...
Current comment-composer follow-up: the latest installed screenshot shows the regular Shared Player bottom composer can be cut off and keyboard-blocked after approval. Source commit `3a2001381a43c26b7e499da699148e0438004c59` keeps the locked fullscreen layout unchanged, makes the regular non-fullscreen lower dock scrollable, enters keyboard-safe comment mode when the default visible composer is focused, closes the Shared Player menu while composing, and keeps Send taps handled while Android keyboard is open. Android EAS Update production runtime `1.0.0` published group `d4461f02-f7a3-4fc7-80e8-90dbfc76581a`, Android update `019f4064-d6e3-7316-95d6-ae6efc5b9a2a`. Installed proof on fresh room `W5AZ7B` showed that was still insufficient: the Android keyboard hid the composer while the action row remained visible. Source commit `cec243533fff7d6859a3397a8a055aa6558a01f6` keeps fullscreen unchanged and hides the regular Shared Player action/control rows while the comment keyboard is active, so the composer and Send button get the reachable slot above the keyboard. Android EAS Update production runtime `1.0.0` published group `4c88f779-570a-4fe8-8089-66eeafee4a6b`, Android update `019f410d-cd39-7875-9253-a6575ce09d8c`. A follow-up installed rerun recovered R3 at `10.0.0.27:42069` and reached the Shared Player, but visible lower-dock controls still did not reliably receive taps because the Android shared-video tap target could sit above the dock. Source commit `547535135002831449ff6dc7281a202ca0c7b5a7` keeps fullscreen unchanged, lowers that video tap target, and raises the existing regular lower dock touch layer. Android EAS Update production runtime `1.0.0` published group `9285127f-acb0-49ec-bcd1-554f439c7497`, Android update `019f412a-b092-73d5-a688-5b2e3c0ce131`. Public production remains No-Go for Watch-Party Live until that fresh OTA proves the lower dock is tappable, the visible bottom composer stays above the keyboard, and a viewer comment can be sent and received by the host.
