...
Installed real-media rerun: a safe public Home media card, `Chi'llywood City Lights`, now classifies as `real-media` with remote playback URL present and no proof/fixture wording. In room `K4ADLA`, R5 hosted from that card and R3 joined the same Party Room; both opened the same Shared Player. R5 showed actual playback at `0:04` / `0:52`, but R3 stayed on a black Shared Player surface while showing `Synced · Playing`, then later returned to `Synced · Paused`. No `Live feed unavailable` / `Live video is temporarily unavailable` alert appeared. Public production remains No-Go for Watch-Party Live sidecar until Google Play-installed proof closes with real non-fixture Home media, actual shared-player playback on both devices, visible regular Shared Player comments at the bottom, host-only playback authority in portrait and fullscreen, viewer comments/reactions/camera request, host approval with matching LiveKit authority, identity-safe LiveKit bubbles, and unchanged fullscreen rails. This lane does not change Premium entitlement logic, billing/provider setup, Watch-Party Party Room behavior, Android App Links, LiveKit routing/heartbeat/cutoffs, Chi'lly Chat/native calls, auth/RLS, live money, payout, or cashout.
