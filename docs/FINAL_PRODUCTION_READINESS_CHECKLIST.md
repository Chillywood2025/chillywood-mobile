...
Watch-Party Live sidecar/shared-player now has source-truth coverage for the remaining app-controlled failure modes without changing the locked fullscreen layout. Source commit `833520e5ba6fad86160b93df1da45cd510b1c433` is on `origin/main`; Android EAS Update production runtime `1.0.0` published group `0d23919f-bedb-40b4-9428-6550bdfd765c`, Android update `019f3da7-dcd6-7732-a757-cb36b1bd7c61`. R3 visibility was recovered over wireless ADB/mDNS for the latest proof packet at `10.0.0.27:41063`, and both devices read back Play-installed v80. Fresh room `V7G4QL` on the latest OTA proved the remaining blocker is source/UI, not device visibility: R5 host rendered the generic shared-playback fallback instead of host/viewer bubbles, while R3 viewer rendered two bubbles with confusing current-user/host preview copy. The custom fullscreen row remains the same left comments rail, center shared player/video surface, and right portrait-reused LiveKit bubble rail. Guard coverage rejects replacing the right rail with fake participants, removing the rails, or falling back to normal native fullscreen for shared playback.
...
Current post-approval follow-up: the newest installed photos and fresh room `V7G4QL` show the Watch-Party Live bubble/feed state is still not production-ready. One phone can show no bubbles or only self, while the other can show confusing current-user/host preview copy. Source commit `cb546f06d65f5bb05ec1c66cccb79ce47ee7039a` is on `origin/main`; Android EAS Update production runtime `1.0.0` published group `9a6ce961-1632-4488-a189-98403ea72b78`, Android update `019f3ff9-39f4-714e-91b9-c9a5de71ba61`. The current source follow-up keeps the locked layout unchanged and makes durable active room membership the base Shared Player bubble roster; realtime presence only enriches display/speaking/LiveKit identity state and cannot delete active members. Room host and membership host remain host, approved speaker membership / `canSpeak` wins over stale presence, self fallback cannot duplicate a membership participant, remote identity aliases that collide with the current device are ignored for presentation, the preview/header line is now a neutral room count, and Player proof logs include membership, presence, rendered bubble, missing bubble, duplicate bubble, and role-map diagnostics. Public production remains No-Go for Watch-Party Live until this fresh OTA is installed-proved.
