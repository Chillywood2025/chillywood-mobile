...
Current post-approval follow-up: the newest installed photos and fresh room `V7G4QL` show the Watch-Party Live bubble/feed state is still not production-ready. One phone can show no bubbles or only self, while the other can show confusing current-user/host preview copy. Source commit `c41c1254ec349b640f5eaa12790731df942b01e8` is on `origin/main`; Android EAS Update production runtime `1.0.0` published group `2554255e-5591-439b-8474-b4de7b01da3f`, Android update `019f3ff1-f211-75a4-be34-20ca98ce0504`. The current source follow-up keeps the locked layout unchanged and makes durable active room membership the base Shared Player bubble roster; realtime presence only enriches display/speaking/LiveKit identity state and cannot delete active members. Room host and membership host remain host, approved speaker membership / `canSpeak` wins over stale presence, self fallback cannot duplicate a membership participant, remote identity aliases that collide with the current device are ignored for presentation, and Player proof logs include membership, presence, rendered bubble, missing bubble, duplicate bubble, and role-map diagnostics. Public production remains No-Go for Watch-Party Live until this fresh OTA is installed-proved.
