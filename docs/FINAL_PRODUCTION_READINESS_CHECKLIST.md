...
| Chat/calls/notifications | Closed for current launch scope; Partial for direct message mutation | `docs/chat/CHAT_CALL_MODERATION_NOTIFICATION_ABUSE.md` and Chi'lly Chat/call/push policies cover exact chat-message reports, participant/thread-context reports, staff evidence scope/case context, blocked/restricted denial, chat-send rate limiting, call/ring dispatch dedupe, notification privacy, safe call metadata, no call content/recording, and scan-gated attachments. | No. | Include message/call/push sanity in release smoke; direct chat-message hide/remove/restore remains a future exact backend lane. |
...
| Chat/call moderation and notification abuse controls | Partial for direct message mutation; launch-safe | Already closed for current supported controls | Exact chat-message reporting is wired; thread-level coverage is participant/thread-context reporting; staff private chat evidence is exact-scope and case/report scoped; blocked/disabled/deleted users fail closed; call/ring notifications are deduped/rate-limited; call content/recording is absent; attachments remain scan-gated. Direct `chat_message` hide/remove/restore is future exact backend work. |
...
| Moderation/reporting | Closed after validation | Already closed | Users can report profiles, Profile media, Platform content, videos, paid videos, live rooms, exact chat messages, comments, replies, specific events, and VIP/subscriber content where the surface exists. Dedicated event report affordance: Closed after validation. Exact chat-message report affordance: Closed after validation. Content takedown decisions: Closed for production decision policy and current backed enforcement after validation. Live-room moderation and incident response: Closed for production policy, current backed host controls, LiveKit token authority, and incident-response proof after validation. Chat/call moderation and notification abuse controls: Partial for direct message hide/remove/restore mutation; Closed for current production reporting, evidence-access policy, blocked/restricted account denial, call/ring dedupe, chat-send rate limiting, attachment scan gating, notification privacy, and proof after validation. Normal reports, DMCA/legal, support, money/refund/access support, security incidents, live safety incidents, notification-abuse incidents, and appeals are separated. Reporter identity stays private by default, duplicate/false reports are deduped and rate-limited, reports do not auto-delete content, evidence/access history are preserved, and staff access requires exact scopes plus case/report context. |
