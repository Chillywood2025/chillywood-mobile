...
Current comment-composer follow-up: the latest installed screenshot shows the regular Shared Player bottom composer can be cut off and keyboard-blocked after approval. The current source follow-up keeps the locked fullscreen layout unchanged, makes the regular non-fullscreen lower dock scrollable, enters keyboard-safe comment mode when the default visible composer is focused, closes the Shared Player menu while composing, and keeps Send taps handled while Android keyboard is open. Public production remains No-Go for Watch-Party Live until a fresh OTA proves the visible bottom composer can scroll above the keyboard and a viewer comment can be sent and received by the host.
