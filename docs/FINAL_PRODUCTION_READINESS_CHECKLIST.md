...
Cross-surface stale identity metadata fix: source-fixed, installed proof pending. One user identity must render consistently across profile, chat, search, circle, followers, and following. Fresh remote profile must win over stale AsyncStorage. Settings/Profile/Chat must agree on the current handle. Circle/Followers/Following must not keep stale handle metadata as primary identity. Existing inbox rows must not show stale participant metadata as primary identity. Platform/owner/admin/moderator/creator surfaces now prefer the same fresh profile identity source where available, while role badges remain separate from handle/name/avatar identity. Source fixed is not installed-app proof, and a v62+ Google Play internal build/proof is required before stale `@user230456` can be considered Closed across inbox, Circle, Followers, Following, shared user cards, and role surfaces.
