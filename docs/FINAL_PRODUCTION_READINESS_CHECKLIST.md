...
| Moderation/reporting | Closed after validation | Already closed | Users can report profiles, Profile media, Platform content, videos, paid videos, live rooms, chat messages, comments, replies, events, and VIP/subscriber content where the surface exists. Normal reports, DMCA/legal, support, money/refund/access support, security incidents, and appeals are separated. Reporter identity stays private by default, duplicate/false reports are deduped and rate-limited, and staff access requires exact scopes plus case/report context. |
