...
Chat Call remote-video / Live action UX sweep: Partial for actual-user proof. `docs/release/CHAT_CALL_REMOTE_VIDEO_LIVE_ACTION_UX_SWEEP.md` records source fixes for remote video not appearing, RTC video cropping on mismatched phone aspect ratios, and Live Watch-Party host action controls staying open/stuck after seat update failure. Actual-user installed-app proof remains Partial until both physical Play-internal v57 phones run the updated code and reproduce the normal visible Chat Call and Live waiting-room paths. No physical phone sideload, provider mutation, live money activation, payout/refund execution, RLS/Premium/auth weakening, or secrets exposure happened.
