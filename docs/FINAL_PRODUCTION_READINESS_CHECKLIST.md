...
Current v78 Answer-action update: Blocked for final two-phone installed closure. `docs/release/GOOGLE_SIGNED_V78_NATIVE_ANSWER_ACTION_FIX.md` records the source/native fix and current proof blocker. Source commit `6c3fbdef23d8ccf9bef90c26d7b6dea33c409b02` changes native CallStyle `Answer` from a broadcast receiver launch to a direct Activity PendingIntent and keeps notification cleanup after safe authenticated accept/decline handling. EAS Build `e01b708a-d049-421b-a16b-1bb1e5399e47` produced Android AAB versionCode `78`, versionName/runtime `1.0.0`, and EAS Submit `1a7f765c-2c34-4cab-8fb6-d10bb422e976` submitted it to Google Play internal testing only. `R5CR120QCBF` updated through Google Play to v78 with installer `com.android.vending`; `R3CXA0DS5JV` is not visible over ADB or Mac USB enumeration, so background voice/video Answer, caller state, Decline, stale Answer safety, same-thread Accept, normal in-app, and room-safe regressions remain unproved on two physical Google Play-installed phones. No Play production submission, sideload, `adb install`, logout, clear data, uninstall/reinstall, Money Center change, provider mutation, live money, payout/cashout change, auth/RLS weakening, broad WebRTC/media rewrite, or broad room-routing change happened.
