...
Provider-Link sandbox proof on May 25, 2026 re-proved the Stripe webhook boundary with a test-mode Stripe CLI `payment_intent.succeeded` event delivered to the enabled sandbox endpoint with `livemode=false` and `pending_webhooks=0`; unsigned direct POST returned `invalid_signature` and `liveMoneyAction:false`. Supabase Edge secret inventory was names-only and found only `STRIPE_SECRET_KEY` plus `STRIPE_WEBHOOK_SECRET` configured; RevenueCat server/webhook secrets, Google Play server/webhook secrets, Stripe Connect client/platform metadata, and internal live-money flags remain missing. RevenueCat and Google Play webhook shells now write sanitized setup-required audit rows when secrets are missing and continue to grant no Premium/subscription access. Android Platform Studio proof is outside the repo at `/tmp/chillywood-proof-2026-05-25T-provider-link-sandbox-proof-android/` and shows sanitized readiness states, planned/locked money tools, no active money claim, and no secret-like values in captured XML.

Connect readiness proof after the payout-readiness pass: Stripe CLI test-mode account reads show provider accounts with `charges_enabled=false`, `payouts_enabled=false`, requirements due, and no completed KYC/tax readiness. `_lib/creatorPayouts.ts` now derives explicit blocked reasons and next actions from provider readiness, KYC, tax/1099, hold, minimum payout, live-money, payout, cash-out, and Stripe production flags. Platform Studio now shows this in `Money Center > Payouts` and `Money Center > Provider Status`; existing backend-created test-mode Stripe setup links and status refresh remain available only where already backed, while payouts/cash-out stay disabled and provider ids, raw requirement JSON, available balances, withdrawals, transfers, and payout release remain hidden. Admin Payouts remains read-only and calls out that provider readiness, KYC, tax/1099, hold clearance, legal/accounting approval, and Owner-approved payout workflow are required before any payout can be called available.
...
- Owner/Admin Money Center lives in Admin Command Center > Money Center with collapsible Overview, Kill Switches, Premium / RevenueCat / Google Play, Sponsors / Ads, Fraud & Risk, Digital Sales, Tips / Watch-Party Seats / Paid Content, Merch, Creator Balance / Ledger, Payouts / Stripe Connect, Provider Webhooks, Tax & Legal, Audit Trail, and Technical Checks sections. Old Admin money params map into these section anchors.
...
- Money switches: `202605270001_platform_money_kill_switches.sql` is applied/aligned in the linked Supabase environment. Defaults are fail-closed with live money off and no money movement.
