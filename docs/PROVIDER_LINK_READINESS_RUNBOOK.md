...
Provider dashboard ownership and access governance is documented in `docs/ops/PROVIDER_DASHBOARD_OWNERSHIP_ACCESS_GOVERNANCE.md`. First Owner / Owner owns provider dashboard accountability; provider roles must be least-privilege; MFA/2FA is required where supported; service accounts are not human staff accounts; API keys and provider secrets must live in secret managers/provider dashboards/EAS/Supabase/GitHub secrets, not repo; provider webhooks must be protected with signature/shared-secret validation where supported; dashboard access proof remains owner-confirmation-required where repo cannot verify it. This reference does not mutate Google Play, RevenueCat, Stripe, payout, purchase, refund, or provider state.

June 4, 2026 Stripe physical merch runtime proof: `stripe-merch-checkout` ACTIVE version `4` and `stripe-merch-webhook` ACTIVE version `5` now have one real sandbox physical-merch Checkout proof. The upgraded proof account authenticated, was temporarily reactivated as operator, completed Stripe test-mode Checkout for `cw_merch_test_tee_sandbox`, and was revoked again. A dedicated Stripe test webhook endpoint sent the real `checkout.session.completed` event to `stripe-merch-webhook` with a valid signature. Backend readback showed one sandbox merch order, one merch order item, and one processed sanitized `stripe_merch_events` row, while merch access grants, RevenueCat/Premium entitlements, payable/paid money-access rows, provider payout-enabled accounts, active proof roles, live money, and payouts all stayed `0`/off. Proof path: `/tmp/chillywood-stripe-merch-sandbox-checkout-proof-20260603/`.

June 4, 2026 Money Center launch-review update: Creator Money Center and Owner/Admin Money Center now carry review-ready sandbox-only/not-payable summaries for the completed Google Play / RevenueCat proof chain. Product Catalog, Provider Events, Purchase Intents, Access Grants, Ledger Events, payable sandbox/setup rows, and failure-path status are visible in the Admin Money Center without exposing secrets. Review packet: `docs/MONEY_CENTER_LAUNCH_REVIEW_PACKET.md`. UI proof doc: `docs/MONEY_CENTER_UI_POLISH_PROOF.md`. This is a provider-readiness/readout polish only; production live money, public buy buttons, payouts, cash-out, fake balances, and Stripe Android digital checkout remain off/absent.

June 4, 2026 sandbox failure/event-pass proof: remote migrations `20260604015548`, `20260604015818`, and `20260604015941` add duplicate access/ledger protections, an admin revoke proof path, and `has_event_pass_access` for existing `creator_events`. Event pass `cw_event_pass_sandbox_099` completed a real Google Play test-card / RevenueCat sandbox purchase after EAS Update group `581630d7-b67f-4159-99e0-43f76a5e1221` fixed the owner/operator sandbox route default. Remote readback showed one event-pass provider event, one consumed intent, one sandbox-only access grant, one sandbox/not-payable ledger row, and 0 payable/paid rows. Admin revoke proof revoked one sandbox Watch-Party Seat Pass grant and wrote a sandbox `reversed` ledger row; expired-intent proof created no provider event/grant/ledger. Remaining provider-specific proof is real provider refund/revoke and delayed-payment pending only if Google Play/RevenueCat tooling exposes those paths. Do not insert fake provider rows or fake sale rows.
...
Sandbox provider product follow-up: the external dashboard product blocker is closed for the first six non-Premium sandbox IDs. Google Play Console has active one-time products for `cw_paid_content_access_sandbox_099`, `cw_watch_party_live_ticket_sandbox_099`, `cw_live_watch_party_access_sandbox_099`, `cw_live_watch_party_seat_sandbox_099`, `cw_creator_tip_sandbox_099`, and `cw_event_pass_sandbox_099`; each has purchase option `sandbox-099`. RevenueCat has matching published Play Store consumable products without entitlements or offerings. Migration `20260603225500_sandbox_digital_product_mappings.sql` maps those products in `monetization_products` as sandbox-only and intent-enabled. EAS update group `9757451f-7817-488e-9b10-fff68372fefd` published `/admin-money-sandbox-purchases`; the route loaded on `R5CR120QCBF`, denied non-operator access, and exposed the sandbox-only launcher only during a temporary operator proof role. The first Creator tip attempt failed with Google Play item-not-found and left a failed/unconsumed intent. The follow-up Play-installed versionCode `23` proof fixed item availability, completed Creator tip `cw_creator_tip_sandbox_099` through Google Play's test card, stored one sandbox RevenueCat/Google Play provider event, consumed one purchase intent, and wrote one sandbox-only/not-payable ledger row with no access grant and no active proof roles after revoke. The remaining provider proof is product-specific resolver testing for ticket/access/seat/content/event products; do not insert fake provider rows or ledger rows.
...
- Stripe CLI is authenticated against the Chi'llywood test account and proved a signed test webhook delivery plus same-event resend without printing or committing API keys or webhook secrets.
