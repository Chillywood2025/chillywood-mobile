...
June 1, 2026 backend entitlement sync follow-up: `revenuecat-webhook` is now deployed as ACTIVE version 4 with real backend sync code for verified RevenueCat Premium events. The function requires `REVENUECAT_WEBHOOK_SECRET`, rejects missing or invalid shared-secret requests, maps entitlement `premium` / product `premium_subscription` to `user_entitlements.source='revenuecat'`, records duplicate-safe `billing_events`, writes sanitized provider-readiness audit, and returns `liveMoneyAction:false`. Supabase write failures now fail the webhook instead of silently claiming success. Real webhook proof is still blocked because the names-only Supabase secret inventory still lacks `REVENUECAT_WEBHOOK_SECRET`; the deployed missing-secret smoke returns `setup_required`, `webhookProcessed:false`, `premiumGranted:false`. Sanitized backend readback shows zero Premium rows, so Platform Studio correctly remains denied even though the RevenueCat client UI can show `Premium is active` after the sandbox purchase. Next provider action is to configure the RevenueCat dashboard webhook and Supabase shared secret, send a real sandbox event, then prove the backend row and creator-tool unlock.
