...
June 1, 2026 backend entitlement sync follow-up: `revenuecat-webhook` is now deployed as ACTIVE version 4 with real backend sync code for verified RevenueCat Premium events. The function requires `REVENUECAT_WEBHOOK_SECRET`, rejects missing or invalid shared-secret requests, maps entitlement `premium` / product `premium_subscription` to `user_entitlements.source='revenuecat'`, records duplicate-safe `billing_events`, writes sanitized provider-readiness audit, handles RevenueCat dashboard `TEST` events as delivery proof with no Premium grant, and returns `liveMoneyAction:false`. Supabase write failures now fail the webhook instead of silently claiming success. The server-side `REVENUECAT_WEBHOOK_SECRET` is now configured in Supabase by name only, and RevenueCat webhook integration `whintgr38699522f7` points at the deployed function with the matching Authorization header. RevenueCat dashboard `TEST` delivery returned HTTP 200; no-secret smoke returns 401; valid shared-secret test returns `test_received`, `webhookProcessed:true`, `premiumGranted:false`, and `liveMoneyAction:false`. Sanitized audit readback shows the test/reject events and no raw provider payload storage. Sanitized backend readback still shows zero Premium rows, because after webhook setup the Play-installed app showed `Premium is not active` and Restore completed with `Premium is not active`. Next provider action is a fresh active sandbox purchase/renewal/restore event, then backend row and creator-tool unlock proof.
...
| RevenueCat | Premium entitlement id | configured in code/dashboard as `premium`; webhook backend sync implemented but real event blocked on missing server secret | false |
| RevenueCat | Offering | configured in code/dashboard as `premium`; purchase/restore sandbox proof passed, backend row proof still blocked on webhook setup | false |
...
| RevenueCat | Premium entitlement id | configured in code/dashboard as `premium`; webhook backend sync implemented but real event blocked on missing server secret | false | false |
| RevenueCat | Offering | configured in code/dashboard as `premium`; purchase/restore sandbox proof passed, backend row proof still blocked on webhook setup | false | false |
