...
Scheduler status: `chillywood-installed-qa-firebase-smoke.timer_daily_cost_capped`. The daily Firebase uploaded-artifact scheduler is enabled/active on `chillywood-prod-01`; it is not Play-installed, Premium Billing, or two-device LiveKit proof.
...
- enabling/linking Google Cloud billing
- arbitrary bucket creation
- Owner/Editor IAM
- project-wide Storage Admin

`installed_product_qa_operator` records installed-app route, role, account-fixture, and device-readiness blockers proactively. Codex caught the current installed traversal blockers manually; this operator now turns that category into first-class QA findings and owner-command requests. Live deployment is active as of 2026-07-13: `installed-product-qa-operator` is token-gated, current OTA `019f596f-1a87-76d8-abe3-14342c8d1cf6` was proved on Play-installed `R5CR120QCBF`, and `watch_once` recorded the current blockers with `source=play_installed` / `discovered_by=autonomous_operator`. The Firebase uploaded-artifact device-lab scheduler is active for bounded route/no-crash smoke; Premium fixture repair is provider-backed only, and two-device proof still requires two Play-installed devices or approved coordinated device lab.

Firebase Test Lab is the first device-lab foundation path in cost-capped cheap mode. The source runner records only `firebase_test_lab_uploaded_artifact` proof and must not call it Play-installed proof. It defaults to virtual-device-only, manual/on-demand or daily/on-change when approved, `FIREBASE_TEST_LAB_MONTHLY_CAP_USD=5`, `FIREBASE_TEST_LAB_PER_RUN_CAP_USD=0.25`, `FIREBASE_TEST_LAB_MAX_SCHEDULED_RUNS_PER_DAY=1`, physical disabled, broad crawls disabled, and two-device Firebase disabled. Unknown free quota can run only when worst-case virtual-device cost is bounded under the per-run cap and monthly remaining budget; unbounded cost still fails closed. One owner-command Tier 1 virtual smoke completed as Firebase matrix `4612242345700782646` with worst-case `costEstimateUsd=0.09`, `billingRisk=low`, and local ledger write. Server matrix `8384645686869695525` later completed from `chillywood-prod-01` using the dedicated results bucket and posted live traversal row `333817c9-b651-4e65-ab66-0d885952d2de` with `source=firebase_test_lab_uploaded_artifact`, `discovered_by=device_lab`, `results_bucket=chillywood-installed-qa-testlab-results`, and `cost_estimate_usd=0.09`. Firebase smoke does not close Premium/RevenueCat/Google Play Billing or two-device LiveKit proof. The owner Mac is not the long-term runner. The target long-term runner is `chillywood-prod-01` through the source-controlled daily systemd templates in `ops/installed-product-qa-operator/systemd/`. Server audit found the root-owned env file, `/opt/chillywood/current` checkout, non-snap Ubuntu `nodejs`/`npm`, Google Cloud SDK `575.0.1`, and `firebase-tools@15.23.0` present with no secrets printed. Owner-authorized Google Console provisioning created dedicated runtime service account `installed-qa-testlab-runner@chillywood-app.iam.gserviceaccount.com`; its key is stored only on the server as `root:root` mode `600`, and server gcloud can read the Test Lab virtual catalog. A current-source APK for commit `4061e2bda8c77dad174b72378db483acb6e906fe` is present on the server with SHA-256 `6bcddf44e9d320d152742537339f3204a7295cd7868554b34146f82c81528b44`, and the server cost guard passes at `costEstimateUsd=0.09`. The exact dedicated results bucket bootstrap is now a standing bounded Level 2 policy, not a per-request approval path: only `gs://chillywood-installed-qa-testlab-results` in `chillywood-app`, seven-day lifecycle, uniform bucket-level access, public access prevention, bucket-level `roles/storage.objectCreator`, and `roles/storage.objectViewer` only if result readback requires it for `installed-qa-testlab-runner@chillywood-app.iam.gserviceaccount.com`. Billing enablement/linking remains owner-only. SSH to `87.99.145.160` recovered before VNC was needed; diagnostics found sshd active, disk/memory healthy, and no reboot/reset/sshd restart was performed. The scheduler runner now uses bounded matrix lifecycle behavior: async start with `--async`, persisted pending matrix id, REST polling with `FIREBASE_TEST_LAB_MAX_WAIT_SECONDS=900`, `FIREBASE_TEST_LAB_POLL_INTERVAL_SECONDS=30`, `FIREBASE_TEST_LAB_MAX_ACTIVE_MATRICES=1`, and systemd start timeout bounds. The bounded service started matrix `4625992954862995833`, posted live backend rows `ff81956d-94e3-49e9-8c80-fae2c12b0dd8` (`matrix_started`), `1dc00369-b5ca-4289-92bc-daf5bae00222` (`matrix_timeout` with `POLL_HTTP_FAILED`), and `282fb154-101c-402b-9539-d3fb8080de51` (`matrix_pending` without starting a duplicate), then exited cleanly. `chillywood-installed-qa-firebase-smoke.timer` is enabled/active daily. The current matrix state is pending/poll-failed rather than completed device proof.
...
`product_intelligence_operator` is registered as a single top-level planning, research, memory, architecture, experiment, scoped-execution, evaluation, capability, and budget foundation. Its nine surfaces are `rachi_cognitive_orchestration`, `research_source_broker`, `intelligence_memory_service`, `architecture_knowledge_graph`, `product_experiment_engine`, `software_engineering_executor`, `independent_evaluation_judge`, `capability_and_tool_broker`, and `model_router_and_budget_controller`.

All are `security_hardened_scaffold_not_deployed`, have no scheduler or production credential, and remain off. They plan and evaluate above the existing specialized systems; they do not replace those systems or receive direct authority over money, user rights, auth/RLS, moderation enforcement, release/OTA, pricing, or provider products. The canonical detailed component contract is `config/autonomy/autonomous-components.json` and the bounded cognitive policy is `config/intelligence/cognitive-platform.json`.
