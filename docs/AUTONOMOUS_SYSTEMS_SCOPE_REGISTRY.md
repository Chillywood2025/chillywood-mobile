...
Approval execution is currently foundation-only because explicit owner/super-admin backing is still incomplete. Current role truth is `platform_role_memberships`; `/admin` is the canonical command center. Rachi may request or recommend, but Rachi cannot approve itself and never outranks the owner. Autonomous operators cannot approve their own Level 3/4 requests. Approved requests expire, and execution must re-run fresh preflight before acting.
