...
- The creator Brand tab no longer exposes reviewer queue controls or `Approve / Reject / Archive` actions. Creator publishing uses the bottom `Publish Changes` action; backend review/RPC permissions remain separate.
...
- Publish notices are state-aware and product-facing: the UI claims the public Platform is updated only when post-publish readback shows selected assets are published, moderation-safe, scan-safe, and not deleted. Normal creator copy must not expose malware-scan/review internals; pending media is described as still getting ready, while blocked media says it cannot be published yet.
- Publish readback now checks both sides of the public gate: the owner Brand Studio row and the public Platform RPC. `Publish Changes` is the creator's approval step: it saves the profile, self-approves only selected owned scan-safe assets, publishes only selected approved scan-safe assets, reloads Brand Studio, then reloads `readPublicPlatformBranding` for the same owner. It only claims public media success when the public readback returns the selected asset. Otherwise the notice classifies the state as safety scan pending, publish/apply retry needed, not publishable yet, or public Platform readback mismatch. Creator-facing copy must not imply there is another normal review step after `Publish Changes`.
- The production publish closeout adds `publish_platform_brand_profile_assets(uuid[], text)`, an owner-only RPC used by the mobile publish path. It reviews and publishes only selected assets owned by the signed-in creator, with `deleted_at is null`, scan status `clean` or `manual_review`, and moderation status `clean` or `reported` before public publish. Wrong-owner, deleted, malware-detected, scan-failed, quarantined, pending-scan, rejected, removed, hidden, and archived assets remain hidden. The creator UI still does not expose reviewer controls.
...
- Malware scanning adds `scan_status` values including `pending_scan`, `scanning`, `clean`, `malware_detected`, `scan_failed`, `manual_review`, and `quarantined`; normal creator UI summarizes them with readiness language such as getting ready, ready, or needs changes. Internal proof/docs may still classify exact scan states, but the in-app creator copy should not expose scan internals.
...
- `20260615142151_platform_brand_owner_publish_assets_rpc.sql`: adds the owner-only selected-asset publish helper used by `Publish Changes`. It keeps storage paths private, writes review/audit metadata for creator self-review, and publishes only selected owned, non-deleted, moderation-safe, scan-safe Brand Studio assets.
