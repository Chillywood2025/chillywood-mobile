...
- The City Lights one-job rows are the canary proof, not the final path. The target path is automatic CDN/HLS for any trusted audited public-safe `media_renditions` row when rollout config is explicitly enabled.
