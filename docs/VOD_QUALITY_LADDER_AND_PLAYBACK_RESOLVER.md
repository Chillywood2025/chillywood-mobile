...
- Production transcode worker design/local proof exists, but the production worker is not deployed. PITR or owner-approved backup/restore readiness is required before future production worker writes, production backfill, or worker activation.
