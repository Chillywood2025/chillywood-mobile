...
| ---: | --- | --- | --- |
| 1 | Physical iPhone camera permission and preview | `PENDING_DEVICE_PROOF_BUILD_6` | Prompt state, allow flow, real preview, camera off/on, lifecycle, and truthful denied/restricted recovery without private imagery. |
| 2 | Physical iPhone microphone capture/publish | `PENDING_DEVICE_PROOF_BUILD_6` | Permission, real local track, mute/unmute, lifecycle, and denied/restricted recovery without recording private conversation. |
| 3 | Physical iPhone photo picker and upload | `PENDING_DEVICE_PROOF_BUILD_6` | Select a non-private HEIC asset, cancel safely, limited-library behavior if offered, prepare/upload/render, and verify item-scoped access. |
...
| 8 | Universal Link verification on a signed physical build | `PENDING_DEVICE_PROOF_BUILD_6` | Cold, warm, and already-running app tests cover authentication and content routes while Android App Links remain unchanged. |
...
| 15 | VoiceOver/Dynamic Type/final device regression | `PENDING_DEVICE_PROOF_BUILD_6` | Representative small/large text, VoiceOver, reduced motion, orientation, iPad, authentication, media, legal, moderation, and account-deletion paths pass. |
