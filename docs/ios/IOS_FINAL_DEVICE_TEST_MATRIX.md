...
**Do not begin this matrix yet.** Build 6 is superseded by semantic application
and backend-contract changes. Every row remains blocked until the replacement
Simulator smoke and internal TestFlight build 7 or higher are recorded here.

| # | Required proof | Status | Completion evidence required |
| ---: | --- | --- | --- |
| 1 | Physical iPhone camera permission and preview | `BLOCKED_NEW_FINAL_CANDIDATE` | Prompt state, allow flow, real preview, camera off/on, lifecycle, and truthful denied/restricted recovery without private imagery. |
| 2 | Physical iPhone microphone capture/publish | `BLOCKED_NEW_FINAL_CANDIDATE` | Permission, real local track, mute/unmute, lifecycle, and denied/restricted recovery without recording private conversation. |
| 3 | Physical iPhone photo picker and upload | `BLOCKED_NEW_FINAL_CANDIDATE` | Select a non-private HEIC asset, cancel safely, limited-library behavior if offered, prepare/upload/render, and verify item-scoped access. |
...
| 5 | APNs foreground delivery | `BLOCKED_NEW_FINAL_CANDIDATE` | Exact final build receives a permitted ordinary notification in foreground with correct route/category/badge behavior and no token exposure. |
| 6 | APNs background delivery | `BLOCKED_NEW_FINAL_CANDIDATE` | Background delivery and response route pass without duplicate handling. |
| 7 | APNs terminated-app delivery | `BLOCKED_NEW_FINAL_CANDIDATE` | Terminated launch opens the intended bounded route exactly once. |
| 8 | Universal Link verification on a signed physical build | `BLOCKED_NEW_FINAL_CANDIDATE` | Cold, warm, and already-running app tests cover authentication and content routes while Android App Links remain unchanged. |
...
| 12 | TestFlight StoreKit purchase | `BLOCKED_NEW_FINAL_CANDIDATE` | After a separately approved bounded sandbox window, an approved account completes only a non-payable internal transaction for each supported product type. |
| 13 | Restore Purchases | `BLOCKED_NEW_FINAL_CANDIDATE` | Premium restore reconciles the authenticated account; tips are not incorrectly restorable and Seat Pass access comes from the verified server ledger. |
| 14 | Renewal/cancellation/refund/revocation | `BLOCKED_NEW_FINAL_CANDIDATE` | RevenueCat/App Store events preserve idempotency and update access according to verified lifecycle state without creating payable value. |
| 15 | VoiceOver/Dynamic Type/final device regression | `BLOCKED_NEW_FINAL_CANDIDATE` | Representative small/large text, VoiceOver, reduced motion, orientation, iPad, authentication, media, legal, moderation, and account-deletion paths pass. |
...
- Use only the pending corrected-source commit, freshly installed/smoked Simulator
  artifact, and exact internal TestFlight build 7 or higher. Build 6 and source
