...
Status: operator/auditor source/proof model with completed City Lights proof, bounded CLI auto-detect cycle, and protected Premium HD row audit. The recovery operator/auditor model is local code plus proof scripts; it does not deploy a production worker, does not run a production queue processor, and does not switch playback. Current scoped row counts are `media_transcode_jobs=10` and `media_renditions=15`, with `5` protected Premium HD rows, no active unfinished jobs, no unsafe CDN rows, and no unsigned public HD rows.
...
- Included data: the one-job pre-write production row counts were `media_transcode_jobs=0` and `media_renditions=0`, so the scoped data artifact used for the proof was intentionally empty. Current scoped production row counts after the bounded auto-transcode cycle are `media_transcode_jobs=10` and `media_renditions=15`.
...
- `npm run backup:media-worker:verify-latest` reads the latest private R2 artifacts for `backups/media-worker/2026/07/10/media-worker-logical-20260710T024048-5de12265dded/`, verifies SHA-256 checksums, parses the manifest, confirms not exposed through the public playback bucket or media domain, and prints the restore-drill command to run next.
...
- `npm run backup:media-worker:status`, `npm run backup:media-worker:verify-latest`, and `npm run backup:media-worker:restore-drill` were run against the latest private R2 backup prefix and passed. `verify-latest` confirmed matching checksums and `media.chillywoodstream.com` returned `404`; `restore-drill` restored `media_transcode_jobs=10` and `media_renditions=15` into disposable PGlite.
...
- Final row counts: `media_transcode_jobs=10`, `media_renditions=15`, with City Lights plus four bounded auto-processed public-safe sources audited for free HLS, five protected Premium HD rows audited behind tokenized Worker access, and one low-resolution unsupported failed job marker.
