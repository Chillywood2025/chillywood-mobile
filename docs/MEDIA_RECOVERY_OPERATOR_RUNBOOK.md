...
- Included data: the one-job pre-write production row counts were `media_transcode_jobs=0` and `media_renditions=0`, so the scoped data artifact used for the proof was intentionally empty. Current scoped production row counts after the bounded auto-transcode cycle are `media_transcode_jobs=6` and `media_renditions=10`.
...
- `npm run backup:media-worker:verify-latest` reads the latest private R2 artifacts for `backups/media-worker/2026/07/10/media-worker-logical-20260710T003002-02eab4b4c5cd/`, verifies SHA-256 checksums, parses the manifest, confirms not exposed through the public playback bucket or media domain, and prints the restore-drill command to run next.
...
- `npm run backup:media-worker:status`, `npm run backup:media-worker:verify-latest`, and `npm run backup:media-worker:restore-drill` were run against the latest private R2 backup prefix and passed. `verify-latest` confirmed matching checksums and `media.chillywoodstream.com` returned `404`; `restore-drill` restored `media_transcode_jobs=6` and `media_renditions=10` into disposable PGlite.
...
- Final row counts: `media_transcode_jobs=6`, `media_renditions=10`, with City Lights plus four bounded auto-processed public-safe sources audited for HLS and one low-resolution unsupported failed job marker.
