...
- Protected Premium HD scope: `5` audited Premium HD rows are stored under `playback/protected/premium/` and served only through `premium-media-proof.chillywoodstream.com` with valid short-lived Premium tokens. Unsigned public HD exposure is `0`; free users and missing/wrong tokens are denied by the Worker. Backend/app token issuance is deployed and OTA-published; installed free/non-Premium fallback is proved, and installed Premium HD playback remains pending only for a Premium-active installed session.
