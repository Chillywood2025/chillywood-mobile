...
Cross-surface stale identity metadata fix: Partial after Play-installed versionCode `63` proof. Fresh remote profile wins on reached current surfaces: Profile showed `user230455` / `@user230455`, Chat inbox search/filter retained the `user230455` / `@user230455` thread, and the fresh direct-thread header showed `user230455` / `@user230455`. Full stale identity closure is not claimed because a separate existing Chat row still displayed `Proof Normal` / `@user230456` and opened to the same stale header on v63. Circle was reachable but the target identity was not visible; Followers, Following, shared user-card, call/room identity, and platform/admin/moderator role surfaces remain Pending installed proof. Google Play internal install is not enough without actual user flow proof, and this lane remains Partial until the old row is resolved or proved to be a distinct non-stale account/thread.
