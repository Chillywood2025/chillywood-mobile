...
Chi'lly Chat Google-signed v60 Direct Chat + Call proof: Partial in `docs/release/GOOGLE_SIGNED_V60_DIRECT_CHAT_CALL_PROOF.md`. Google Play internal v60 was delivered and installed by Google Play on both attached phones with installer `com.android.vending`; no logout, uninstall, reinstall, clear-data, reset session, sideload, or manual APK install happened. Fresh handle proof improved: Settings/Profile/Chat search/direct-thread header showed `@user230455`, and visible Chat search -> direct-thread open/create passed after live authenticated RPC repair migrations. Full actual-user call closure is still No-Go/Partial: receiver elsewhere-in-app saw the incoming call banner, but tapping it opened `This Chi'lly Chat thread could not be found.` and both phones did not join. The remaining blocker is consistent with receiver readback being blocked by the current platform-owner chat guard on owner-to-user direct threads; no auth/RLS/chat/account-status permission weakening was made. Existing inbox row metadata can still show stale `@user230456`. Video, background push/ringing, full cleanup, and two-normal-user call proof remain launch blockers for full Chi'lly Chat call closure. No service-role chat proof was counted, no provider/live-money mutation happened, and liveMoneyEnabled remains OFF.
...
Chat Call remote-video / Live action UX sweep: `docs/release/CHAT_CALL_REMOTE_VIDEO_LIVE_ACTION_UX_SWEEP.md` records a targeted source fix for the latest tester-visible realtime issues. Remote Chi'lly Chat video could be hidden by stale camera presence state or Android audio-first RTC track order, fullscreen RTC video could crop on mismatched phone aspect ratios, and Live Watch-Party host participant controls could stay open/stuck after `Seat update unavailable`. Source fixes are applied, but actual-user installed-app proof remains Partial until both physical Play-internal v57 phones pick up this code and reproduce the normal visible Chat Call and Live waiting-room paths. No physical phone sideload, provider mutation, live money activation, payout/refund execution, RLS/Premium/auth weakening, or secrets exposure happened.
