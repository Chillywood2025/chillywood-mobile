...
Cross-surface stale identity metadata fix: source-fixed, No-Go/Partial for installed proof. Fresh remote profile must win over stale AsyncStorage, Settings/Profile/Chat must agree on the current handle, and existing inbox rows plus Circle/Followers/Following/shared user-card/platform-role surfaces must not show stale `@user230456` as primary identity. Platform/owner/admin/moderator/creator role badges may show role/status, but they must not cause stale handle/name/avatar metadata to win. Source fixed is not installed-app proof; Google Play internal install is not enough without actual user flow proof, so v62+ Google Play internal installed proof is required.
