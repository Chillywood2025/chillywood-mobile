...
June 10, 2026 Chi'lly Chat call notification/ringtone foundation update: `docs/CHILLY_CHAT_CALL_NOTIFICATION_RINGTONE_SYSTEM.md` records the implementation for direct-thread call invites, incoming in-app call sheet, call history cards, call settings, and Android notification channels. Migration `202606100001_chilly_chat_call_invites_and_ringtones.sql` is remote-applied, and post-apply dry-run reports the remote database is up to date. This reduces the Chi'lly Chat call UX blocker but does not fully close production notification proof: two-user Android proof is required, and background call push dispatch needs the approved server dispatch path. No bundled ringtone audio assets currently exist, so custom background push sounds still require a native build with local files; downloaded/imported sounds remain in-app-only for V1. No LiveKit token issuer, communication room authority, Watch-Party route ownership, Player behavior, auth, Premium/content safety, money state, payouts, Stripe Android checkout, or admin authority changed.
