...
| 7. RevenueCat/Google provider proof | Partial, active sandbox entitlement bridge and non-Premium denial closed for test scope | P1 for production monetized launch | No while production Premium/live money claims stay off | Yes for production monetized launch | Play-installed v12, RevenueCat/Google sandbox purchase, restore, real RevenueCat webhook event, backend `user_entitlements.source='revenuecat'` active row, Platform Studio unlock, and separate non-Premium runtime denial are proved. The webhook requires the shared secret, no-secret smoke returns 401, dashboard `TEST` delivery returns no Premium grant, real `INITIAL_PURCHASE` returned `premiumGranted:true`, sanitized readback shows no raw provider payload storage, and a disposable non-Premium user with zero Premium rows/zero platform roles saw clean `Premium required` copy. Money Center remains setup/not-active, and no fake Premium/live-money action exists. | Keep the purchase shell closed unless intentionally opened for reviewer sandbox testing, and keep live money/tickets/tips/paid content/payouts off. Production Premium remains unclaimed until the submitted build/provider release path is approved |
