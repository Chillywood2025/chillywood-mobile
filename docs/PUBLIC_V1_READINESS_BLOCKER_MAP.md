...
June 1, 2026 signed-in purchase-shell proof at `/tmp/chillywood-play-installed-premium-sandbox-purchase-proof-20260601/` clears the Play-installed purchase/restore blocker. The Play-installed v12 app opens signed in, owner-approved EAS update group `b678522a-8734-49a1-a582-f2bc6743c756` temporarily opened the Premium shell, Google Play displayed the sandbox `Chi'llywood Premium` subscription with the always-approves test payment method, purchase completed, `/subscribe` showed `Premium is active`, and restore completed with `Purchases restored. Premium is active.` The shell was closed again with EAS update group `82f7e7fd-d213-4f50-9c5d-6e6a328884db`, and current source keeps `PREMIUM_PURCHASE_SHELL_ON_HOLD = true`. Remaining blocker: backend `user_entitlements` sync/readback and Premium creator-tool unlock are not closed; Platform Studio still denied with clean `Premium required` copy after purchase.

June 1, 2026 RevenueCat backend entitlement sync proof at `/tmp/chillywood-revenuecat-backend-entitlement-sync-proof-20260601/` implements and configures the missing webhook bridge but keeps active-entitlement proof blocked honestly. `revenuecat-webhook` ACTIVE version 4 now verifies the shared webhook secret, maps real RevenueCat Premium events to `user_entitlements.source='revenuecat'`, writes duplicate-safe `billing_events`, records sanitized audit with no live-money action, and handles dashboard `TEST` events without granting Premium. Supabase now has `REVENUECAT_WEBHOOK_SECRET` configured by name only, RevenueCat webhook integration `whintgr38699522f7` points at the deployed function, dashboard `TEST` delivery returned HTTP 200, no-secret smoke returns 401, and valid shared-secret test returns `test_received`. Sanitized backend readback still shows zero Premium rows; after webhook setup `/subscribe` and Restore Purchases show Premium not active, so no real active entitlement event was available to sync. Remaining provider blocker: run a fresh active sandbox purchase/renewal/restore event, verify the backend row, then prove Premium creator-tool unlock.
...
| 7. RevenueCat/Google provider proof | Partial | P1 for monetized launch | No while production Premium/live money claims stay off | Yes for monetized launch | Play-installed v12, RevenueCat/Google sandbox purchase, restore, and RevenueCat webhook delivery setup are proved. `revenuecat-webhook` ACTIVE version 4 implements verified backend sync into `user_entitlements`; Supabase has `REVENUECAT_WEBHOOK_SECRET`; dashboard `TEST` delivery returned HTTP 200; no-secret smoke returns 401; valid shared-secret test returns `test_received`; sanitized audit has test/reject events. Sanitized backend readback still shows zero Premium rows because post-setup restore returned Premium not active. Money Center remains setup/not-active, and no fake Premium/live-money action exists. | Run a fresh active sandbox purchase/renewal/restore event, verify an active `user_entitlements` row from `source='revenuecat'`, prove Premium creator-tool unlock, reprove non-Premium denial, and keep live money/tickets/tips/paid content/payouts off |
