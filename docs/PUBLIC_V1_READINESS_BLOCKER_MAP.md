...
| 7. RevenueCat/Google provider proof | Partial, active sandbox entitlement bridge closed for licensed tester | P1 for production monetized launch | No while production Premium/live money claims stay off | Yes for production monetized launch | Play-installed v12, RevenueCat/Google sandbox purchase, restore, real RevenueCat webhook event, backend `user_entitlements.source='revenuecat'` active row, and Platform Studio unlock are proved for the licensed tester. The webhook requires the shared secret, no-secret smoke returns 401, dashboard `TEST` delivery returns no Premium grant, real `INITIAL_PURCHASE` returned `premiumGranted:true`, and sanitized readback shows no raw provider payload storage. Money Center remains setup/not-active, and no fake Premium/live-money action exists. | Reprove non-Premium denial with a separate safe runtime account if available, keep the purchase shell closed unless intentionally opened for reviewer sandbox testing, and keep live money/tickets/tips/paid content/payouts off. Production Premium remains unclaimed until the submitted build/provider release path is approved |
