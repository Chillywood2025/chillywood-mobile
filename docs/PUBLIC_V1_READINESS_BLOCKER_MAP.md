...
June 1, 2026 RevenueCat backend entitlement sync proof at `/tmp/chillywood-revenuecat-backend-entitlement-sync-proof-20260601/` implements the missing webhook bridge but keeps provider proof blocked honestly. `revenuecat-webhook` ACTIVE version 4 now verifies the shared webhook secret, maps real RevenueCat Premium events to `user_entitlements.source='revenuecat'`, writes duplicate-safe `billing_events`, and records sanitized audit with no live-money action. The deployed missing-secret smoke still returns `setup_required` because `REVENUECAT_WEBHOOK_SECRET` is not configured server-side, and sanitized backend readback shows zero Premium rows. Android proof shows `/subscribe` still says `Premium is active`, but Platform Studio still denies with `Premium required`, proving there is no UI-only creator-tool unlock. Remaining provider blocker: configure the RevenueCat webhook secret/dashboard event path, process a real sandbox event, verify the backend row, then prove Premium creator-tool unlock.
...
| 7. RevenueCat/Google provider proof | Partial | P1 for monetized launch | No while production Premium/live money claims stay off | Yes for monetized launch | Play-installed v12, RevenueCat/Google sandbox purchase, and restore are proved. `revenuecat-webhook` ACTIVE version 4 now implements verified backend sync into `user_entitlements`, but the deployed missing-secret smoke still returns setup-required because `REVENUECAT_WEBHOOK_SECRET` is absent. Sanitized backend readback shows zero Premium rows, and Platform Studio still denies with `Premium required` while `/subscribe` shows RevenueCat active. Money Center remains setup/not-active, and no fake Premium/live-money action exists. | Configure RevenueCat webhook secret in Supabase and dashboard, process a real sandbox event, verify an active `user_entitlements` row from `source='revenuecat'`, prove Premium creator-tool unlock, reprove non-Premium denial, and keep live money/tickets/tips/paid content/payouts off |
