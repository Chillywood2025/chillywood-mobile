...
June 3, 2026 real sandbox money blocker update: `revenuecat-webhook` ACTIVE version `8` now mirrors future real Premium RevenueCat events into the shared money-access tables, but no fake sale/event/ledger row was inserted. Premium is the only mapped sandbox digital product; paid content, Watch-Party ticket, Live Watch-Party access, Live Watch-Party seat, tip, and event pass proof are blocked by missing RevenueCat/Google Play products and catalog mappings. Current-main Admin money visuals are now captured from EAS APK build `cc88ce26-6e94-4adb-9768-d0483c12505a` versionCode `22` after explicitly uninstalling Play/EAS v21 because in-place install failed with signature mismatch. Proof at `/tmp/chillywood-real-sandbox-digital-sales-proof-20260603/` shows Product Catalog `8`, Shared Access Grants `0`, Provider Rows `14`, Money Audit Explorer `78` safe events, sandbox/setup/not-payable labels, and sanitized detail; temporary owner/operator roles were revoked. Safety switches have live money, payouts, paid content, tips, merch, tickets, access, and seats off; provider webhook/RevenueCat readiness remains sandbox-only.
