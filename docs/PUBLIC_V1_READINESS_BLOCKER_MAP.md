...
June 3, 2026 Google Play sandbox item availability closeout: the exact internal-test opt-in link fixed tester availability, replacing the prior EAS/internal `installer=null` package with a Play-installed build and updating to Play-delivered versionCode `23` from EAS build `766b8015-cb3a-43ba-910d-fa442a45e9be`. Creator tip `cw_creator_tip_sandbox_099` then completed a real Google Play test-card sandbox purchase through RevenueCat. Remote readback showed one sandbox provider event, one consumed purchase intent, zero access grants, one sandbox-only/not-payable ledger row, zero payable/paid rows, and zero active proof roles after revoke. The remaining blocker is not item availability; it is product-specific resolver proof for Watch-Party tickets, Live Watch-Party access/seat, paid content, and event pass, plus refund/pending/idempotency proof where tooling supports it. No fake sale/provider/ledger row or production money was created.
