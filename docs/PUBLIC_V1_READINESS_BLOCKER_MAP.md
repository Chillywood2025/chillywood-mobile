...
June 3, 2026 sandbox provider product follow-up: the six proposed Google Play products now exist as active one-time products with purchase option `sandbox-099`, and RevenueCat has matching published Play Store consumable products with no entitlements, no offerings, and no transactions yet. Migration `20260603225500_sandbox_digital_product_mappings.sql` is remote-applied and connector readback confirms six sandbox catalog rows with exact provider product IDs and purchase intents enabled. EAS update group `9757451f-7817-488e-9b10-fff68372fefd` published `/admin-money-sandbox-purchases`; Android route proof on `R5CR120QCBF` showed non-operator denial, temporary operator-only sandbox launcher access, and post-revoke denial. A real Creator tip purchase attempt created one sandbox intent, then Google Play returned item-not-found; the intent was marked failed and unconsumed. Remaining blocker moves to Google Play item availability for the signed/tester install, then completed real sandbox purchases must prove provider events, access grants, sandbox not-payable ledgers, resolver behavior, Creator Money Center, and Owner/Admin drilldowns. No fake sales or production money were created.
