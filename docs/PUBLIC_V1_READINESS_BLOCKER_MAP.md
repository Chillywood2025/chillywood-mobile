...
June 5, 2026 creator monetization setup flow update: `docs/CREATOR_MONETIZATION_IN_APP_FLOWS.md` records the repo-side in-app setup layer for approved creators/internal testers. Migrations `20260605000610_creator_monetization_in_app_setup_flows.sql` and `20260605002000_bound_creator_monetization_setup_access.sql` are remote-applied and add sandbox/not-payable creator monetization config rows plus safe creator/admin RPCs bound to owner/operator or active beta/internal tester access. Platform Studio now links to `/creator-monetization-setup`, where approved users can bind real source UUIDs to approved Google Play / RevenueCat sandbox tiers or the Stripe physical merch sandbox tier. Production purchase activation, payouts, cash-out, withdrawal, transfer, payable balances, arbitrary Android digital prices, fake sales, Stripe Android digital checkout, LiveKit publish, host/speaker/mod/admin authority, route ownership changes, and safety bypass remain blocked. Remaining work is Play-installed Android visual/provider proof and future production approval planning, not backend rail construction.
