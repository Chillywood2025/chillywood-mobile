...
June 10, 2026 Chi'lly Chat call notification/ringtone foundation update: `docs/CHILLY_CHAT_CALL_NOTIFICATION_RINGTONE_SYSTEM.md` records the implementation for direct-thread call invites, incoming in-app call sheet, call history cards, call settings, and Android notification channels. Migration `202606100001_chilly_chat_call_invites_and_ringtones.sql` is remote-applied, and post-apply dry-run reports the remote database is up to date. Bundled CC0 Chi'lly Chat call sounds are now present under `assets/sounds/chilly-chat/`, with provenance in `docs/CHILLY_CHAT_SOUND_LICENSES.md`; the active call notification channel is versioned as `chilly_chat_calls_v2` and uses `chilly_ring.wav` after a native/internal Android build. This reduces the Chi'lly Chat call UX blocker but does not fully close production notification proof: two-user Android proof is required, background call push dispatch needs the approved server dispatch path, and bundled background sound proof requires a native build because OTA alone cannot add native notification sound resources. Downloaded/imported sounds remain in-app-only for V1. No LiveKit token issuer, communication room authority, Watch-Party route ownership, Player behavior, auth, Premium/content safety, money state, payouts, Stripe Android checkout, or admin authority changed.

June 10, 2026 live-room wake/back/overlay stabilization update: `docs/LIVE_ROOM_WAKE_LOCK_BACK_OVERLAY_PROOF.md` records the targeted fix for tester-reported Live Stage / Watch-Party Live idle blackout, wrong Back destination, and overlay lock behavior. Watch-Party Live and Live Stage now use focused-screen `expo-keep-awake` activation, so active room surfaces can prevent Android sleep once the Play/internal runtime includes the native module. Back/leave/error paths now return to Watch-Party entry or canonical Party Room context instead of falling through to Home. Live Stage Android Back returns from Stage to the embedded Live Room surface first, the existing 10-second overlay auto-hide remains, tap-to-reveal remains active, and locked controls keep the overlay visible without arming auto-hide. Final device proof still needs a Play/internal build/runtime containing `expo-keep-awake`; this is not fully proved by OTA alone if the installed binary lacks the native module. No LiveKit token issuer, publish authority, host approval, route ownership, Party Room behavior, old-room handling, Premium/content safety, money state, payouts, cash-out, Stripe Android checkout, fake participants, or secrets changed.

June 5, 2026 Shared Player custom fullscreen rails update: `docs/SHARED_PLAYER_CUSTOM_FULLSCREEN_RAILS.md` records the repo-side fix for the repeated shared fullscreen visual mismatch. Watch-Party Live Shared Player fullscreen is now a real three-zone layout in `app/player/[id].tsx`: left dark rail for existing room comments/input/Send, center flex stage for the existing shared player/video surface, and right dark rail for compact participant bubbles from `liveBubbleParticipants`. The fullscreen path no longer mounts the full Watch-Party social card, duplicate LiveKit media surface, `Shared Player` fallback card, or `Shared playback stays here...` fallback copy. Android visual proof still needs an active Premium-capable session; the available local proof account is `premium` status `canceled`, so the app correctly blocks the direct Watch-Party route. No LiveKit token issuer, route ownership, old-room handling, host approval, canPublish logic, money state, payout state, fake participants, fake comments, or secrets changed.

June 5, 2026 Owner/Admin search and permission audit hardening: `docs/OWNER_ADMIN_SEARCH_PERMISSION_AUDIT_HARDENING.md` closes the latest Admin staff-control confusion blocker. Proof on Play-installed `R5CR120QCBF` used EAS Update group `fda01165-2608-4c82-8079-2436f429ad74` / Android update `019e99a0-3b76-7475-a129-cf3d787cd4f1` and captured search results, regular-directory user rows, no-results/clear state, exact selected-target summary, Grant/Revoke confirmation modals, keyboard-safe cancel behavior, scoped permission active/expired/will-change previews, invalid past-expiration copy, protected Owner rules, audit/history readout, cleanup readback, and post-revoke `/admin` denial under `/tmp/chillywood-owner-admin-search-permission-audit-proof-20260605/`. Temporary proof Owner role `39` was revoked after screenshots; final readback shows active temp proof roles `0`, active Owner count `1`, production-enabled configs `0`, payout-enabled configs `0`, and payable/paid rows `0`. No backend authority, self-grant/final Owner protection, production money, payouts, Stripe Android checkout, LiveKit token issuer, route ownership, Premium gates, or content safety changed.

June 5, 2026 device plus emulator live-room sweep: `docs/DEVICE_EMULATOR_LIVE_ROOM_TEST_SWEEP.md` records a targeted attempt to close the two-session Watch-Party/Live Stage blocker with physical device `R5CR120QCBF` plus emulator `emulator-5554`. Physical proof captured Watch-Party Premium gate, Live Stage unavailable state, route-backed Watch-Party ticket / Live access / Live seat gates, `canPublish=false`, no host power, no production money, no payout, and background/foreground recovery under `/tmp/chillywood-device-emulator-live-room-test-sweep-20260605/`. The full two-session host/viewer room proof remains blocked because the available physical session was not Premium-host eligible and the emulator was unstable: Expo dev-launcher state, System UI ANR, package-service failure, and hung current-debug APK install. No LiveKit token issuer, route ownership, old-room handling, host approval, money state, or payout state changed.

June 5, 2026 internal testing stabilization sweep: `docs/INTERNAL_TESTING_STABILIZATION_SWEEP.md` records a Play-installed tester sweep on `R5CR120QCBF` after EAS Update group `4cd86764-44c4-4a93-bd0b-274473b36cdc` / Android update `019e980c-fca8-78db-b44e-6551a6d4d0f4`. Startup, Home, Settings/session, Profile, Explore, Library, Live, Platform Studio gate, Player, comments, Watch-Party/Live Stage gates, creator monetization setup, internal sandbox mode, Premium sandbox mode, payout readiness, Chi'lly Chat inbox, and Admin denial were captured under `/tmp/chillywood-internal-testing-stabilization-sweep-20260605/`. Fixed issues were limited to signed-in Platform Studio gate action copy/routing and missing route-backed proof cards on unavailable room fixture branches. No production money, payout, cash-out, withdrawal, transfer, payable balance, Stripe Android digital checkout, LiveKit authority, route ownership, Premium/content safety, or Owner/Admin authority changed.

June 5, 2026 production money policy operations readiness: `docs/PRODUCTION_MONEY_POLICY_OPERATIONS_READINESS.md` and `docs/PRODUCTION_MONEY_READINESS_INDEX.md` add the legal, tax, fraud/risk, support, refund/return, merch fulfillment, payout operations, and Owner/Admin approval-gate framework for a future production activation lane. This reduces the planning blocker for production approval review, but it does not activate production money, production merch checkout, payout execution, cash-out, withdrawal, transfer, payable balances, Stripe Android digital checkout, LiveKit authority changes, or route ownership changes. Remaining production blockers are external/legal/provider/support/fraud/Owner approvals plus a future explicit activation lane.

June 5, 2026 route-backed monetization visual proof completion: `docs/ROUTE_BACKED_MONETIZATION_VISUAL_PROOF.md` closes the previous contextual viewer-gate/Admin drilldown blocker. Play-installed proof on `R5CR120QCBF` captured route-backed viewer gates for paid content, Watch-Party Live ticket, Live Watch-Party access pass, Live Watch-Party seat pass, and event pass, plus Owner/Admin Money Center readouts for Product Catalog, Provider Events/Webhooks, Purchase Intents, Access Grants, Ledger Events, Merch Products/Orders, Payout Readiness, Money Center Overview, Money Audit Explorer, and Technical Checks. Remote readback still has `live_money_enabled=false`, `payouts_enabled=false`, `cashout_enabled=false`, production/payout/payable/publish/host-power config rows `0`, payable/paid money-access rows `0`, payout requests `0`, provider payout-enabled accounts `0`, merch access grants `0`, and active temp/proof roles `0`.

June 5, 2026 creator monetization setup flow completion: `docs/CREATOR_MONETIZATION_SETUP_COMPLETION_MATRIX.md` records the completed in-app setup matrix for approved creators/internal testers. Migrations `20260605000610_creator_monetization_in_app_setup_flows.sql` and `20260605002000_bound_creator_monetization_setup_access.sql` remain the schema basis. Platform Studio links to `/creator-monetization-setup`, where approved users can bind real source UUIDs to approved Google Play / RevenueCat sandbox tiers or the Stripe physical merch sandbox tier. Play-installed proof saved every setup row; remote readback proves provider events, consumed intents, access grants where appropriate, sandbox/not-payable ledger rows, merch readiness, payout readiness read-only, and zero production/payout/payable/publish/host-power config rows. Production purchase activation, payouts, cash-out, withdrawal, transfer, payable balances, arbitrary Android digital prices, fake sales, Stripe Android digital checkout, LiveKit publish, host/speaker/mod/admin authority, route ownership changes, and safety bypass remain blocked. Remaining work is production approval planning or provider-tooling regression proof, not backend rail construction.

June 5, 2026 creator monetization viewer-gate/Admin QA: `docs/CREATOR_MONETIZATION_VIEWER_GATE_ADMIN_QA.md` captures Android setup/tier/internal-sandbox/merch/payout-readiness screens, a direct paid-content Player route blocker, and correct Admin denial for the non-admin tester. Remote readback confirmed all sandbox provider/setup/readiness counts remained safe. Its visual blocker is superseded by the route-backed visual proof doc above.

June 4, 2026 internal tester sandbox purchase mode update: `docs/INTERNAL_TESTER_SANDBOX_PURCHASE_MODE.md` records the bounded tester-only opening for sandbox purchases and Owner/Admin controls. Public/default purchase surfaces remain closed, but approved testers can run clearly labeled Google Play / RevenueCat sandbox Premium and digital-product tests plus Stripe physical merch sandbox checkout. Owner/Admin Money Center shows `Internal Sandbox Testing` status and tester-tool routing. Payout readiness is read-only: no payout request, simulation, cash-out, withdrawal, transfer, payable balance, or payout activation. This reduces internal-test friction without activating production money, public buy buttons, fake sales, Stripe Android digital checkout, LiveKit authority, route ownership changes, or safety bypass.

June 4, 2026 Standalone Player overlay/fullscreen update: `docs/STANDALONE_PLAYER_OVERLAY_FULLSCREEN_PROOF.md` records the targeted fix for the standalone Player black top/bottom blocks, bottom blur, portrait-only fullscreen, and fullscreen crop/clutter issue. All standalone Player surfaces now mirror the same control layout: video fills the media card, Share / Report / `1x` speed / Watch-Party Live sit as compact top overlays, progress/time/fullscreen sit as compact bottom overlays, and Back sits below the player. Fullscreen suppresses the normal Player framework depth overlay, rotates landscape through the native orientation module, is aspect-preserving, hides Discussion/comments, uses overlay controls, and exits through Android hardware Back. Local release APK versionCode `24` captured proof at `/tmp/chillywood-standalone-player-overlay-fullscreen-proof-20260604/`; the Play-installed v23 app was documented before replacement because native orientation could not be OTA-proved on the portrait-locked binary. This reduces standalone Player visual risk without changing playback, media resolver/access logic, Watch-Party Live CTA, LiveKit token issuer, route ownership, Premium/content safety, money state, or Admin authority.

June 4, 2026 Player/live-room touch polish update: `docs/PLAYER_LIVE_ROOMS_TOUCH_POLISH.md` records the targeted viewer-surface pass after the broader Public V1 visual polish. Player top actions, Player access/unavailable cards, creator-video comments/replies, Watch-Party waiting-room controls, Live Stage unavailable gates, and shared room control primitives now have clearer touch affordance, accessibility labels/roles, disabled/busy states, and cleaner dark mobile styling. The Player media-frame bottom edge now masks the visible source/render artifact without changing playback behavior. EAS Update group `00f95dd5-5e05-4bf7-8256-b74ee374b7ab` / Android update `019e92bd-6642-7cde-9280-e89333c714d4` captured proof at `/tmp/chillywood-player-live-rooms-touch-polish-proof-20260604/`. This reduces viewer-surface polish risk without changing Player playback, media resolver logic, LiveKit token issuer, LiveKit publish authority, Watch-Party/Live Stage route ownership, Party Room behavior, old-room handling, host approval, Premium/content safety gates, money state, or Admin authority.

June 4, 2026 Public V1 visual/touch polish update: `docs/PUBLIC_V1_VISUAL_TOUCH_POLISH.md` records the reusable public/creator UI pattern and first application pass. Home discovery/creator/Rachi rails, public Platform action/section surfaces, login primary action/status, and Platform Studio content empty/error/edit actions now use modern sections, status pills, 44px buttons, improved empty states, and elevated form actions. EAS Update group `3f98fb2e-2cfb-4a13-89e7-b0e32609707f` / Android update `019e9273-c86e-7789-9b1a-6a9aed785f16` captured proof at `/tmp/chillywood-public-v1-visual-touch-polish-proof-20260604/`. Player, Watch-Party Live, and Live Stage remain behavior-preserved/no-change audited surfaces in this lane. This reduces visual consistency risk but does not change backend behavior, Premium gates, content safety, LiveKit, route ownership, money state, or Admin authority.

June 4, 2026 Owner/Admin UI polish update: `docs/OWNER_ADMIN_TABS_UI_UX_POLISH.md` records the reusable collapsible Owner/Admin tab pattern now used in Roles & Permissions, Users, Permission Templates, and Live Cost Guard. The change improves mobile scanability, click affordance, protected-rule readability, and save/reset action placement without changing backend authority, protected Owner rules, money state, LiveKit behavior, route ownership, Player behavior, or terminology. Remaining admin polish is targeted visual QA, not a public-launch blocker by itself.
...
June 4, 2026 Stripe merch/payout readiness update: physical merch now has repo-side sandbox Stripe readiness plus one real Stripe test-mode runtime checkout proof, but production merch and payouts remain blocked. The physical merch path is server-side/operator-only sandbox checkout plus signed Stripe webhook handling, constrained to physical goods and no digital access. Runtime proof produced one sandbox paid/processing merch order, one merch order item, and one processed sanitized Stripe merch event, while digital access grants, RevenueCat/Premium entitlements, payable/paid money-access rows, provider payout-enabled accounts, active proof roles, live money, and payouts stayed `0`/off. The later Stripe Connect proof completed sandbox payout-readiness account/onboarding/status proof, while production payouts, payout activation, cash-out/withdraw/transfer, public merch launch, and any Stripe Android digital checkout remain blocked.

June 4, 2026 Stripe Connect payout-readiness proof update: existing sandbox Connect functions now prove the payout-readiness readout without activating payouts. The upgraded proof account reused a real Stripe test-mode Express connected account, created a sandbox onboarding link with the approved HTTPS Chi'llwood origin, and refreshed provider status to `pending_kyc` / `onboarding_in_progress` with `charges_enabled=false`, `payouts_enabled=false`, details not submitted, and transfers capability inactive. Remote readback kept provider payout-enabled accounts `0`, payout requests `0`, payable/paid creator payout ledger rows `0`, payable/paid money-access rows `0`, payout-readiness access grants `0`, Stripe Connect entitlements `0`, Stripe Connect Premium entitlements `0`, active proof roles `0`, `live_money_enabled=off`, and `payouts_enabled=off`. Public V1 still blocks production payouts, cash-out/withdraw/transfer, provider payout execution, and payout simulation because the provider account is not payout-enabled.
...
June 4, 2026 sandbox money failure/event-pass blocker update: event pass is no longer blocked by a missing backing model for sandbox proof. `has_event_pass_access` now resolves existing `creator_events`, and `cw_event_pass_sandbox_099` completed a real Google Play / RevenueCat sandbox purchase with sandbox-only/not-payable provider, intent, grant, and ledger rows. Duplicate/idempotency remote readback found zero duplicate provider/grant/ledger rows and 0 payable/paid rows; admin revoke proof removed one sandbox Watch-Party ticket grant from resolver access; expired intent proof created no provider event/grant/ledger. Remaining blockers are real provider refund/revoke and delayed-payment pending proof only if Google Play/RevenueCat tooling is available. Production live money, public buy buttons, payouts, cash-out, fake balances, and Stripe Android digital checkout remain blocked/off.

June 3, 2026 money-access blocker update: the shared catalog/access-grants architecture is now repo-backed, remote-applied, type-refreshed, and still setup/readiness-only. Premium is still the only sandbox-proved digital product path and remains backed by RevenueCat/Google Play plus `user_entitlements`; the purchase shell is closed by default. Paid content, Watch-Party tickets, Live Watch-Party access/seat passes, tips, event passes, merch checkout, payouts, and production payable ledgers remain blockers until provider mappings, policy proof, kill-switch approval, refund/revoke handling, and payout readiness are separately closed. The migration passed local lint and remote push/post-apply dry-run; `supabase migration list` remains unavailable in this shell due to linked CLI SASL auth. Android proof paths `/tmp/chillywood-money-access-grants-remote-proof-20260603/`, `/tmp/chillywood-money-center-signed-in-proof-20260603/`, and `/tmp/chillywood-real-sandbox-digital-sales-proof-20260603/` show signed-out denial, signed-in non-Premium/admin denial, temporary upgraded Creator Money Center readiness/not-payable state, current-main Owner/Admin Product Catalog / Provider Rows / Access Grants / Money Audit Explorer readouts, sanitized detail proof, and post-revoke Admin denial.

June 3, 2026 real sandbox money blocker update: `revenuecat-webhook` ACTIVE version `8` now mirrors future real Premium RevenueCat events into the shared money-access tables, but no fake sale/event/ledger row was inserted. At that point Premium was the only mapped sandbox digital product and non-Premium proof was blocked by missing RevenueCat/Google Play products and catalog mappings; later June 3/June 4 lanes closed those sandbox mappings and purchases. Current-main Admin money visuals are captured from EAS APK build `cc88ce26-6e94-4adb-9768-d0483c12505a` versionCode `22` after explicitly uninstalling Play/EAS v21 because in-place install failed with signature mismatch. Proof at `/tmp/chillywood-real-sandbox-digital-sales-proof-20260603/` shows Product Catalog `8`, Shared Access Grants `0`, Provider Rows `14`, Money Audit Explorer `78` safe events, sandbox/setup/not-payable labels, and sanitized detail; temporary owner/operator roles were revoked. Safety switches have live money, payouts, paid content, tips, merch, tickets, access, and seats off; provider webhook/RevenueCat readiness remains sandbox-only.

June 3, 2026 sandbox product mapping blocker update: migration `20260603190000_money_purchase_intents.sql` is remote-applied, typegen refreshed, post-apply dry-run clean, and `revenuecat-webhook` ACTIVE version `9` now requires a pending purchase intent before dynamic non-Premium RevenueCat/Google Play events can create access grants or sandbox not-payable ledger rows. This closes the repo-side generic-product-to-specific-source binding gap. The blocker is now external product setup: create/verify `cw_paid_content_access_sandbox_099`, `cw_watch_party_live_ticket_sandbox_099`, `cw_live_watch_party_access_sandbox_099`, `cw_live_watch_party_seat_sandbox_099`, `cw_creator_tip_sandbox_099`, and `cw_event_pass_sandbox_099` in Google Play and RevenueCat. No real non-Premium sandbox purchase was possible today, and no fake sale fallback was used.

June 3, 2026 sandbox provider product follow-up: the six proposed Google Play products now exist as active one-time products with purchase option `sandbox-099`, and RevenueCat has matching published Play Store consumable products with no entitlements or offerings. Migration `20260603225500_sandbox_digital_product_mappings.sql` is remote-applied and connector readback confirms six sandbox catalog rows with exact provider product IDs and purchase intents enabled. EAS update group `9757451f-7817-488e-9b10-fff68372fefd` published `/admin-money-sandbox-purchases`; Android route proof on `R5CR120QCBF` showed non-operator denial, temporary operator-only sandbox launcher access, and post-revoke denial. The first real Creator tip attempt created one sandbox intent and failed Google Play item availability; the follow-up Play-installed versionCode `23` proof fixed item availability and completed Creator tip through Google Play's test card. Remote readback now shows one sandbox provider event, one consumed purchase intent, zero access grants, one sandbox-only/not-payable ledger row, zero payable/paid rows, and zero active proof roles after revoke. Remaining proof is product-specific resolver proof for Watch-Party tickets, Live Watch-Party access/seat, paid content, and event pass. No fake sales or production money were created.

June 3, 2026 Google Play sandbox item availability closeout: the exact internal-test opt-in link fixed tester availability, replacing the prior EAS/internal `installer=null` package with a Play-installed build and updating to Play-delivered versionCode `23` from EAS build `766b8015-cb3a-43ba-910d-fa442a45e9be`. Creator tip `cw_creator_tip_sandbox_099` then completed a real Google Play test-card sandbox purchase through RevenueCat. Later lanes completed product-specific resolver proof for Watch-Party ticket, Live Watch-Party access/seat, paid content, and event pass, plus idempotency/admin-revoke/expired-intent safety. Remaining money blockers are real provider refund/revoke and delayed-payment pending only if tooling supports them, plus future production approval lanes. No fake sale/provider/ledger row or production money was created.

June 3, 2026 access-product resolver proof closeout: Watch-Party Live ticket, Live Watch-Party access pass, Live Watch-Party seat pass, and paid content access completed real Google Play / RevenueCat sandbox purchases and webhook processing from the Play-installed versionCode `23` build. The backend consumed four new purchase intents, created four sandbox-only access grants, wrote four additional sandbox/not-payable ledger rows, and kept payable/paid rows at `0`. Resolver RPCs proved ticket/access/seat buyers are viewer-only with `canPublish:false` and host approval still required; non-buyers are denied. Paid content resolver proved buyer access, non-buyer denial, and draft-content policy denial before grant checks. Event pass was still blocked in that lane, then closed by the June 4 event-pass backing proof. No fake sale/provider/access/ledger row or production money was created.
Branch at audit start: `main...origin/main`

June 6, 2026 Auth email branding readiness update: Chi’llwood now has repo-tracked Supabase Auth templates under `docs/auth-email-templates/`, exact subject lines, and `docs/SUPABASE_AUTH_EMAIL_BRANDING_RUNBOOK.md`. Signup confirmation now uses `chillywoodmobile://auth/callback` (legacy `chillywoodmobile://auth/confirm` remains compatible); forgot-password passes `chillywoodmobile://reset-password`; root deep-link handling routes reset, auth confirm, auth callback, and legacy auth callback links back into app screens without exposing tokens in analytics. Remaining blocker is external Supabase SMTP/template/redirect allowlist setup and sanitized live email proof. Do not claim branded sender delivery until custom SMTP, templates, and allowlisted redirects are verified in the hosted project.
...
June 1, 2026 Play-installed proof check initially found `installer=null`, but that blocker is superseded by the later internal-test v12 install, RevenueCat mapping proof, and sandbox purchase/restore proof. The current provider blocker is backend entitlement sync from a real RevenueCat webhook event.

June 1, 2026 Google Play API upload readiness check found the API path after a broader local search. The current local AAB is still debug-signed (`CN=Android Debug`) and must not be uploaded, but `artifacts/google-play-proof/chillywood-v12.aab` exists as a non-debug signed candidate with SHA-256 `e256d62de976fbf1b930e5c81cda921f2798ce55f0e4b421139f624e5d2956c1`. Service-account credentials exist outside the repo under `/Users/loverslane/secrets/chillywood/`, and legacy ADC for `chillywood-revenuecat-play@chillywood-app.iam.gserviceaccount.com` can create/read/delete Android Publisher edits. Internal track currently reports completed release `1.0.0` with versionCode `12`; alpha/beta/production are empty. No bundle was uploaded and no edit/track/tester state changed.

June 1, 2026 Play-installed versionCode 12 proof moved the blocker from Play install to app-sign-in/purchase-shell. Proof at `/tmp/chillywood-play-installed-v12-premium-proof-20260601/` shows `R5CR120QCBF` accepted the internal-test invite, installed from Google Play, and now reports `com.chillywood.mobile` with `installer=com.android.vending`, `versionCode=12`, `versionName=1.0.0`, and APK signing version 3. Play Console read-only proof shows internal testing active on release `1.0.0` / versionCode `12` with tester list `Chi'llywood Internal Testers`. RevenueCat dashboard proof confirms product `premium_subscription:monthly`, subscription id `premium_subscription`, base plan `monthly`, entitlement `premium`, offering `premium`, Android app `appd24db94dd8`, and package `com.chillywood.mobile`. Purchase/restore still is not claimed because the reinstall cleared the Chi'llywood app session, no safe app test-account password was available in-session, and the Play-installed v12 build still has `PREMIUM_PURCHASE_SHELL_ON_HOLD = true`. Next blocker is a bounded purchase-shell build/update plus app test-account sign-in; production Premium and all money/tickets/tips/paid-content/payout features remain off.

June 1, 2026 signed-in purchase-shell proof at `/tmp/chillywood-play-installed-premium-sandbox-purchase-proof-20260601/` clears the Play-installed purchase/restore blocker. The Play-installed v12 app opens signed in, owner-approved EAS update group `b678522a-8734-49a1-a582-f2bc6743c756` temporarily opened the Premium shell, Google Play displayed the sandbox `Chi'llywood Premium` subscription with the always-approves test payment method, purchase completed, `/subscribe` showed `Premium is active`, and restore completed with `Purchases restored. Premium is active.` The shell was closed again with EAS update group `82f7e7fd-d213-4f50-9c5d-6e6a328884db`, and current source keeps `PREMIUM_PURCHASE_SHELL_ON_HOLD = true`. The then-open backend entitlement blocker is superseded by the fresh sandbox entitlement proof below.
...
June 1, 2026 Premium reviewer readiness proof at `/tmp/chillywood-premium-reviewer-test-readiness-20260601/` confirms the test posture is ready-but-closed, not production-live. The Play-installed v13 app still resolves from `com.android.vending`, `/subscribe` shows purchase `Temporarily unavailable`, the current EAS production update is closed-shell group `5668cdaa-cd5b-4553-bd91-7b786323fd22`, `revenuecat-webhook` remains ACTIVE version `7`, and Admin Money Center shows `Live money off`. No fresh purchase was run because approved reviewer/app credentials were not available and read-only Play API track/product readback returned `403`; this does not invalidate the earlier licensed-tester sandbox purchase/restore/webhook/backend entitlement/Platform Studio unlock proof. The remaining reviewer-specific step is external: enter safe credentials only in Play Console/App access and explicitly approve a bounded shell-opening update for the review window.

June 1, 2026 Google Play external acceptance tracker at `/tmp/chillywood-google-play-external-acceptance-20260601/` adds `docs/google-play/EXTERNAL_ACCEPTANCE_TRACKER.md` as the current field-level P0 tracker. It confirms repo/operator packets exist for App details, Privacy Policy, App access, Ads, Data Safety, Content Rating, Account deletion, UGC/moderation, financial declarations, permissions, Store listing, release upload, closed/internal testing, reviewer notes, and release notes, but none are claimed submitted or accepted in Play Console. The current local repo AAB is still debug-signed and must not be uploaded. P0 remains external until Play Console fields are saved/submitted/accepted and owner/legal approves the claims.
...
| --- | --- | --- | --- | --- | --- | --- |
| 1. Current release Android AAB/APK build/install/open proof | Partial, current-HEAD local build/install/open route-smoke portion closed; Play-installed v13 proof exists | P1 release signing / diagnostics / broader account-state smoke | No for controlled Android test | Yes until Play-upload signing and final diagnostics are proved | Fresh current-HEAD proof: APK 196M sha256 `abc67ba63c4679ca005d9b3fcb9dc2a5286dd74c48525f1580c7d1ea94f5ed33`; AAB 126M sha256 `57f8f8da17f21959ef7d3f4abb661fad5135757caa277d2b9a03ddec192ad199`; install `Success` on `R5CR120QCBF`; route screenshots in `/tmp/chillywood-current-head-play-upload-proof-20260530/android/`. June 1 device proof shows Play-installed versionCode `13`, versionName `1.0.0`, installer `com.android.vending`. | Local Gradle release is debug-signed (`CN=Android Debug`) and must not be uploaded. Use owner-approved EAS/Play upload signing or corrected release signing before actual Play upload; full signed-out/signed-in route sweep, release diagnostics/log-redaction, Crashlytics/Performance/vitals proof remain |
...
| 7. RevenueCat/Google provider proof | Partial, active sandbox entitlement bridge and non-Premium denial closed for test scope | P1 for production monetized launch | No while production Premium/live money claims stay off | Yes for production monetized launch | Play-installed v12/v13, RevenueCat/Google sandbox purchase, restore, real RevenueCat webhook event, backend `user_entitlements.source='revenuecat'` active row, Platform Studio unlock, and separate non-Premium runtime denial are proved. The webhook requires the shared secret, no-secret smoke returns 401, dashboard `TEST` delivery returns no Premium grant, real `INITIAL_PURCHASE` returned `premiumGranted:true`, sanitized readback shows no raw provider payload storage, and a disposable non-Premium user with zero Premium rows/zero platform roles saw clean `Premium required` copy. EAS production readback shows the current update is closed-shell group `5668cdaa-cd5b-4553-bd91-7b786323fd22`. Money Center remains setup/not-active, and no fake Premium/live-money action exists. | Keep the purchase shell closed unless intentionally opened for reviewer sandbox testing. Fresh reviewer purchase proof is blocked until safe reviewer credentials are entered outside the repo and Play tester/product readback is available or manually confirmed. Keep live money/tickets/tips/paid content/payouts off. Production Premium remains unclaimed until the submitted build/provider release path is approved |
...
| 4. Home / Explore / Library | Partial-ready | P1 | No for closed test | `/tmp/chillywood-home-continue-watching-proof-20260529/`, `/tmp/chillywood-explore-typeahead-admin-search-proof-20260529/`, `/tmp/chillywood-explore-people-search-proof-20260529/`, `/tmp/chillywood-public-v1-blocker-burndown-proof-20260529/`, `/tmp/chillywood-search-typeahead-social-discovery-proof-20260605/` | `app/(tabs)/index.tsx`, `app/(tabs)/explore.tsx`, `app/(tabs)/my-list.tsx`, `_lib/publicPeopleSearch.ts`, `_lib/discoveryFeed.ts` | Search/typeahead modernization is in code; Android capture is pending for social discovery polish path. | Public Discovery Release Smoke |
| 5. Profile / Chi’lly Circle / Chi’lly Chat | Partial | P1 | Yes before broad social launch | `/tmp/chillywood-profile-production-ui-proof-20260525/`, `/tmp/chillywood-profile-viewer-state-proof-20260525/`, `/tmp/chillywood-profile-social-interaction-proof-20260525/`, `/tmp/chillywood-profile-photo-picker-proof-20260529/`, `/tmp/chillywood-search-typeahead-social-discovery-proof-20260605/` | `app/profile/[userId].tsx`, `components/ProfileSocialFeedCard.tsx`, `_lib/profileMedia.ts`, `_lib/friendGraph.ts`, `_lib/chat.ts`, `_lib/socialAttachments.ts`, `app/chilly-circle.tsx`, `app/chat/index.tsx` | Chi’lly Circle local list/search + People suggestions and Chi’lly Chat inbox typeahead are implemented; Android proof capture for search flows remains pending. | Profile/Chat Social Discovery Polish |
