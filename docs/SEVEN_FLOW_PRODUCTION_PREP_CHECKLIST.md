...
Provider dashboard access was not used for this prep pass. Google Play and RevenueCat production readiness is therefore marked `Pending provider verification` unless already proved by repo-local sandbox proof.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Premium | `premium_subscription` | Subscription | Pending provider verification for production; sandbox proof exists | Pending provider verification for production; entitlement `premium` expected | `user_entitlements.entitlement_key=premium` | `/subscribe`, Premium gates, Settings Premium management | App-wide Premium entitlement only | No creator access grant, creator earning, payout, paid video unlock, VIP, ticket, event pass, room authority, or LiveKit publish authority | `revoked_at` and provider entitlement readback block Premium | Store/provider plus app support; no instant refund promise | Keep purchase shell off | RevenueCat customer, Google Play subscription, entitlement row, restore/readback, revoke/readback | Yes | Owner approval and production provider final check |
| Tips | `cw_creator_tip_sandbox_099`; production ID pending | One-time consumable | Sandbox proved; production product pending provider verification | Sandbox proved; production mapping pending provider verification | Ledger/readback only; no durable access | Public Platform tip/support sheet, Money Center | None | No Premium, paid video, ticket, subscription, VIP, event, badge, ranking, LiveKit, payout, or payable balance | No access revoke; support can mark/review local records only | Accidental/duplicate/unauthorized review is manual/external; no instant refund promise | `tips_enabled=off`; global `live_money_enabled=off` | Intent, provider event, ledger/readback, not-payable state, support case | Yes | Production product and owner activation; payouts remain off |
| Paid Video | `cw_paid_content_access_sandbox_099`; production ID pending | One-time consumable | Sandbox proved; production product pending provider verification | Sandbox proved; production mapping pending provider verification | `paid_content_access` grant for one video/source | `/player/[id]` | Exact video/source access only | No Premium, other videos, subscription, VIP, ticket, event, LiveKit authority, payout, or payable balance | Revoke exact target grant; route/readback locks target | Manual/provider review for access failure, early removal, DMCA/removal, or platform fault | `paid_content_enabled=off`; global `live_money_enabled=off` | Paywall, intent, provider event, access grant, content grant, revoke/readback | Yes | Production product and owner activation; content/support readiness check |
| Watch-Party Ticket | `cw_watch_party_live_ticket_sandbox_099`; production ID pending | One-time consumable | Sandbox proved; production product pending provider verification | Sandbox proved; production mapping pending provider verification | `watch_party_live_ticket` grant for one Party Room / Watch-Party target | `/watch-party/[partyId]` | Same-room ticket access only | No Premium, other room, Live Stage route, LiveKit publish, host, speaker, moderator, paid video, VIP, subscription, event pass, payout, or payable balance | Revoke same-room grant/resolver | Manual/provider review for ended/failed/no-show/platform fault | `watch_party_tickets_enabled=off`; global `live_money_enabled=off` | Room id, offer id, intent, provider event, ticket row, grant, revoke/readback | Yes | Production product and owner activation; room operations readiness |
| Channel Subscription | `channel_subscription_sandbox_monthly_499:monthly`; production ID/base plan pending | Subscription | Sandbox base plan proved; production product/base plan pending provider verification | Sandbox product and entitlement `creator_channel_subscription` proved; production mapping pending provider verification | `channel_subscription` grant/subscription state for one creator channel | `/channel-subscription/[creatorId]`, Public Platform subscriber area, Money Center | Subscriber access for one creator channel | No Premium, VIP, paid video, ticket, event, other creator subscription, LiveKit authority, payout, or payable balance | Provider lifecycle/revoke/expiration and effective access resolver | Manual/provider review for missing entitlement, cancellation/expiration, creator inactivity, or subscriber-only access issue | `digital_sales_enabled=off`; global `live_money_enabled=off` | Product/base plan, subscription row, provider lifecycle event, access grant, effective access | Yes | Production subscription/base plan and owner activation; lifecycle smoke |
| VIP | `cw_vip_pass_sandbox_499`; production ID pending | One-time non-consumable | Sandbox proved; production product pending provider verification | Sandbox proved; production mapping pending provider verification | `vip_pass` grant/pass state for one creator | `/vip-pass/[creatorId]`, creator VIP area, Money Center | VIP access for one creator | No Premium, channel subscription, paid video, ticket, event, other creator VIP, LiveKit authority, payout, or payable balance | Revoke exact creator VIP grant/pass | Manual/provider review for missing access, unavailable perks, early removal, or misrepresentation | `digital_sales_enabled=off`; global `live_money_enabled=off` | Creator id, offer id, provider event, VIP pass, access grant, revoke/readback | Yes | Production product and owner activation; support/perks readiness |
| Event Pass | `cw_event_pass_sandbox_099`; production ID pending | One-time consumable | Sandbox proved; production product pending provider verification | Sandbox proved; production mapping pending provider verification | `event_pass` grant/pass for one creator event | `/event/[eventId]`, Public Platform event cards, Money Center | Pass for one event only | No Premium, VIP, subscription, paid video, room ticket, other event, LiveKit authority, payout, or payable balance | Revoke exact event pass; canceled/ended policy still gates | Manual/provider review for canceled/rescheduled/ended/unavailable event | `digital_sales_enabled=off`; global `live_money_enabled=off` | Event id, offer id, provider event, pass row, access grant, cancel/expire/revoke readback | Yes | Production product and owner activation; event operations readiness |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Premium | `premium_subscription` | Same unless owner approves a different product | Subscription | Google Play / RevenueCat | Pending provider verification for active subscription/base plan | Pending provider verification for product, offering/package, entitlement `premium` | Premium offering/package must expose the active Google Play product | `user_entitlements` Premium only | RevenueCat restore plus backend entitlement readback | Manual/provider refund; entitlement revoke/readback | Pending provider verification |
| Tips | `cw_creator_tip_sandbox_099` | Pending | One-time consumable | Google Play / RevenueCat | Pending production one-time product verification | Pending production product/offering/package verification | Tip package only; no entitlement | Ledger/readback only | Not restorable as access | Manual/external; no access revoke | Pending provider verification |
| Paid Video | `cw_paid_content_access_sandbox_099` | Pending | One-time consumable | Google Play / RevenueCat | Pending production one-time product verification | Pending production product/offering/package verification | Paid-content package | Exact `paid_content_access` target | Route/readback restores access from backend grant | Manual/external refund; exact revoke | Pending provider verification |
| Watch-Party Ticket | `cw_watch_party_live_ticket_sandbox_099` | Pending | One-time consumable | Google Play / RevenueCat | Pending production one-time product verification | Pending production product/offering/package verification | Ticket package | Exact `watch_party_live_ticket` target | Route/readback restores access from backend grant | Manual/external refund; exact revoke | Pending provider verification |
| Channel Subscription | `channel_subscription_sandbox_monthly_499:monthly` | Pending | Subscription | Google Play / RevenueCat | Pending production subscription and base plan verification | Pending production product/offering/package and entitlement verification | Creator-channel subscription package | Exact creator `channel_subscription` | Provider lifecycle + backend effective access | Manual/external refund; lifecycle revoke/expire | Pending provider verification |
| VIP | `cw_vip_pass_sandbox_499` | Pending | One-time non-consumable | Google Play / RevenueCat | Pending production one-time product verification | Pending production product/offering/package verification | VIP package | Exact creator `vip_pass` | Backend exact-target readback; provider ownership prevents duplicate purchase | Manual/external refund; exact revoke | Pending provider verification |
| Event Pass | `cw_event_pass_sandbox_099` | Pending | One-time consumable | Google Play / RevenueCat | Pending production one-time product verification | Pending production product/offering/package verification | Event pass package | Exact `event_pass` target | Backend exact-target readback | Manual/external refund; exact revoke/expire | Pending provider verification |
