...
| Permanent purge/de-identification | Pass for controlled production path | The dedicated production enablement lane proves controlled owner/operator single-user purge/de-identification for an expired scheduled-deletion disposable proof account. Batch auto-purge remains disabled/default-off. No broad production auto-purge or legal compliance claim is made. |
...
- Permanent purge/de-identification: Closed for controlled production path. `docs/ACCOUNT_PURGE_DEIDENTIFICATION_POLICY.md`, `docs/ACCOUNT_PURGE_PRODUCTION_RUNBOOK.md`, migration `20260624235742_account_purge_production_enablement.sql`, migration `20260625000344_account_purge_production_audit_readback.sql`, and artifact `/tmp/app-account-purge-production-enable-proof-20260625000935/` prove dry-run eligibility, owner/operator-only single-user purge, active/restore-window/protected/non-admin denial, sanitized audit readback, idempotency, public fail-closed readback, private-feature denial, and batch disabled/default-off behavior. No broad auto-purge job, provider refund, or live-money action is claimed.
