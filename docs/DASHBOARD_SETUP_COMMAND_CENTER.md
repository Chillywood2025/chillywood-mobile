...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Google Play Console | Android app record, testing tracks, subscriptions, Data Safety, content rating, account deletion, listing, Play signing, releases | `https://play.google.com/console` | Product owner + release manager | `app.json`, `eas.json`, legal/support pages, billing helpers, store runbooks | Package verification complete for `com.chillywood.mobile`; EAS build `aedded50-3818-4d7b-8bcf-9bb9c2d36513`, versionCode `2`, was manually uploaded to Internal Testing as `Internal test v2 - RevenueCat Premium setup`; Android Publisher package visibility now returns `200`; subscription `premium_subscription` is still missing and Play subscription create returns `400 FAILED_PRECONDITION` because the developer payments profile is not registered; build `96e60082-00b1-48dc-843b-132ce82a7710` completed for versionCode `3` and EAS submitted it to the internal track only as submission `920c452f-1b9f-401c-94c9-616801684939` | Play service account JSON if EAS submit later; subscription product/base-plan ids | Google Play, RevenueCat, EAS submit credentials | Play service account JSON, screenshots with private account data | Subscription product, internal test, Data Safety, content rating, account deletion URL, AAB upload accepted; package ownership already verified | External Setup Pending | Register the Play developer payments profile, create/activate Google Play subscription `premium_subscription` with monthly $9.99 base plan, then connect the product in RevenueCat. |
