...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Google Play Console | Android app record, testing tracks, subscriptions, Data Safety, content rating, account deletion, listing, Play signing, releases | `https://play.google.com/console` | Product owner + release manager | `app.json`, `eas.json`, legal/support pages, billing helpers, store runbooks | Package verification complete for `com.chillywood.mobile`; EAS build `aedded50-3818-4d7b-8bcf-9bb9c2d36513`, versionCode `2`, was manually uploaded to Internal Testing as `Internal test v2 - RevenueCat Premium setup`; Android Publisher package visibility now returns `200`; build `96e60082-00b1-48dc-843b-132ce82a7710` completed for versionCode `3` and EAS submitted it to the internal track only as submission `920c452f-1b9f-401c-94c9-616801684939`; subscription `premium_subscription` now exists with active monthly base plan `monthly`, billing period `P1M`, and US price `$9.99/month`; physical-device internal-test proof passed no-charge test purchase, restore, cancellation/expiry, and missing-entitlement blocking | Play service account JSON if EAS submit later; subscription product/base-plan ids | Google Play, RevenueCat, EAS submit credentials | Play service account JSON, screenshots with private account data | Subscription product, internal test, purchase/restore/expiry proof, Data Safety, content rating, account deletion URL, AAB upload accepted; package ownership already verified | Premium Proof Passed / External Setup Pending | Complete Data Safety, content rating, listing, account deletion acceptance, and final release smoke. |
| RevenueCat | Store product bridge, Premium entitlement, offerings, purchase/restore customer info | `https://app.revenuecat.com` | Billing owner | `react-native-purchases`, `_lib/revenuecat.ts`, `_lib/monetization.ts`, `_lib/premiumEntitlements.ts`, `/subscribe` | API-proved app/public key/entitlement `premium`/offering `premium`; Android public SDK key is in EAS production env; product `premium_subscription:monthly` exists and is attached to entitlement `premium`; offering `premium` is current; package `$rc_monthly` exists under offering `premium` and is attached to `premium_subscription:monthly` with eligibility `all`; May 15 attached-device proof showed active sandbox Play Store entitlement after purchase, active entitlement after restore, and zero active entitlements after cancellation/test-cycle expiry | `EXPO_PUBLIC_REVENUECAT_ANDROID_PUBLIC_SDK_KEY`, optional dev public key, RevenueCat private/API/webhook keys later | Public SDK key in EAS env; private keys/webhooks in RevenueCat/backend only | Private API keys, webhook secret, receipts, purchase tokens | Offer loads, license tester purchase activates entitlement `premium`, restore works, expired/revoked blocks | Premium Proof Passed / Release Smoke Pending | Preserve mapping and rerun only if RevenueCat/Play product setup changes. |
...
| Expo / EAS Dashboard | Build profiles, Android signing credentials, EAS env, update channels, project ownership | `https://expo.dev/accounts/chillywood2025/projects/chillywood-mobile` | Release manager | `eas.json`, `app.json`, Expo Updates URL, release runbook | Project configured; production AAB build `aedded50-3818-4d7b-8bcf-9bb9c2d36513` completed with versionCode `2` and was manually uploaded to Internal Testing; Android submit profile targets internal track; build `96e60082-00b1-48dc-843b-132ce82a7710` completed for versionCode `3`; EAS submitted versionCode `3` to the internal track only as submission `920c452f-1b9f-401c-94c9-616801684939` | `EXPO_TOKEN`, EAS env vars, Android signing credentials/keystore | Expo/EAS dashboard/credentials/env; GitHub secrets if CI build is used | Expo token, keystore, keystore passwords, credential JSON | Preview build, production AAB, signing verification, internal test upload | External Setup Pending | Install the internal-test build from Play, run release smoke, and keep EAS env secrets out of repo. |
...
- Google Play license tester purchase opens and completes. Passed May 15, 2026 on physical device `R3CXA0DS5JV`.
- RevenueCat customer info shows active entitlement `premium`. Passed through RevenueCat API after the sandbox Play Store purchase.
- Restore works after reinstall/clear state. Restore button proof passed in the same signed-in app session; reinstall/clear-state restore remains optional release smoke.
- Expired, canceled, revoked, or missing entitlement states remain blocked. Passed after Google Play cancellation, test-cycle expiry, RevenueCat active-entitlements count dropping to zero, app Recheck returning `Entitlement Missing`, and Watch-Party showing the Premium-required gate.
...
1. **Google Play app record + package verification**
   ...
   - Proof status: Play app/package verification is complete for `com.chillywood.mobile`; selected signing certificate fingerprint `2B:A7:84:1B:70:6B:E8:69:E8:0F:EA:DA:43:8D:14:33:34:D6:E8:70:99:63:FA:B9:63:94:07:4E:9D:D0:95:43` is verified. Internal testing versionCode `3` is Play-installed and Premium purchase/restore/expiry proof passed.
   - Blocks next: final Play listing/Data Safety/account deletion acceptance and broader release-route smoke.
...
2. **RevenueCat product / entitlement / offering**
   ...
   - Blocks next: only setup regression monitoring and final release smoke; `/subscribe` offer load, purchase proof, restore proof, and expiry blocking have passed on internal testing.
