...
- Stripe Connect Test-Mode Backend Functions are pushed and deployed as backend-only/test-mode functions: account, onboarding-link, account-sync, and webhook flows use internal auth validation where applicable, remain blocked for real provider proof until backend `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` are configured, and add no Admin payout controls, Connect Stripe UI, transfer, payout, checkout session, production Stripe call, or live money behavior
