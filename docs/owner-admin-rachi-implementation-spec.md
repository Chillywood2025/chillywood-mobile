...
- Money Flow Control Plane guard files: `_lib/moneyFlowControl.ts`, `docs/MONEY_FLOW_CONTROL_RUNBOOK.md`, `scripts/proof-money-flow-control.mjs`, `scripts/guard-money-flow-control.mjs`, and `/admin` Money Center > Money Flow Control in `app/admin.tsx`. This registers `money_flow_control` as read-only/foundation guarded scope. It does not create payout release, charges, cashout, production checkout, provider mode switches, fake revenue, fake payable balances, or manual Premium toggles.
