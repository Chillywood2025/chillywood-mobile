...
- Owner/Admin/Moderator authority and tap behavior contract files: `_lib/platformRoleActionMatrix.ts`, `_lib/adminActionRegistry.ts`, `docs/OWNER_ADMIN_MODERATOR_AUTHORITY_AND_TAP_MATRIX.md`, `docs/ADMIN_ACTION_REGISTRY.md`, `docs/MODERATOR_AUTONOMOUS_BOUNDARIES.md`, `scripts/proof-owner-admin-moderator-tap-matrix.mjs`, `scripts/proof-admin-action-registry.mjs`, `scripts/proof-moderator-autonomous-boundaries.mjs`, `scripts/guard-owner-admin-moderator-tap-policy.mjs`, `scripts/guard-admin-action-registry.mjs`, and `scripts/guard-moderator-autonomous-boundaries.mjs`. Every active owner/admin/moderator tap on canonical `/admin` must be registered with role gate, approval level, backing, reason/audit requirements, denial copy, and test ID. Unsupported or high-risk actions must stay read-only, disabled, hidden, or routed through Owner Command / Autonomous Approval. Moderator scope stays exact and case-scoped; Rachi can request/recommend but cannot approve; no god panel or duplicate admin route is allowed.
