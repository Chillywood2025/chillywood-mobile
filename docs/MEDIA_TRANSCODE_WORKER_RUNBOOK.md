...
Source/original object-storage note: Hetzner Object Storage -> R2 private origin is shutdown-ready by active-reference semantics. Trusted copier `media-object-storage-migration` copied/readback verified `16` existing distinct refs, storage-metadata backup/restore passed, and DB batch `media-object-storage-r2-2026-07-10T21-27-45-643Z` migrated `24` copied+verified row refs to private R2 origin bucket `chillywood-media-origin`. Future worker source reads should prefer R2 private origin for migrated metadata, while Hetzner read fallback remains retained during the fallback-retention window. The final zero-ref audit still has `7` raw `media_scan_jobs` history refs, but all `7` are resolved historical/stale with `activeUnresolvedHetznerObjectRefs=0`; Hetzner Object Storage is ready only for owner-controlled shutdown after retention/export. This does not cover Hetzner LiveKit; do not shut down Hetzner LiveKit.
