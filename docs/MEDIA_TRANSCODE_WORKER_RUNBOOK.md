...
Source/original object-storage note: Hetzner Object Storage -> R2 private origin is Partial. Trusted copier `media-object-storage-migration` is deployed and inventory-proved, but future worker source reads should prefer private R2 origin bucket `chillywood-media-origin` only after verified copy/readback/checksum and DB metadata migration, while Hetzner fallback retained during the migration window. Current production still has Hetzner/S3 object-storage refs. The missing backend R2 private-origin write config blocker is cleared, but copy is now blocked before DB metadata update by `source_object_head_failed_404` on the first selected legacy source row even after virtual-host and path-style legacy source fallback. No DB metadata was migrated, no media rows were written by the migration lane, and Hetzner Object Storage is not ready for shutdown. This does not cover Hetzner LiveKit; do not shut down Hetzner LiveKit.
