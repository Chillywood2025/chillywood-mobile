...
Source/original object-storage note: Hetzner Object Storage -> R2 private origin is Partial. Trusted copier `media-object-storage-migration` copied/readback verified `16` existing distinct refs, storage-metadata backup/restore passed, and DB batch `media-object-storage-r2-2026-07-10T21-27-45-643Z` migrated `24` copied+verified row refs to private R2 origin bucket `chillywood-media-origin`. Future worker source reads should prefer R2 private origin for migrated metadata, while Hetzner read fallback remains retained during the migration window. Zero-ref audit still reports `7` unresolved `media_scan_jobs` refs, so Hetzner Object Storage is not ready for shutdown. This does not cover Hetzner LiveKit; do not shut down Hetzner LiveKit.
