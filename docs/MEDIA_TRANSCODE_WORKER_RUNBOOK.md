...
Source/original object-storage note: Hetzner Object Storage -> R2 private origin is Partial. Trusted copier `media-object-storage-migration` is deployed and inventory-proved, but future worker source reads should prefer private R2 origin bucket `chillywood-media-origin` only after verified copy/readback/checksum and DB metadata migration, while Hetzner fallback retained during the migration window. Current production still has Hetzner/S3 object-storage refs. The missing backend R2 private-origin write config blocker is cleared, full reconciliation found `16` existing distinct refs copied/readback verified in R2 private origin, `5` `missing_404` refs, `1` unsupported ref, `9` duplicate row refs, and no permission denials. No DB metadata was migrated because the affected storage-metadata backup/restore gate did not close in this pass, no media rows were written by the migration lane, and Hetzner Object Storage is not ready for shutdown. This does not cover Hetzner LiveKit; do not shut down Hetzner LiveKit.
...
HD ladder policy: do not upscale. A 480p source can generate only 360p/480p; a 720p source can add 720p; a 1080p source can add 720p/1080p. Premium HD token claims, the protected Cloudflare Worker verifier, and the Supabase `premium-media-playback-token` issuer are live/proved for protected Premium rows. The current HD pass generated `4` 720p rows and `1` 1080p row only for HD-capable public-safe sources; City Lights stayed SD-only at 854x480. Android OTA `019f4a0b-efff-71c9-bae5-9198bb001160` carries app resolver integration; installed free/non-Premium fallback is proved, and installed Premium HD playback remains pending for a Premium-active installed session.
...
Automation operator source/proof status: `_lib/mediaAutomationController.ts`, `_lib/mediaAutomationDiscovery.ts`, `_lib/mediaAutomationJobs.ts`, `_lib/mediaAutomationWorkerLoop.ts`, and `scripts/media-automation-cli.mjs` model a future scheduled/batched worker without deploying it. Automation defaults to off, dry-run writes nothing, one-job mode requires a source allowlist, batch mode requires owner approval plus max-batch and backup gates, continuous-limited mode is denied while the scheduled backup/restore gate is open, and continuous-full remains blocked. Proofs `npm run proof:media-automation-controller`, `npm run proof:media-automation-discovery`, `npm run proof:media-automation-batch-planner`, and `npm run proof:media-automation-worker-loop` verify unsafe candidates are excluded, already-audited HLS rows are skipped unless future force is approved, leases are required, audit pass is required before resolver trust, audit failure quarantines and pauses automation, rollback scope is exact, and no production playback switch occurs.

Auto-detect CLI worker model: `_lib/mediaAutomationBatchPolicy.ts` and `scripts/media-automation-cli.mjs` add auto-detect source/proof commands. Normal CLI operation discovers candidates and calculates batch size automatically; manual source ids and manual batch size are not required. `run-auto` requires `MEDIA_AUTOMATION_RUN_CONFIRM=I_UNDERSTAND_AUTO_DETECT_BATCH`, fresh backup/restore, no active unfinished jobs, no unsafe CDN rows, dry-run pass, exact rollback, audit, fallback, and emergency-stop gates. The source/proof build still does not process media or write production rows.

Autonomous operating model: `docs/CHILLYWOOD_AUTONOMOUS_APP_OPERATING_MODEL.md` governs future worker operations. Level 0/1 worker-adjacent work such as eligible media discovery, safe batch sizing, scoped backups, restore drills, public-safe transcode work inside caps, post-write audit, scoped rollback, fallback playback, and telemetry reporting should proceed without owner approval when all gates pass. Level 2 batch automation may proceed only with emergency stop, caps, backup gate, audit, rollback/quarantine, fallback, and reporting active. Level 3/4 boundaries still require owner approval: broad catalog backfill, deployed worker service, cron/scheduler, paid provider/PITR changes, destructive production DB changes, auth/RLS changes, Premium entitlement changes, payout/cashout, public/private exposure changes, app-store release, payment production mutation, legal/compliance, and public marketing claims. Private, Premium, original/master, unscanned, moderation-blocked, missing-source, unsupported, or denied media always stop.

## Scheduled R2 Logical Backup Gate

Status: manual logical backup runner implemented, latest post-HD backup complete, scheduler not deployed. `scripts/run-media-worker-logical-backup.mjs` is disabled/dry-run by default and can write/upload only when manually invoked with `MEDIA_BACKUP_RUNNER_ENABLED=true`, `MEDIA_BACKUP_MODE=write`, a private R2 backup bucket, and `MEDIA_BACKUP_R2_PREFIX=backups/media-worker/`. `MEDIA_BACKUP_EXPORT_MODE=auto|pg_dump|js` supports both the existing dump-tool path and a Node JS SELECT export fallback; when `pg_dump`/`psql` are missing, `auto` resolves to JS and emits `schema.sql.gz` plus `data-media-worker.jsonl.gz`. `MEDIA_BACKUP_DATABASE_SOURCE=linked` uses Supabase CLI linked read-only queries instead of requiring or printing a raw database URL; `MEDIA_BACKUP_DATABASE_URL` remains available for safe URL-based environments. Backup operation is CLI-only from Codex/local terminal. No GitHub Actions workflow, cron schedule, deployed scheduler, continuous worker, queue processor, broad backfill, or playback switch is enabled. Latest backup completed after protected Premium HD row promotion at private R2 prefix `backups/media-worker/2026/07/10/media-worker-logical-20260710T024048-5de12265dded/`; row counts were `media_transcode_jobs=10` and `media_renditions=15`.
