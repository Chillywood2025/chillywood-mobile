...
This document defines Chi'llwood money-risk rules for production readiness while live money remains off.
...
Creator risk signals include new account monetization, sudden sales spikes, high refund rate, high chargeback rate, repeated reports, copyright complaints, suspicious live-room sales, suspicious paid-content access, suspicious merch orders, blocked or banned account history, identity mismatch, KYC incompleteness, tax incompleteness, account takeover signals, device changes, and attempts to move buyers to prohibited payment rails.

## Transaction Risk Signals

Transaction risk signals include repeated failed payments, many small purchases from the same account, unusual purchase velocity, refunds shortly after access, chargebacks, region mismatch where provider data makes that visible, repeated merch shipping issues, many purchases from newly created accounts, and support complaints indicating misleading access claims.

## Allowed Risk Actions

Risk actions may include blocking production activation, pausing payout eligibility, holding funds after activation, revoking monetization, removing content, canceling rooms or events, refunding customers, denying future payouts, escalating to Owner/Admin, requiring additional verification, or documenting a policy violation. Risk actions need audit reasons and should avoid exposing sensitive provider details in public UI.
