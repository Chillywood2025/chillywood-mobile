...
This document defines how Chi'llywood should handle disputes and chargebacks for future production money.
...
Disputes should be triaged by product rail. Digital disputes may involve Google Play, RevenueCat, access logs, content status, room/event status, and support messages. Merch disputes may involve Stripe, order records, shipping or tracking proof, fulfillment notes, return records, and customer messages. Payout disputes may involve creator payout eligibility, refund reserve, and hold rules. Intake should identify the provider deadline and decision owner.

## Evidence Collection

Evidence may include purchase event IDs, purchase intent records, access grants, ledger rows, entitlement state, content or room availability, moderation state, event status, merch order items, fulfillment proof, support messages, refund decisions, and admin audit notes. Evidence should be sanitized for support use and should not expose provider secrets or raw private payloads.

## Decision And Aftermath

Possible decisions include accepting the dispute, issuing a refund, contesting with evidence, revoking access after reversal, placing creator funds on hold after production activation, updating fraud rules, or correcting support copy. Chargeback outcomes should be reconciled to ledger/order records and included in risk review. Sandbox proof should never be treated as disputed real money.
