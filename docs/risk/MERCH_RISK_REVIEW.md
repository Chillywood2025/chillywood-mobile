...
This document defines risk checks for physical merch before and after production launch.
...
Merch risk signals include suspicious order velocity, mismatched shipping and account information, repeated failed payments, high refund rate, damaged-item claims without evidence, many orders to risky addresses, out-of-stock sales, misleading product descriptions, fulfillment delays, suspicious creator-created products, and chargeback patterns. Manual fulfillment increases risk because shipping proof and support process must be disciplined.

## Review Actions

Actions may include holding fulfillment, canceling an order, refunding an order, blocking a product, requiring owner review, pausing a creator's merch ability, limiting quantity, escalating to support, or updating product copy. No action should grant or revoke digital access because merch should not create digital access.

## Evidence

Merch risk review should collect product listing, order status, payment status, fulfillment status, shipping records, tracking information if any, support messages, customer claim evidence, refund records, Stripe event identifiers, and admin audit notes. Provider secrets and raw payloads should not appear in Owner/Admin screenshots or support replies.
