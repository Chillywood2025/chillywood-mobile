...
| Legal / Support / Content Policy | External Setup Pending | `app/privacy.tsx`, `app/terms.tsx`, `app/account-deletion.tsx`, `app/community-guidelines.tsx`, `app/copyright.tsx`, `components/system/support-screen.tsx`, `docs/ACCOUNT_LEGAL_DATA_SAFETY_RUNBOOK.md` | Bundled Privacy, Terms, Account Deletion, Community Guidelines, and Copyright/DMCA pages exist; Settings and Support link to them. Signup now links Terms/Privacy/Guidelines before account creation. Terms/Guidelines/Copyright/Account Deletion now include draft UGC responsibility, moderation rights, user-content license, DMCA/repeat-infringer, liability/disclaimer, and retention language. | Attorney/legal approval, final DMCA agent/contact decision, configured public URLs, support email confirmation, Play Store policy URL proof, Play Data Safety entry acceptance, and release-route proof. | V1 required | Follow `docs/ACCOUNT_LEGAL_DATA_SAFETY_RUNBOOK.md`, then prove signup/legal links in release build. |
...
| Store Readiness | External Setup Pending | Play Store console, runtime config, legal/support URLs, `docs/PLAY_STORE_LISTING_CONTENT_RATING_RUNBOOK.md` | App-side policy pages and Premium UX are ready to point at configured store/legal surfaces. The store listing/content-rating runbook now documents draft copy, app category guidance, asset gaps, screenshot plan, content-rating prep, Data Safety consistency, UGC policy readiness, and the signup acceptance posture. | Store listing approval, Play-ready icon export, feature graphic, screenshots, support/legal URLs, privacy declarations, content rating, target audience, UGC policy answers, attorney/legal approval, billing product setup. | V1 required | Follow `docs/PLAY_STORE_LISTING_CONTENT_RATING_RUNBOOK.md`, then complete Play Console setup and prove in release build/internal testing. |
