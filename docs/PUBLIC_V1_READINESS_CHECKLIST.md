...
| Supabase / RLS | Implemented / Proof Pending | Supabase migrations, `supabase/database.types.ts`, `_lib/supabase.ts`, `docs/SUPABASE_REMOTE_PUBLIC_V1_RUNBOOK.md` | Local and remote migration history through `202604260004` is aligned for creator-video Watch-Party and Public v1 hardening. `livekit-token` is deployed ACTIVE. Local RLS proof covers key creator media, billing, moderation, event, notification, reminder, and room lanes. | Linked DB lint/schema inspection is blocked until approved remote DB login auth is available. Full live Supabase proof is still needed for public/draft/non-owner creator media, Storage API delete/remove, admin writes, entitlement states, and access gates. | V1 required | Follow `docs/SUPABASE_REMOTE_PUBLIC_V1_RUNBOOK.md`; do not push migrations blindly, and keep `supabase/.temp/` uncommitted. |
