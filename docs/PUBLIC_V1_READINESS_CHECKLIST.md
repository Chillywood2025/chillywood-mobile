...
| Live Stage / LiveKit | Implemented / Proof Pending | `app/watch-party/live-stage/[partyId].tsx`, `components/watch-party-live/livekit-stage-media-surface.tsx`, `docs/LIVEKIT_PRODUCTION_READINESS_RUNBOOK.md` | Current main preserves two-device Live First and Live Watch-Party proof; Live Stage remains separate from title/content Party Room. The production domain/TURN/TLS/network proof plan now has a dedicated runbook. | Production DNS/TLS/WebSocket, TURN/firewall, Supabase token-secret, cellular/Wi-Fi, sustained two-phone live/watch retention, audio, and release log proof. | V1 required for live launch | Follow `docs/LIVEKIT_PRODUCTION_READINESS_RUNBOOK.md`, then use a bounded two-phone proof runner. |
