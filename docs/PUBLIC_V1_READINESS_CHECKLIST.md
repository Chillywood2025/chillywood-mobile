...
| Live Stage / LiveKit | Implemented / Proof Pending | `app/watch-party/live-stage/[partyId].tsx`, `components/watch-party-live/livekit-stage-media-surface.tsx`, `_lib/liveEffects.ts`, `components/live/live-effects-sheet.tsx`, `docs/LIVEKIT_PRODUCTION_READINESS_RUNBOOK.md`, `docs/LIVE_LAYOUT_AND_EFFECTS_SYSTEM.md` | Current main preserves two-device Live First and Live Watch-Party proof; Live Stage remains separate from title/content Party Room. The layout foundation makes Live Stage people-first with the Chi'lly Party Members grid visible for host/participants and adds honest effects catalog UI. `Off` is the only real active effect. | Production DNS/TLS/WebSocket, TURN/firewall, Supabase token-secret, cellular/Wi-Fi, sustained two-phone live/watch retention, audio, release log proof, layout proof, request-to-join proof, and effects-foundation proof. | V1 required for live launch | Follow `docs/LIVE_LAYOUT_AND_EFFECTS_SYSTEM.md` and `docs/LIVEKIT_PRODUCTION_READINESS_RUNBOOK.md`, then use a bounded two-phone proof runner. |
