...
| Legal / Support / Content Policy | External Setup Pending | `app/privacy.tsx`, `app/terms.tsx`, `app/account-deletion.tsx`, `app/community-guidelines.tsx`, `app/copyright.tsx`, `components/system/support-screen.tsx`, `docs/ACCOUNT_LEGAL_DATA_SAFETY_RUNBOOK.md` | Bundled Privacy, Terms, Account Deletion, Community Guidelines, and Copyright/DMCA pages exist; Settings and Support link to them. The account/legal/Data Safety runbook now maps current app data categories and manual Play Console steps. | Final legal review, configured public URLs, support email confirmation, Play Store policy URL proof, Play Data Safety entry acceptance. | V1 required | Follow `docs/ACCOUNT_LEGAL_DATA_SAFETY_RUNBOOK.md`, then prove links in release build. |
...
| Store Readiness | External Setup Pending | Play Store console, runtime config, legal/support URLs, `docs/PLAY_STORE_LISTING_CONTENT_RATING_RUNBOOK.md` | App-side policy pages and Premium UX are ready to point at configured store/legal surfaces. The store listing/content-rating runbook now documents draft copy, app category guidance, asset gaps, screenshot plan, content-rating prep, Data Safety consistency, and UGC policy readiness. | Store listing approval, Play-ready icon export, feature graphic, screenshots, support/legal URLs, privacy declarations, content rating, target audience, UGC policy acceptance, billing product setup. | V1 required | Follow `docs/PLAY_STORE_LISTING_CONTENT_RATING_RUNBOOK.md`, then complete Play Console setup and prove in release build/internal testing. |
