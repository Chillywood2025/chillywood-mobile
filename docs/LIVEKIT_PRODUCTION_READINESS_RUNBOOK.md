...
## July 10/11, 2026 Global LiveKit Router Eligibility Recovery

The latest installed LiveKit failure was global router eligibility, not R2/media storage, Premium, or just the Live tab entry point. `livekit-token` was deployed and fail-closed correctly, but requests for LiveKit-backed surfaces could not receive usable token contracts because the router returned `no_eligible_livekit_server`. This affected every surface that depends on the routed token boundary: Live Stage, Watch-Party Live / Shared Player camera seats, Party Room live camera sidecar paths, and `chat-call` if that surface uses `livekit-token`.

Production readback classified the exact rejection as `stale_heartbeat`. `chillywood-prod-01` was still registered `active`, `wss://live.chillywoodstream.com` was reachable, Docker/Caddy/LiveKit containers were running, and capacity/CPU/RAM/packet-loss limits were not the blocker. The heartbeat monitor service on the host was running, but its call to the Supabase heartbeat monitor function returned `NOT_FOUND_FUNCTION_BLOB`, so legitimate heartbeats stopped refreshing `livekit_servers.last_heartbeat_at` and the router correctly rejected new token requests after the 120-second stale cutoff.

Repair was operational and scoped: redeploy the existing `livekit-heartbeat-monitor` Edge Function source, then run the legitimate host heartbeat monitor path once from `chillywood-prod-01`. No stale-heartbeat cutoff was loosened, no fake heartbeat rows were inserted, no unhealthy server was marked healthy, no LiveKit API secrets or participant tokens were printed, and no LiveKit server restart was required. Post-repair readback showed fresh `livekit-heartbeat-monitor` heartbeats, `chillywood-prod-01` eligible with heartbeat age under the 120-second cutoff, and no fresh `no_eligible_livekit_server` audit blockers.

Installed Play proof on `R5CR120QCBF` used the approved Google Play / RevenueCat sandbox flow to renew Premium, then entered Live Stage through the Live tab path. The app received a `live-stage` token contract with `participantRole=host`, `room_join=true`, `can_subscribe=true`, and `can_publish=true`; the Stage rendered a visible local camera surface with `shouldRenderLiveKitStage=true` and `publishLocalStageCamera=true`. This closes the one-device installed Live Stage proof for the router-recovery lane. Fresh installed Watch-Party Live / Party Room sidecar two-device proof remains separate; historical token audit rows for `watch-party-live` and `chat-call` were successful, and all surfaces share the repaired router eligibility path.

Do not describe this incident as a Live tab button regression. The Live tab is only one entry path. Future LiveKit proof must check router eligibility and token issuance across all routed LiveKit surfaces before claiming closure. Do not touch R2/storage or Premium entitlement logic for this class of failure.

## July 11, 2026 LiveKit Autonomous Reliability Loop

The LiveKit Autonomous Operator is now active as a controlled reliability loop scoped to LiveKit health and recovery state only. It covers Live Stage, Watch-Party Live / Shared Player, Party Room live camera sidecar paths, `chat-call` when routed through `livekit-token`, the token issuer, router, heartbeat monitor, and host-agent recommendations. The operator classifies stale heartbeat, no eligible server, missing function blob, token issuer unavailability, websocket or host service failure, stopped heartbeat monitor, stale/full capacity counters, token `nbf` skew, app token-validation regressions, renderable-contract clearing, fallback roster flash, surface mount regressions, roster/bubble-grid regressions, identity mismatch, and camera-track gaps.

Allowed autonomous repair remains narrow: legitimate heartbeat monitor execution, heartbeat monitor service/timer recovery when host proof shows it stopped, counter refresh through the monitor path, scoped stale-assignment cleanup planning, affected-surface pause/reporting, audit/recovery event writes, and source/CLI recommendation for a known missing Edge Function blob. It must not write fake heartbeats, loosen stale cutoff thresholds, mark a server healthy without host proof, rotate LiveKit/TURN secrets, change routing policy beyond approved bounds, replace servers, perform destructive DB work, or mutate billing/provider/Premium state.

Client render/token telemetry is now part of the loop. Authenticated app users can submit only sanitized `render_event_ingest` events; they cannot trigger recovery or directly mark health. Events include token received, bounded `nbf` grace used, future-`nbf` rejected, expired token rejected, renderable contract set/preserved/cleared, surface mount, fallback roster shown/suppressed, camera preparing, camera track present, bubble grid rendered, identity mismatch, and connection-state changes. The July 11 JWT `nbf` grace class is now detectable: +0-5 second `nbf` skew is normal grace, while materially future `nbf` is `token_time_skew_blocker`.

`livekit-operator` is token-gated with `LIVEKIT_OPERATOR_TOKEN_SHA256`; operator tables are RLS-enabled with client writes revoked. Manual `watch_once` activation proof returned `healthy` / `eligible_server_available`, `eligibleServerCount=1`, planned `audit_only`, and `executionStatus=not_executed`; no recovery was needed. Missing operator token returns `401 operator_token_required`, and unauthenticated render telemetry returns `401 authenticated_user_required`. Continuous scheduling is active through `chillywood-livekit-operator-watch-once.timer` on `chillywood-prod-01`, which runs every five minutes, uses systemd privilege restrictions, has no service-role key, and uses only the narrow operator token in `/etc/chillywood/livekit-operator.env`. GitHub Actions remains a prepared template at `ops/livekit-operator/github-actions/livekit-operator-reliability-loop.yml` because the available GitHub OAuth credential cannot push workflow files without `workflow` scope. The Cloudflare Cron Worker template remains inactive because live schedule attachment was blocked by Cloudflare error `10063` until a Workers subdomain exists.

The policy guard fails if the operator gains broad DB mutation, logs participant tokens/secrets, ignores `no_eligible_livekit_server`, confuses Shared Player proof with Live Stage proof, lets client telemetry trigger recovery directly, permits fake heartbeat writes, loosens stale cutoff, or downgrades Level 3/4 actions to autonomous repair.

Watch-Party Live Shared Player now smooths the transient bubble transition: when a same-room non-expired renderable contract exists or token/authority refresh is actively progressing, the app keeps a stable in-surface LiveKit connecting/camera-preparing shell for a 1600 ms grace window instead of flashing the separate fallback roster. Hard room errors, expired contracts, and room mismatches still fall back safely. This does not change fullscreen rails, LiveKit backend routing, Premium, R2, storage, or billing behavior.

## July 7, 2026 Watch-Party Live Source-Truth Follow-Up

Source audit found the remaining Watch-Party Live sidecar/shared-player failure was not Premium, Party Room, or LiveKit server liveness. The repair keeps the Shared Player fullscreen rails unchanged while tightening source truth around the sidecar handoff: Party Room and Player entry now share the same canonical Premium access-key helper, installed proof can distinguish real remote Home media from bundled/proof/fixture fallback, Watch-Party Live token contracts must match desired host/speaker canPublish authority before publish-ready UI, camera-seat requests carry request versions so duplicate pending events do not reopen an X-closed review, pending approval stays on one stable host review path, and participant-specific LiveKit bubbles require identity-matched tracks. `npm run proof:watch-party-seat-request` imports the real helper module instead of a duplicate fake model. Source commit `44d776fe117386d3cd0a3400d488ed82e11c8bf6` is on `origin/main` and OTA-published to Android production runtime `1.0.0` as update group `d4c9fe3a-1f92-4fa6-8252-5920aecc59b4`, Android update `019f3d20-16a3-772b-9b4e-870e4d9deeeb`.

This is a Watch-Party Live source/guard/proof lane only. It does not change LiveKit routing, heartbeat, stale cutoff, server registry, Premium gates, Watch-Party Party Room behavior, Android App Links, Chi'lly Chat/native calls, auth/RLS, billing/provider settings, payout, or cashout behavior. Installed proof is still required before public production readiness: use Play-installed v80 or newer, latest OTA, Premium-active sandbox testers, strict real non-fixture Home media, same Party Room/shared-player path on both devices, actual playback, identity-safe bubbles, host-only playback authority, viewer social controls, camera request/approval, and unchanged fullscreen rails.
