...
Backend LiveKit routing was healthy in the latest installed attempt: `eligibleServerCount=1`, heartbeat age below `staleHeartbeatSeconds=120`, `chillywood-prod-01` healthy, and fresh redacted `live-stage:success` token audit. The installed attempt proved R3 could complete the approved Google Play / RevenueCat sandbox Premium flow, create live room `T7S75E`, tap `Continue to Live Stage`, and reach `LIVE STAGE` as host. It still did not reach two-device Stage / `2 in room` because R5 remained non-Premium and was blocked at the Premium-required Live Stage gate with R5 sandbox setup unavailable. Treat this as a second-device Premium sandbox setup blocker for installed Stage proof, not a LiveKit router/heartbeat regression and not a reason to loosen routing eligibility.

Future Stage closure should use two Premium-active Play-installed devices and prove only the missing installed UX steps: viewer requests a seat, host dismisses safely, viewer requests again, host approves, both reach Stage / `2 in room`, viewer toggles self-hero locally, real host appears first in the party box, and host roles/controls remain unchanged.
