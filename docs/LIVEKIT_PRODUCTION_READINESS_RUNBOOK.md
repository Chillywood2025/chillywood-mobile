...
`livekit-operator` is token-gated with `LIVEKIT_OPERATOR_TOKEN_SHA256`; operator tables are RLS-enabled with client writes revoked. Manual `watch_once` activation proof returned `healthy` / `eligible_server_available`, `eligibleServerCount=1`, planned `audit_only`, and `executionStatus=not_executed`; no recovery was needed. Missing operator token returns `401 operator_token_required`, and unauthenticated render telemetry returns `401 authenticated_user_required`. A scheduled-loop template exists at `ops/livekit-operator/github-actions/livekit-operator-reliability-loop.yml`, but it is not installed in `.github/workflows` yet because the available GitHub OAuth credential cannot push workflow files without `workflow` scope. Do not claim continuous scheduled operation until that workflow is installed and a scheduled run is proved.
