...
The current Live Stage room UX source lane is fixed but not installed-closed. Follow-up source commit `3f49af76d50897947ac1a19bec4def2f22300875` supersedes the earlier `50db5cabf237b42d269aac15f45120ebcb983a03` self-hero pass by fixing the installed manual regressions: default host-hero layout includes viewer/self in the party box, self-hero uses immediate local camera/avatar/initials fallback instead of `Live feed is syncing`, the real host remains first in the party box during self-hero mode, and host pending-request card tap opens the seat sheet without hiding/removing the participant card. EAS Update production Android runtime `1.0.0` published group `860e3d56-c894-478a-92dc-7a0d2a0345de`, Android update `019f3526-e2bc-77a9-b3f3-27ab50b867a4`.

Backend LiveKit routing was healthy after the follow-up OTA: `eligibleServerCount=1`, `noEligibleServerCountRecent=0`, heartbeat age below `staleHeartbeatSeconds=120`, `chillywood-prod-01` healthy, and no rejection reasons. Both R5 and R3 can complete the approved Google Play / RevenueCat sandbox Premium flow and read back `Premium is active.`, but installed proof still did not reach two-device Stage / `2 in room` because the approximately five-minute Google Play sandbox subscription renewal window expired during navigation. R5 created live room `TT8NTT`; R3 found it and reached `Join Now`; then R3 hit the Premium-required gate after expiry, and R5 later hit the Premium-required gate before `Continue to Live Stage`. Treat this as a sandbox proof-window timing blocker for installed Stage proof, not a LiveKit router/heartbeat regression and not a reason to loosen routing eligibility.

Future Stage closure should use two simultaneously Premium-active Play-installed devices and prove only the missing installed UX steps: default viewer host-hero layout shows self in the party box, self-hero toggles instantly without `Live feed is syncing`, real host appears first in the party box, toggle-off returns to host-as-hero, viewer requests a seat, host opens the sheet by tapping the stable participant card, X close preserves request/card, `Not now` clears the request while keeping the participant visible, viewer requests again, host approves, both reach Stage / `2 in room`, and host roles/controls remain unchanged.
