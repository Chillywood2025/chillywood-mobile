...
## July 10/11, 2026 LiveKit Autonomous Operator Source Lane

The follow-up operating layer is the LiveKit Autonomous Operator, which is scoped to LiveKit health and recovery state only. It covers Live Stage, Watch-Party Live / Shared Player, Party Room live camera sidecar paths, `chat-call` when routed through `livekit-token`, the token issuer, router, heartbeat monitor, and host-agent recommendations. The operator classifies stale heartbeat, no eligible server, missing function blob, token issuer unavailability, websocket or host service failure, stopped heartbeat monitor, stale/full capacity counters, render-surface flicker, missing render contracts, identity mismatch, and camera-track gaps.
...
`livekit-operator` is token-gated with `LIVEKIT_OPERATOR_TOKEN_SHA256`; operator tables are RLS-enabled with client writes revoked. The policy guard fails if the operator gains broad DB mutation, logs participant tokens/secrets, ignores `no_eligible_livekit_server`, confuses Shared Player proof with Live Stage proof, or downgrades Level 3/4 actions to autonomous repair.

Deployment note: migration `20260711043323_livekit_autonomous_operator` is applied and `livekit-operator` is deployed. Secret names `LIVEKIT_OPERATOR_TOKEN_SHA256`, `LIVEKIT_HEARTBEAT_MONITOR_FUNCTION_URL`, and `LIVEKIT_HEARTBEAT_MONITOR_SECRET` are present. A one-time ephemeral operator token proved `health_snapshot` can read router state and write the scoped snapshot path, returning `healthy` / `eligible_server_available`; the token hash was then reset to a disabled non-granting value. Future live operator use requires rotating `LIVEKIT_OPERATOR_TOKEN_SHA256` to the hash of an owner-held token stored outside git.
