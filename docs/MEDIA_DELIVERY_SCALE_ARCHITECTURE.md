...
Hetzner Object Storage migration status: shutdown-ready by active-reference semantics. Trusted backend copier `media-object-storage-migration` is deployed with operator-token hash auth. Initial copier inventory found `31` Hetzner/S3 object-storage row refs (`22` distinct object refs); `16` existing distinct refs were copied/readback verified in R2 private origin, and storage-metadata backup/restore passed at `backups/media-object-storage/storage-metadata-2026-07-10T21-26-24-644Z/`. Transactional DB batch `media-object-storage-r2-2026-07-10T21-27-45-643Z` migrated `24` copied+verified row refs to `cloudflare_r2` / `chillywood-media-origin`; new uploads now use R2 private origin with Hetzner writes disabled and read fallback retained. Final stale-ref resolution audit reports `7` raw `media_scan_jobs` history refs, all resolved historical/stale, with `activeUnresolvedHetznerObjectRefs=0` and shutdown readiness true after owner-controlled retention/export. No Hetzner objects were deleted, no source/original object is public, and Hetzner LiveKit is separate; do not shut down Hetzner LiveKit, `chillywood-prod-01`, or `live.chillywoodstream.com`.
