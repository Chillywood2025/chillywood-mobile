...
Premium HD token mode is source/proof foundation only. `_lib/mediaPremiumCdnToken.ts` models short-lived token claims scoped to viewer, source, rendition label, and protected path; `_lib/mediaPlaybackCdnEligibility.ts` can accept token mode only for `visibility=premium` 720p/1080p HLS rows under `playback/premium/` or `playback/protected/premium/` with active Premium entitlement proof and a tokenized URL signer. Without the signer, HD falls back/blocks. This does not generate 720p/1080p media, does not expose Premium HD through unsigned `playback/public/`, and does not change current production playback.

Protected Premium HD delivery architecture: `workers/premium-media-access/worker.mjs` is the chosen source/proof path. The future Cloudflare Worker sits in front of the protected Premium R2 prefix, verifies compact HMAC tokens using placeholder env `PREMIUM_CDN_TOKEN_SECRET`, checks `premiumEntitlement=true`, user/session scope, source id, rendition label, expiry, protected path prefix, and denied path segments, then proxies only the matching R2 object through `PREMIUM_MEDIA_R2_BUCKET`. It denies missing/expired/wrong-source/wrong-path/wrong-rendition/non-Premium/private/original/unscanned/moderation-blocked requests, redacts token values in logs, and is not deployed. `npm run proof:premium-cdn-worker` proves the Worker allow/deny matrix and mock R2 proxy behavior without printing tokens or serving real HD media.
...
- The protected Premium Worker source is not live. HD generation still requires future source-aware transcode, protected-prefix upload, trusted rows, Worker deployment, token issuance from backend entitlement proof, installed playback proof, fallback proof, and rollback proof before Premium HD can be activated.
