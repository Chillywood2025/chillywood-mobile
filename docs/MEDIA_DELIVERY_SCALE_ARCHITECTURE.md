...
Scan automation status: source/proofed and no production writes. `_lib/mediaScanAutomation.ts`, `scripts/media-scan-cli.mjs`, `npm run media-scan:status`, `npm run media-scan:plan`, `npm run media-scan:dry-run`, and `npm run proof:media-scan-automation` classify public scan candidates, build a one-item scan plan, validate scanner results, and prove ffprobe media-readability. The scanner proof is explicitly not malware scanning or content moderation. Private/Premium rows are skipped and redacted, already audited HLS rows are skipped, unscanned rows cannot become transcode-eligible without clean scanner proof plus moderation-safe readback, and `media-scan:run-one` is fail-closed in this source/proof build even with confirmation. Existing trusted scan writes must use service-role scanner authority and existing scan RPC/policy; clients cannot mark media clean.
