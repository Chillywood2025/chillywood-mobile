...
Media automation operator status: source/proof/templates only, not deployed. `_lib/mediaAutomationController.ts`, `_lib/chillywoodAutonomyPolicy.ts`, `_lib/mediaAutomationDiscovery.ts`, `_lib/mediaAutomationBatchPolicy.ts`, `_lib/mediaAutomationBackfillPolicy.ts`, `_lib/mediaAutomationJobs.ts`, `_lib/mediaAutomationQueueProcessor.ts`, `_lib/mediaAutomationWorkerLoop.ts`, `scripts/media-automation-cli.mjs`, disabled templates under `ops/media-automation/systemd/`, and `docs/MEDIA_AUTOMATION_OPERATOR_RUNBOOK.md` define the scale automation path for Level 0-4 autonomy policy, scheduled-backup-aware mode control, eligible public-safe discovery, adaptive batch sizing, capped backfill policy, dry-run job planning, lease-bound queue/worker loop, post-write audit, R2/HLS output policy, rollback/quarantine, telemetry event shapes, report-only status, and CLI-only batch controls. Automation defaults to `off`; `dry_run` writes nothing; `auto_detect` discovers/plans; `auto_detect_run` runs only after confirmation and gates; `continuous_limited` remains blocked unless scheduled backup/restore, telemetry, retry/dead-letter, max concurrency, max jobs/run, audit, rollback, output/cache validation, and unsafe-row gates pass; `continuous_full_blocked` remains blocked. `MEDIA_AUTOMATION_RUN_CONFIRM=I_UNDERSTAND_BATCH_AUTOMATION`, `MEDIA_AUTOMATION_RUN_CONFIRM=I_UNDERSTAND_AUTO_DETECT_BATCH`, `MEDIA_AUTOMATION_CONTINUOUS_ONCE_CONFIRM=I_UNDERSTAND_ONE_CONTINUOUS_LIMITED_CYCLE`, and `MEDIA_AUTOMATION_BROAD_BACKFILL_CONFIRM=I_UNDERSTAND_BROAD_BACKFILL_RISK` are the future execution confirmations. The current CLI still fails closed for run execution in this source/proof build. No live daemon, cron, scheduler, queue processor, production media processing, production DB writes, broad backfill, or production playback broadening is live.

Auto-detect CLI automation status: source/proofed, not deployed as a daemon. `_lib/mediaAutomationBatchPolicy.ts` and updated CLI commands make normal operation automatic for eligible public-safe videos: `media-automation:discover` finds candidates, `media-automation:plan-auto` calculates safe batch size, `media-automation:dry-run-auto` builds exact job/rollback plans, `media-automation:report` summarizes safe status, and `media-automation:run-auto` remains fail-closed unless `MEDIA_AUTOMATION_RUN_CONFIRM=I_UNDERSTAND_AUTO_DETECT_BATCH` plus backup/restore, active-job, unsafe-row, dry-run, audit, rollback, fallback, and emergency-stop gates pass. `media-automation:run-continuous-once` is a bounded future one-iteration command that remains fail-closed in this source/proof build unless `MEDIA_AUTOMATION_CONTINUOUS_ONCE_CONFIRM=I_UNDERSTAND_ONE_CONTINUOUS_LIMITED_CYCLE` and every gate passes. Manual source ids and manual batch size are not required for normal auto mode. Batch size starts at `1`, can grow after clean runs, drops to `1` after failure, reduces or blocks under CPU/disk pressure or high error rate, and is `0` when backups/restores are stale, active unfinished jobs exist, unsafe CDN rows exist, or no eligible candidate exists. Private, Premium, original/master, unscanned, moderation-blocked, missing-source, unsupported, active-job, denied, and already-processed candidates are excluded. No cron, scheduler, daemon, queue processor, production media processing, production DB writes, broad backfill, or global playback switch is live.

Catalog readiness automation status: source/proofed and read-only. `_lib/mediaCatalogReadiness.ts`, `scripts/media-catalog-readiness-cli.mjs`, `docs/MEDIA_CATALOG_READINESS_RUNBOOK.md`, `npm run media-catalog:status`, `npm run media-catalog:readiness-plan`, `npm run media-catalog:scan-plan`, and `npm run proof:media-catalog-readiness` classify catalog rows before transcode. Linked readback reports `ready_for_transcode=0`, `already_audited_hls=1`, `needs_scan=5`, `private_excluded=12`, `premium_excluded=9`, and zero moderation-review, original/master, missing-source, unsupported, blocked, or denied rows. The `needs_scan` rows are scan candidates only; they cannot become transcode candidates until trusted scanner proof and moderation-safe readback promote them. The readiness CLI does not execute scans, mark media clean, write production rows, process media, upload media, or switch playback, and it redacts private URL-like values.

Scan automation status: source/proofed and no production writes. `_lib/mediaScanAutomation.ts`, `scripts/media-scan-cli.mjs`, `npm run media-scan:status`, `npm run media-scan:plan`, `npm run media-scan:dry-run`, `npm run media-scan:run-auto`, `npm run proof:media-scan-automation`, and `npm run proof:media-scan-auto-cycle` classify public scan candidates, build one-item or bounded auto-scan plans, validate scanner results, and prove ffprobe media-readability. The scanner proof is explicitly not malware scanning or content moderation. Private/Premium rows are skipped and redacted, already audited HLS rows are skipped, unscanned rows cannot become transcode-eligible without clean scanner proof plus moderation-safe readback. Trusted private scan-download access for S3/Hetzner candidates is source-wired through the existing backend `media-storage create_download_url` signer using `MEDIA_SCAN_DOWNLOAD_ACCESS_TOKEN`; raw local service-role and S3 keys are not required for that download path, and signed URLs must never be printed. Current runtime is blocked at `trusted_scan_download_access_missing` because no token was present; trusted scan result writes still require service-role scanner authority and existing scan RPC/policy, and clients cannot mark media clean.
