...
Hetzner Object Storage migration status: Partial. Private R2 origin bucket `chillywood-media-origin` exists, source defines the fail-closed config contract for future private source/original uploads through `MEDIA_ORIGIN_PROVIDER=cloudflare_r2`, `MEDIA_ORIGIN_PRIVATE_ONLY=true`, and `MEDIA_ORIGIN_PUBLIC_PLAYBACK_DISABLED=true`, and trusted backend copier `media-object-storage-migration` is deployed with operator-token hash auth. Live copier inventory still found `31` Hetzner/S3 object-storage reference rows (`22` distinct object refs), so shutdown is not ready. Copy stopped before mutation because R2 private-origin write config is missing in backend secrets. No media was processed, no media rows were written, no objects were copied, and Hetzner fallback retained. Hetzner LiveKit is separate; do not shut down Hetzner LiveKit, `chillywood-prod-01`, or `live.chillywoodstream.com`.
