...
Trusted backend migration dry-run status: `npm run proof:media-rendition-migration-dry-run` passes static SQL validation in the current shell. Local/shadow apply and live RLS role simulation are pending because Docker/local Supabase/Postgres, `psql`, and `MEDIA_RENDITION_DRY_RUN_DATABASE_URL` are not available; the script refuses production-looking URLs and can run temporary-database RLS checks only when a safe local/shadow URL is supplied. Production migration is not applied and production playback remains unchanged.
...
- Trusted backend migration dry-run proof is static-only in this shell; local/shadow apply and RLS role simulation remain pending until a safe local/shadow Postgres URL is provided.
