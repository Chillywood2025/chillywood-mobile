...
Trusted backend migration path status: `docs/MEDIA_TRANSCODE_RENDITION_MIGRATION_PLAN.md`, draft migration `supabase/migrations/20260709033207_trusted_media_transcode_renditions.sql`, and `npm run proof:media-rendition-migration-policy` design and statically prove the server-owned `media_transcode_jobs` plus `media_renditions` path. The draft migration is not applied; production DB writes, production rendition writes, production transcode worker, and production playback remain unchanged.

Trusted backend migration dry-run status: `npm run proof:media-rendition-migration-dry-run` passes static SQL validation and runtime apply/RLS checks in an in-memory disposable local Postgres runtime via `@electric-sql/pglite`. The proof uses no network DB URL, no production Supabase project, and no production service-role key; it applies the draft migration, verifies tables/indexes/RLS/policies/grants, proves anon/authenticated trusted writes are denied, proves service-role/worker writes pass, proves resolver-safe anon select returns one clean public-ready row, proves unsafe/original/Premium/private/non-public-prefix rows cannot become public CDN eligible, and proves production-looking DB URLs are refused. Production migration is not applied and production playback remains unchanged.
...
- Trusted backend migration path is designed and proofed with a draft, unapplied SQL migration for server-owned `media_transcode_jobs` and `media_renditions`; clients cannot write trusted readiness, public-safe, path, worker version, or source hash fields.
- Trusted backend migration dry-run proof now passes in the embedded disposable local Postgres runtime; a real Supabase staging/shadow apply still requires explicit owner approval and a safe non-production database.
...
- Production trusted `video_renditions` or replacement rendition-metadata writes remain planned; the current trusted rendition metadata foundation and trusted backend migration path are source/proof-only unless the owner explicitly approves applying the draft migration.
...
- `docs/MEDIA_TRANSCODE_RENDITION_MIGRATION_PLAN.md` and draft migration `supabase/migrations/20260709033207_trusted_media_transcode_renditions.sql` define the future server-owned tables. This draft creates `media_transcode_jobs` for worker lifecycle state and `media_renditions` for trusted playback metadata, but it has not been applied and does not switch playback.
- The draft RLS contract revokes client writes, grants worker writes through `service_role`, and adds explicit false insert/update/delete policies for `anon` and `authenticated`; clients cannot mark rows ready, set `public_playback_path`, set `is_public_playback_safe`, set worker identity, or create trusted CDN eligibility.
...
- Draft backend-owned tables are designed as `media_transcode_jobs` and `media_renditions`; they are not applied to production. An external durable queue may still be used later for worker orchestration.
