...
- Protected Premium HD generation is closed for the current HD-capable public-safe set. HD outputs were generated only when source dimensions supported them, uploaded under `playback/protected/premium/`, audited before ready rows were promoted, and verified through the token-validating Worker. City Lights remains SD-only because the source is 854x480. Installed app Premium HD proof remains a separate future milestone.
