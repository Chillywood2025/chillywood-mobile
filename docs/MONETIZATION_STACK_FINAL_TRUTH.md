...
Creator setup update: `docs/CREATOR_MONETIZATION_IN_APP_FLOWS.md` adds in-app creator-facing sandbox setup flows. Creators/internal testers can save sandbox configs only through approved product tiers and real source UUIDs; Android digital tiers stay Google Play / RevenueCat, merch stays Stripe physical goods, and payout readiness stays read-only. Migrations `20260605000610_creator_monetization_in_app_setup_flows.sql` and `20260605002000_bound_creator_monetization_setup_access.sql` are remote-applied, constrain setup rows to `sandbox` / `not_payable`, and bind the save RPC to owner/operator or active beta/internal tester access. Production money, payouts, LiveKit publish, and host authority remain off.
