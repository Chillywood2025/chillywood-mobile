...
June 21 refund / credit / payout-hold foundation update: `docs/REFUND_CREDIT_PAYOUT_HOLD_FOUNDATION.md`, `_lib/moneyRefundPolicy.ts`, migration `20260621091458_refund_credit_payout_hold_foundation.sql`, and `guard:refund-credit-payout-hold-policy` add the rule foundation for future refund eligibility, in-app credit review, creator obligation review, and creator payout holds. This is foundation-only. No real refunds are executed, no provider refund APIs are called, no production credits are spendable, no payout holds release money, no payable creator balances are created, live money remains off, payouts remain off, and purchase/access behavior is unchanged.
