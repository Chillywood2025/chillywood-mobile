# Creator Payout Support Playbook
...
This playbook defines creator payout support while payout execution remains unavailable.
...
Creators may ask why payouts are not available, whether sandbox sales are earnings, what KYC or tax status means, whether they can cash out, what holds mean, whether Stripe Connect is connected, when production payouts will launch, or how refunds affect future earnings. Current support answers must be direct: payouts are not active, cash-out is not available, sandbox rows are not payable, and production activation requires future approval.

## Evidence To Check

Support may check safe payout readiness readouts, app-level payout switch state, Stripe Connect readiness status, KYC/tax labels, hold category, account standing, and policy status. Support should not expose sensitive identity, tax, financial, or provider payload data. Complex cases escalate to Owner/Admin and tax/legal review.

## Support Limits

Support cannot release money, create payouts, approve KYC, provide tax advice, promise launch timing, override risk holds, or mark sandbox rows payable. If a future production lane activates payouts, this playbook should be updated with exact eligibility, threshold, schedule, and escalation rules.
