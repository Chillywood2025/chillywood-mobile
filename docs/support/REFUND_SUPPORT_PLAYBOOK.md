...
This playbook defines how support should handle refund requests without promising outcomes that are not approved.
...
Support should identify the product, provider, purchase date, user account, access state, content or event status, safety state, reason for refund, and whether the request is a duplicate or chargeback risk. Premium and Android digital goods may involve Google Play and RevenueCat. Merch may involve Stripe and fulfillment records. Support should avoid asking for full payment credentials.

## Decision Factors

Decision factors include whether access was delivered, whether the content or event was canceled or removed, whether the user was blocked for safety reasons, whether merch was damaged or missing, whether the request is abusive, whether provider rules allow refund, and whether a chargeback already exists. Refunds may revoke access or trigger ledger reversals after production activation.

## User Reply Principles

Replies should be clear, calm, and specific. They should say what was reviewed, what happens next, and whether provider action is needed. They should not guarantee refunds, blame providers, disclose internal fraud signals, or promise access where safety rules deny it.
