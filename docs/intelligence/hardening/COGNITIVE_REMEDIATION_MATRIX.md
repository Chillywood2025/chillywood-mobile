...
## Fourteenth exact-head retest findings

The fourteenth isolated retest reviewed
`6821a7921e19596ef18c331a0bb9d6d2e52620d0`. All three lanes reported zero
P0, but independently reproduced order-independent and duplicate fragment
streams, alternate numbered research-query aliases, additional Unicode
confusables, split private identifiers, nested provider-policy composition,
credential-filename omissions, and bounded SQL text availability. Reviewer D
was correctly withheld because the zero-P1 gate was not met. The scaffold
remains `security_hardening_in_progress`.

| Retest finding | Severity | Exact defect | Corrective implementation | Regression | Status | Independent retest |
|---|---:|---|---|---|---|---|
| A14-FRAGMENT-001 | P1 | Duplicated, shared, indexed, and ordinal token streams could reconstruct sensitive labels without a single trustworthy object order. | Use bounded target reachability over the complete fragment multiset, preserve duplicates, accept position/index/ordinal aliases, and fail closed on duplicate positions. | R-92/R-93; focused pgTAP fragment/position matrix | fixed locally | pending |
| A14-URL-001 | P1 | Alternate query aliases such as `part[0][chunk]`, mixed prefixes, normalized prefixes, and Unicode decimal indexes could reconstruct credentials after per-value inspection. | Normalize each query key, detect its bounded numeric index regardless of nesting syntax, reconstruct global and grouped streams in both received and numeric order, and reject duplicate indexes before DNS. | R-94 with zero-DNS assertions | fixed locally | pending |
| B14-UNICODE-001 | P1 | Cyrillic shha, dotless i, and script g confusables could hide credential labels from one or more source/database boundaries. | Extend the detection-only security skeleton consistently in TypeScript and PostgreSQL, retaining the existing Unicode decimal normalization. | R-95; focused pgTAP confusable matrix | fixed locally | pending |
| B14-CREDENTIAL-001 | P1 | Arbitrarily granular, permuted, filler-bearing, and nested sibling key/value fragments could compose credentials without matching a finite pair list. | Replace finite fragment-pair checks with bounded dynamic target assembly plus explicit safe status/state assignment controls. | R-92; focused pgTAP granular/nested credential matrix | fixed locally | pending |
| B14-PRIVATE-001 | P1 | Nonadjacent, duplicated, prefixed/suffixed, two-part, and short-chunk phone/email/IP components could reconstruct private identifiers. | Add bounded structural phone/email/IPv4 composition checks over normalized leaves while preserving exact reviewed counter/status controls. | R-92; focused pgTAP private-fragment matrix | fixed locally | pending |
| B14-POSITION-001 | P1 | `index`/`ordinal` aliases and duplicated positions were not governed identically to the original positioned-fragment contract. | Normalize all reviewed position aliases, support reviewed chunk aliases, and reject ambiguous position reuse transactionally. | R-93; focused pgTAP position-alias matrix | fixed locally | pending |
| C14-PROVIDER-001 | P1 | Nested sibling provider-policy fragments and structured AWS `NotAction`, Kubernetes wildcards, and GCP included permissions could request broad authority without owner review. | Compose reviewed privilege targets across bounded nested fragments and add exact structured provider-policy checks. | R-95 | fixed locally | pending |
| C14-URL-001 | P1 | Multi-prefix numbered URL fragments could be distributed across query groups and evade group-local reconstruction. | Add a global bounded indexed-fragment stream in addition to per-prefix streams, with ambiguity rejection before DNS. | R-94 | fixed locally | pending |
| C14-CREDENTIAL-001 | P1 | Nested sibling labels and values could compose a credential without a direct assignment string. | Treat object keys and string leaves as one bounded detection envelope and require an explicit reviewed safe status/state value to avoid rejection. | R-92; focused pgTAP nested-sibling matrix | fixed locally | pending |
| A14/B14/C14-SCOPE-001 | P2/P3 | Provider denial/reversal wording and safe international/status evidence produced both missed scope expansion and false positives. | Accept only exact bounded reinforcing denials, detect explicit negation reversal, remove blanket non-ASCII authority heuristics, and retain structured policy review. | R-95 safe/unsafe provider matrix | fixed locally | pending |
| A14-PATH-001 | P2 | Common service-account, package-manager, SSH, and cloud credential filenames were not denied by the executor path contract. | Extend the closed credential-path grammar for reviewed service account, Firebase admin, AWS, npm/netrc, SSH, and secrets files. | R-96 source/runtime path matrix | fixed locally | pending |
| B14-BOUNDS-001 | P2 | Whitespace-only text at 32–64 KB could consume seconds in PostgreSQL before rejection. | Fail closed on stored cognitive text above the reviewed 4,000-byte retention envelope before recursive decoding or pattern analysis. | focused pgTAP 64 KB `performs_ok` assertion | fixed locally | pending |
| A14-SUPPLY-001 | P2 | The locked dependency tree still reports 23 inherited advisories, including three high-severity advisories. | No dependency changed and no audit fix ran; separately reviewed dependency upgrades remain a deployment blocker. | read-only dependency audit | accepted deployment blocker | pending |

## Fifteenth exact-head retest findings

The fifteenth isolated retest reviewed
`4b42964ae50a3ed6aea99f9d57565082590cf563`. Reviewer A reported
P0=0/P1=2/P2=3/P3=1, Reviewer B reported P0=0/P1=4/P2=1/P3=0, and
Reviewer C reported P0=0/P1=4/P2=2/P3=0. Reviewer D was correctly withheld.
The overlapping failures reduced to positioned/semantic fragment contracts,
query/path egress reconstruction, Unicode/phone database parity, provider
policy semantics, and credential-path coverage. The scaffold remains
`security_hardening_in_progress`.

| Retest finding | Severity | Exact defect | Corrective implementation | Regression | Status | Independent retest |
|---|---:|---|---|---|---|---|
| A15/B15-POSITION-001 | P1 | `index`/`ordinal` with `value`, `order`, `idx`, case-normalized aliases, mixed aliases, negative positions, and positions above 127 could be ignored or reconstructed inconsistently. | Parse a closed normalized alias set, require exactly one position and one fragment alias, permit `value` for every position alias, and reject ambiguity or any invalid range before retention. | R-97; focused pgTAP alias/range matrix | fixed locally | pending |
| A15/B15/C15-SEMANTIC-001 | P1 | Semantic sibling keys such as first/second/third could reorder raw credential fragments outside insertion or lexical order. | Collect bounded semantic object fragments and inspect their canonical ordinal order in TypeScript and PostgreSQL. | R-97; focused pgTAP semantic-sibling assertion | fixed locally | pending |
| A15/C15-URL-001 | P1 | Out-of-range, word, repeated position/chunk, non-numbered query, and path fragments could reconstruct credentials after per-value inspection and reach DNS. | Inspect bounded contiguous query-value, key/value, paired-position, word-index, global-index, and decoded path sequences before DNS in both static and runtime boundaries. | R-98 with zero-DNS assertions | fixed locally | pending |
| B15-FRAGMENT-001 | P1 | Inline safe-looking values such as `=status`/`=enabled` and single-character fragments could hide credential assignments. | Restrict the status exception to reviewed separate authorization/password status metadata; every other assembled sensitive label fails closed. | R-97; focused pgTAP fragmented assignment matrix | fixed locally | pending |
| B15-PRIVATE-001 | P1 | Letter/punctuation-suffixed and two-part phone fragments could compose a private identifier. | Count digits across every bounded digit-bearing leaf, reject 10–15 digit compositions, and preserve only reviewed two-counter forms with at least four digits per counter. | R-97; focused pgTAP phone/private matrix | fixed locally | pending |
| B15-UNICODE-001 | P1 | Armenian ho, Latin small-cap i, and Latin iota confusables survived direct and encoded SQL inspection. | Extend the shared detection skeleton in TypeScript/runtime/PostgreSQL and inspect percent/base64 forms through the same classifier. | R-99; focused pgTAP direct/encoded confusable matrix | fixed locally | pending |
| A15/C15-PROVIDER-001 | P1/P2 | Direct/nested AWS, Azure, Kubernetes, GCP, and GitHub wildcard/admin/impersonation policy shapes and additional denial reversals were retained. | Inspect direct action/permission wildcards, NotResource, assignable scopes, non-resource URLs, set-IAM-policy, administration writes, impersonation, and exact reversal statements. | R-99 provider matrix | fixed locally | pending |
| A15/C15-PATH-001 | P1/P2 | Git, Docker, cloud-default, service-account, language-registry, Kubernetes, HTTP-auth, auth, and token credential filenames passed confinement. | Extend the identical lexical/runtime deny grammar for the reviewed credential filename families and test actual confined-path resolution. | R-99 path matrix | fixed locally | pending |
| A15/B15/C15-FALSEPOS-001 | P2/P3 | Reinforcing future/international denials, safe status/counter arrays, and unrelated duplicate numeric query suffixes were rejected. | Recognize exact reviewed denial translations/forms, structured status metadata, bounded numeric counter pairs, and inspect duplicate URL indexes by content instead of rejecting the index collision alone. | R-97/R-98/R-99; focused pgTAP safe controls | fixed locally | pending |
| A15-SUPPLY-001 | P2 | The locked dependency tree still reports 23 inherited advisories, including three high-severity advisories. | No dependency changed and no audit fix ran; separately reviewed dependency upgrades remain a deployment blocker. | read-only dependency audit | accepted deployment blocker | pending |

## Sixteenth exact-head retest findings

The sixteenth isolated retest reviewed
`cf2f328a1d4ed9f0c0732df32d2fb5d485d3b37b`. Reviewer A reported
P0=0/P1=2/P2=3/P3=1, Reviewer B reported P0=0/P1=5/P2=2/P3=0, and
Reviewer C reported P0=0/P1=6/P2=3/P3=0. Reviewer D was correctly withheld.
The scaffold remains `security_hardening_in_progress`.

| Retest finding | Severity | Exact defect | Corrective implementation | Regression | Status | Independent retest |
|---|---:|---|---|---|---|---|
| A16/B16/C16-FRAGMENT-001 | P1 | Sequence/offset/rank/slot aliases, duplicate semantic positions, head/mid/tail and header/body roles, and unordered string leaves reconstructed credentials outside the prior finite grammar. | Normalize extended position/fragment aliases, generate bounded duplicate-position alternatives, inspect semantic roles and bounded small-envelope permutations, and preserve invalid-position rejection. | R-100; focused pgTAP semantic/position matrix | fixed locally | pending |
| A16/B16-UNICODE-001 | P1 | Latin alpha and small-cap letter confusables hid password, token, and secret assignments through plain and encoded SQL/source boundaries. | Extend the shared detection-only Unicode security skeleton consistently in TypeScript, runtime, and PostgreSQL. | R-100; focused pgTAP plain/percent/base64/hex matrix | fixed locally | pending |
| B16-CREDENTIAL-001 | P1 | `passwd` and `passphrase` assignments were absent from the closed credential vocabulary. | Add both exact labels to source, runtime, SQL, fragment, key, and identifier classifiers. | R-100; focused pgTAP assignment assertions | fixed locally | pending |
| B16-STATUS-001 | P1 | Standalone `=` or `:` fragments allowed literal credential assignments to use the safe-status exception. | Make any assignment marker disable the status exception while preserving separator-free state/status metadata. | R-100; focused pgTAP assignment/status matrix | fixed locally | pending |
| B16-PRIVATE-001 | P1 | Two numeric leaves could persist reconstructed ten-digit phone identifiers. | Reject 10–15 digit array compositions, retain only exact reviewed date/counter, year-series, version, and long opaque-counter forms, and keep object metadata context-bound. | R-100; focused pgTAP phone/counter matrix | fixed locally | pending |
| A16/C16-URL-001 | P1 | Duplicate indexes, sequence/payload pairs, extended ordinals, Roman indexes, labeled/matrix paths, hashes, and static/runtime divergence allowed synthetic credentials to reach DNS. | Use equivalent bounded alternative reconstruction for query, path, matrix, and hash fragments in both validators; block before DNS and separate general secret candidates from structured private-data candidates. | R-101 zero-DNS matrix | fixed locally | pending |
| A16/C16-PROVIDER-001 | P1/P2 | AWS role/policy escalation, Azure data actions, Kubernetes impersonation, GCP editor/token-creator operations, GitHub write permissions, and denial-reversal paraphrases did not always request owner review. | Add exact provider-native operation semantics, nested structured traversal, and reversal/contraction classification; explicit Deny and French/German denial controls remain non-escalating. | R-102 | fixed locally | pending |
| A16/C16-PATH-001 | P1/P2 | Additional database, cloud CLI, Kubernetes, GitHub, Azure, package-manager, vault, and YAML credential files passed executor confinement. | Extend the identical lexical/runtime credential-file grammar and exercise real confined-path resolution. | R-103 | fixed locally | pending |
