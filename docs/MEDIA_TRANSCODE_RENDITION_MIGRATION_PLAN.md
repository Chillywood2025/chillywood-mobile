...
Status: production schema applied, with scoped audited rows from the City Lights one-job proof, bounded CLI auto-detect cycle, and protected Premium HD pass. Migration `supabase/migrations/20260709033207_trusted_media_transcode_renditions.sql` was applied to production on 2026-07-09 for project `bmkkhihfbmsnnmcqkoly` (`Chillywood2025's Project`), with tables/indexes/RLS/policies/grants read back afterward. Additive migration `supabase/migrations/20260710023000_media_premium_hd_protected_renditions.sql` adds protected Premium HD metadata and constraints. Current production counts are `media_transcode_jobs=10`, `media_renditions=15`, `active_unfinished_jobs=0`, `unsafe_cdn_rows=0`, and unsigned public HD exposure `0`. This plan does not switch production playback broadly, does not migrate existing videos blindly, does not backfill media rows broadly, and does not make a production transcode worker live.
...
9. Trusted rows for safe Level 0/1 audited public media: complete for City Lights plus four bounded auto-detected public-safe sources (`media_transcode_jobs=10`, `media_renditions=15`), including `5` protected Premium HD rows behind tokenized Worker access; future batches remain capped, audited, rollback-scoped, and blocked for private/Premium/original/unscanned/moderation-blocked media.
