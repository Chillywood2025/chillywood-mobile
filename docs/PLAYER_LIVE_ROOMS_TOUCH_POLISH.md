...
June 5, 2026 shared/standalone fullscreen follow-up: standalone fullscreen now keeps the same cover-fit presentation used by the normal standalone media card, so standalone title, creator-video, and other standalone Player surfaces no longer switch back to black side letterboxing when fullscreen is entered. Shared Watch-Party playback now mirrors the standalone overlay direction with compact Share, Report, and speed controls over the video, no Watch-Party Live handoff toggle, and 5-second idle auto-hide for shared Player chrome. Tap-to-play still routes through the existing shared playback tap handler. Shared fullscreen rotates landscape but keeps the video in contain/letterbox mode like the requested reference instead of cropping to cover the whole screen. Fullscreen side rails are persistent and minimal: the real room comments composer stays on the left, and display-only participant bubbles stay on the right. The fullscreen overlay no longer mounts the full Watch-Party social card, placeholder shared-player card, or LiveKit media surface, preventing the bubble flicker/disappear/freeze path seen in tester screenshots. The non-fullscreen shared dock has additional bottom room so the comments composer is not cut off without changing Player media size. The Android hardware Back button exits shared fullscreen before returning to the Party Room. EAS Update group `d32d4cdb-d39f-4815-a397-07648f0b694a` / Android update `019e9a29-c855-74d2-a2c2-3adab2aa1ea1` was published on runtime `1.0.0` before the final shared-fullscreen rail correction. Prior proof path is `/tmp/chillywood-player-shared-fullscreen-polish-proof-20260605/`; the available route-backed Watch-Party fixture returned the expected unavailable card, so admitted shared-player fullscreen visual proof still needs a live/admitted Party Room session. This follow-up is presentation-only: it does not change media resolver logic, playback sync authority, LiveKit token issuance, host approval, route ownership, money state, access grants, or content safety.
...
- Shared Watch-Party playback now uses compact in-video overlay controls and shared fullscreen side rails for comments plus display-only room bubbles, while keeping LiveKit publish/host authority unchanged.
