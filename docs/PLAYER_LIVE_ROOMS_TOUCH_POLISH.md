...
June 10, 2026 shared-player participant rail follow-up: the Watch-Party Live shared/fullscreen participant bubble surface now always scrolls vertically inside its own rail (`components/watch-party-live/livekit-stage-media-surface.tsx`). The backed roster/order remains unchanged: current user sorts first and renders as `You`, joined users without camera tracks still render as avatar/initial/status placeholders, and extra participant tiles overflow only inside the participant rail. EAS Update group `28f14786-d65a-4751-a9d8-db399140a6ac` / Android update `019eb4c5-4263-7ba2-9f25-09f6570b1762` published this JS-only update. Validation passed typecheck, runtime validation, and diff checks. Real 2-user/overflow screenshots still require a second joined participant/device; none were faked.
...
- Shared Player fullscreen rails now have a dedicated exact-component follow-up in `docs/SHARED_PLAYER_RAILS_EXACT_COMPONENT_PROOF.md`: the right rail reuses the regular shared-player `renderParticipantPanel` / `renderSharedPlayerParticipantBubble` path, and the left comments rail is compacted without changing room comment behavior.
