...
June 5, 2026 shared/standalone fullscreen follow-up: standalone fullscreen now keeps the same cover-fit presentation used by the normal standalone media card, so standalone title, creator-video, and other standalone Player surfaces no longer switch back to black side letterboxing when fullscreen is entered. Shared Watch-Party playback now mirrors the standalone overlay direction with compact Share, Report, and speed controls over the video, no Watch-Party Live handoff toggle, and 5-second idle auto-hide for shared Player chrome. Tap-to-play still routes through the existing shared playback tap handler. Shared fullscreen rotates landscape but keeps the video in contain/letterbox mode like the requested reference instead of cropping to cover the whole screen. Fullscreen now reuses the regular shared Player participant bubble renderer on the right side, not a separately invented fullscreen bubble. The same room comments composer is compacted into the left black gutter so it does not cover video content. The fullscreen overlay no longer mounts the full Watch-Party social card, placeholder shared-player card, or duplicate LiveKit media surface. The non-fullscreen shared dock has additional bottom room so the comments composer is not cut off without changing Player media size. The Android hardware Back button exits shared fullscreen before returning to the Party Room. Prior EAS Update group `808798e4-1010-49f3-aa4b-d189ff9a8232` / Android update `019e9a3e-9a9a-7f2c-81ea-71faec45c9ba` was published on runtime `1.0.0`; the corrected gutter/bubble reuse still needs a new OTA and admitted shared-player fullscreen visual proof. Prior proof path is `/tmp/chillywood-player-shared-fullscreen-polish-proof-20260605/`; the available route-backed Watch-Party fixture returned the expected unavailable card. This follow-up is presentation-only: it does not change media resolver logic, playback sync authority, LiveKit token issuance, host approval, route ownership, money state, access grants, or content safety.
...
- Shared Watch-Party playback now uses compact in-video overlay controls and shared fullscreen side rails for guttered comments plus the regular shared participant bubble renderer, while keeping LiveKit publish/host authority unchanged.
