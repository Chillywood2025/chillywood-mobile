...
: "${PLATFORM_RECOVERY_OPERATOR_TOKEN:?missing PLATFORM_RECOVERY_OPERATOR_TOKEN}"

SOURCE="${OPERATOR_SOURCE:-systemd_timer:chillywood-prod-01:platform_recovery_operator}"
TMP_RESPONSE="$(mktemp)"
...
printf 'platform-recovery-operator-watch-once status=%s response=%s\n' "${HTTP_STATUS}" "${REDACTED_RESPONSE}"
