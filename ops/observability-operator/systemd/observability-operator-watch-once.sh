...
SOURCE="${OPERATOR_SOURCE:-systemd_timer:chillywood-prod-01:observability_runtime_operator}"
TMP_RESPONSE="$(mktemp)"
...
HTTP_STATUS="$(
  curl --silent --show-error --max-time 25 --retry 2 --retry-delay 2 \
    ...
    --data '{"action":"watch_once","scheduler":"systemd_timer","operator_id":"observability_runtime_operator","source":"'"${SOURCE}"'","highRiskExecuted":false,"releaseActionExecuted":false,"secretsLogged":false,"piiStored":false}'
)"
