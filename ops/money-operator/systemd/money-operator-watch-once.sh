...
: "${MONEY_OPERATOR_TOKEN:?missing MONEY_OPERATOR_TOKEN}"

SOURCE="${OPERATOR_SOURCE:-systemd_timer:chillywood-prod-01:money_flow_control}"
TMP_RESPONSE="$(mktemp)"
...
printf 'money-operator-watch-once status=%s response=%s\n' "${HTTP_STATUS}" "${REDACTED_RESPONSE}"
