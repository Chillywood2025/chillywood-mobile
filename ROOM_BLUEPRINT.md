...
- Discovery Feed Foundation D2 plus Discovery/Spectator D3-D10, D7B, D7C, D7D readiness, and D7G provide backend/read-model, Home/Channel metadata, metadata-only spectator, eligibility, trigger-foundation, deterministic ranking, broadcast-session schema groundwork, backend-only start/stop/status functions, a private D7D test Egress path, and metadata-only ad/CTV policy labels. `/spectate/[itemId]` is metadata-only and does not grant spectator playback, public HLS/Egress, LiveKit participant tokens, room mutation, fake ranking, fake trending, fake counts, real ad playback, CTV inventory, or protected-content public video. `room_broadcast_sessions` is admin/operator-owned; public watchability and spectator playback are constrained false, and real Egress ids/private HLS locations are allowed only for private `D7D_TEST_` proof rows. D7D private Egress proof is complete; D7E now targets Hetzner Object Storage but remains blocked until a public/custom HLS base URL and outside-LiveKit `.m3u8` fetch/playback proof exist. D7F stays foundation/readout only until then.
