...
- Discovery Feed Foundation D2 plus Discovery/Spectator D3-D10, D7B, D7C, D7D readiness, D7E proof, D7F public-safe gate, and D7G provide backend/read-model, Home/Channel metadata, spectator eligibility, trigger-foundation, deterministic ranking, broadcast-session schema groundwork, backend-only start/stop/status functions, a private D7D test Egress path, real Hetzner HLS delivery proof, controlled public-safe spectator playback gating, and metadata-only ad/CTV policy labels. `/spectate/[itemId]` may render watch-only HLS playback only when `spectator-playback` returns an approved public-safe live state from `spectator_hls_playback_records`; otherwise it remains metadata/blocked/unavailable. D7F does not grant LiveKit participant tokens, mic/camera controls, room mutation, fake ranking, fake trending, fake counts, real ad playback, CTV inventory, Premium full-room bypass, or protected/title/private/ticketed public video.
