...
- Discovery Feed Foundation D2 plus Discovery/Spectator D3-D10, D7B, D7C, D7D readiness, D7E proof, D7F public-safe gate, D7G, and the Spectator child-room relay provide backend/read-model, Home/Channel metadata, spectator eligibility, trigger-foundation, deterministic ranking, broadcast-session schema groundwork, backend-only start/stop/status functions, a private D7D test Egress path, real Hetzner HLS delivery proof, controlled public-safe spectator playback gating, metadata-only ad/CTV policy labels, and server-verified child room creation from eligible public-safe Spectator sources. `/spectate/[itemId]` may render watch-only playback only when `spectator-playback` returns an approved public-safe state from `spectator_hls_playback_records`; otherwise it remains metadata/blocked/unavailable. Eligible Spectator sources can create child Watch-Party Live rooms or child Live Watch-Party reaction rooms linked through `spectator_child_room_sources` with `root_source_id`, source attribution, and no nested chain wording when a safe eligible fixture/source exists. The May 26 Android fixture closeout added proof-scoped safe fixtures, added narrow signed-out public-safe Spectator reads, hardened deployed `spectator-playback` controlled URLs to HTTPS, and proved on `R5CR120QCBF` that an eligible content/player Spectator source can start Watch-Party Live child room `5SR4TQ` with source attribution and no original host controls/member list. Signed-out handoff, private/source-ended/reuse-disabled UI states, and private/blocked/ended/reuse-disabled backend denials are proved without child ids or token fields. The replay fixture closeout applied `202605260006_spectator_replay_archive_fixture.sql` and proved replay Watch-Party Live child room `NSHU7J` from a `replay_available_later` source with `source_is_live=false`, safe source attribution, controlled HTTPS playback, and a visible child shared Player frame. Successful Live Watch-Party / Reaction Room launch still needs a true live-stage-compatible safe fixture; public-safe candidate discovery returned zero true live-compatible rows, so replay/VOD is not claimed as live. This relay does not grant original LiveKit participant tokens, publish permission, mic/camera controls in the original room, original host controls, original member lists, raw playback paths, fake live status, fake ranking, fake trending, fake counts, real ad playback, CTV inventory, Premium full-room bypass, or protected/title/private/ticketed public video.
...
- Viewers can see and do only what the current room mode permits.
...
## 7. Standalone Player
Relationship to other rooms: Standalone Player is the solo playback route and the launch surface for `Watch-Party Live`. Social viewing is a deliberate escalation from here, not the default state. Player copy should keep `Watch-Party Live`, `Live Watch-Party`, and `Party Room` distinct.
