...
- Admin Usage Metering Foundations 37-39 is pushed as schema/helper/read-only Admin foundation, and the usage remote migration/type refresh is complete. Ledger Systems 4A-4D finance foundation is still local schema/helper/read-only Admin foundation until a later finance Supabase pass. Real bandwidth/provider logs, LiveKit billing truth, storage billing truth, finance provider imports, payout provider integration, network billing actions, sponsor checkout, fraud enforcement, and event writers remain later separately scoped lanes.
