...
- Admin Usage Metering Foundations 37-39 is pushed as local schema/helper/read-only Admin foundation. Ledger Systems 4A-4D finance foundation is also pushed as local schema/helper/read-only Admin foundation. Remote migration application, generated type refresh, real bandwidth/provider logs, LiveKit billing truth, storage billing truth, finance provider imports, payout provider integration, network billing actions, sponsor checkout, fraud enforcement, and event writers remain later separately scoped lanes.
...
- The usage metering migration has not been applied remotely and generated database types were not edited in the foundation pass. Any remote apply/type refresh must be separately authorized and proved.
