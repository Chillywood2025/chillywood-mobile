...
- Ads Config V1D1 is pushed: `_lib/appConfig.ts` normalizes optional `app_configurations.config.adsLaunch` through code-owned defaults and `app/admin.tsx` shows that source read-only; runtime ad slots/controllers are not wired to app config yet and default runtime remains hidden
...
- Ads V1D1 does not change default runtime behavior: Home `NativeAdSlot` and root `InterstitialController` still use `ADS_LAUNCH_CONFIG_DEFAULTS` unless a future scoped pass wires normalized app-config reads, and Admin `runtimeControls.ads_enabled` is not used as the Ads Launch runtime source.
