...
requireText(admin, 'const ADMIN_MAIN_TAB_KEYS = [', "Admin must keep an explicit canonical top-level tab set.");
for (const key of ["home", "payments", "users", "reports", "live-ops", "rachi", "cognitive", "legal", "system", "owner-security"]) {
...
for (const legacy of ["telemetry", "alerts", "automation", "incidents", "turn", "subscriptions", "creator-subs", "money-sandbox", "money-ops"]) {
  forbidText(admin, `ADMIN_MAIN_TAB_KEYS = [\n  \"${legacy}\"`, `Specialized ${legacy} tool must not become a standalone first Admin tab.`);
}
...
requireText(moderation, 'role === "owner" || role === "super_admin"', "Client authority must recognize Owner and Super Admin together.");
