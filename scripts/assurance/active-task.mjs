...
function observeEngineeringScope(lease, currentHead, currentProtectedMain = null, effectiveReservationResolution = null) {
  ...
  if (observation && implementation && aggregate
    ...
    observation = deriveTrustedImplementationScopeObservation(observation, effectiveReservationResolution);
  }
...
export function resolveEngineeringArtifactInput({ lease, suppliedPacket = null, suppliedCertificate = null, head = null, root = ROOT } = {}) {
  ...
  const exactArtifact = readTaskArtifactAtGitHead(artifactPath, head, root);
  if (!exactArtifact) return { ok: false, findings: ["PREIMPLEMENTATION_ADMITTED_ARTIFACT_CONTRACT_UNSUPPORTED"] };
  const artifact = exactArtifact.artifact;
  ...
  return { ok: true, packet: artifact, certificate: artifact?.certificate ?? null, taskArtifactBytes: exactArtifact.bytes, taskArtifactHash: exactArtifact.artifactHash };
}

export function admittedFiniteTaskCommandRule({ contractCommand, featureId, engineeringAuthority, taskArtifact, taskArtifactHash, lease } = {}) {
  const admitted = engineeringAuthority?.ok === true
    && engineeringAuthority?.classification === "PREIMPLEMENTATION_ENGINEERING_CLEAR"
    && engineeringAuthority?.derivedGate?.clear === true
    && engineeringAuthority.derivedGate.verificationMode === "ADMITTED_FROZEN_FINITE_TASK_V2"
    && taskArtifact?.primaryDomain === featureId
    && sha64(taskArtifactHash)
    && taskArtifactHash === lease?.closure?.artifactHash;
  if (!admitted) return null;
  if (contractCommand === "assurance:plan") return { id: "admitted-finite-task-plan", file: "node", args: ["scripts/assurance/plan.mjs", `--feature=${featureId}`], resultContract: { type: "assurance-json-v1", command: "assurance:plan" }, contractCommand };
  const testPaths = taskArtifact?.implementationPlan?.tests;
  const testsBound = Array.isArray(testPaths) && testPaths.length > 0
    && stableJson([...testPaths].sort()) === stableJson([...(lease?.artifactReservation?.testEvidencePaths ?? [])].sort())
    && testPaths.every((file) => /^(?:tests|supabase\/tests)\/[A-Za-z0-9_./-]+$/u.test(file));
  if (contractCommand !== "focused auth/RLS suite" || featureId !== "auth-session-password-recovery" || !testsBound) return null;
  return {
    id: "deferred:admitted-focused-auth-rls-suite",
    file: null,
    args: [],
    resultContract: { type: "deferred-task-contract", executable: false, reason: "IMPLEMENTATION_TEST_EVIDENCE_PENDING", task: taskArtifact.taskId, taskArtifactHash, testEvidencePaths: [...testPaths] },
    contractCommand,
  };
}
...
export function validateEngineeringTaskAuthority({ doctrineTruth, featureId, phase = "IMPLEMENTATION", lease, baseLease = lease, effectiveReservationResolution = null, closurePacket, certificate, taskArtifactBytes = null, sourcePushed = false, samePr = true, branch, currentMain, currentHead, implementationPr, implementationMerged = false, bootstrapExpired = false, sourceChanging = true, readOnlyDiagnostic = false, scopeObservation, taskIdentityObservation, implementationIdentity, ownerJurisdictionAuthority } = {}) {
  ...
  const admittedFiniteTaskArtifact = wrappedFiniteTaskArtifact
    ...
    && typeof closurePacket?.artifactId === "string"
    && closurePacket.artifactId === String(closurePacket?.taskId ?? "").toUpperCase().replaceAll("-", "_")
    && closurePacket?.closure?.id === "ENGINEERING_CLOSURE_PACKET_V1"
    && closurePacket?.closure?.classification === "ENGINEERING_CLOSURE_PACKET_V1"
    && closurePacket?.certificate?.id === "BOUNDED_ENGINEERING_COMPLETENESS_CERTIFICATE_V1"
    && closurePacket?.certificate?.classification === "BOUNDED_ENGINEERING_COMPLETENESS_CERTIFICATE_V1" ? closurePacket : null;
  ...
  if (bootstrap) {
    const gate = evaluatePreimplementationGate(effectivePacket, { certificate: effectiveCertificate, taskIdentityObservation, bootstrapAuthority: { branch, base: DOCTRINE_BASE, currentMain, doctrineStatus: doctrineTruth?.status, implementationMerged, bootstrapExpired, productTask: false, featureId } });
  ...
  const findings = [];
  {
    ...
    const planningOnly = !finiteLeasePresent || !artifactFrozen;
    const graph = generateDomainGraph(); const closureDomains = effectivePacket?.sections?.C_AFFECTED_DOMAIN_CLOSURE ? [effectivePacket.sections.C_AFFECTED_DOMAIN_CLOSURE.primaryDomain, ...effectivePacket.sections.C_AFFECTED_DOMAIN_CLOSURE.includedDependencies].sort() : null;
    ...
    if (planningOnly) {
      ...
      if (!effectivePacket || effectivePacket.id !== "ENGINEERING_CLOSURE_PACKET_V1") findings.push("PREIMPLEMENTATION_AFFECTED_DOMAIN_INCOMPLETE");
    ...
    if (reservationValid && (changedPaths.length > reservation.maximumFiles || (Number.isInteger(changedLines) && changedLines > reservation.maximumLines) || changedPaths.some((file) => matches(file, highRiskPaths) && !highRiskAuthorized(file) && !(admittedReservationBound && file.startsWith("supabase/migrations/") && admittedPlanPaths.includes(file))))) findings.push("FINITE_TASK_ARTIFACT_RESERVATION_SCOPE_VIOLATION");
    if (!samePr) findings.push("ASSURANCE_RECURSIVE_BOOTSTRAP_CYCLE");
    const clear = !planningOnly && derivedGate?.clear === true && findings.length === 0;
    const classification = findings.length ? "BOUND_INCOMPLETE" : clear ? "PREIMPLEMENTATION_ENGINEERING_CLEAR" : "ENGINEERING_DISCOVERY_RESERVED";
    const runtimeState = clear && changedPaths.some((file) => file !== reservation?.closureArtifactPath) ? "IMPLEMENTATION" : classification;
    return { ok: findings.length === 0, findings: [...new Set(findings)].sort(), classification, runtimeState, productSourceMutationAllowed: clear, authorityPreserved: true, evidenceInvalidated: sourcePushed, admissionPrRequired: planningOnly, persistedPhase: phase, phasePlanningOnly, planningOnly, artifactFrozen, finiteLeasePresent, derivedGate, derivedClearanceContext: clear ? { packetHash: sha256(stableJson(effectivePacket ?? null)), certificateHash: sha256(stableJson(effectiveCertificate ?? null)), task: effectivePacket?.task ?? null, pr: implementationPr == null ? null : String(implementationPr), leaseId: lease?.leaseId == null ? null : String(lease.leaseId), phase } : null };
  }
}
...
export function resolveFiniteTaskImplementation(truth, identity, facts, binding, lease) {
  ...
  const reservationProjection = effectiveResolution ? {
    ...
    ...(effectiveResolution.status === "AMENDED_WITH_TEST_ADAPTATION" ? {
      scopeBase: effectiveResolution.scopeBase,
      aggregateReservation: effectiveResolution.aggregateReservation,
      testAdaptationReservation: effectiveResolution.testAdaptationReservation,
      scopePartitions: effectiveResolution.scopePartitions,
      testAdaptationsConsumed: effectiveResolution.testAdaptationsConsumed,
      testAdaptationReceipt: effectiveResolution.testAdaptationReceipt
    } : {}),
  ...
  const candidateLease = effectiveResolution?.effectiveLease ?? lease;
  const evaluated = evaluateFiniteTaskCandidate({
    lease: candidateLease,
    registry: truth.finiteTaskLeases,
    candidate,
    effectiveReservationResolution: effectiveResolution
  });
...
function resolveStructuredImplementation(truth, identity, facts, binding) {
  ...
  if (baseFiniteLease) {
    return resolveFiniteTaskImplementation(truth, identity, facts, binding, baseFiniteLease);
  }
  const findings = [];
  const open = truth?.openImplementationPrs ?? [];
  if (!Array.isArray(open)) return { ok: false, findings: ["IMPLEMENTATION_INVENTORY_MALFORMED"] };
...
export function activeTask(facts = {}) {
  ...
  if (baseFiniteTaskLease) {
    ...
    finiteTaskCandidate ??= terminalLease
      ? {
          ...
          head: identity.head,
          tree: identity.tree,
          ...(terminalOutcome?.schemaVersion === 2
            && Array.isArray(terminalOutcome?.scopePartitions?.aggregate?.actualPaths)
            && Number.isInteger(terminalOutcome?.scopePartitions?.aggregate?.canonicalChangedLines) ? {
  ...
  const engineeringScopeObservation = terminalHistoricalLease ? null : observeEngineeringScope(engineeringLease, synchronizedImplementationHead, identity.originMainHead, finiteTaskEffectiveReservationResolution);
  const closureArtifactHash = exactArtifact?.taskArtifactHash ?? exactArtifact?.artifactHash ?? null;
  const implementationIdentity = engineeringLease ? createImplementationIdentityObservation({
    ...
    currentProtectedMain: identity.originMainHead,
    finiteLeaseId: engineeringLease.leaseId,
  ...
  const safe = redactActiveTaskPacket(built.packet);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (result.ok) {
    ...
    const authorityBytes = Number(result.packet?.authority?.bytes ?? Buffer.byteLength(text));
    emit("assurance:active-task", true, {
      packetSha256,
      packetBytes: Buffer.byteLength(text),
      canonicalCurrentTruthBytes: authorityBytes,
      excludedContextBytes: Math.max(0, authorityBytes - Buffer.byteLength(text)),
      artifactLocation,
