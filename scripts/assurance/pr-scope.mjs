...
if (highRisk.length > 1) findings.push({ id: "ASSURANCE_MIXED_HIGH_RISK_SCOPE", status: "BLOCKED_INTERNAL", domains: highRisk });
if (highRisk.length && highRisk.some((domain) => !objectiveDomains.includes(domain))) findings.push({ id: "ASSURANCE_OBJECTIVE_OMITS_AFFECTED_DOMAIN", status: "BLOCKED_INTERNAL", domains: highRisk.filter((domain) => !objectiveDomains.includes(domain)) });
...
emit("assurance:pr-scope", findings.length === 0, {
  ...
  domains, highRiskDomains: highRisk, objectiveDomains, budget, waiver: waiver ? waiver.contractId : null, classified, findings
