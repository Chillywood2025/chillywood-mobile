...
export function validateUntrustedAssuranceControlTaskContextObservation({ result, authorityProof = null, expectedIdentity = null, githubEvent, gitCommand = git, environment = process.env } = {}) {
  try {
    ...
    const profile = ASSURANCE_CONTROL_SOURCE_ONLY_PROFILES.find(({ paths, maximumFiles, maximumChangedLines }) => stableJson(changedPaths) === stableJson(paths) && budget?.maximumFiles === maximumFiles && budget?.maximumChangedLines === maximumChangedLines && budget?.maximumHandAuthoredNetLines === maximumChangedLines && result?.budget?.files === maximumFiles && result?.budget?.lines === maximumChangedLines && result.changedFiles === maximumFiles);
    const exact = authorityVerified && result?.ok === true && result?.mode === "GITHUB_EVENT_TASK_CONTEXT" && context?.ok === true && empty(context?.findings) && empty(result?.findings) && githubExecutionIdentityValid(executionIdentity) && stableJson(child) === stableJson(executionIdentity)
      ...
      && profile && stableJson(changedPaths) === stableJson(observedPaths) && result.changedFiles === observedPaths.length
