...
assertIncludes(chillyChatNativeCallRoutes, "TRUSTED_ANDROID_ACTIONS.has(nativeCallAction)", "native-store replay must be limited to explicit Answer and Decline actions");
...
assertIncludes(chillyChatNativeCallRouteBuffer, "registerTrustedAndroidNativeActionStorePayload", "only validated native-store output may create Android transition provenance");
assertIncludes(chillyChatNativeCallRouteBuffer, "authenticatedUserId", "native-store consumption must bind the exact authenticated user before deleting native state");
assertIncludes(layout, "consumePendingAndroidNativeCallRoute({ authenticatedUserId })", "the root bridge must drain a native action only after authenticated session binding");
assertIncludes(nativeCallTransitionProvenance, 'source: "android_native_action_store"', "Android provenance must use the canonical shared platform source");
assertIncludes(nativeCallTransitionProvenance, "consumeMountedAndroidNativeCallRoute", "mounted Android routes must consume the canonical claim once");
assertIncludes(nativeCallTransitionProvenance, "subscribeToTrustedAndroidNativeActionRoutes", "same-thread warm actions must use the in-memory trusted producer notification");
assertIncludes(chatThread, "subscribeToTrustedAndroidNativeActionRoutes", "the mounted same-thread surface must consume warm Android claims without URL authority");
...
assertIncludes(nativeCallPlugin, "val declineIntent = buildActionPendingIntent(context, data, ACTION_DECLINE, 2)", "native Decline must use the trusted explicit receiver path");
assertIncludes(nativeCallPlugin, "PendingIntent.getBroadcast", "trusted notification actions must target an explicit broadcast receiver");
assertIncludes(nativeCallPlugin, "PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE", "native action PendingIntents must remain immutable");
assertNotIncludes(nativeCallPlugin, "PendingIntent.FLAG_MUTABLE", "native action PendingIntents must not become mutable");
...
assertIncludes(nativeCallPlugin, "captureTrustedNotificationAction(", "the non-exported receiver must persist a validated native action before launching the Activity");
assertIncludes(nativeCallPlugin, "Intent(Intent.ACTION_MAIN)", "trusted native actions must use a neutral explicit Activity launch");
assertIncludes(nativeCallPlugin, "data = null", "trusted native Activity launch must carry no authoritative URL data");
assertNotIncludes(nativeCallPlugin, "openDeepLinkForAction", "native Answer/Decline must not manufacture route authority from a deep link");
assertNotIncludes(nativeCallPlugin, "captureForActivity", "MainActivity must not manufacture native action authority from URLs or intents");
...
assertIncludes(nativeCallPlugin, "KEY_CAPTURE_GENERATION_COUNTER", "native actions must carry a distinct monotonic capture generation");
assertIncludes(nativeCallPlugin, "storedSchemaVersion != SCHEMA_VERSION", "pre-correction restored state must be invalidated before duplicate handling");
assertIncludes(nativeCallPlugin, 'ensureModernBackupSection(root, "cross-platform-transfer")', "current Android SDK cross-platform transfer must exclude transient authority");
