...
[
  ...
  ["Party Room Pass actual tappable CTA testID", "money-feature-watch_party_ticket-cta"],
...
const flowChecks = [
  ...
  },
  {
    ...
    viewerChecks: [
      ["viewer route resolves Party Room Pass access", watchPartyRoute, "resolvePaidWatchPartyTicketAccess(snapshot.room.partyId)"],
      ["viewer route renders ticket lock card", watchPartyRoute, "watch-party-ticket-lock-card"],
      ["viewer route has Party Room Pass purchase action", watchPartyRoute, "watch-party-ticket-purchase-button"],
      ["viewer route labels Party Room Pass gate clearly", watchPartyRoute, "Party Room Pass required"],
      ["viewer route keeps contextual Party Room CTA", watchPartyRoute, "Join Party Room —"],
      ["viewer route has simple Party Room Pass unavailable copy", watchPartyRoute, "Party Room Pass purchases are temporarily unavailable while setup is being finalized."],
      ["viewer route uses Party Room Pass purchase helper", watchPartyRoute, "purchasePaidWatchPartyTicket"],
      ...
      ["viewer canceled state is clear", paidWatchPartyTickets, "Party Room Pass purchase was canceled. Nothing changed."],
  ...
  },
  {
    ...
    viewerChecks: [
      ...
      ["viewer copy keeps unrelated unlocks separate", eventRoute, "other Events, or other creator content"],
