...
const forbidSentenceMatch = (label, content, pattern, description) => {
  const sentences = content
    .split(/(?<=[.\n])\s+/)
    .map((sentence) => sentence.trim())
    .filter(Boolean);
  const allowedDenial = /\b(?:did not|do not|not performed|avoided|not happen|no production)\b/i;
  const hit = sentences.find((sentence) => pattern.test(sentence) && !allowedDenial.test(sentence));
  if (hit) failures.push(`${label} contains forbidden ${description}`);
};
...
const docs = [
  ["Play internal AAB doc", doc],
  ["Android tester binary doc", binaryDoc],
  ["tester update doc", testerUpdate],
...
for (const [label, content] of docs) {
  forbidMatch(label, content, /submitted? (?:the app )?to Play production(?![^.\n]*(?:did not|not performed|avoided))/i, "production Play submission claim");
  forbidSentenceMatch(label, content, /promoted? (?:a release )?to production/i, "production promotion claim");
  forbidMatch(label, content, /mutated (?:Google Play products?|Google Play base plans?|RevenueCat|Stripe|provider dashboards?)(?![^.\n]*(?:did not|not performed))/i, "provider/product mutation claim");
  forbidMatch(label, content, /Google Play products?\/base plans? (?:changed|mutated|created|activated)/i, "Google Play product/base-plan mutation claim");
  forbidMatch(label, content, /RevenueCat mapping (?:changed|updated|mutated)/i, "RevenueCat mapping change claim");
...
].forEach((needle) => requireText("Play internal AAB doc", doc, needle));
...
if (failures.length) {
  console.error("Play internal test AAB policy guard failed:");
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log("Play internal test AAB policy guard passed.");
console.log("- no production submission, provider/product mutation, money activation, package change, or secret exposure was introduced.");
