...
assertIncludes(architecture, "Backup/PITR activation gate: Blocked for production worker writes/backfill/activation.", "backup PITR worker activation gate");
...
assertIncludes(architecture, "Production transcode worker deployment remains planned and blocked by the Backup/PITR gate until PITR or an owner-approved restore path is verified; no worker writes/backfill while the gate is Blocked or Partial.", "production worker deployment planned checkpoint");
...
assertIncludes(currentState, "Backup/PITR gate status is Blocked for production worker writes/backfill/activation", "current state backup PITR worker gate");
...
assertIncludes(currentState, "No production worker writes or backfill while the gate is Blocked or Partial.", "current state backup PITR no-write rule");
...
assertIncludes(nextTask, "Backup/PITR gate is Blocked for production worker activation", "next task backup PITR gate");
...
assertIncludes(nextTask, "No production worker writes or backfill while the gate is Blocked or Partial.", "next task backup PITR no-write rule");
...
assertIncludes(nextTask, "Next real media milestone is explicit owner approval for a staged one-job worker activation lane or controlled trusted-row pipeline, not production playback migration", "next task worker activation gate next step");
...
assertIncludes(mediaMigrationPlan, "Backup/PITR gate: Blocked for production worker writes/backfill/activation.", "trusted rendition migration plan backup PITR gate");
...
assertIncludes(mediaMigrationPlan, "No production worker writes or backfill while the backup/PITR gate is Blocked or Partial.", "trusted rendition migration plan no-write backup gate");
...
assertIncludes(mediaTranscodeWorkerRunbook, "Backup/PITR gate status: Blocked for production worker writes/backfill/activation.", "media transcode worker runbook backup gate blocked");
...
assertIncludes(mediaTranscodeWorkerRunbook, "No production worker writes or backfill while the backup/PITR gate is Blocked or Partial.", "media transcode worker runbook no-write backup gate");
...
assertIncludes(mediaTranscodeBackupGateProof, "Backup/PITR gate status: Blocked for production worker writes/backfill/activation.", "backup PITR proof blocked docs assertion");
...
assertIncludes(mediaRecoveryOperatorRunbook, "Status: operator/auditor source/proof-only, with a private R2 logical backup proof artifact.", "media recovery operator runbook status");
...
assertIncludes(mediaRecoveryOperatorRunbook, "backups/media-worker/2026/07/09/media-worker-logical-20260709-one-job-readiness-0712c0fbc441/", "media recovery operator runbook backup prefix");
...
assertIncludes(architecture, "R2 logical backup/restore gate status: Closed for one-job proof readiness only.", "architecture one-job logical backup gate");
assertIncludes(currentState, "R2-backed logical backup/restore readiness is Closed for one-job proof readiness only, not continuous production automation.", "current state one-job logical backup gate");
