...
assertIncludes(architecture, "Production transcode worker runbook status: `docs/MEDIA_TRANSCODE_WORKER_RUNBOOK.md` defines the future worker runtime, inputs, safety checks, processing flow, failure behavior, security, rollback, and activation gates. No production worker is deployed and no production queue processor is running; the first owner-approved one-job proof wrote exactly one City Lights job plus two audited HLS rendition rows. Playback may use those rows only through the audited public-row rollout gates, with signed-origin fallback still mandatory.", "production transcode worker runbook status");
...
assertIncludes(architecture, "First one-job production proof status: current production row counts are `media_transcode_jobs=1` and `media_renditions=2`, scoped only to City Lights source `c28e3838-7d2e-4f48-a8ad-73e3100f8cf1`.", "trusted backend migration one-job row status");
...
assertIncludes(currentState, "Production HLS/transcoding is not broadly live: there is no deployed production backend transcode queue/service worker, no trusted production `video_renditions` rows for these HLS assets, and no production playback switch.", "current state production HLS boundary");
...
assertIncludes(architecture, "`ready_for_transcode=5`, `already_audited_hls=1`, `needs_scan=0`, `private_excluded=12`, `premium_excluded=9`", "catalog readiness linked readback");
...
assertIncludes(mediaRecoveryOperatorRunbook, "Status: operator/auditor source/proof model with one completed owner-approved production proof job.", "media recovery operator runbook status");
...
assertIncludes(mediaRecoveryOperatorRunbook, "First real manual backup completed on 2026-07-09 under private prefix `backups/media-worker/2026/07/09/media-worker-logical-20260709T152048-8820af024114/`", "media recovery operator runbook completed real backup");
