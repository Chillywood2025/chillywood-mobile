...
assertIncludes(architecture, "Trusted backend migration dry-run proof now passes in the embedded disposable local Postgres runtime; a real Supabase staging/shadow apply still requires explicit owner approval and a safe non-production database.", "trusted backend migration dry-run checkpoint");
...
assertIncludes(nextTask, "Trusted backend migration dry-run now passes static SQL validation plus runtime apply/RLS proof through `npm run proof:media-rendition-migration-dry-run` using an in-memory disposable `@electric-sql/pglite` Postgres runtime.", "next task trusted backend migration dry-run");
