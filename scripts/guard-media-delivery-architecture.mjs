...
assertIncludes(architecture, "Production data/write boundary: no production media backfill, production `video_renditions` write, production resolver bridge, deployed production transcode worker, broad queue processor, or production playback switch is live.", "trusted backend migration production boundary");
...
assertIncludes(mediaRecoveryOperatorRunbook, "First real manual backup remains pending until required private write-mode env is present.", "media recovery operator runbook pending real backup");
...
assertIncludes(architecture, "`npm run proof:media-worker-backup-runner` proves dry-run behavior, missing-env fail-closed behavior, public bucket/domain denial, manifest/checksum generation, JS JSONL restore into disposable PGlite, and resolver-safe query.", "architecture backup runner proof");
assertIncludes(architecture, "First real manual backup remains pending unless required private write-mode env is supplied.", "architecture pending real manual backup");
...
assertIncludes(currentState, "The first real manual backup remains pending unless required private write-mode env is supplied.", "current state pending real backup");
...
assertIncludes(nextTask, "The first real manual backup remains pending unless required private write-mode env is supplied.", "next task pending real backup");
