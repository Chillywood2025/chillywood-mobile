...
const bridgeLifecycle = read("_lib/iosNativeCallBridgeLifecycle.mjs");
...
requireText(rootLayout, "activeNativeAuthorityKeyRef", "The native bridge must distinguish initial cold-start hydration from an in-process account transition.");
requireText(rootLayout, "resolveIosNativeCallBridgeLifecycle", "The native bridge must use the executable cold-start lifecycle policy.");
requireText(bridgeLifecycle, 'authorityStatus === "loading" && !hadActiveAuthority', "Initial session hydration must preserve the exact persisted PushKit binding.");
