...
async function runLinkedScanForEntry(entry) {
  ...
  try {
    ...
    return {
      ...
      readabilityStatus: download.scanResult.status === "clean" ? "readable" : "unreadable",
      malwareScanStateChanged: recordResult?.result?.malwareScanStateChanged === true,
...
if (mode === "run-one") {
  ...
  if (dataSource === "linked") {
    ...
    safeExit(0, {
      ...
      scannedCount: 1,
      readabilityPassedCount: result.readabilityStatus === "readable" ? 1 : 0,
      readabilityFailedCount: result.readabilityStatus === "unreadable" ? 1 : 0,
      selectedSource: {
        sourceType: result.sourceType,
      ...
      scannerTypeDisclosure: "ffprobe_media_readability_only_not_malware_or_content_moderation",
      scanResult: {
        status: result.readabilityStatus,
...
if (mode === "run-auto") {
  ...
  if (dataSource === "linked") {
    ...
    safeExit(0, {
      ...
      readabilityPassedCount: results.filter((result) => result.readabilityStatus === "readable").length,
      readabilityFailedCount: results.filter((result) => result.readabilityStatus === "unreadable").length,
      skippedPrivateCount: output.skippedPrivateCount,
      skippedPremiumCount: output.skippedPremiumCount,
      ...
      results: results.map((result) => ({
        ...
        storageProvider: result.storageProvider,
        downloadedBytes: result.downloadedBytes,
        readabilityStatus: result.readabilityStatus,
        malwareScanStateChanged: result.malwareScanStateChanged,
        scannerType: result.scannerType,
