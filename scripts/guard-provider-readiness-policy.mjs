#!/usr/bin/env node

import { readFileSync } from "node:fs";
import path from "node:path";

const root = process.cwd();
const read = (relativePath) => readFileSync(path.join(root, relativePath), "utf8");

const fail = (message) => {
  console.error(`Provider readiness policy guard failed: ${message}`);
  process.exitCode = 1;
};

const assertIncludes = (source, needle, label) => {
  if (!source.includes(needle)) fail(`${label} is missing ${needle}`);
};

const assertNotIncludes = (source, needle, label) => {
  if (source.includes(needle)) fail(`${label} must not include ${needle}`);
};

const migration = read("supabase/migrations/202605250002_provider_link_readiness_scaffold.sql");
const appHelper = read("_lib/providerReadiness.ts");
const edgeShared = read("supabase/functions/_shared/provider-readiness.ts");
const edgeReadiness = read("supabase/functions/provider-readiness/index.ts");
const revenueCatWebhook = read("supabase/functions/revenuecat-webhook/index.ts");
const revenueCatAuthorityCloseout = read("supabase/migrations/20260824034109_creator_money_authority_integrity_closeout.sql");
const googlePlayWebhook = read("supabase/functions/google-play-webhook/index.ts");
const channelSettings = read("app/channel-settings.tsx");
const packageJson = read("package.json");
const config = read("supabase/config.toml");
const sandboxProofMigration = read("supabase/migrations/202605250004_provider_link_sandbox_proof_status.sql");
const moneySwitchMigration = read("supabase/migrations/202605270001_platform_money_kill_switches.sql");

assertIncludes(packageJson, "guard:provider-readiness-policy", "package guard script");

assertIncludes(migration, 'create table if not exists public."provider_readiness_status"', "readiness status table");
assertIncludes(migration, 'create table if not exists public."provider_readiness_audit_log"', "readiness audit table");
assertIncludes(migration, '"is_live_money_enabled" boolean not null default false', "live money default false");
assertIncludes(migration, '"is_live_money_enabled" = false or "status" = \'active\'', "live money requires active status");
assertIncludes(migration, 'provider_readiness_status_active_requires_proof_check', "active requires proof check");
assertIncludes(migration, 'alter table public."provider_readiness_status" enable row level security', "readiness RLS");
assertIncludes(migration, 'alter table public."provider_readiness_audit_log" enable row level security', "audit RLS");
assertIncludes(migration, 'revoke all on table public."provider_readiness_status" from anon, authenticated', "direct readiness revoke");
assertIncludes(migration, 'revoke all on function public."get_provider_readiness_summary"() from public', "RPC public revoke");
assertIncludes(migration, 'grant execute on function public."get_provider_readiness_summary"() to authenticated', "authenticated safe RPC");
assertIncludes(migration, 'provider_readiness_summary_requested', "readiness summary audit");
assertIncludes(migration, "'tips_enabled', false", "tips seed disabled");
assertIncludes(migration, "'payout_created', false", "payout seed disabled");
assertIncludes(migration, "'transfer_created', false", "transfer seed disabled");
assertIncludes(migration, "'balance_created', false", "balance seed disabled");
assertIncludes(migration, "'premium_granted', false", "premium grant seed disabled");
assertNotIncludes(migration, "is_live_money_enabled\", true", "live money true seed");

assertIncludes(appHelper, "readProviderReadinessSummary", "app safe readiness reader");
assertIncludes(appHelper, "getProviderReadinessFallbackSummary", "fail-closed app fallback");
assertIncludes(appHelper, "isLiveMoneyEnabled: false", "fallback live money false");
assertIncludes(appHelper, "findProviderReadinessSummary", "readiness lookup helper");
assertNotIncludes(appHelper, "STRIPE_SECRET_KEY", "app helper Stripe secret");
assertNotIncludes(appHelper, "REVENUECAT_SECRET_API_KEY", "app helper RevenueCat secret");
assertNotIncludes(appHelper, "GOOGLE_PLAY_SERVICE_ACCOUNT_JSON", "app helper Google secret");

assertIncludes(edgeShared, "ProviderReadinessAdapter", "provider adapter interface");
assertIncludes(edgeShared, "RevenueCatAdapter", "RevenueCat adapter");
assertIncludes(edgeShared, "GooglePlayAdapter", "Google Play adapter");
assertIncludes(edgeShared, "StripeAdapter", "Stripe adapter");
assertIncludes(edgeShared, "StripeConnectAdapter", "Stripe Connect adapter");
assertIncludes(edgeShared, "liveMoneyAction: false", "edge adapter no live money");
assertIncludes(edgeShared, "secret_values_logged: false", "audit secret redaction marker");

assertIncludes(edgeReadiness, "authenticateBearerUser", "readiness auth");
assertIncludes(edgeReadiness, "readProviderReadinessRows", "readiness sanitized rows");
assertIncludes(edgeReadiness, "liveMoneyAction: false", "readiness no live money");

assertIncludes(revenueCatWebhook, "REVENUECAT_WEBHOOK_SECRET", "RevenueCat webhook secret env");
assertIncludes(revenueCatWebhook, "writePremiumEntitlementFromRevenueCatEvent", "RevenueCat backend entitlement sync");
assertIncludes(revenueCatWebhook, '.rpc("process_revenuecat_premium_event_atomic"', "RevenueCat atomic Premium projector");
assertIncludes(revenueCatWebhook, '.rpc("process_revenuecat_app_store_event_atomic"', "RevenueCat atomic App Store creator-money projector");
assertIncludes(revenueCatWebhook, '.rpc("process_revenuecat_google_play_event_atomic"', "RevenueCat atomic Google Play creator-money projector");
assertIncludes(revenueCatWebhook, '.rpc("process_revenuecat_terminal_event_atomic"', "RevenueCat atomic cross-domain terminal dispatcher");
assertIncludes(revenueCatWebhook, '.rpc("reserve_revenuecat_webhook_ingress_event"', "RevenueCat immutable signed ingress reservation");
assertIncludes(revenueCatWebhook, "await reserveRevenueCatWebhookIngress(adminConfig.client, event, rawBody)", "RevenueCat pre-routing ingress reservation");
assertIncludes(revenueCatWebhook, "TERMINAL_DISPATCH_EVENT_TYPES.has(terminalEventType)", "RevenueCat non-active lifecycle dispatch");
assertIncludes(revenueCatWebhook, "&& terminalOriginalTransactionId", "RevenueCat terminal original-transaction binding");
assertIncludes(revenueCatWebhook, "p_original_transaction_id: originalTransactionId", "RevenueCat exact original-transaction binding");
assertIncludes(revenueCatWebhook, "const productIdentityResolved = rawProductIdentityUsable", "RevenueCat exact catalog identity decision");
assertIncludes(revenueCatWebhook, "&& hasExactPremiumStoreProductIdentity(productResolution, providerProductIdentity)", "RevenueCat exact catalog cardinality decision");
assertIncludes(revenueCatWebhook, "p_product_id: resolvedProductId", "RevenueCat unresolved catalog durable projector routing");
assertIncludes(revenueCatWebhook, "p_store_mapping_id: resolvedStoreMappingId", "RevenueCat unresolved mapping durable projector routing");
assertIncludes(revenueCatWebhook, "const ignoredEvent = toText(result.status) === \"ignored\"", "RevenueCat ignored authority handling");
assertIncludes(revenueCatWebhook, "moneyAccessMirrored: !entitlementWrite.staleEvent && !entitlementWrite.ignoredEvent", "RevenueCat ignored/stale mirror block");
assertIncludes(revenueCatWebhook, "premiumGranted: entitlementWrite.entitlementActive", "RevenueCat no fake premium");
assertIncludes(revenueCatWebhook, "verifySharedWebhookSecret", "RevenueCat signature requirement");
assertIncludes(revenueCatWebhook, "revenuecat_webhook_setup_required", "RevenueCat missing-secret audit");
assertNotIncludes(revenueCatWebhook, "readGooglePlayPurchaseSwitchState", "pre-atomic Google Play switch return");
assertNotIncludes(revenueCatWebhook, "readAppStorePurchaseSwitchState", "pre-atomic App Store switch return");
assertIncludes(revenueCatAuthorityCloseout, 'create or replace function public."process_revenuecat_premium_event_atomic"', "RevenueCat Premium authority wrapper");
assertIncludes(revenueCatAuthorityCloseout, 'create or replace function public."record_revenuecat_premium_ignored_internal"', "RevenueCat durable ignored event projector");
assertIncludes(revenueCatAuthorityCloseout, 'revenuecat_premium_event_id_identity_mismatch', "RevenueCat immutable event identity mismatch block");
assertIncludes(revenueCatAuthorityCloseout, 'create table public."revenuecat_premium_transaction_authority"', "RevenueCat Premium transaction authority binding");
assertIncludes(revenueCatAuthorityCloseout, 'unique ("provider","original_transaction_id")', "RevenueCat unique Premium transaction identity");
assertIncludes(revenueCatAuthorityCloseout, 'premium_original_transaction_initial_replay', "RevenueCat Premium cross-event replay block");
assertIncludes(revenueCatAuthorityCloseout, 'premium_original_transaction_terminal_or_blocked', "RevenueCat Premium sticky terminal block");
assertIncludes(revenueCatAuthorityCloseout, 'revenuecat_premium_original_transaction_subject_mismatch', "RevenueCat Premium cross-user transaction block");
assertIncludes(revenueCatAuthorityCloseout, 'premium_original_transaction_id_required', "RevenueCat missing transaction ignored state");
assertIncludes(revenueCatAuthorityCloseout, 'premium_store_product_resolution_missing_or_ambiguous', "RevenueCat unresolved catalog tombstone");
assertIncludes(revenueCatAuthorityCloseout, 'revenuecat_provider_event_replay_mismatch', "RevenueCat creator-money replay mismatch block");
assertIncludes(revenueCatAuthorityCloseout, 'revenuecat_unbound_initial_authority', "RevenueCat ignored original-transaction tombstone");
assertIncludes(revenueCatAuthorityCloseout, 'unbound_initial_original_transaction_reserved', "RevenueCat tombstone replay block");
assertIncludes(revenueCatAuthorityCloseout, 'create or replace function public."process_revenuecat_premium_transfer_atomic"', "RevenueCat atomic transfer wrapper");
assertIncludes(revenueCatAuthorityCloseout, 'create or replace function public."process_revenuecat_terminal_event_atomic"', "RevenueCat atomic terminal authority dispatcher");
assertIncludes(revenueCatAuthorityCloseout, 'create or replace function public."reserve_revenuecat_webhook_ingress_event"', "RevenueCat immutable ingress reservation RPC");
assertIncludes(revenueCatAuthorityCloseout, 'revenuecat_webhook_ingress_identity_mismatch', "RevenueCat changed-payload replay rejection");
assertIncludes(revenueCatAuthorityCloseout, 'terminal_dispatch_binding_ambiguous', "RevenueCat ambiguous terminal domain block");
assertIncludes(revenueCatAuthorityCloseout, 'terminal_dispatch_binding_missing', "RevenueCat missing terminal domain tombstone");
assertIncludes(revenueCatAuthorityCloseout, "'premium_provider_rail_disabled'", "RevenueCat durable disabled-rail transfer result");
assertIncludes(revenueCatAuthorityCloseout, 'v_event."raw_payload_hash" is distinct from p_raw_payload_hash', "RevenueCat ignored replay payload binding");
assertIncludes(revenueCatAuthorityCloseout, 'provider_event."raw_payload_hash"=entitlement."metadata"->>\'revenuecat_event_hash\'', "RevenueCat entitlement payload binding");
assertIncludes(revenueCatAuthorityCloseout, 'provider_event."provider_event_id"=entitlement."metadata"->>\'revenuecat_event_id\'', "RevenueCat entitlement event binding");
assertIncludes(revenueCatAuthorityCloseout, 'entitlement."source"=\'revenuecat\'', "RevenueCat authoritative entitlement source");
assertIncludes(revenueCatAuthorityCloseout, "then 'UNKNOWN'", "RevenueCat invalid finite authority readback");
assertIncludes(revenueCatAuthorityCloseout, "'entitlementStatus','unknown'", "RevenueCat ignored entitlement state");
assertIncludes(revenueCatAuthorityCloseout, "'entitlementActive',false", "RevenueCat ignored grant block");
assertIncludes(revenueCatAuthorityCloseout, "'grantStatus','blocked'", "RevenueCat ignored access block");
assertIncludes(revenueCatAuthorityCloseout, 'revoke all on function public."process_revenuecat_premium_event_atomic"', "RevenueCat public Premium RPC revoke");
assertIncludes(revenueCatAuthorityCloseout, ') to service_role;', "RevenueCat service-only RPC grant");
assertIncludes(googlePlayWebhook, "GOOGLE_PLAY_WEBHOOK_SECRET", "Google Play webhook secret env");
assertIncludes(googlePlayWebhook, "subscriptionGranted: false", "Google Play no fake subscription");
assertIncludes(googlePlayWebhook, "verifySharedWebhookSecret", "Google Play signature requirement");
assertIncludes(googlePlayWebhook, "google_play_webhook_setup_required", "Google Play missing-secret audit");

assertIncludes(sandboxProofMigration, "'sandbox_ready'", "Stripe sandbox proof status");
assertIncludes(sandboxProofMigration, "'stripe_webhook_sandbox_ready_proved'", "Stripe sandbox proof audit");
assertIncludes(sandboxProofMigration, "'revenuecat_webhook_setup_required_proved'", "RevenueCat setup-required proof audit");
assertIncludes(sandboxProofMigration, "'google_play_webhook_setup_required_proved'", "Google Play setup-required proof audit");
assertIncludes(sandboxProofMigration, '"is_live_money_enabled" = false', "sandbox proof live money remains false");
assertIncludes(sandboxProofMigration, "'CHILLYWOOD_LIVE_MONEY_ENABLED', 'missing'", "live money flag missing proof");
assertNotIncludes(sandboxProofMigration, "'status' = 'active'", "sandbox proof must not force active");
assertNotIncludes(sandboxProofMigration, '"is_live_money_enabled" = true', "sandbox proof must not enable live money");

assertIncludes(config, "[functions.provider-readiness]", "provider readiness function config");
assertIncludes(config, "[functions.revenuecat-webhook]", "RevenueCat webhook function config");
assertIncludes(config, "[functions.google-play-webhook]", "Google Play webhook function config");

assertIncludes(channelSettings, "readProviderReadinessSummary", "Studio readiness integration");
assertIncludes(channelSettings, "readMoneyFeatureFlagSummary", "Studio money switch integration");
assertIncludes(channelSettings, "Money Center", "Studio Money Center title");
assertIncludes(channelSettings, "Provider Status", "Studio provider status section");
assertIncludes(channelSettings, "Provider checks are the source of readiness truth.", "Studio provider source-of-truth copy");
assertIncludes(channelSettings, "No withdrawal, transfer, cash-out, or payout release action is available.", "Studio payout lock copy");
assertNotIncludes(channelSettings, "{ id: \"payouts\", label: \"Payouts\" }", "separate payout tab");
assertNotIncludes(channelSettings, "{ id: \"revenue\", label: \"Revenue\" }", "separate revenue tab");
assertNotIncludes(channelSettings, "{ id: \"monetize\", label: \"Monetize\" }", "separate monetize tab");
assertNotIncludes(channelSettings, "STRIPE_SECRET_KEY", "Studio Stripe secret");
assertNotIncludes(channelSettings, "REVENUECAT_SECRET_API_KEY", "Studio RevenueCat secret");
assertNotIncludes(channelSettings, "GOOGLE_PLAY_SERVICE_ACCOUNT_JSON", "Studio Google secret");

assertIncludes(moneySwitchMigration, "get_money_feature_flags_summary", "Money switch sanitized readiness companion RPC");
assertIncludes(moneySwitchMigration, "live_money_enabled", "Money switch live money lock");
assertIncludes(moneySwitchMigration, "assert_money_feature_allowed", "backend money capability guard");

if (process.exitCode) {
  process.exit();
}

console.log("Provider readiness policy guard passed.");
