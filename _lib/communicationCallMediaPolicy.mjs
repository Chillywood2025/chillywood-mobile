...
export function createIosAcceptedCallKitMediaDescriptor(input) {
  const claim = input?.trustedNativeClaim;
  const ids = [input?.authenticatedUserId, input?.threadId, input?.inviteId, input?.roomId, input?.callUuid].map(callId);
  ...
  if (input?.serverAccepted !== true || input?.callKitAnswerQueued !== true || input?.inviteStatus !== "accepted"
    ...
    || [claim.authenticatedUserId, claim.threadId, claim.inviteId, claim.nativeIdentity].some((id, index) => id !== ids[[0, 1, 2, 4][index]])
    || !Number.isFinite(now) || now < claim.consumedAtMonotonicMs || now >= claim.expiresAtMonotonicMs) return null;
  const descriptor = Object.freeze({
    authenticatedUserId: ids[0], callUuid: ids[4], claimId: claim.claimId, inviteId: ids[2], mediaProvider,
    nativeEventGeneration: claim.nativeEventGeneration, platform: "ios", roomId: ids[3], source: claim.source, threadId: ids[1],
  });
...
function canSettleIosAcceptedCallKitMediaFailure(input) {
  ...
  const exact = [input?.authenticatedUserId, input?.threadId, input?.inviteId, input?.roomId, input?.callUuid, input?.mediaProvider]
    .map(callId).every((value, index) => value === [descriptor?.authenticatedUserId, descriptor?.threadId, descriptor?.inviteId, descriptor?.roomId, descriptor?.callUuid, descriptor?.mediaProvider][index]);
  return !!descriptor && input?.platform === "ios" && exact && ((state === "active" && input?.channelState === "error"
    && input?.inviteStatus === "accepted") || state === "terminal_confirmed" || state === "room_left");
}

const exactIosAcceptedInvite = (invite, expected) => !!invite
  && [invite.id, invite.threadId, invite.communicationRoomId, invite.calleeUserId].map(callId)
    .every((value, index) => value === [expected.inviteId, expected.threadId, expected.roomId, expected.authenticatedUserId][index])
...
export async function terminateIosAcceptedNativeAnswer(input, operations) {
  const expected = {authenticatedUserId: callId(input?.authenticatedUserId), callUuid: callId(input?.callUuid), inviteId: callId(input?.invite?.id), mediaProvider: callId(input?.invite?.mediaProvider), roomId: callId(input?.invite?.communicationRoomId), threadId: callId(input?.threadId)};
...
export async function completeIosAcceptedNativeAnswer(input, operations) {
  const expected = {authenticatedUserId: callId(input?.authenticatedUserId), callUuid: callId(input?.callUuid), inviteId: callId(input?.invite?.id), mediaProvider: callId(input?.invite?.mediaProvider), roomId: callId(input?.invite?.communicationRoomId), threadId: callId(input?.threadId)};
  if (input?.serverAccepted !== true || input?.invite?.status !== "accepted" || !exactIosAcceptedInvite(input?.invite, expected) || !exactIosAnswerClaim(input?.trustedNativeClaim, expected)
