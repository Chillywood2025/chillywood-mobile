...
const transitionWitnesses = [
  { id: "chat-in-app-remote-media", domain: "chat-call", kind: "positive", commands: [{ type: "invite", generation: 1, provider: "livekit" }, { type: "accept_in_app" }, { type: "issue_token" }, { type: "connect_room" }, { type: "publish_local" }, { type: "remote_participant" }, { type: "subscribe_remote" }, { type: "first_media" }], reached: (s) => s.remoteMedia && s.acceptanceAuthority === "in_app" },
  { id: "chat-native-accept", domain: "chat-call", kind: "positive", commands: [{ type: "invite", generation: 1, provider: "livekit" }, { type: "native_action", actionId: 1, generation: 1, expiresAt: 5 }, { type: "auth_ready" }, { type: "react_ready" }, { type: "consume_native" }, { type: "accept_native", actionId: 1, generation: 1 }], reached: (s) => s.inviteStatus === "accepted" && s.acceptanceAuthority === "native_action" },
  { id: "chat-expired-native-denied", domain: "chat-call", kind: "negative", commands: [{ type: "invite", generation: 1, provider: "livekit" }, { type: "native_action", actionId: 1, generation: 1, expiresAt: 2 }, { type: "tick", amount: 2 }, { type: "accept_native", actionId: 1, generation: 1 }], reached: (s) => s.inviteStatus === "ringing" && s.expiredNativeActions.includes("action-1-1") },
  ...
  { id: "ota-compatible-new-source", domain: "ota-build", commands: [{ type: "install_build", buildId: "build-a", packageId: "com.chillywood.mobile", signedArtifactId: "artifact-a", platform: "android", environment: "internal", runtime: "runtime-1", channel: "internal", nativeDigest: "native-a", sourceCommit: "source-a", embeddedSafe: true, providedCapabilities: ["camera"] }, { type: "activate_update", targetBuildId: "build-a", packageId: "com.chillywood.mobile", updateId: "update-b", group: "group-b", platform: "android", environment: "internal", runtime: "runtime-1", channel: "internal", nativeDigest: "native-a", sourceCommit: "source-b", requiredCapabilities: ["camera"] }], reached: (s) => s.currentUpdate?.sourceCommit === "source-b" && s.currentUpdate?.targetBuildId === s.build?.buildId },
  ...
  { id: "pushkit-report-complete-native-acceptance", domain: "notification-native-action", kind: "positive", commands: [{ type: "receive_push", id: "push", mustReport: true }, { type: "report_callkit", id: "push", mustReport: true }, { type: "complete_push", id: "push", mustReport: true }, { type: "native_answer", id: "witness", expiresAt: 5 }, { type: "auth_ready" }, { type: "react_ready" }, { type: "consume_action", id: "witness" }, { type: "server_accept", id: "witness" }], reached: (s) => s.serverStatus === "accepted" && s.callKitReported.includes("push") && s.completedPushes.includes("push") },
...
export function runPropertyModels(options = {}) {
  ...
  const results = ids.map((id) => {
    ...
    const witnesses = transitionWitnesses.filter(({ domain }) => domain === id).map(({ id: witnessId, kind = "positive", commands, reached }) => {
      const execution = runCommands(id, commands);
      return { id: witnessId, kind, reached: reached(execution.state), commandCount: commands.length, commands, violations: execution.violations };
    });
...
export function runEscapedDefectChecks() {
  ...
  return {
    ...
    canonicalFixtureCount: results.filter(({ classification }) => classification === "canonical").length,
    supplementalFixtureCount: results.filter(({ classification }) => classification === "supplemental").length,
    results
  };
}
