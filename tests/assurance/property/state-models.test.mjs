...
const expectedWitnessCounts = { "chat-call": 3 };
...
for (const [domain, definition] of Object.entries(modelDefinitions)) {
  test(`${domain} preserves its invariants with replayable deterministic commands`, () => {
    ...
    assert.equal(report.witnessCount, expectedWitnessCounts[domain] ?? 1);
    ...
    if (domain === "chat-call") assert.deepEqual(report.results[0].transitionWitnesses.map(({ kind }) => kind), ["positive", "positive", "negative"]);
  });
}
