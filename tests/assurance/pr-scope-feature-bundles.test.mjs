...
test("assurance-control observer accepts only exact protected source-only profiles and strips authority claims", () => {
  ...
  assert.deepEqual(tokenWiringProfile, { profileId: "PHASE1_SOURCE_AUTHORITY_TOKEN_WIRING_V1", paths: [".github/workflows/phase1-ci.yml", "scripts/assurance/engineering-closure.mjs", "scripts/assurance/lib.mjs", "tests/assurance/engineering-doctrine.test.mjs", "tests/assurance/pr-scope-feature-bundles.test.mjs", "tests/assurance/source-readiness-wrapper.test.mjs"], maximumFiles: 6, maximumChangedLines: 260 });
  ...
  assert.equal(resolveAssuranceControlSourceOnlyProfile({ changedPaths: tokenWiringProfile.paths, budget: { maximumFiles: 6, maximumChangedLines: 260, maximumHandAuthoredNetLines: 260 }, changedFiles: 6 })?.profileId, tokenWiringProfile.profileId);
  assert.equal(resolveAssuranceControlSourceOnlyProfile({ changedPaths: tokenWiringProfile.paths.slice(1), budget: { maximumFiles: 6, maximumChangedLines: 260, maximumHandAuthoredNetLines: 260 }, changedFiles: 5 }), null);
  ...
  const fixedPointSubset = fixedPointProfile.paths.slice(1);
  assert.equal(resolveAssuranceControlSourceOnlyProfile({ changedPaths: fixedPointSubset, budget: { maximumFiles: 7, maximumChangedLines: 1800, maximumHandAuthoredNetLines: 1800 }, changedFiles: fixedPointSubset.length })?.profileId, fixedPointProfile.profileId);
  assert.equal(resolveAssuranceControlSourceOnlyProfile({ changedPaths: [...fixedPointSubset, "app/index.tsx"].sort(), budget: { maximumFiles: 7, maximumChangedLines: 1800, maximumHandAuthoredNetLines: 1800 }, changedFiles: fixedPointSubset.length + 1 }), null);
