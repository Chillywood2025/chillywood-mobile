...
const admissionFixture = () => {
  ...
  const lease = { leaseId: taskArtifact.taskId, featureId: taskArtifact.primaryDomain, implementationPr: 229, implementationBranch: implementation.branch, admittedSeedHead: seedHead, admittedSeedTree: seedTree, admittedBase: implementation.baseSha, protectedAdmissionPr: 230, ownerAuthorizationCommentId: implementation.ownerCommentId, domain: taskArtifact.primaryDomain, domainOwnership: "ACTIVE", taskState: "PREIMPLEMENTATION_ENGINEERING_CLEAR", allowedPaths, scopeBudget: { maximumFiles: 30, maximumChangedLines: 3600 }, artifactReservation: { closureArtifactPath: artifactPath, allowedDomains: domains, pathGlobs: allowedPaths, testEvidencePaths: [allowedPaths.at(-1)], maximumFiles: 30, maximumLines: 3600, excludedHighRiskPaths: [] }, recursionBudget: { maximumAdmissionPrs: 1, maximumFinalSourceBindingPrs: 0, maximumMergeProvenancePrs: 0, maximumPostMergeTruthPrs: 1 } };
...
const clearanceCases = [
  ...
  [33, "source descendant retains the finite lease identity", () => assert.equal(admissionFixture().truthRecord.finiteTaskLeases.tasks[0].leaseId, "pre-release-identity-entitlement-authority-v1")],
