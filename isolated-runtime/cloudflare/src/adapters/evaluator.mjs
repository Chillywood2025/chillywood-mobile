...
const LIVEKIT_FIXTURE_BINDING_KEYS = Object.freeze([
  "condition",
...
const LIVEKIT_FIXTURE_CONDITIONS = Object.freeze({
...
const approvedRouteFamilyId = (route) => {
  if (!APPROVED_MAPPING_ROUTES.has(route)) return null;
  return route
    .replace(/([a-z0-9])([A-Z])/gu, "$1-$2")
    .toLowerCase() + ".main";
};
export const routeTimingNoFindingReasons = (run, metrics) => {
  ...
  const resolutionKind = text(metrics.resolutionKind);
  const expectedFinalState = ROUTE_RESOLUTION_STATES[resolutionKind];
  const evidenceHashes = run?.metric_manifest?.evidenceHashes;

  if (
    !ROUTE_IDENTITY.test(text(metrics.appVersion)) ||
    !ROUTE_IDENTITY.test(text(metrics.appBuild)) ||
    !ROUTE_IDENTITY.test(text(metrics.runtimeVersion)) ||
    !ROUTE_IDENTITY.test(text(metrics.channel))
  ) {
    reasons.add("route_timing_app_identity_required");
  }
  if (
    text(metrics.platform) !== run.platform ||
    text(metrics.routeOrSurface) !== run.route_or_surface ||
    !ROUTE_FAMILY_ID.test(text(metrics.routeFamilyId)) ||
    !HASH.test(text(metrics.routeFamilyBindingHash)) ||
    text(metrics.routeFamilyId) !==
      approvedRouteFamilyId(text(run.route_or_surface)) ||
    !baselineBindingIsValid(run, metrics)
  ) {
    reasons.add("route_timing_route_family_binding_required");
  }
  if (
    text(metrics.runtimeIdentityHash) !== run.runtime_identity_hash ||
    text(metrics.buildRuntimeHash) !== run.source_build_hash ||
    text(metrics.sanitizedEvidenceHash) !== run.evidence_manifest_hash ||
  ...
  if (
    text(metrics.installedProofStatus) !== run.physical_proof_status ||
  ...
  if (
    ...
    text(metrics.networkState) !== "ready" ||
  ...
  if (
    !expectedFinalState ||
    text(metrics.finalObservedState) !== expectedFinalState ||
  ...
  if (
    !ROUTE_INTERACTION_EVIDENCE.has(
      text(metrics.interactionEvidenceKind),
    ) ||
    !HASH.test(text(metrics.interactionEvidenceHash))
  ) {
    reasons.add("route_timing_interaction_evidence_required");
  }
  if (text(metrics.findingDisposition) !== "no_finding") {
...
const objectiveAccessibilityBindingIsValid = (run, metrics) => {
  const binding = OBJECTIVE_ACCESSIBILITY_BINDINGS.get(
    text(metrics.routeFamilyMappingId),
  );
  return !!binding &&
    binding.objectiveAccessibilityOnly === true &&
    binding.allowsVisualDensityComparison === false &&
    text(run.route_or_surface) === binding.routeOrSurface &&
    text(metrics.routeFamilyMappingHash) === binding.bindingHash &&
    text(metrics.componentIdentityHash) ===
      OBJECTIVE_ACCESSIBILITY_COMPONENT_SET_HASH &&
    text(metrics.surfaceFamily) === binding.surfaceFamily &&
    text(metrics.exceptionContractId) === binding.exceptionContractId &&
    text(metrics.exceptionContractHash) ===
      EXCEPTION_CONTRACT_HASHES[binding.exceptionContractId] &&
    metrics.exceptionVersioned === true &&
    text(metrics.exceptionType) === "non_media_surface" &&
    text(metrics.targetClassification) !== "meets_wcag_aa_minimum_only";
};
...
const liveKitMetricContractIsValid = (metrics) => {
  if (
    ...
    !LIVEKIT_SCENARIO_TYPES.has(metrics.scenarioType) ||
...
export const deterministicNoFindingReasons = (
  ...
  if (observationKind === "search_accessibility") {
    ...
    if (
      text(metrics.resultState) !== "success" ||
      text(metrics.expectedState) !== text(metrics.observedState) ||
      metricNumber(metrics, "unresolvedStateCount") !== 0 ||
      elapsedDuration === null ||
      maximumDuration === null ||
      elapsedDuration > maximumDuration
    ...
    const failureCategory = deriveIndependentLiveKitFailureCategory(metrics);
    if (
      text(metrics.scenarioType) === "bounded_failure_fixture" ||
...
export const deterministicResolutionReasons = (
  ...
  if (!resolutionMetrics || !detectionMetrics) {
    ...
    if (
      ...
      text(resolutionMetrics.localMediaSource) !==
        text(detectionMetrics.localMediaSource) ||
      text(resolutionMetrics.installedRuntimeIdentityHash) !==
        text(run.runtime_identity_hash) ||
      text(resolutionMetrics.installedSourceBuildHash) !==
        text(run.source_build_hash)
    ) {
      reasons.add("resolution_measurement_identity_mismatch");
    }
    ...
    if (!failureCategory) {
      ...
      text(resolutionMetrics.stageFailureCategory) !== failureCategory
    ) {
    ...
    for (const reason of routeTimingNoFindingReasons(run, resolutionMetrics)) {
      reasons.add(reason);
    }
  }
  return Object.freeze([...reasons].sort());
...
const normalizeNoFinding = (payload) =>
  exactKeys(payload, NO_FINDING_KEYS) &&
...
const normalizeLiveKitNoFindingAttestation = (payload) =>
  exactKeys(payload, LIVEKIT_NO_FINDING_ATTESTATION_KEYS) &&
...
const liveKitFixtureConditionMatches = (binding) => {
  if (
    !isRecord(binding) ||
    !exactKeys(binding, LIVEKIT_FIXTURE_BINDING_KEYS) ||
    !isRecord(binding.condition) ||
    !exactKeys(binding.condition, [
      "expectedFailureCategory",
      "injectedCondition",
      "timeoutMs",
      "triggerStage",
    ]) ||
    typeof binding.fixtureType !== "string"
  ) {
    return false;
  }
  const expected = LIVEKIT_FIXTURE_CONDITIONS[binding.fixtureType];
  return !!expected &&
    binding.condition.expectedFailureCategory ===
      expected.expectedFailureCategory &&
    binding.condition.injectedCondition === expected.injectedCondition &&
    binding.condition.timeoutMs === expected.timeoutMs &&
    binding.condition.triggerStage === expected.triggerStage;
};

const readLiveKitNoFindingAttestationSnapshot = (
  snapshot,
  request,
  now = Date.now(),
) => {
  ...
  const manifest = run.metric_manifest;
  const metrics = metricObject(run);
  const binding = manifest.failureFixtureBinding;
  const evidenceHashes = manifest.evidenceHashes;
  const derivedFailureCategory =
    deriveIndependentLiveKitFailureCategory(metrics);
  if (
    run.sentinel_key !== "livekit_experience_sentinel" ||
    run.result_status !== "failed" ||
    run.physical_proof_status !== "installed_ui_observed" ||
    manifest.observationKind !== "livekit_experience" ||
    !liveKitMetricContractIsValid(metrics) ||
    metrics.scenarioType !== "bounded_failure_fixture" ||
    !derivedFailureCategory ||
    derivedFailureCategory === "none" ||
    metrics.stageFailureCategory !== derivedFailureCategory ||
    !liveKitFixtureConditionMatches(binding) ||
    binding.condition.expectedFailureCategory !==
      derivedFailureCategory ||
    binding.principal !== "cognitive_livekit_experience_collector" ||
    !HASH.test(text(binding.fixtureId)) ||
    !HASH.test(text(binding.fixtureAttestationHash)) ||
    !HASH.test(text(binding.syntheticRoomNameHash)) ||
    !HASH.test(text(binding.roomRunCorrelationHash)) ||
    !SOURCE_COMMIT.test(text(binding.sourceCommit)) ||
    metrics.roomRunCorrelationHash !== binding.roomRunCorrelationHash ||
    metrics.installedRoomRunCorrelationHash !==
      binding.roomRunCorrelationHash ||
    !Array.isArray(evidenceHashes) ||
    !evidenceHashes.includes(run.evidence_manifest_hash) ||
    !evidenceHashes.includes(binding.fixtureAttestationHash)
  ) {
    return null;
  }
  return Object.freeze({
...
const assessmentHashForNoFinding = async (database, run) => {
...
const evaluateNoFinding = ready(
  ...
  async ({ assertActive, database, env, payload, signal }) => {
    const request = normalizeNoFinding(payload);
    if (!request) throw new Error("sentinel_no_finding_payload_rejected");
    const snapshot = await database.call("productQualityEvaluatorSnapshot", [
      request.sentinelRunId,
      null,
    ]);
    const selected = readDetectionSnapshot(snapshot, request);
    if (!selected) throw new Error("sentinel_no_finding_snapshot_rejected");
    const assessmentHash = await assessmentHashForNoFinding(
      database,
      selected.run,
    );
    if (!assessmentHash) {
      throw new Error("sentinel_no_finding_assessment_hash_rejected");
    }
    return recordEvaluatorProof({
      assertActive,
      assessmentHash,
      assessmentKind: "run_no_finding",
      database,
      env,
      reasons: deterministicNoFindingReasons(
        selected.run,
        selected.context,
      ),
      run: selected.run,
...
const attestLiveKitBoundedFailureNoFinding = ready(
  ...
  async ({ assertActive, database, env, payload, signal }) => {
    const request = normalizeLiveKitNoFindingAttestation(payload);
    if (!request) {
      throw new Error("livekit_no_finding_attestation_payload_rejected");
    }
    const snapshot = await database.call("productQualityEvaluatorSnapshot", [
      request.sentinelRunId,
      null,
    ]);
    const selected = readLiveKitNoFindingAttestationSnapshot(
      snapshot,
      request,
    );
    if (!selected) {
      throw new Error("livekit_no_finding_attestation_snapshot_rejected");
    }
    const evaluatorOutputHash = await hashJson({
      derivedFailureCategory: selected.derivedFailureCategory,
      evidenceManifestHash: selected.run.evidence_manifest_hash,
      evaluationKind: "livekit_bounded_failure_no_finding",
      fixtureAttestationHash:
        selected.binding.fixtureAttestationHash,
      fixtureId: selected.binding.fixtureId,
      sentinelRunId: selected.run.id,
      sourceBuildHash: selected.run.source_build_hash,
    ...
    const attestationHash = await sha256Hex([
      ...
      selected.derivedFailureCategory,
      evaluatorOutputHash,
    ].join("|"));
    await assertInvocationActive({ assertActive, signal });
    const result = await database.call(
    ...
    if (
      ...
      throw new Error("livekit_no_finding_attestation_readback_rejected");
    }
    return Object.freeze({
      ...result,
      evaluatorOutputHash,
...
export const PRODUCT_QUALITY_EVALUATOR_ADAPTERS = Object.freeze({
  ...
  evaluate_sentinel_no_finding: evaluateNoFinding,
